...
- Marker choice:
    - Use one primary family marker by file family.
    - Do not mix family markers in one `it()` unless explicitly required by a validator rule.
    - Never do blind cross-family marker rewrites. Any migration must be scoped per folder and validated per file.
