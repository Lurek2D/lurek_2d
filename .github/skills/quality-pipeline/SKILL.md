...
## Domain Knowledge
- Quality gate order: `python tools/dev/parallel_cargo.py fmt check` — formatting, `python tools/dev/parallel_cargo.py clippy --deny-warnings` — zero-warning lint, `python tools/dev/parallel_cargo.py test rust` — Rust tests, `python tools/dev/parallel_cargo.py test lua` — Lua harness, `python tools/gen_all_docs.py` — docs freshness. Running them out of order wastes time — fmt and clippy failures are cheap to fix early.
- Use the correct entry point for the change type: Lua API change → `python tools/validate/validate_generated_lua_stubs.py`. Module structure change → `python tools/validate/validate_module_coverage.py`.
