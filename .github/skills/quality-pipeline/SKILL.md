...
- Generated files (`docs/api/lurek.md`, `docs/api/lurek.lua`, `docs/api/lureksome.md`) must be committed in the same PR as the source changes that produced them. CI catches stale generated files by running the generator and diffing. A PR with stale generated docs will fail CI.
