...
## Domain Knowledge
- Quality gate order: `tools/python.cmd tools/dev/parallel_cargo.py fmt check` â€” formatting, `tools/python.cmd tools/dev/parallel_cargo.py clippy --deny-warnings` â€” zero-warning lint, `tools/python.cmd tools/dev/parallel_cargo.py test rust` â€” Rust tests, `tools/python.cmd tools/dev/parallel_cargo.py test lua` â€” Lua harness, `tools/python.cmd tools/gen_all_docs.py` â€” docs freshness. Running them out of order wastes time â€” fmt and clippy failures are cheap to fix early.
- Use the correct entry point for the change type: Lua API change â†’ `tools/python.cmd tools/validate/validate_generated_lua_stubs.py`. Module structure change â†’ `tools/python.cmd tools/validate/validate_module_coverage.py`.
- `tools/python.cmd tools/audit/quality_report.py` outputs a module-by-module quality score. Interpret it as a routing signal: modules with score < 70 are candidates for an audit task.
...
- `tools/python.cmd tools/audit/doc_coverage.py` and `tools/python.cmd tools/audit/test_coverage.py` are health indicators, not gates. Run them before major releases or module audits to identify where quality investment is needed.
