...
```
lua.draw() callback:
  → Lua calls luna.graphics.drawImage(img, x, y)
...
Users provide WGSL fragment (or vertex+fragment) source via `luna.graphics.newShader`:
...
- User uniforms go at group 2+; set via `luna.graphics.sendShaderUniform(shader, name, value)`
...
```lua
-- Lua usage:
local c = luna.graphics.newCanvas(512, 512)
luna.graphics.setCanvas(c)
-- ... draw calls render to canvas texture ...
luna.graphics.setCanvas()               -- back to screen
luna.graphics.draw(c, 0, 0)
```
