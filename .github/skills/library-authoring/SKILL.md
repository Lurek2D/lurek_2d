...
- How to register a library in the test harness: open `tests/lua_tests.rs`, find the `lua_library_*` test list, and add a `lua_library_<name>` entry pointing to `tests/lua/library/test_library_<name>.lua`. This file is what `cargo test --test lua` runs.
