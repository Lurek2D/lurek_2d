...
- How to register a library in the test harness: open `tests/lua_reorg_tests.rs`, find the `lua_library_*` test list, and add a `lua_library_<name>` entry pointing to `tests/lua_reorg/library/test_library_<name>.lua`. This file is what `cargo test --test lua` runs.
- How library documentation is generated: LDoc comment tags on public functions in `init.lua` are read by `tools/docs/gen_lib_docs.py`. Run `tools/python.cmd tools/docs/gen_lib_docs.py` after any API change and check `docs/api/lureksome.md` for the updated output.
...
- When updating a library API: update `init.lua`, then update `example.lua` to call the new signature, then update `tests/lua_reorg/library/test_library_<name>.lua` to test the new behavior, then regenerate docs. All four in the same commit, no exceptions.
## Companion File Index
- None.

## References
- library/
- tests/lua_reorg/library/
