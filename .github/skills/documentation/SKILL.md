...
- Docs have a clear hierarchy: `docs/specs/<module>.md` is the canonical module contract (what the module does, its public API, and its invariants). `wiki/` pages are task-oriented guides for game authors. `docs/architecture/developer-workflow.md` is for contributor workflow. `docs/architecture/` is for high-level system design. Never duplicate content across tiers — link instead.
