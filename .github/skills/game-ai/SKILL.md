...
- How blackboards work: `src/patterns/blackboard.rs` is a typed key-value store with parent chain lookup. An agent's blackboard looks up its parent (world global blackboard) when a key is not found locally. Writes always stay local. Three value types: `Number(f64)`, `Bool(bool)`, `Text(String)`. Use blackboards to pass signals between agents and between AI subsystems — for example, write `"player_seen" = true` on the world blackboard so any agent can read it.
