...
## Domain Knowledge
- File type selection rule: system prompt = always-on global rules; agent = role identity, owned scope, and routing; skill = HOW-TO domain knowledge loaded on demand; prompt = a focused multi-step workflow for one recurring task. Mixing these purposes into the wrong file type breaks discoverability.
...
- Validation is mandatory before any CAG commit. Run `python tools/validate/cag_validate.py` for schema compliance, `python tools/audit/cag_link_check.py --strict` for file reference integrity.
