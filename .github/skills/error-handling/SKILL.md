...
## Domain Knowledge
- `src/runtime/error.rs` defines `EngineError` with distinct variants — never collapse distinct failure classes into a generic `String` variant. Each variant enables targeted recovery logic.
...
- Recoverable vs. fatal classification: `asset_not_found` is recoverable — log at warn, return `nil` or a fallback.
...
- Five distinct failure classes to treat separately: invalid content, missing required resource, unsupported runtime state, unavailable external resource, and engine defect. Each implies a different response — reject input, fallback, reject call, retry, and panic respectively.
