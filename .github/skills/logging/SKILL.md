...
- Luna2D log crate facade: `log::error!`, `warn!`, `info!`, `debug!`, `trace!`
...
Luna2D uses the `log` crate (`log = "0.4"`) as the logging facade. The concrete backend is `env_logger`, initialised at startup in `main.rs`.
...
```powershell
# Show all output from luna2d at info level and above
...
# Show debug from luna2d, but silence wgpu noise
$env:RUST_LOG = "luna2d=debug,wgpu_core=warn,wgpu_hal=warn"
...
`RUST_LOG` filter strings use the Rust module path. Luna2D module paths follow the pattern `luna2d::<module>`:

| Module | Filter string |
|--------|--------------|
| All engine code | `luna2d=debug` |
...
**Format rule**: `"<module>: <what happened> <values>"` — include the module name prefix when the log target is `luna2d` (all output mixed together): makes grep filtering easy.
...
cargo run -- demos/hello_world 2>&1 | Tee-Object logs/run.log
```

For production log files, consider adding a `WriteLogger` via the `fern` or `simplelog` crate alongside `env_logger` — but do not add new logging crates to Luna2D's `Cargo.toml` without a design decision.
