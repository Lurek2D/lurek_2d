...
## Domain Knowledge
- The five tier dependency graph is: Foundations â†’ Core Runtime â†’ Platform Services â†’ Feature Systems â†’ Edge/Integration. Any import that goes upward from a lower tier to a higher tier is a binding constraint violation.
- How to audit the current dependency graph: run `tools/python.cmd tools/audit/dep_graph.py` or use `cargo tree --edges all | grep -E "->.*src/"`. Compare the result against the tier table.
