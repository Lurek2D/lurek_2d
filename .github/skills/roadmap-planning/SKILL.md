...
- Quality gate alignment: the Done When criterion for any phase touching `src/` must include `cargo test` and `cargo clippy -- -D warnings`. Done When for any phase touching `docs/` must include `tools/python.cmd tools/gen_all_docs.py` with clean diff.
...
- When a phase changes a public API, it must include a sub-step for migration notes in `docs/specs/<module>.md`. These are not optional follow-up tasks â€” they are part of the phase.
