...
- Stale chunk detection: a chunk is stale when its `last_modified` timestamp is older than the source file that generates or governs it. Run `tools/python.cmd tools/audit/cag_link_check.py` as a proxy stale-link detector for CAG chunks.
...
- Coverage gap reporting: `tools/python.cmd tools/audit/doc_coverage.py --retrieval` outputs modules with no retrievable spec. Each gap is a source that must be added or a module that needs a spec created.
