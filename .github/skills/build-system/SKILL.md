...
- Output locations: `dev` → `target/debug/lurek2d.exe`, `release` → `target/release/lurek2d.exe`, final artifacts → `build/debug/` and `build/release/`. Packaging scripts read from `build/`, not `target/`.
...
- `tools/dev/parallel_cargo.py` wraps cargo for the workspace. Use it for clippy, fmt, test, and doc commands — it handles Cargo target threading and output formatting.
