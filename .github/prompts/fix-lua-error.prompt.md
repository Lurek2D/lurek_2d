...
4. `attempt to index a nil value (global 'lurek')` → `lurek` table not constructed; check `create_lua_vm()`
...
11. `src/lua_api/mod.rs` — `<module>_api::register(&lua, &lurek, state.clone())?`
