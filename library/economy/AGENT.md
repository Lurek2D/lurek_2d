...
| **Status**            | full                                                                                                                                                                                                                      |
| **Optional bindings** | `lurek.math.clamp` (delegated by `Resource:_clamp` when available), `lurek.serialize.toJson/fromJson` (recommended for save serialisation), `lurek.patterns.newEventBus` (transaction event bus from `manager:getEventBus()`) |
