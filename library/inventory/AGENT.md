...
| **Optional bindings** | `lurek.patterns.newEventBus` (change-event bus from `inv:getEventBus()`), `lurek.serial.toJson/fromJson` (save round-trip), `lurek.save.SaveManager` (recommended collector host), deepCopy helper (P4 lift target for `item:clone()`) |
## Summary
