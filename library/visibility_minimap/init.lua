-- @module library.visibility_minimap
--- @status full
--- Helper for syncing visibility masks into minimap fog and raw data layers.

local visibility_minimap = {}

local VisibilityMinimap = {}
VisibilityMinimap.__index = VisibilityMinimap
...
--- @param opts table
--- @return table
function visibility_minimap.new(opts)
    opts = opts or {}

    local visibility = opts.visibility
    if not visibility then
        error("visibility_minimap.new requires opts.visibility")
    end

    local width = opts.width
    local height = opts.height
    if not width or not height then
        error("visibility_minimap.new requires opts.width and opts.height")
    end

    local self = setmetatable({}, VisibilityMinimap)
...
--- @param opts table?
--- @return table
function VisibilityMinimap:syncFog(player, opts)
    opts = opts or {}
...
--- @param opts table?
--- @return table
function VisibilityMinimap:syncVisibleLayer(player, layer, opts)
    opts = opts or {}
    local hidden_value = opts.hidden_value or 0
...
--- @param opts table?
--- @return table
function VisibilityMinimap:syncActionLayer(player, layer, opts)
    opts = opts or {}
    local hidden_value = opts.hidden_value or 0
...
--- @return table
function VisibilityMinimap:getMinimap()
    return self.minimap
end

return visibility_minimap
