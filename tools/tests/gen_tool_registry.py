...
This script scans durable Python and script-based tools under `tools/`, extracts
their summaries, and rewrites:
...
def iter_tool_files() -> list[Path]:
    """Return durable script tools, excluding tests and cache folders."""
    files: list[Path] = []
    for path in sorted(TOOLS_DIR.rglob("*")):
        if not path.is_file() or path.suffix.lower() not in {".py", ".ps1", ".cmd", ".bat", ".sh", ".nsi"}:
            continue
...
def summary_from_docstring(path: Path) -> str:
    ...
    if path.suffix.lower() != ".py":
        for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if text and not text.startswith(("@echo", "set ")):
                return text
        return "Script tool without an inline summary."
...
def write_tools_readme(registry: list[dict[str, str]]) -> None:
    ...
    with readme_path.open("w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write("Single registry for durable Python, PowerShell, CMD, shell, and installer tooling under `tools/`.\n\n")
