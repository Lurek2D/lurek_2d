...
- `audit/audit_module.py` - audit_module.py â€” Lurek2D module quality audit tool. [durable; mcp:no]
- `audit/cag_coverage.py` - Report active Codex CAG coverage for roles, skills, contracts, and domains. [durable; mcp:no]
- `audit/cag_link_check.py` - Check links and structured References across active Codex skills and contracts. [durable; mcp:candidate]
...
- `audit/docs_quality.py` - Audit documentation source/generated ownership and coverage gates. [durable; mcp:no]
...
- `audit/example_coverage.py` - Cross-reference Lua examples against the lurek.* API and enforce 5-16 useful LOC per owner block. [durable; mcp:candidate]
...
- `audit/sprite_perf_gate.py` - Run release-mode sprite scenarios and enforce their checked-in performance baseline. [durable; mcp:no]
...
- `audit/ui_api_inventory.py` - Validate the canonical generated inventory for the public ``lurek.ui`` API. [durable; mcp:no]
- `audit/ui_boundary_check.py` - Enforce the UI/render ownership boundary described in the UI architecture spec. [durable; mcp:no]
...
- `demos/capture_demo_gifs.py` - Create preview.gif files for lurek_2d_content/games demos from existing screen.png. [targeted-maintenance; mcp:no]
- `demos/game_catalog.py` - Shared discovery and classification helpers for lurek_2d_content/games catalogs. [targeted-maintenance; mcp:no]
- `demos/gen_demo_catalog.py` - Generate lurek_2d_content/games/README.md and catalog.json from audited metadata. [targeted-maintenance; mcp:no]
...
- `demos/gen_game_readmes.py` - gen_game_readmes.py — Generate or repair README.md files for lurek_2d_content/games/ projects. [targeted-maintenance; mcp:no]
...
- `demos/smoke_sweep.py` - Smoke-sweep every playable project under lurek_2d_content/games/ and every single-file [targeted-maintenance; mcp:no]
...
- `docs/gen_lua_api_html_wrapper.py` - gen_lua_api_html_wrapper.py — Generate HTML index wrapper for Lua API docs-general in ../lurek_2D_pages/lua-docs/. [durable; mcp:no]
...
- `docs/gen_module_pages.py` - Generate temporary MkDocs input for Pages module guides from: [durable; mcp:no]
...
- `fix/spec_docstring_apply.py` - spec_docstring_apply.py -- deprecated legacy spec-to-docstring migration tool. [targeted-maintenance; mcp:no]
...
## /ui
- `ui/fix_layouts.py` - Fix and validate Lurek2D TOML layout files. [durable; mcp:no]
- `ui/snap_to_grid.py` - Snap every pixel-coordinate field in Lurek2D TOML layout files to a grid. [durable; mcp:no]

## /validate
- `validate/_cag_common.py` - Shared discovery and parsing helpers for the active Codex CAG layer. [internal; mcp:no]
- `validate/cag_validate.py` - Validate the active Codex CAG contracts, role registry, skills, and domains. [durable; mcp:candidate]
...
- `validate/validate_library.py` - Validate Lureksome libraries under lurek_2d_content/library/. [durable; mcp:candidate]
