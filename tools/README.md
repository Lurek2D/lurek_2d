...
Single registry for durable Python, PowerShell, CMD, shell, and installer tooling under `tools/`.
...
- `audit/docs_quality.py` [audit; durable; mcp:no] - Audit documentation source/generated ownership and coverage gates.
...
- `audit/sprite_perf_gate.py` [audit; durable; mcp:no] - Run release-mode sprite scenarios and enforce their checked-in performance baseline.
...
- `dev/headroom_runtime.py` [developer; developer-workflow; mcp:no] - Manage a repo-local Headroom runtime for Codex and MCP workflows.
...
- `dev/setup_python_path.ps1` [developer; developer-workflow; mcp:no] - param(
- `dev/test_fix_loop.py` [developer; developer-workflow; mcp:no] - Agent-friendly test-run / fix / re-run loop for cargo test.

## dist

- `dist/dist.ps1` [packaging; durable; mcp:no] - Requires -Version 5.1
- `dist/dist.sh` [packaging; durable; mcp:no] - !/usr/bin/env bash
- `dist/install.ps1` [packaging; durable; mcp:no] - Requires -Version 5.1
- `dist/install.sh` [packaging; durable; mcp:no] - !/usr/bin/env bash
- `dist/installer.nsi` [packaging; durable; mcp:no] - Lurek2D NSIS Installer Script
- `dist/pack.ps1` [packaging; durable; mcp:no] - tools/pack.ps1
...
- `dist/register_lurek_filetype.ps1` [packaging; durable; mcp:no] - param(
- `dist/release.ps1` [packaging; durable; mcp:no] - Requires -Version 5.1

## docs

- `docs/check_freshness.py` [generator; durable; mcp:no] - Regenerate docs and fail if generated outputs are stale.
...
- `docs/migrate_spec_manual_overlays.py` [generator; durable; mcp:no] - Migrate hand-written spec sections into docs/specs/manual overlays.
- `docs/module_registry.py` [generator; durable; mcp:no] - Load the canonical Lurek2D module registry.
...
- `fix/spec_docstring_apply.py` [maintenance; targeted-maintenance; mcp:no] - spec_docstring_apply.py -- deprecated legacy spec-to-docstring migration tool.
...
- `rag/build_index.py` [query; durable; mcp:no] - Build the local DuckDB RAG index for Lurek2D docs, code, tests, and Codex assets.
- `rag/context.py` [query; durable; mcp:no] - Build an agent-friendly context bundle from the local Lurek2D RAG index.
- `rag/contract.py` [query; durable; mcp:no] - Shared RAG contract constants loaded from ``rag_contract.json``.
- `rag/eval.py` [query; durable; mcp:no] - Evaluate local RAG recall against a prompt baseline for agent workflows.
- `rag/generate_contract.py` [query; durable; mcp:no] - Generate the checked-in RAG limit/watch contract from the canonical TOML.
- `rag/insights.py` [query; durable; mcp:no] - Run SQL insights and audits over the local DuckDB RAG index.
- `rag/query.py` [query; durable; mcp:candidate] - Query and read the local DuckDB RAG index for Lurek2D.
- `rag/read.py` [query; durable; mcp:no] - Read full chunks from the local Lurek2D RAG index by chunk id.
- `rag/state.py` [query; durable; mcp:no] - Shared state, configuration, and tokenization helpers for the local RAG.
...
- `gen_all_docs.py` [meta; durable; mcp:no] - Regenerate the full Lurek2D documentation pipeline in one command.
- `python.cmd` [meta; durable; mcp:no] - setlocal
...
- `ui/fix_layouts.py` [maintenance; durable; mcp:no] - Fix and validate Lurek2D TOML layout files.
