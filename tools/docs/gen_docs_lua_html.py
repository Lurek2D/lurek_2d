...
ICON_CANDIDATES = [
    WORKSPACE_ROOT / "lurek_2d_extension" / "vscode" / "media" / "lurek-logo.png",
...
def _render_sidebar(ordered: list[str], modules: dict[str, Any], current_module: str | None, site_base: str, current_page: str) -> str:
    ...
    for name in ordered:
        ...
        module_links.append(
            f'<a{active} href="{escape(site_base)}/{_module_href(name)}">lurek.{escape(namespace)}</a>'
        )
...
def _page_template(*, title: str, site_base: str, asset_prefix: str, sidebar_html: str, body_html: str, icon_path: str | None) -> str:
    icon_img = (
        f'<img class="topnav-logo-img" src="{escape(asset_prefix)}/{escape(icon_path)}" alt="">'
        if icon_path
        else '<div class="topnav-logo-orb" aria-hidden="true"></div>'
    )
...
<body>
  <header class="topnav">
    <div class="topnav-left">
      <a class="topnav-home" href="../index.html" title="Back to portal">
      ...
      <a class="topnav-brand" href="{escape(site_base)}/index.html">
        {icon_img}
        <span>Lua API</span>
      </a>
    </div>
  ...
  <div class="shell">
    <aside class="sidebar">
      <label class="search-box">
        <span class="sr-only">Search API</span>
        <input id="search" type="search" placeholder="Search module, class, function…">
      </label>
      <div class="search-filters">
        <select id="filter-module" aria-label="Filter by module"><option value="">All modules</option></select>
        <select id="filter-kind" aria-label="Filter by type"><option value="">All types</option></select>
      </div>
...
def _write_assets(output_dir: Path, search_index: list[dict[str, str]]) -> str | None:
    ...
    css = f"""/* ── Lurek Docs Engine — Lua theme ────────────────────── */
:root {{
  /* accent tokens — override these for Rust (orange) */
  --accent:        {LUREK_COLORS['ink']};
  --accent-mid:    {LUREK_COLORS['sky']};
  --accent-light:  #dbeeff;
  --accent-bg:     #f0f8ff;
  --accent-border: #bad8ef;
  --accent-glow:   rgba(127, 196, 231, 0.22);
  --accent-hover:  rgba(127, 196, 231, 0.13);
  --code-bg:       #0d1b2a;
  --code-fg:       #cee8ff;
  --code-border:   #1e3a52;
...
.topnav-home:hover {{ background: var(--accent-hover); color: var(--accent); }}
.topnav-sep {{ color: var(--border); font-size: 1.1rem; user-select: none; }}
.topnav-brand {{
  display: inline-flex;
  align-items: center;
  gap: 8px;
...
.topnav-brand:hover {{ background: var(--accent-hover); }}
.topnav-logo-img {{ width: 24px; height: 24px; border-radius: 5px; object-fit: cover; }}
.topnav-logo-orb {{
...
.topnav-xlink:hover {{ border-color: var(--accent-mid); color: var(--accent); background: var(--accent-hover); }}
...
.search-filters select:focus {{ border-color: var(--accent-mid); }}
...
.search-result {{ display: block; padding: 8px 10px; border-radius: 4px; }}
.search-result:hover {{ background: var(--accent-hover); }}
.search-result strong {{ display: block; font-size: 0.88rem; }}
.search-result span {{ display: block; color: var(--muted); font-size: 0.8rem; }}
.search-link {{
  display: block;
  margin: 4px 10px 8px;
  padding: 8px 10px;
...
.global-nav {{ padding: 0 8px; margin-bottom: 6px; }}
.global-link {{
  display: block;
  padding: 7px 10px;
  border-radius: var(--radius);
...
.global-link:hover {{ background: var(--accent-hover); color: var(--accent); }}
.global-link.active {{
...
.module-nav {{ padding: 0 8px; display: grid; gap: 1px; }}
.module-nav a {{
  display: flex;
...
.module-nav a:hover {{ background: var(--accent-hover); color: var(--accent); }}
.module-nav a.active {{
  background: var(--accent-light);
  color: var(--accent);
  font-weight: 600;
...
.nav-meta {{ font-size: 0.75rem; color: var(--muted); white-space: nowrap; }}
...
.compact-header .page-header h1 {{ font-size: clamp(1.3rem, 2.5vw, 1.7rem); }}
...
.item-row:hover {{ border-color: var(--accent-mid); box-shadow: var(--shadow-xs); }}
.item-row-main p {{ margin: 4px 0 0; color: var(--muted); font-size: 0.88rem; }}
.item-row-main h3 a {{ color: var(--accent); }}
.item-row-main h3 a:hover {{ text-decoration: underline; }}
.item-row-meta {{
  display: flex;
  flex-wrap: wrap;
  gap: 5px;
...
.item-row-meta span, .item-row-meta a {{
  padding: 2px 8px;
  border-radius: 999px;
  background: var(--accent-hover);
  border: 1px solid var(--accent-border);
...
.api-card:hover {{ border-color: var(--accent-mid); }}
.api-card p {{ margin: 6px 0 0; color: var(--muted); font-size: 0.88rem; }}
.api-card h3 a {{ color: var(--accent); }}
...
.api-card pre, .example-block pre {{
  margin-top: 10px;
  padding: 12px 14px;
  border-radius: var(--radius);
  background: var(--code-bg);
...
/* tokens / badges */
.stat-row, .api-meta, .crumbs, .token-row {{
  display: flex;
  flex-wrap: wrap;
  gap: 6px 10px;
  margin-top: 10px;
}}
.api-meta span, .stat-row span, .crumbs a, .crumbs span,
.token-row span, .token-row a {{
  padding: 3px 10px;
  border-radius: 999px;
  background: var(--accent-hover);
  border: 1px solid var(--accent-border);
...
.api-meta-header {{ margin-top: 14px; }}
...
.api-row .item-row-main {{ display: grid; gap: 3px; }}
...
.hero-copy {{ margin-top: 6px; color: var(--muted); }}
