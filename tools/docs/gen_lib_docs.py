...
* ``@see library.<name>.<fn>``        – cross-link into ``lureksome.md``
* ``@raise description``              – raised-error description
* ``@within section``                 – grouping label inside the module

Usage:
    python tools/docs/gen_lib_docs.py                  # generate docs/api/lureksome.{md,lua}
    python tools/docs/gen_lib_docs.py --module dialog  # generate API docs from one library
...
LUREK_NAMESPACES = {
    ...
    "image", "save", "serialize", "ecs", "mods", "filesystem", "pathfind",
...
def _render_see(see_list: list[str], from_libs_dir: bool) -> str:
    ...
    rel_lib = "../api/lureksome.md" if from_libs_dir else "lureksome.md"
    out = []
    for tgt in see_list:
...
def render_aggregate_md(modules: dict) -> str:
    out: list[str] = []
    out.append("# Lureksome Library API Reference")
    out.append("")
    ...
    out.append(
        ...
        "[`lurek.md`](../api/lurek.md). Cross-library references "
...
def scan_library() -> dict:
    ...
    for init_lua in sorted(LIB_DIR.glob("*/init.lua")):
        ...
        source = init_lua.read_text(encoding="utf-8", errors="replace")
...
def generate_per_module(modules: dict) -> int:
    ...
    print("Per-library library pages are retired; docs/api/lureksome.md is canonical.")
    return 0


# ── API-style outputs (docs/api/lureksome.md + docs/api/lureksome.lua) ───────

API_MD_PATH = REPO_ROOT / "docs" / "api" / "lureksome.md"
API_LUA_PATH = REPO_ROOT / "docs" / "api" / "lureksome.lua"
...
def render_api_md(modules: dict) -> str:
    """Render docs/api/lureksome.md in the same style as docs/api/lurek.md.
...
def render_luacats(modules: dict) -> str:
    ...
    for module_name in sorted(modules.keys()):
        ...
        for nested in sorted(nested_tables, key=lambda value: (value.count("."), value)):
            ...
            out.append(f"{stub_prefix}.{nested} = {{}}")
            out.append("")
        ...
        for fn in info["functions"]:
            ...
            if ":" in raw_name:
                ...
                args = fn["args"]
                out.append(f"function {stub_prefix}.{short}({args}) end")
            out.append("")
...
def generate_api_files(modules: dict) -> None:
    """Generate docs/api/lureksome.md and docs/api/lureksome.lua."""
...
def main() -> int:
    ...
    parser.add_argument("--api", action="store_true",
                        help="Generate docs/api/lureksome.md + docs/api/lureksome.lua")
    ...
    print("Generating API docs (lureksome.md + lureksome.lua) …")
