...
- **`collect_docs.py`**: collect_docs.py — Lurek2D rich structured API docs-general collector.
- **`gen_docs_lua.py`**: gen_docs_lua.py -- Generate Lua API reference from logs/data/lua_api_data.json.
- **`gen_docs_lua_html.py`**: Generate compatibility redirects for legacy ``/lua-docs`` URLs.
- **`gen_docs_rust.py`**: gen_docs_rust.py — Generate compact inline Rust API reference from logs/data/rust_api_data.json.
- **`gen_engine_docs.py`**: gen_engine_docs.py — Generate per-module docs-general for Lurek2D Rust engine source.
...
- **`gen_lua_api_html_wrapper.py`**: gen_lua_api_html_wrapper.py — Generate HTML index wrapper for Lua API docs-general in pages/lua-docs/.
- **`gen_lua_binding_reports.py`**: Generate source-derived Lua binding snapshots from src/lua_api/*.rs.
- **`gen_lua_dev_docs.py`**: gen_lua_dev_docs.py — Generate Lua developer docs-general from lua_api *.rs files.
...
- **`gen_module_pages.py`**: Generate per-module MkDocs pages in docs/lua/ from:
...
- **`gen_test_docs.py`**: gen_test_docs.py — Generate human-readable test docs-general for Lurek2D.
