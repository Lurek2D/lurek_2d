...
class ApiDataService {
    ...
    searchFunctions(query) {
        ...
        for (const fn of this.allFunctions.values()) {
            if (fn.fullPath.toLowerCase().includes(lower) ||
    ...
    upsertParam(func, name, type, description) {
        ...
        if (existing) {
            ...
            const isOptional = description.toLowerCase().startsWith("optional") ||
...
function inferReturnType(returns) {
    ...
    if (lower === "nil" || lower === "none")
        return "nil";
    if (lower.startsWith("number") || lower.startsWith("`number`"))
        return "number";
    if (lower.startsWith("string") || lower.startsWith("`string`"))
        return "string";
    if (lower.startsWith("boolean") || lower.startsWith("`boolean`"))
        return "boolean";
    if (lower.startsWith("table") || lower.startsWith("`table`"))
        return "table";
    if (lower.startsWith("integer") || lower.startsWith("`integer`"))
        return "number";
    if (lower.startsWith("function") || lower.startsWith("`function`"))
        return "function";
    if (lower.includes(","))
        return "multiple";
    return returns;
...
function inferTypeFromDesc(desc) {
    ...
    if (lower.includes("boolean"))
        return "boolean";
    if (lower.includes("string") || lower.includes("name"))
        return "string";
    ...
    if (lower.includes("table"))
        return "table";
    if (lower.includes("function") || lower.includes("callback"))
        return "function";
    if (lower.includes("draw mode") || lower.includes("'fill'") || lower.includes("'line'"))
        return "DrawMode";
    if (lower.includes("blend mode"))
        return "BlendMode";
    return "any";
}
