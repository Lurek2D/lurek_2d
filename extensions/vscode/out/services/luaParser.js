...
exports.LuaDocumentAnalyzer = exports.TokenType = void 0;
var TokenType;
(function (TokenType) {
...
class LuaDocumentAnalyzer {
        ...
        while (pos < len) {
            ...
            if (isDigit(ch) || (ch === "." && pos + 1 < len && isDigit(text[pos + 1]))) {
                ...
                if (ch === "0" && end + 1 < len && (text[end + 1] === "x" || text[end + 1] === "X")) {
                    ...
                    while (end < len && isHexDigit(text[end]))
                        end++;
                }
                else if (ch === "0" && end + 1 < len && (text[end + 1] === "b" || text[end + 1] === "B")) {
                    end += 2;
                    while (end < len && (text[end] === "0" || text[end] === "1"))
                        end++;
                }
                else {
                    while (end < len && isDigit(text[end]))
                        end++;
                    if (end < len && text[end] === ".") {
                        end++;
                        while (end < len && isDigit(text[end]))
            ...
            if (isIdentStart(ch)) {
                const startCol = col;
                let end = pos + 1;
                while (end < len && isIdentPart(text[end]))
                    end++;
                const value = text.slice(pos, end);
        ...
        while (i < toks.length && peek()?.type !== TokenType.EOF) {
            ...
            if (match(TokenType.Keyword, "function")) {
                ...
                if (peek()?.type === TokenType.Identifier) {
                    ...
                    if (fullName.startsWith("lurek.") && LUNA_CALLBACKS.has(shortName)) {
                        callbacks.push(sym);
                    }
                    // Add parameters
                    for (const pName of params.names) {
    ...
    getWordAtPosition(text, line, column) {
        ...
        let start = column;
        let end = column;
        while (start > 0 && isIdentPart(lineText[start - 1]))
            start--;
        while (end < lineText.length && isIdentPart(lineText[end]))
            end++;
    ...
    isInsideComment(text, line, column) {
        ...
        for (const tok of tokens) {
            ...
            const endLine = tok.line + countNewlines(tok.value);
            if (tok.line === endLine) {
                if (tok.line === line && column >= tok.column)
                    return true;
            }
            else {
                if (line > tok.line && line < endLine)
...
exports.LuaDocumentAnalyzer = LuaDocumentAnalyzer;
