...
exports.ApiCoverageEditor = void 0;
...
// ── Editor class ──────────────────────────────────────────────────────────────
class ApiCoverageEditor extends shared_js_1.WebviewEditor {
    covData = null;
    scanning = false;
    static open(context) {
    ...
    runScan() {
        ...
        cp.exec(cmd, { cwd: ws }, (err, stdout, stderr) => {
            this.scanning = false;
            if (err) {
    ...
    openTestFile(moduleName) {
        const ws = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!ws)
            return;
        const testPath = path.join(ws, "tests", "lua", "unit", `test_${moduleName}.lua`);
        if (fs.existsSync(testPath)) {
            vscode.workspace.openTextDocument(testPath).then((doc) => vscode.window.showTextDocument(doc));
        }
    }
    // ── HTML ────────────────────────────────────────────────────────────────────
    getHtml() {
        const nonce = (0, shared_js_1.getNonce)();
        const css = `
...
`;
        return (0, shared_js_1.wrapHtml)(nonce, "API Test Coverage", css, body, scripts);
    }
}
exports.ApiCoverageEditor = ApiCoverageEditor;
//# sourceMappingURL=apiCoverageEditor.js.map