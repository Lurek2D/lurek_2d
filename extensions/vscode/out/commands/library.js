...
exports.registerLibraryCommands = registerLibraryCommands;
...
function registerLibraryCommands(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.library.browse", async () => {
        ...
        const picked = await vscode.window.showQuickPick(patterns.map((p) => ({
            label: p.name,
        ...
        if (action.label === "Preview") {
            ...
            if (fs.existsSync(dest)) {
                const overwrite = await vscode.window.showWarningMessage(`libs/${picked.label}.lua already exists. Overwrite?`, "Yes", "No");
                if (overwrite !== "Yes") {
                    return;
                }
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.library.insertSnippet", async () => {
        const categories = getCategories();
        const catPick = await vscode.window.showQuickPick(categories.map((c) => ({ label: c })), { placeHolder: "Choose snippet category" });
        if (!catPick) {
            return;
        }
        const snippetsInCat = SNIPPETS.filter((s) => s.category === catPick.label);
        const snippetPick = await vscode.window.showQuickPick(snippetsInCat.map((s) => ({ label: s.label, snippet: s })), { placeHolder: `${catPick.label} snippets` });
        if (!snippetPick) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.library.newPattern", async () => {
        const editor = vscode.window.activeTextEditor;
        ...
        if (fs.existsSync(destFile)) {
            const overwrite = await vscode.window.showWarningMessage(`Pattern "${name}" already exists. Overwrite?`, "Yes", "No");
            if (overwrite !== "Yes") {
                return;
            }
        }
