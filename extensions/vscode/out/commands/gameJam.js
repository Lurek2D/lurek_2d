...
exports.registerGameJamCommands = registerGameJamCommands;
...
function registerGameJamCommands(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.gameJam.quickStart", async () => {
        // Pick template
        const picked = await vscode.window.showQuickPick(TEMPLATES.map((t) => ({ label: t.label, description: t.description, template: t })), { placeHolder: "Choose a game template" });
        if (!picked) {
            return;
        }
        ...
        fs.writeFileSync(path.join(projectDir, "assets", "README.md"), "# Assets\n\nPlace your game assets (images, sounds, fonts) in this folder.\n", "utf-8");
        // Open the folder
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.gameJam.addModule", async () => {
        const picked = await vscode.window.showQuickPick(MODULES.map((m) => ({ label: m.label, description: m.description, module: m })), { placeHolder: "Choose a module to add" });
        if (!picked) {
            return;
        }
        ...
        if (fs.existsSync(destFile)) {
            const overwrite = await vscode.window.showWarningMessage(`libs/${mod.patternFile} already exists. Overwrite?`, "Yes", "No");
            if (overwrite !== "Yes") {
                return;
            }
        }
    ...
    // ── lurek.gameJam.timer ───────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("lurek.gameJam.timer", async () => {
