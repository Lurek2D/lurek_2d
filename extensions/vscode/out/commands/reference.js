...
exports.openApiDocs = openApiDocs;
...
async function browseApi() {
    ...
    if (!fs.existsSync(apiPath)) {
        vscode.window.showWarningMessage("API reference not found. Run 'python tools/gen_lua_api.py' to generate it.");
        return;
    }
...
async function openApiDocs() {
    ...
    const apiPath = path.join(root, "docs", "API", "lua_api_reference_generated.md");
    if (!fs.existsSync(apiPath)) {
        vscode.window.showWarningMessage("API reference not found. Run 'python tools/gen_lua_api.py' first.");
        return;
    }
    const doc = await vscode.workspace.openTextDocument(apiPath);
