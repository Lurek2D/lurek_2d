...
function register(context, apiData) {
    ...
    const docSymbolProvider = vscode.languages.registerDocumentSymbolProvider(LUA_SELECTOR, {
        provideDocumentSymbols(document) {
            ...
            try {
                ...
                for (const req of info.requires) {
                    ...
                    symbols.push(new vscode.DocumentSymbol(req.localName, `require("${req.modulePath}")`, vscode.SymbolKind.Module, range, range));
                }
                ...
                for (const sym of info.symbols) {
                    ...
                    if (sym.kind === "function") {
                        ...
                        const docSym = new vscode.DocumentSymbol(sym.name, "table", vscode.SymbolKind.Object, range, selRange);
            ...
            for (const fileUri of files) {
                try {
                    ...
                    for (const sym of info.symbols) {
                        ...
                        const location = new vscode.Location(fileUri, new vscode.Position(sym.line, sym.column));
                        results.push(new vscode.SymbolInformation(sym.name, kind, sym.scope ?? "", location));
                    }
                }
                catch { /* skip */ }
            }
...
function fallbackDocumentSymbols(lines) {
    const symbols = [];
    for (let i = 0; i < lines.length; i++) {
        ...
        if (requireMatch) {
            ...
            symbols.push(new vscode.DocumentSymbol(requireMatch[1], `require("${requireMatch[2]}")`, vscode.SymbolKind.Module, range, range));
            continue;
        }
    }
    return symbols;
