...
function scanDocument(document) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const classDefMatch = line.match(/\b(?:local\s+)?(\w+)\s*=\s*\{\s*\}/);
        if (classDefMatch) {
        ...
        const methodMatch = line.match(/\bfunction\s+(\w+):(\w+)\s*\(/);
        if (methodMatch) {
        ...
        const instanceMatch = line.match(/\blocal\s+(\w+)\s*=\s*(\w+)[:.](new|create)\s*\(/);
        if (instanceMatch) {
        ...
        const setmetaMatch = line.match(/\blocal\s+(\w+)\s*=\s*setmetatable\s*\([^,]*,\s*(\w+)\s*\)/) ??
...
function getMethodsForVar(varName, position, varTypes, classes) {
    ...
    const factoryVar = varTypes.find((v) => v.varName === varName && v.line < position.line);
    if (factoryVar) {
        const typeInfo = Object.values(FACTORY_TYPES).find((t) => t.typeName === factoryVar.typeName);
        if (typeInfo)
            return typeInfo.methods;
    }
    // Check OOP class instances
    for (const cls of classes) {
        const instance = cls.instances.find((inst) => inst.varName === varName && inst.line < position.line);
...
function register(context, _apiData) {
    ...
    const colonProvider = vscode.languages.registerCompletionItemProvider(LUA_SELECTOR, {
    ...
    const dotProvider = vscode.languages.registerCompletionItemProvider(LUA_SELECTOR, {
        provideCompletionItems(document, position) {
            const lineText = document.lineAt(position).text;
            const before = lineText.substring(0, position.character);
