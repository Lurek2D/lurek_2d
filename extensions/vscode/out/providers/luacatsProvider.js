...
function parseDocument(document) {
    ...
    const registry = { version: document.version, classes, instanceTypes };
    docRegistries.set(key, registry);
    return registry;
}
// ── Resolve the class name for a variable at a position ──────
function resolveClassForWord(document, word, registry) {
    // Direct match (hovering on class name itself)
    if (registry.classes.has(word))
        return registry.classes.get(word);
    // Instance lookup
    const className = registry.instanceTypes.get(word);
    if (className)
        return registry.classes.get(className);
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider(LUA_SELECTOR, {
        provideCompletionItems(document, position) {
            ...
            for (const method of cls.methods) {
                ...
                item.insertText = new vscode.SnippetString(method.params
                    ? `${method.name}(\${1})`
                    : `${method.name}()`);
                items.push(item);
            }
...
//# sourceMappingURL=luacatsProvider.js.map