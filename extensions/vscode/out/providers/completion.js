...
const CONSTRUCTOR_RETURN_TYPES = {
...
function buildStdlibSnippet(fn) {
...
function sortPriority(fn) {
    if (FREQUENT_FUNCTIONS.has(fn.name))
        return "0" + fn.name;
    if (fn.deprecated)
        return "2" + fn.name;
    return "1" + fn.name;
...
function inferObjectType(document, position, objName, apiData) {
    const lines = document.getText().split("\n");
    for (let lineIdx = position.line; lineIdx >= 0; lineIdx--) {
        const assignMatch = lines[lineIdx].match(new RegExp(`(?:local\\s+)?${escapeRegex(objName)}\\s*=\\s*(lurek\\.[\\w.]+)\\s*\\(`));
        if (assignMatch) {
    ...
    if (apiData.getMethods(capitalName).length > 0)
        return capitalName;
    return undefined;
}
...
function register(context, apiData) {
    ...
    const mainProvider = vscode.languages.registerCompletionItemProvider(LUA_SELECTOR, {
        provideCompletionItems(document, position) {
            ...
            if (moduleFuncMatch) {
                ...
                return funcs
                    ...
                    .map((fn, idx) => {
                    const item = new vscode.CompletionItem(fn.name, fn.isMethod ? vscode.CompletionItemKind.Method : vscode.CompletionItemKind.Function);
            ...
            if (moduleMatch) {
                ...
                for (const modName of apiData.getModuleNames()) {
                    if (partial && !modName.toLowerCase().startsWith(partial))
                        continue;
                ...
                for (const cb of apiData.getCallbacks()) {
                    if (partial && !cb.name.toLowerCase().startsWith(partial))
                        continue;
            ...
            if (stdlibMatch) {
                ...
                if (libFuncs.length === 0)
                    return undefined;
                return libFuncs
                    .filter(fn => !partial || fn.name.toLowerCase().startsWith(partial))
                    .map(fn => {
                    const isConst = fn.parameters.length === 0 && !fn.signature.includes("(");
                    const item = new vscode.CompletionItem(fn.name, isConst ? vscode.CompletionItemKind.Constant : vscode.CompletionItemKind.Function);
                    item.detail = fn.signature;
                    item.documentation = buildFunctionDoc(fn);
                    if (!isConst) {
            ...
            if (colonMatch) {
                ...
                try {
                    ...
                    for (const cls of classes) {
                        if (cls.name.toLowerCase() !== objName.toLowerCase() && cls.name !== objectType)
                            continue;
                        for (const method of cls.methods) {
                            if (partial && !method.name.toLowerCase().startsWith(partial))
                                continue;
            ...
            if (/(?:^|[\s=(,{;])[\w]*$/.test(before) && !before.match(/\.\w*$/) && !before.match(/:\w*$/)) {
                ...
                for (const g of LUA_BUILTINS) {
                    ...
                    if (g.snippet)
                        item.insertText = new vscode.SnippetString(g.snippet);
                ...
                // lurek namespace
                const lunaItem = new vscode.CompletionItem("lurek", vscode.CompletionItemKind.Module);
                ...
                try {
                    ...
                    for (const sym of info.symbols) {
                        if (sym.isLocal || sym.kind === "parameter")
                            continue;
                        if (seen.has(sym.name))
                            continue;
                        seen.add(sym.name);
    ...
    const stringProvider = vscode.languages.registerCompletionItemProvider(LUA_SELECTOR, {
        provideCompletionItems(document, position) {
    ...
    const requireProvider = vscode.languages.registerCompletionItemProvider(LUA_SELECTOR, {
        async provideCompletionItems(document, position) {
            ...
            try {
                ...
                for (const fileUri of luaFiles) {
                    ...
                    if (partial && !dotPath.toLowerCase().startsWith(partial.toLowerCase()))
                        continue;
