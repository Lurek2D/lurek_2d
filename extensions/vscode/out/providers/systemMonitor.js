...
exports.openSystemMonitor = openSystemMonitor;
...
[PSCustomObject]@{
  ...
  NetRecvBps = [long]$net.Sum[1]
    ...
    try {
        const { stdout: gpuOut } = await execFileAsync("nvidia-smi", ["--query-gpu=utilization.gpu,memory.used", "--format=csv,noheader,nounits"], { timeout: 2000 });
...
function startPolling() {
    ...
    _pollInterval = setInterval(async () => {
        ...
        if (_history.length > MAX_SAMPLES)
            _history.shift();
        if (_panel?.visible) {
