...
const EASING_FUNCTIONS = {
    ...
    inBounce: {
        fn: (t) => {
            ...
            if (r < 2 / 2.75) {
                const u = r - 1.5 / 2.75;
                return 1 - (7.5625 * u * u + 0.75);
            }
            if (r < 2.5 / 2.75) {
                const u = r - 2.25 / 2.75;
                return 1 - (7.5625 * u * u + 0.9375);
            }
            const u = r - 2.625 / 2.75;
            return 1 - (7.5625 * u * u + 0.984375);
        },
...
function renderEasingChart(name, fn) {
    const W = 20;
    const H = 8;
    const samples = [];
    for (let i = 0; i <= W; i++) {
...
function register(context, apiData) {
    ...
    const apiHover = vscode.languages.registerHoverProvider(LUA_SELECTOR, {
        provideHover(document, position) {
            // ── A: Luna2D API functions (lurek.module.func) ──
            ...
            if (funcRange) {
                ...
                if (fn)
                    return new vscode.Hover(buildRichHover(fn), funcRange);
            }
            // ── D: Luna callbacks (lurek.callback) ──
            ...
            if (stdlibRange) {
                ...
                if (match)
                    return new vscode.Hover(buildRichHover(match), stdlibRange);
            }
                ...
                try {
                    ...
                    for (const sym of info.symbols) {
                        if (sym.name === word && sym.line <= position.line) {
                            ...
                            if (sym.scope)
                                md.appendMarkdown(` · scope: \`${sym.scope}\``);
                            md.isTrusted = true;
    ...
    const physicsGravityHover = vscode.languages.registerHoverProvider(LUA_SELECTOR, {
        provideHover(document, position) {
            ...
            if (!/lurek\.physics\.newWorld/.test(line))
                return undefined;
            ...
            const md = new vscode.MarkdownString(`**Gravity Y = ${num} px/s²**\n\n` +
