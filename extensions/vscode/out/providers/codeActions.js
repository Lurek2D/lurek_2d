...
function register(context, apiData) {
    const provider = vscode.languages.registerCodeActionsProvider(LUA_SELECTOR, {
...
function createRemoveUnusedRequire(document, diag) {
    const action = new vscode.CodeAction('Remove unused require', vscode.CodeActionKind.QuickFix);
...
function createGenerateCallbacks(document, diag) {
    ...
    if (missing.length === 0)
        return [];
    const action = new vscode.CodeAction('Generate Luna2D callbacks', vscode.CodeActionKind.QuickFix);
    action.edit = new vscode.WorkspaceEdit();
    const stubs = [];
...
function createConvertColorRange(document, diag) {
    ...
    const action = new vscode.CodeAction('Convert to 0-1 color range', vscode.CodeActionKind.QuickFix);
    action.edit = new vscode.WorkspaceEdit();
...
function createExtractFunction(document, range) {
    const action = new vscode.CodeAction('Extract to local function', vscode.CodeActionKind.RefactorExtract);
    action.edit = new vscode.WorkspaceEdit();
...
function createConvertToLocal(document, line, match) {
    const action = new vscode.CodeAction('Convert to local variable', vscode.CodeActionKind.QuickFix);
    action.edit = new vscode.WorkspaceEdit();
...
function createWrapRequirePcall(document, line) {
    ...
    const action = new vscode.CodeAction('Wrap require in pcall', vscode.CodeActionKind.QuickFix);
    action.edit = new vscode.WorkspaceEdit();
...
function createInlineVariable(document, line, match) {
    const action = new vscode.CodeAction(`Inline variable '${match[2]}'`, vscode.CodeActionKind.RefactorInline);
    action.edit = new vscode.WorkspaceEdit();
    ...
    action.edit.replace(document.uri, document.lineAt(line).range, `${indent}-- TODO: inline '${match[2]}' = ${rhs}`);
    return action;
}
...
function tryCreateStateMapConversion(document, startLine) {
    ...
    for (let i = startLine; i < Math.min(startLine + 40, document.lineCount); i++) {
        ...
        if (document.lineAt(i).text.trimStart() === 'end')
            break;
    }
    const cases = [];
    let i = 0;
    ...
    const action = new vscode.CodeAction(`Convert if/elseif chain to state-map (${mapName})`, vscode.CodeActionKind.RefactorRewrite);
    action.edit = new vscode.WorkspaceEdit();
    const replaceRange = new vscode.Range(startLine, 0, startLine + lines.length - 1, document.lineAt(startLine + lines.length - 1).range.end.character);
...
function createAddTypeAnnotation(document, line, varName) {
    const action = new vscode.CodeAction(`Add ---@type annotation for '${varName}'`, vscode.CodeActionKind.RefactorRewrite);
    action.edit = new vscode.WorkspaceEdit();
...
function createGenerateTostring(document, line, className) {
    const action = new vscode.CodeAction(`Generate __tostring for ${className}`, vscode.CodeActionKind.QuickFix);
    action.edit = new vscode.WorkspaceEdit();
    const indent = (document.lineAt(line).text.match(/^(\s*)/) ?? ['', ''])[1];
