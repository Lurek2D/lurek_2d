...
function register(context, apiData) {
    context.subscriptions.push(vscode.languages.registerDocumentSemanticTokensProvider(LUA_SELECTOR, {
...
// ── Core computation ─────────────────────────────────────────
function computeSemanticTokens(document, apiData) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        switch (tok.type) {
            case luaParser_js_1.TokenType.Keyword:
                pushToken(builder, tok, 'keyword', []);
                break;
            case luaParser_js_1.TokenType.Comment:
                classifyComment(builder, tok);
                break;
            case luaParser_js_1.TokenType.String:
                classifyString(builder, tok, enumValues);
                break;
            case luaParser_js_1.TokenType.Number:
                pushToken(builder, tok, 'number', []);
                break;
            case luaParser_js_1.TokenType.Operator:
                pushToken(builder, tok, 'operator', []);
                break;
            case luaParser_js_1.TokenType.Identifier:
                classifyIdentifier(builder, tok, prevCode, nextCode, tokens, i, paramNames, localNames, localFuncNames, declaredLines, lunaFuncNames, deprecatedFuncs, apiData);
                break;
        }
    }
...
function classifyIdentifier(builder, tok, prevCode, nextCode, allTokens, idx, paramNames, localNames, localFuncNames, declaredLines, lunaFuncNames, deprecatedFuncs, apiData) {
    ...
    if (name === 'luna') {
        ...
        if (nextCode?.value === '.') {
            ...
            if (afterDot?.type === luaParser_js_1.TokenType.Identifier && LUNA_CALLBACKS.has(afterDot.value)) {
    ...
    if (prevCode?.type === luaParser_js_1.TokenType.Keyword && prevCode.value === 'function') {
...
function classifyComment(builder, tok) {
...
function classifyString(builder, tok, enumValues) {
...
function findPrevNonWhitespace(tokens, idx) {
    for (let i = idx - 1; i >= 0; i--) {
        if (tokens[i].type !== luaParser_js_1.TokenType.Whitespace)
            return tokens[i];
    }
    return undefined;
}
...
function findNextNonWhitespaceAfter(tokens, idx, skip) {
    let found = 0;
    for (let i = idx + 1; i < tokens.length; i++) {
        if (tokens[i].type !== luaParser_js_1.TokenType.Whitespace) {
            found++;
...
function getIdentifierChain(tokens, idx) {
    ...
    while (i >= 0) {
        ...
        // Expect `.` or `:`
        if (tokens[i].type === luaParser_js_1.TokenType.Punctuation && (tokens[i].value === '.' || tokens[i].value === ':')) {
            const sep = tokens[i].value;
            i--;
            // Skip whitespace
            while (i >= 0 && tokens[i].type === luaParser_js_1.TokenType.Whitespace)
                i--;
            if (i >= 0 && tokens[i].type === luaParser_js_1.TokenType.Identifier) {
