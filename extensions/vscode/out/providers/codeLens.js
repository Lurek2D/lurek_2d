...
class LuaCodeLensProvider {
    ...
    provideCodeLenses(document) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const lunaCallbackMatch = funcName.match(/^lurek\.(\w+)$/);
...
function buildVariableInspector(context) {
    const barItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 95);
...
function register(context, _apiData) {
    ...
    context.subscriptions.push(vscode.languages.registerCodeLensProvider(LUA_SELECTOR, provider));
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.codelens.findRefs", async (_uri, pos) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lurek.codeLens.toggle", () => {
        ...
        vscode.window.showInformationMessage(`Luna Code Lens ${!current ? "enabled" : "disabled"}`);
    }));
}
