...
function register(context, _apiData) {
    ...
    context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider(LUA_SELECTOR, provider), vscode.languages.registerDocumentRangeFormattingEditProvider(LUA_SELECTOR, provider));
}
// ── Core formatting ──────────────────────────────────────────
...
function formatLines(lines, indentStr) {
    ...
    const mlState = { inBlockComment: false, inLongString: false, closingPattern: '' };
...
// ── Indent computation ───────────────────────────────────────
function computeDeIndent(trimmed) {
    const stripped = stripStrings(trimmed);
    return computeDeIndentFromStripped(stripped);
}
function computeDeIndentFromStripped(stripped) {
    let delta = 0;
...
function computeAddIndent(trimmed) {
    const stripped = stripStrings(trimmed);
    return computeAddIndentFromStripped(stripped);
}
function computeAddIndentFromStripped(stripped) {
 ...
 * - `{ a = 1, b = 2 }`
 */
function isSingleLineBlock(stripped) {
...
function detectLongBracketOpen(line) {
    // --[[ or --[=[ block comment
...
function countLongBracketLevel(text, pos) {
    if (text[pos] !== '[')
        return -1;
    let level = 0;
    let p = pos + 1;
...
// ── Helpers ──────────────────────────────────────────────────
function makeIndent(indentStr, level) {
