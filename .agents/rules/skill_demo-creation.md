...
- How to write `main.lua` for a demo: start with `function lurek.load()` for one-time setup, `function lurek.update(dt)` for per-frame logic, and `function lurek.draw()` for all draw calls. The demo must not use `require` on library modules unless demonstrating that specific library — bare `lurek.*` calls only.
