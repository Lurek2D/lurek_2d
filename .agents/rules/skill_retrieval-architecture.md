...
- Local RAG System: Located at `tools/rag/rag_index.db`, it uses SQLite FTS5 for zero-dependency retrieval. It natively parses `logs/data/lua_api_data.json` to inject rich `type='api'` structured chunks. Configuration (chunk sizes, target paths, BM25 ranking weights) is maintained in `tools/rag/rag.toml`.
- RAG UX: The VS Code extension automatically maintains the index on file saves via a filesystem watcher. Human developers can search via the `Lurek2D: Search Knowledge Base (RAG)` Webview panel, while agents interact via the `lurek2d.ragSearch` and `lurek2d.ragBuildIndex` MCP tools.
