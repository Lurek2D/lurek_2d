...
- Generated files (`docs/api/lurek.md`, `docs/api/lurek.lua`, `docs/api/lureksome.md`) should be indexed at lower priority than their sources (`src/lua_api/*_api.rs`, `library/*/init.lua`). When both the source and the generated output match a query, prefer the source — it is what gets edited.
