...
local function _wrap()
    local _old_r = lurek.ready
    lurek.ready = function(...)
        local ret = nil
        if type(_old_r) == 'function' then ret = _old_r(...) end
        if lurek.event and lurek.event.quit then lurek.event.quit() end
        return ret
    end
    local _old_p = lurek.process
    lurek.process = function(...)
        local ret = nil
        if type(_old_p) == 'function' then ret = _old_p(...) end
        if lurek.event and lurek.event.quit then lurek.event.quit() end
        return ret
    end
end
