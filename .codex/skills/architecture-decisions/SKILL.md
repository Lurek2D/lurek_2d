...
- Required fields per option: name, approach summary in 2â€“3 sentences, fit with repo architecture, migration cost, validation path, long-term maintenance burden. Every option needs all six fields; missing fields signal the option is under-researched.
...
- Decision output format: option name chosen, one sentence per rejected option naming the decisive reason it lost, the main residual risk of the chosen option, and the next owner. The handoff must be actionable â€” "implement in `src/<module>/`" is better than "proceed with engineering".
