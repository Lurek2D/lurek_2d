...
- RenderPass count rule: one RenderPass per phase. Adding a mid-frame RenderPass for a single effect is almost always wrong â€” batch the draw into the existing transparent pass instead.
...
- Texture format convention: all game textures must use `wgpu::TextureFormat::Rgba8UnormSrgb` unless explicitly documented otherwise. A format mismatch between texture and bind-group layout produces black output on some GPUs and a validation error on others â€” both symptoms are confusing to diagnose.
