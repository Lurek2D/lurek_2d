...
impl ErrorScreen {
    /// Build error screen model from plain text where first non-empty line becomes the title and remainder is body.
    pub fn from_error(msg: &str) -> Self {
        ...
        let title = if let Some(idx) = title_idx {
        ...
        let body = if let Some(idx) = title_idx {
            if idx + 1 < lines.len() {
                lines[idx + 1..].join("\n")
            } else {
                String::new()
