# `app` â€” Agent Reference

| Property       | Value                                                        |
|----------------|--------------------------------------------------------------|
| **Tier**       | Baseline â€” application lifecycle substrate                   |
| **Status**     | Implemented â€” Full                                           |
| **Lua API**    | â€” (no dedicated `lurek.*` namespace; lifecycle callbacks are `lurek.init`, `lurek.ready`, `lurek.process`, `lurek.render`, etc.) |
| **Source**     | `src/app/`                                                   |
...
The `RunState` machine (`Running â†’ Error â†’ Restarting`) handles hot-reload and
...
| `app_winit.rs`       | Legacy / dead file â€” not declared in `mod.rs`; preserved for reference. |
| `debug_overlay.rs`   | `DebugOverlay` â€” FPS and draw-call counter rendered top-right.          |
| `error_screen.rs`    | `ErrorScreen` â€” blue error display with word-wrap and traceback text.   |
