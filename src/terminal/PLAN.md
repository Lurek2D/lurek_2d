...
Ten plik przedstawia plan rozwoju modułu `terminal` po analizie istniejącego kodu źródłowego i zrealizowanych zmian.

## Status realizacji (DONE / TODO)

### DONE

* MUST: skróty klawiaturowe terminala (clipboard + edycja tekstu) — zrealizowane.
* MUST: przeniesienie helperów tekstowych do `text_utils` — zrealizowane.
* SHOULD: rozszerzenie testów widgetów terminalowych (focus/zagnieżdżenia) — zrealizowane.

### TODO

* SHOULD: ograniczenie klonowania bufora siatki przy kompozycji.
* COULD: feature-flag modułu `terminal`.

## Stan obecny

W module wprowadzono już następujące funkcjonalności:
