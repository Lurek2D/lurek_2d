//! Bidirectional A★ for halved search space on large navigation grids.
//!
//! This module is part of Lurek2D's `pathfind` subsystem and implements
//! `bidirectional_astar()`, which alternates forward/backward node expansions
//! to find a meeting point and splices the two partial paths into one route.
//!
...
/// A node in the open-set priority queue, ordered by ascending f-cost.
#[derive(Debug, Clone)]
struct BNode {
    f: f32,
    g: f32,
...
/// Octile/Manhattan heuristic distance between two grid cells.
...
/// Returns the walkable neighbours of `(x, y)` for a `unit_size`×`unit_size` footprint.
...
/// Walks the `came_from` table backwards from `end_coord` to `anchor_idx` and
/// returns the resulting path with `anchor_idx` as the first element.
///
/// # Parameters
/// - `came_from` — `&[u32]`.
/// - `w` — `u32` grid width.
/// - `anchor_idx` — `usize` flat index of the path start.
/// - `end_coord` — `(u32, u32)` path end (0-based).
///
/// # Returns
/// `Vec<(u32, u32)>`.
fn reconstruct_half(
...
/// Run bidirectional A★ search on `grid` from `start` to `goal`.
///
/// Alternates node expansions between a forward search (start → goal) and a
/// backward search (goal → start). When the expanding wave from one direction
/// reaches a node already closed by the other direction, a meeting point is
/// established and the two partial paths are spliced together into one route.
///
/// # Parameters
/// - `grid` — `&NavGrid`.
/// - `start` — `(u32, u32)` start cell (0-based).
/// - `goal` — `(u32, u32)` goal cell (0-based).
/// - `unit_size` — `u32` NxN footprint; 1 for single-cell units.
/// - `max_nodes` — `u32` node expansion budget; 0 = unlimited.
///
/// # Returns
/// `(Option<Vec<(u32, u32)>>, bool)`.
///
/// Returns `(Some(path), true)` when the goal is reached, `(Some(partial), false)`
/// when `max_nodes` is exceeded before the meeting point, and `(None, false)` when
/// start or goal is not walkable or no path exists.
pub fn bidirectional_astar(
    ...
    let _ = h; // used implicitly through NavGrid bounds checks
    ...
    loop {
        ...
        // Expand the frontier with the smaller f-cost; break ties in favour of forward.
        ...
        if expand_fwd {
            ...
            // Meeting point: this node was already closed by the backward search.
            if bwd_closed[cur_idx] {
                ...
                // bwd_path = [goal, ..., meet]; reversed = [meet, ..., goal];
                // skip the duplicated meeting node.
            ...
            // Meeting point: this node was already closed by the forward search.
            ...
            if max_nodes > 0 && expanded >= max_nodes {
                // No complete forward-to-meeting path yet — return partial from start.
                return (None, false);
            }
...
mod tests {
    ...
    fn iteration_limit_triggers_flag() {
        ...
        // Second bool is true only when the meeting point is found (complete path).
        // With only 5 expansions on a 50×50 grid, the search cannot complete.
