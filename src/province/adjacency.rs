...
/// # Example
///
/// To detect rivers: pass a map containing `{river_color_id: "river"}`.
pub fn detect_adjacency_with_tags(
    ...
    let is_tag_pixel = |id: u32| -> bool {
        tag_pixel_colors.contains_key(&id)
    };
    ...
    if !tag_pixel_colors.is_empty() {
        for y in 0..height {
            for x in 0..width {
                ...
                // Collect non-zero, non-tag province IDs in the four cardinal neighbours.
                ...
                // Every pair of distinct provinces bordering this tag pixel shares a tagged edge.
