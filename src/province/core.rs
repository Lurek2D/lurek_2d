//! Core province data structures: [`Province`], [`AdjacencyEdge`], and [`ProvinceMap`].
...
/// Error type for province operations.
#[derive(Debug, Error)]
...
///
/// All game-specific data (terrain, owner, region, etc.) is stored externally
/// via [`ProvinceProperties`](super::properties::ProvinceProperties). The
/// province struct itself is a pure spatial record.
pub struct Province {
    ...
    /// Position slots within the province.
    ///
    /// Slot 0 is the primary position (e.g. capital). Additional slots may be
    /// used for infrastructure, unit staging areas, etc. — the game developer
...
impl Province {
    ...
    /// data, call position calculation to populate `positions`.
...
impl ProvinceMap {
    ...
    ///
    /// Each entry contains the province ID that owns the corresponding pixel.
