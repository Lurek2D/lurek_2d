...
//!
//! - Data types: `ProvinceRecord`, `ProvinceRegistry`.
//! - Implementation: `ProvinceRegistry`.
//! - Public methods: `new`, `from_grid`, `from_png`, `width`, and 37 more.
//! - Contains 44 method implementations.
...
pub struct ProvinceRegistry {
    ...
    /// Manually overridden border types keyed by normalised (lo, hi) pair.
    border_types: HashMap<(ProvinceId, ProvinceId), BorderType>,
    /// Registered border type configs indexed by type id.
    border_type_configs: Vec<Option<BorderTypeConfig>>,
    ...
    /// Config-driven map mode registry.
    map_modes: MapModeRegistry,
...
impl ProvinceRegistry {
    ...
    pub fn from_grid(grid: &ProvinceGrid) -> Self {
        ...
        let pairs: Vec<(ProvinceId, ProvinceId)> = grid
            .adjacencies()
            .iter()
            .map(|(a, b, _)| (ProvinceId(*a), ProvinceId(*b)))
            .collect();
        ...
        for (id, rec) in provinces.iter_mut() {
            if let Some(n) = sum_n.get(&id.0).copied() {
                if n > 0.0 {
                    let cx = (sum_x[&id.0] / n) as f32;
                    let cy = (sum_y[&id.0] / n) as f32;
    ...
    /// Set the border type for the (a, b) pair and record a BorderType change.
    pub fn set_border_type(&mut self, a: ProvinceId, b: ProvinceId, border_type: BorderType) {
        let key = Self::norm_pair(a, b);
        self.border_types.insert(key, border_type);
        self.bump_change(ProvinceChange::BorderType {
            province_a: key.0,
            province_b: key.1,
            border_type,
        });
    }
    /// Return the stored border type for the (a, b) pair, or None if not explicitly set.
    pub fn get_border_type(&self, a: ProvinceId, b: ProvinceId) -> Option<BorderType> {
        self.border_types.get(&Self::norm_pair(a, b)).copied()
    }
    /// Register a border type config by ID. Grows the config vec as needed.
    pub fn register_border_type(&mut self, type_id: u8, config: BorderTypeConfig) {
        let idx = type_id as usize;
        if self.border_type_configs.len() <= idx {
            self.border_type_configs.resize(idx + 1, None);
        }
        self.border_type_configs[idx] = Some(config);
    }
    /// Return the config for a border type ID, or None if not registered.
    pub fn get_border_type_config(&self, type_id: u8) -> Option<&BorderTypeConfig> {
        self.border_type_configs
    ...
    pub fn register_map_mode(&mut self, name: &str, config: MapModeConfig) {
        self.map_modes.register(name, config);
    }
    /// Set the active map mode by name. Returns true if mode exists.
    pub fn set_map_mode(&mut self, name: &str) -> bool {
        let ok = self.map_modes.set_active(name);
        if ok {
            self.bump_change(ProvinceChange::MapModeChanged {
                mode: name.to_string(),
            });
        }
        ok
    ...
    pub fn active_map_mode(&self) -> &str {
        self.map_modes.active_name()
    }
    /// Get the active map mode config.
    pub fn map_mode_config(&self) -> &MapModeConfig {
        self.map_modes.active_config()
    }
    /// Get config for a named map mode.
    pub fn get_map_mode_config(&self, name: &str) -> Option<&MapModeConfig> {
