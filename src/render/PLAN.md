...
Ten plik przedstawia plan rozwoju modułu `render` po analizie istniejącego kodu źródłowego i zrealizowanych zmian.

## Status realizacji (DONE / TODO)

### DONE

* MUST: adaptacyjny poziom szczegółowości okręgów (Adaptive circle LOD) — zrealizowane.
* MUST: wydzielenie wspólnych helperów i redukcja duplikacji w `postfx_pipeline.rs` — zrealizowane.

### TODO

* MUST: rozbicie `gpu_renderer.rs` na podmoduły.
* MUST: cache geometrii statycznej (static draw caching).
* SHOULD: GPU instancing.
* SHOULD: antyaliasing linii i kształtów.
* SHOULD: uporządkowanie nazewnictwa zakresów (`render` vs `pipeline` vs `effect`).
