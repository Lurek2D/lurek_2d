...
impl DataFrame {
    /// Parallel element-wise transform on a column.
    /// Uses rayon when the column has 10,000+ rows; sequential otherwise.
    pub fn par_apply_column<F>(&mut self, col: ColRef, f: F) -> Result<(), String>
    where
        F: Fn(&CellValue) -> CellValue + Send + Sync,
    {
        let ci = self.resolve_col(col)?;
        let col_data = &self.data[ci];
