...
//! - Public methods: `group_agg`, `par_group_agg`, `pivot`, `corr`, and 1 more.
...
impl DataFrame {
    ...
    /// Parallel group-by aggregation — partitions by group column, aggregates each partition in parallel.
    /// Falls back to sequential for small group counts.
    pub fn par_group_agg(
        &self,
        group_col: ColRef,
        agg_col: ColRef,
        agg_fn: AggFn,
        ...
        // Build group partitions (sequential — requires stable insertion order)
        let mut key_order: Vec<String> = Vec::new();
        ...
        use rayon::prelude::*;
        let results: Vec<CellValue> = key_order
            .par_iter()
            .map(|key| {
                let indices = &groups[key];
                match agg_fn {
                    AggFn::Count => {
                        let count = indices.iter().filter(|&&i| !data[aci][i].is_nil()).count();
                    ...
                    _ => {
                        let nums: Vec<f64> = indices
                            .iter()
                            .filter_map(|&i| data[aci][i].as_number())
                            .collect();
                        if nums.is_empty() {
                        ...
                        match agg_fn {
                            ...
                            AggFn::Min => CellValue::Number(
                                nums.iter().copied().fold(f64::INFINITY, f64::min),
                            ),
                            AggFn::Max => CellValue::Number(
