...
pub struct AwarenessGrid {
    ...
    pending_events: Vec<AwarenessEvent>,
    /// Fog configuration.
...
impl AwarenessGrid {
    ...
    pub fn new(region_count: u32, player_count: u32) -> Self {
        let states =
            vec![vec![AwarenessState::Hidden; region_count as usize]; player_count as usize];
        let flags = vec![AwarenessFlags::NONE; region_count as usize];
    ...
    pub fn reveal(&mut self, player_id: u32, region_id: u32, reveal_flags: AwarenessFlags) {
        let allies = self.ownership.allies_of(player_id);
        for ally in &allies {
            if let Some(state) = self.get_state_mut(*ally, region_id) {
                let old = *state;
                *state = AwarenessState::Visible;
                if old != AwarenessState::Visible {
                    self.pending_events.push(AwarenessEvent::Revealed {
                        player_id: *ally,
                        region_id,
                    });
                }
    ...
    pub fn hide(&mut self, player_id: u32, region_id: u32) {
        ...
        for ally in &allies {
            if let Some(state) = self.get_state_mut(*ally, region_id) {
                if *state == AwarenessState::Visible {
                    *state = AwarenessState::Discovered;
                    self.pending_events.push(AwarenessEvent::Hidden {
                        player_id: *ally,
                        region_id,
                    });
                }
    ...
    pub fn get_state(&self, player_id: u32, region_id: u32) -> AwarenessState {
        self.states
    ...
    pub fn get_fog_intensity(&self, player_id: u32, region_id: u32) -> f32 {
        match self.get_state(player_id, region_id) {
            AwarenessState::Visible => 0.0,
            AwarenessState::Discovered => self.fog_config.discovered_intensity,
            AwarenessState::Hidden => 1.0,
            AwarenessState::Custom(level) => 1.0 - (level as f32 / 255.0),
        }
    }
    ...
    pub fn get_flags(&self, region_id: u32) -> AwarenessFlags {
        self.flags
            .get(region_id as usize)
            .copied()
            .unwrap_or(AwarenessFlags::NONE)
    }

    /// Set visibility flags for a region.
    pub fn set_flags(&mut self, region_id: u32, flags: AwarenessFlags) {
    ...
    pub fn drain_events(&mut self) -> Vec<AwarenessEvent> {
    ...
    pub fn reveal_all(&mut self, player_id: u32) {
        for region_id in 0..self.region_count {
            self.reveal(player_id, region_id, AwarenessFlags::ALL);
        }
    }
    ...
    pub fn reset(&mut self, player_id: u32) {
        if let Some(states) = self.states.get_mut(player_id as usize) {
            states.fill(AwarenessState::Hidden);
        }
        self.fog_intensity.fill(1.0);
    }

    fn get_state_mut(&mut self, player_id: u32, region_id: u32) -> Option<&mut AwarenessState> {
        self.states
