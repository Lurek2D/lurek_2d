...
mod tests {
    ...
    fn make_test_image() -> ImageData {
        ...
        // Red province (top-left 2×2)
        for y in 0..2 {
            for x in 0..2 {
                img.set_pixel(x, y, 255, 0, 0, 255);
            }
        }
        // Green province (top-right 2×2)
        for y in 0..2 {
            for x in 2..4 {
                img.set_pixel(x, y, 0, 255, 0, 255);
            }
        }
        // Blue province (row 2, full width)
        for x in 0..4 {
            img.set_pixel(x, 2, 0, 0, 255, 255);
        }
        // Row 3 = black (background, ID 0)
        img
    }
    ...
    fn adjacency_detected_between_neighbors() {
        ...
        // Red–Green, Red–Blue, Green–Blue should all be present
