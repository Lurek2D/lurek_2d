...
/// Parses a Lua follow easing name into a camera easing mode.
fn parse_follow_easing(name: &str) -> CameraEasing {
    match name.to_ascii_lowercase().as_str() {
        "smoothstep" | "smooth" => CameraEasing::SmoothStep,
        "easeout" | "ease_out" | "ease-out" => CameraEasing::EaseOutCubic,
        _ => CameraEasing::Linear,
    }
}
/// Parses an optional Lua zoom easing name into a zoom tween easing mode.
fn parse_zoom_easing(name: Option<String>) -> CameraEasing {
    match name
    ...
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("getFollowEasing", |_, this, ()| {
            let mode = match this.inner.borrow().get_follow_easing() {
                ...
                CameraEasing::EaseOutCubic => "easeout",
            };
            Ok(mode)
        });
        // -- setDeadZone --
