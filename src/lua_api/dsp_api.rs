//! `src/lua_api/dsp_api.rs` registers the `lurek.dsp` Lua boundary for dsp behavior, converts Lua values into engine types, validates arguments and error messages, and exposes userdata or callbacks while keeping implementation state in Rust modules.
