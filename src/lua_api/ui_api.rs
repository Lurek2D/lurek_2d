...
fn create_widget_table<'a>(
    ...
    /// @summary When true and wrap is false, overflowing text is truncated with "â€¦".
    /// @param | self | LUiWidget | The widget instance.
            ...
            match g.widgets.get(idx) {
                Some(WidgetKind::NinePatch(np)) => {
                    ...
                    for (i, s) in slices.iter().enumerate() {
                        ...
                        /// The 'sx' field value exposed to Lua scripts.
                        st.set("sx", s.0)?;
                        /// The 'sy' field value exposed to Lua scripts.
                        st.set("sy", s.1)?;
                        /// The 'sw' field value exposed to Lua scripts.
                        st.set("sw", s.2)?;
                        /// The 'sh' field value exposed to Lua scripts.
                        st.set("sh", s.3)?;
                        /// The 'dx' field value exposed to Lua scripts.
                        st.set("dx", s.4)?;
                        /// The 'dy' field value exposed to Lua scripts.
                        st.set("dy", s.5)?;
                        /// The 'dw' field value exposed to Lua scripts.
                        st.set("dw", s.6)?;
                        /// The 'dh' field value exposed to Lua scripts.
                        st.set("dh", s.7)?;
                        result.set(i + 1, st)?;
                    }
...
pub fn register(lua: &Lua, luna: &LuaTable, state: Rc<RefCell<SharedState>>) -> LuaResult<()> {
    ...
    tbl.set(
        ...
        lua.create_function(move |_, (dx, dy): (f32, f32)| {
            Ok(c.borrow_mut().focus_direction(dx, dy))
        })?,
    )?;
    ...
    tbl.set(
        ...
        lua.create_function(move |_, (w, h): (f32, f32)| {
            c.borrow_mut().update_resolution(w, h);
            Ok(())
        })?,
    )?;
    ...
    /// @return | integer | End index (exclusive).
    let c = ctx.clone();
    tbl.set(
        "visibleRange",
        lua.create_function(
        ...
        lua.create_function(
            move |_,
                ...
                let ease = easing
                    ...
                    .and_then(EasingFunction::parse_str)
                    .unwrap_or(EasingFunction::Linear);
                Ok(c.borrow_mut().animate_color(idx, from, to, duration, ease))
            },
        )?,
    )?;
    ...
    /// The 'ui' field value exposed to Lua scripts.
    luna.set("ui", tbl)?;
    Ok(())
}
...
pub struct LuaLineChart {
    /// Inner wrapped value.
    pub inner: crate::charts::LineChart,
}
...
pub struct LuaBarChart {
    /// Inner wrapped value.
    pub inner: crate::charts::BarChart,
}
...
pub struct LuaScatterPlot {
    /// Inner wrapped value.
    pub inner: crate::charts::ScatterPlot,
}
...
pub struct LuaPieChart {
    /// Inner wrapped value.
    pub inner: crate::charts::PieChart,
}
...
pub struct LuaAreaChart {
    /// Inner wrapped value.
    pub inner: crate::charts::AreaChart,
}
