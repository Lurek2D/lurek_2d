//! `src/lua_api/animation_api.rs` registers the `lurek.animation` Lua boundary for animation behavior, converts Lua values into engine types, validates arguments and error messages, and exposes userdata or callbacks while keeping implementation state in Rust modules.
...
pub struct LuaAnimation {
    ...
    inner: Animation,
    /// Spritesheet stored by `setImage`; used by `draw` when no image argument is supplied.
    stored_image: Option<LuaImage>,
}
    ...
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method_mut("setImage", |_, this, image_ud: LuaAnyUserData| {
            let image = image_ud.borrow::<LuaImage>().map_err(|_| {
                LuaError::RuntimeError("LAnimation:setImage: argument must be LImage".into())
            })?;
        ...
        /// Accepts two call forms: `draw(image, x, y, opts)` uses the supplied atlas directly;
        /// `draw(x, y, opts)` uses the image previously stored with `setImage`.
        ...
        methods.add_method("draw", |_, this, args: LuaMultiValue| {
            draw_animation_frame_from_args("LAnimation:draw", &this.inner, &this.stored_image, args)
        });
        // -- pollEvents --
...
impl LuaUserData for LuaAnimStateMachine {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        /// @return | boolean | True when a frame draw command was queued; false when no current frame is active.
        methods.add_method("draw", |_, this, args: LuaMultiValue| {
            draw_animation_frame_from_args(
                "LAnimStateMachine:draw",
                this.inner.get_animation(),
                &this.stored_image,
                args,
            )
        });
...
/// Parsed transform options for animation draw helpers.
struct AnimationDrawOptions {
    x: f32,
    y: f32,
    rotation: f32,
    sx: f32,
...
/// Extract optional draw transform values from Lua arguments.
fn parse_animation_draw_options(
    method_name: &str,
    x: Option<f32>,
    y: Option<f32>,
    opts: Option<LuaTable>,
) -> LuaResult<AnimationDrawOptions> {
    let mut out = AnimationDrawOptions {
        x: x.unwrap_or(0.0),
        y: y.unwrap_or(0.0),
        rotation: 0.0,
        sx: 1.0,
    ...
    if let Some(opts) = opts {
        out.rotation = optional_number_field(method_name, &opts, "rotation")?.unwrap_or(0.0);
        let scale = optional_number_field(method_name, &opts, "scale")?.unwrap_or(1.0);
        out.sx = optional_number_field(method_name, &opts, "scaleX")?.unwrap_or(scale);
        out.sy = optional_number_field(method_name, &opts, "scaleY")?.unwrap_or(scale);
        out.ox = optional_number_field(method_name, &opts, "originX")?.unwrap_or(0.0);
        out.oy = optional_number_field(method_name, &opts, "originY")?.unwrap_or(0.0);
    }
    Ok(out)
}

/// Read an optional numeric field from a Lua options table.
fn optional_number_field(
    ...
    match opts.get::<_, LuaValue>(field)? {
        ...
        LuaValue::Integer(n) => Ok(Some(n as f32)),
        LuaValue::Number(n) => Ok(Some(n as f32)),
        other => Err(LuaError::RuntimeError(format!(
            "{method_name}: option '{field}' must be a number, got {}",
            other.type_name()
        ))),
    }
}

/// Queue a draw command for an animation's current frame using a resolved `LuaImage`, returning false when no frame is active.
fn draw_animation_frame(
    ...
    let (tex_w, tex_h) = {
        ...
        let texture = st.textures.get(texture_key).ok_or_else(|| {
            LuaError::RuntimeError(format!(
                "{method_name}: image handle is not valid or was released"
            ))
        })?;
        (texture.width as f32, texture.height as f32)
    };
    let params = AnimRenderParams {
        texture_key,
        tex_w,
        tex_h,
        x: options.x,
    ...
    let Some(command) = animation.generate_render_command(&params) else {
        return Ok(false);
    };
    state.borrow_mut().render_commands.push(command);
    Ok(true)
}

/// Dispatch a draw call by inspecting the first variadic argument.
/// If arg[0] is `LImage` userdata the explicit-image form is used; otherwise the stored image is used.
fn draw_animation_frame_from_args(
    method_name: &str,
    animation: &Animation,
    stored_image: &Option<LuaImage>,
    args: LuaMultiValue,
    ...
    let first = iter.next();
    let is_image_ud = matches!(&first, Some(LuaValue::UserData(_)));
    if is_image_ud {
        let ud = match first {
        ...
        draw_animation_frame(method_name, animation, &image, x, y, opts)
    } else {
        let image = stored_image.as_ref().ok_or_else(|| {
            LuaError::RuntimeError(format!(
                "{method_name}: no image stored; call setImage first or pass an LImage as the first argument"
            ))
        })?.clone();
        let x = parse_opt_coord(method_name, "x", first.as_ref())?;
        let y = parse_opt_coord(method_name, "y", iter.next().as_ref())?;
        let opts = parse_opt_table_arg(method_name, "opts", iter.next())?;
        draw_animation_frame(method_name, animation, &image, x, y, opts)
    }
...
fn parse_opt_coord(
    ...
    match val {
        ...
        Some(LuaValue::Integer(n)) => Ok(Some(*n as f32)),
        Some(other) => Err(LuaError::RuntimeError(format!(
            "{method_name}: '{field}' must be a number, got {}",
            other.type_name()
        ))),
    }
}
...
fn parse_opt_table_arg<'lua>(
    ...
    match val {
        ...
        Some(other) => Err(LuaError::RuntimeError(format!(
            "{method_name}: '{field}' must be a table, got {}",
            other.type_name()
        ))),
    }
}
