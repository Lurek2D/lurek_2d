//! `src/lua_api/spine_api.rs` registers the `lurek.spine` Lua boundary for spine behavior, converts Lua values into engine types, validates arguments and error messages, and exposes userdata or callbacks while keeping implementation state in Rust modules.
