//! `src/lua_api/parallax_api.rs` registers the `lurek.parallax` Lua boundary for parallax behavior, converts Lua values into engine types, validates arguments and error messages, and exposes userdata or callbacks while keeping implementation state in Rust modules.
