...
pub fn register(lua: &Lua, luna: &LuaTable, _state: Rc<RefCell<SharedState>>) -> LuaResult<()> {
    ...
    tbl.set(
        ...
        lua.create_function(|lua, (fmt, vals): (String, LuaMultiValue)| {
            ...
            let bd = binary::pack(&fmt, &pvs).map_err(LuaError::RuntimeError)?;
    ...
    tbl.set(
        ...
        lua.create_function(
            |lua, (fmt, raw, offset): (String, LuaString, Option<usize>)| {
                let (values, next_pos) = binary::unpack(&fmt, raw.as_bytes(), offset.unwrap_or(0))
    ...
    tbl.set(
        ...
        lua.create_function(|_, (fmt, vals): (String, LuaMultiValue)| {
            let pvs = lua_values_to_pack(vals);
            binary::get_packed_size(&fmt, &pvs)
                .map_err(LuaError::RuntimeError)
                .map(|n| n as i64)
        })?,
    )?;
    ...
    tbl.set(
        ...
        lua.create_function(
            |lua, (format_str, raw_data, level): (String, LuaString, Option<u32>)| {
                ...
                let result = binary::compress(raw_data.as_bytes(), format, level.unwrap_or(6))
                    .map_err(LuaError::RuntimeError)?;
    ...
    tbl.set(
        ...
        lua.create_function(
            |lua, (format_str, chunks, level): (String, LuaValue, Option<u32>)| {
                ...
                let result = binary::compress_chunks(&chunk_refs, format, level.unwrap_or(6))
                    .map_err(LuaError::RuntimeError)?;
                lua.create_string(&result)
            },
        )?,
    ...
    tbl.set(
        ...
        lua.create_function(|lua, (format_str, chunks): (String, LuaValue)| {
            ...
            let chunk_refs: Vec<&[u8]> = chunks.iter().map(|c| c.as_slice()).collect();
            let result =
                binary::decompress_chunks(&chunk_refs, format).map_err(LuaError::RuntimeError)?;
            lua.create_string(&result)
        })?,
    )?;
    // -- encode --
    ...
    tbl.set(
        ...
        lua.create_function(|lua, (fmt, vals): (String, LuaMultiValue)| {
            ...
            let bd = binary::bin_write(&fmt, &bvs).map_err(LuaError::RuntimeError)?;
            lua.create_string(bd.as_bytes())
        })?,
    )?;
    // -- read --
    ...
    tbl.set(
        ...
        lua.create_function(
            |lua, (fmt, raw, offset): (String, LuaString, Option<usize>)| {
                let (bvs, _) = binary::bin_read(&fmt, raw.as_bytes(), offset.unwrap_or(0))
                    .map_err(LuaError::RuntimeError)?;
