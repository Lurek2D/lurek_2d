//! Registers the `lurek.serialize` Lua API for multi-format encode, decode, validation, and CSV options.
...
/// Builds CSV options from explicit Lua arguments.
fn csv_options_from_args(delim: Option<String>, headers: Option<bool>) -> CsvOptions {
    CsvOptions {
...
fn parse_format_arg(op: &str, format: Option<&str>) -> LuaResult<Option<SerialFormat>> {
    format
        .map(|name| {
            SerialFormat::parse(name).ok_or_else(|| {
                LuaError::RuntimeError(format!(
                    "{op}: unknown format (expected json/toml/csv/xml/ini/msgpack)"
                ))
            })
        })
        .transpose()
...
fn encode_msgpack_value<'lua>(lua: &'lua Lua, value: LuaValue<'lua>) -> LuaResult<LuaString<'lua>> {
    ...
    let bytes = crate::serialize::to_msgpack(&val).map_err(LuaError::RuntimeError)?;
    lua.create_string(&bytes)
}

/// Validates a Lua value against a Lua schema table and returns the Lua-facing result tuple.
fn validate_value(value: LuaValue, schema: LuaValue) -> LuaResult<(bool, Option<String>)> {
    let val = from_lua(&value)?;
    let sch = from_lua(&schema)?;
    match crate::serialize::validate_schema(&val, &sch) {
        Ok(()) => Ok((true, None)),
...
pub fn register(lua: &Lua, lurek: &LuaTable, _state: Rc<RefCell<SharedState>>) -> LuaResult<()> {
    ...
    tbl.set(
        ...
        lua.create_function(|lua, s: String| {
            let val = crate::serialize::from_json(&s).map_err(LuaError::RuntimeError)?;
            to_lua(lua, &val)
        })?,
    )?;
    ...
    tbl.set(
        "fromToml",
        lua.create_function(|lua, s: String| {
            let val = crate::serialize::from_toml(&s).map_err(LuaError::RuntimeError)?;
            to_lua(lua, &val)
        })?,
    )?;
    ...
    tbl.set(
        "fromIni",
        lua.create_function(|lua, s: String| {
            let val = crate::serialize::from_ini(&s).map_err(LuaError::RuntimeError)?;
            to_lua(lua, &val)
        })?,
    )?;
    ...
    tbl.set(
        ...
        lua.create_function(|_, value: LuaValue| {
            let val = from_lua(&value)?;
            crate::serialize::to_toml(&val).map_err(LuaError::RuntimeError)
        })?,
    )?;
    ...
    tbl.set(
        "decodeXml",
        lua.create_function(|lua, s: String| {
            let val = crate::serialize::from_xml(&s).map_err(LuaError::RuntimeError)?;
            crate::serialize::lua_table::to_lua(lua, &val)
        })?,
    )?;
    ...
    tbl.set(
        ...
        lua.create_function(|_, s: String| {
            Ok(crate::serialize::detect_format(&s).map(|f| f.as_str().to_string()))
        })?,
    )?;
