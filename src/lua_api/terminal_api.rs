//! `src/lua_api/terminal_api.rs` registers the `lurek.terminal` Lua boundary for terminal behavior, converts Lua values into engine types, validates arguments and error messages, and exposes userdata or callbacks while keeping implementation state in Rust modules.
...
enum CallbackKind {
    /// Button click callback slot exposed by the lurek engine.
    Click,
        ...
        lua.create_function(|lua, text: String| {
            ...
            for (i, span) in spans.iter().enumerate() {
                ...
                if let Some(ref c) = span.fg {
                    ...
                    ct.set("b", c.b)?;
                    /// The 'fg' field value exposed to Lua scripts.
                    t.set("fg", ct)?;
                }
                if let Some(ref c) = span.bg {
                    ...
                    /// The 'b' field value exposed to Lua scripts.
                    ct.set("b", c.b)?;
                    /// The 'bg' field value exposed to Lua scripts.
                    t.set("bg", ct)?;
                }
                arr.set(i + 1, t)?;
            }
