//! Registers the `lurek.patterns` Lua API for pattern graphs, node ticking, flushing, and pattern userdata.
...
impl LuaUserData for LuaBlackboard {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("clear", |_, this, key: String| {
            this.board.borrow_mut().remove(&key);
            Ok(())
        });
        // -- keys --
        ...
        methods.add_method("keys", |lua, this, ()| {
            let keys = this.board.borrow().keys();
