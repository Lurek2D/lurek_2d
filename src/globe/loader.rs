...
//!
//! - Functions: `load_from_toml_str`, `load_from_toml_file`, `load_from_png_file`, `generate_voronoi_provinces`.
//! - Implementation: `TomlRegionBuilder`.
...
struct TomlRegion {
    /// Region identifier.
    id: RegionId,
    /// Region centroid as latitude and longitude.
    centroid: [f32; 2],
    /// Region polygon vertices as latitude and longitude pairs.
    vertices: Vec<[f32; 2]>,
...
fn toml_region_to_region(tp: TomlRegion) -> Region {
    ...
    let mut p = Region::new(tp.id, verts);
...
/// Parse a TOML region list using the local lightweight parser.
fn parse_toml_region_list(src: &str) -> Result<Vec<TomlRegion>, String> {
    let mut regions: Vec<TomlRegion> = Vec::new();
    let mut current: Option<TomlRegionBuilder> = None;
    ...
    for (line_no, raw_line) in src.lines().enumerate() {
        ...
        if line == "[[province]]" {
            if let Some(b) = current.take() {
                regions.push(b.build().map_err(|e| format!("line {}: {}", line_no, e))?);
            }
            current = Some(TomlRegionBuilder::default());
            in_attrs = false;
            continue;
        }
    ...
    if let Some(b) = current {
        regions.push(b.build()?);
    }
    Ok(regions)
}
/// Incremental TOML region builder used by the lightweight parser.
#[derive(Default)]
struct TomlRegionBuilder {
    /// Optional region identifier.
    id: Option<RegionId>,
    /// Optional centroid pair.
    centroid: Option<[f32; 2]>,
    /// Collected region vertices.
    vertices: Vec<[f32; 2]>,
...
impl TomlRegionBuilder {
    /// Finalize the builder into a TOML region or return a missing-field error.
    fn build(self) -> Result<TomlRegion, String> {
        Ok(TomlRegion {
...
/// Parse one non-attribute TOML field into the region builder.
fn parse_field(line: &str, b: &mut TomlRegionBuilder) -> Result<(), String> {
    let eq = line
        .find('=')
