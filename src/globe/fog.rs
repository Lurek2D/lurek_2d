...
/// Compact fog state for all supported regions.
#[derive(Debug, Clone)]
pub struct FogMask {
    /// Packed fog state per region id.
    states: Vec<u8>,
}
impl FogMask {
    /// Create a mask with every region hidden.
    pub fn all_hidden() -> Self {
        Self {
            states: vec![FogState::Hidden as u8; MAX_REGIONS],
        }
    }
    /// Create a mask with every region visible.
    pub fn all_visible() -> Self {
        Self {
            states: vec![FogState::Visible as u8; MAX_REGIONS],
        }
    }
    /// Return true when the region is visible.
    pub fn is_visible(&self, id: RegionId) -> bool {
    ...
    /// Return the stored fog state for a region id.
    pub fn state(&self, id: RegionId) -> FogState {
        let id = id.0 as usize;
        if id >= MAX_REGIONS {
    ...
    /// Set the fog state for a region id when it is in range.
    pub fn set_state(&mut self, id: RegionId, state: FogState) {
        let id = id.0 as usize;
        if id < MAX_REGIONS {
            self.states[id] = state as u8;
        }
    }
    /// Mark a region as visible. This function is part of the public API.
    pub fn reveal(&mut self, id: RegionId) {
        self.set_state(id, FogState::Visible);
    }
    /// Mark a region as hidden. This function is part of the public API.
    pub fn hide(&mut self, id: RegionId) {
        self.set_state(id, FogState::Hidden);
    }
    /// Mark a region as explored. This function is part of the public API.
    pub fn explore(&mut self, id: RegionId) {
        self.set_state(id, FogState::Explored);
    }
    /// Toggle a region between visible and hidden.
    pub fn toggle(&mut self, id: RegionId) {
        let idu = id.0 as usize;
        if idu < MAX_REGIONS {
    ...
    /// Reveal every region in the supplied iterator.
    pub fn reveal_batch(&mut self, ids: impl Iterator<Item = RegionId>) {
        for id in ids {
            self.reveal(id);
        }
    }
    /// Return all visible region ids in ascending order.
    pub fn visible_ids(&self) -> Vec<RegionId> {
        self.states
            .iter()
            .enumerate()
            .filter_map(|(i, s)| {
                if *s == FogState::Visible as u8 {
                    Some(RegionId(i as u32))
                } else {
                    None
                }
            })
    ...
    /// Return all explored region ids in ascending order.
    pub fn explored_ids(&self) -> Vec<RegionId> {
        self.states
            .iter()
            .enumerate()
            .filter_map(|(i, s)| {
    ...
    /// Build a mask that reveals the supplied region ids.
    pub fn from_visible_ids(ids: &[RegionId]) -> Self {
    ...
    /// Count visible regions in the mask.
    ...
    /// Count explored regions in the mask.
    ...
    pub fn to_base64(&self) -> String {
        let packed_len = (MAX_REGIONS * 2).div_ceil(8);
    ...
    pub fn from_base64(s: &str) -> Result<Self, String> {
        ...
        let mut states = vec![FogState::Hidden as u8; MAX_REGIONS];
...
impl FogStore {
    ...
    /// Return true when the viewer can see the region.
    pub fn is_visible(&self, viewer: &str, id: RegionId) -> bool {
        match self.masks.get(viewer) {
            Some(mask) => mask.is_visible(id),
            None => true,
        }
    }
    /// Reveal a fog region for a specific viewer.
    pub fn reveal(&mut self, viewer: &str, id: RegionId) {
        self.get_or_insert(viewer).reveal(id);
    }
    /// Mark a region as explored for a viewer.
    pub fn explore(&mut self, viewer: &str, id: RegionId) {
        self.get_or_insert(viewer).explore(id);
    }
    /// Hide a region for a viewer. This function is part of the public API.
    pub fn hide(&mut self, viewer: &str, id: RegionId) {
        self.get_or_insert(viewer).hide(id);
    }
    /// Return the visible region ids for a viewer when the viewer exists.
    pub fn visible_ids(&self, viewer: &str) -> Option<Vec<RegionId>> {
        self.masks.get(viewer).map(FogMask::visible_ids)
    }
    /// Return the explored region ids for a viewer when the viewer exists.
    pub fn explored_ids(&self, viewer: &str) -> Option<Vec<RegionId>> {
        self.masks.get(viewer).map(FogMask::explored_ids)
    }
    /// Return the state for a viewer or visible when the viewer has no mask.
    pub fn state(&self, viewer: &str, id: RegionId) -> FogState {
        self.masks
            .get(viewer)
            .map(|m| m.state(id))
            .unwrap_or(FogState::Visible)
    }
    /// Set the fog state for a viewer and region.
    pub fn set_state(&mut self, viewer: &str, id: RegionId, state: FogState) {
    ...
    /// Replace a viewer mask with one built from visible region ids.
    pub fn load(&mut self, viewer: &str, ids: &[RegionId]) {
        self.masks
