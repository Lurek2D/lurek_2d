...
Ten plik przedstawia plan rozwoju modułu `sprite` po analizie istniejącego kodu źródłowego i zrealizowanych zmian.

## Status realizacji (DONE / TODO)

### DONE

* MUST: `newAtlasPacker` (API Lua runtime atlas packer) — zrealizowane.
* MUST: ograniczenie alokacji w `SpriteSheet::get_row/get_column` — zrealizowane.

### TODO

* SHOULD: wsparcie lit sprites (normal-map).
* SHOULD: deduplikacja parserów Aseprite między `sprite` i `animation`.
* SHOULD: rozszerzenie testów parserów atlasów (error-path).
* COULD: binarny format atlasów.
* COULD: fuzz targety parserów atlasów.

## Stan obecny

W module wprowadzono już następujące ułatwienia:
