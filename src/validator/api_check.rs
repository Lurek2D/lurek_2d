...
impl ApiComplianceRule {
    ...
    pub fn with_defaults() -> Self {
        Self {
            known_apis: vec![
                "lurek.sprite".into(), "lurek.audio".into(), "lurek.input".into(),
                "lurek.physics".into(), "lurek.math".into(), "lurek.timer".into(),
                "lurek.window".into(), "lurek.render".into(), "lurek.font".into(),
                "lurek.camera".into(), "lurek.tween".into(), "lurek.scene".into(),
                "lurek.event".into(), "lurek.data".into(), "lurek.ui".into(),
                "lurek.particle".into(), "lurek.tilemap".into(), "lurek.animation".into(),
                "lurek.network".into(), "lurek.save".into(), "lurek.thread".into(),
                "lurek.ai".into(), "lurek.pathfind".into(), "lurek.effect".into(),
                "lurek.mapblock".into(), "lurek.cursor".into(), "lurek.grep".into(),
                "lurek.validator".into(), "lurek.mods".into(),
            ],
        }
    }
}
...
impl ValidationRule for ApiComplianceRule {
    fn id(&self) -> &str { "api-compliance" }
    fn description(&self) -> &str { "Checks that lurek.* API calls reference known modules" }
    ...
    fn validate(&self, path: &Path, content: &str) -> Vec<Violation> {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            while let Some(pos) = line[search_from..].find("lurek.") {
                ...
                let module_end = after.find(|c: char| !c.is_alphanumeric() && c != '_').unwrap_or(after.len());
                if module_end > 0 {
                    let module_name = format!("lurek.{}", &after[..module_end]);
                    if !self.known_apis.iter().any(|api| module_name.starts_with(api)) {
                        violations.push(
                            Violation::new("api-compliance", Severity::Warning, path.to_path_buf(),
