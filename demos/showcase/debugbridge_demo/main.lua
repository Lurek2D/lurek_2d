...
if not luna.debugbridge then
    ...
    function luna.draw()
        ...
        luna.render.print("Debug Bridge Demo", 20, 20)
        luna.render.setColor(0.7, 0.7, 0.7)
        luna.render.print("luna.debugbridge is not available in this build.", 20, 60)
...
function luna.draw()
    ...
    luna.render.print("Debug Bridge Demo", 20, y)
    y = y + 30

    -- Server status
    luna.render.setColor(0.7, 1, 0.7)
    luna.render.print("Server running: " .. tostring(db.isRunning()), 20, y)
    y = y + 20
    luna.render.print("Port: " .. db.getPort(), 20, y)
    y = y + 20
    luna.render.print("Connected clients: " .. db.getClientCount(), 20, y)
    y = y + 30

    -- Performance
    local perf = db.getPerformance()
    luna.render.setColor(0.7, 0.8, 1)
    luna.render.print(string.format("FPS: %.1f  |  avg dt: %.3fms", perf.fps or 0, (perf.avgDt or 0) * 1000), 20, y)
    y = y + 30

    -- Print history (last 5)
    luna.render.setColor(1, 0.8, 0.6)
    luna.render.print("Print history (last 5):", 20, y)
    y = y + 20

    local history = db.getPrintHistory(5)
    for i = 1, #history do
        luna.render.setColor(0.8, 0.8, 0.8)
        luna.render.print(history[i].message, 30, y)
        y = y + 16
    end
    y = y + 20

    -- Instructions
    luna.render.setColor(0.5, 0.6, 0.5)
    luna.render.print("Connect via TCP to interact:", 20, y)
    y = y + 18
    luna.render.print('  {"id":1,"method":"ping"}', 30, y)
    y = y + 18
    luna.render.print('  {"id":2,"method":"eval","params":{"code":"return 2+2"}}', 30, y)
    y = y + 18
    luna.render.print('  {"id":3,"method":"getPerformance"}', 30, y)
    y = y + 30
