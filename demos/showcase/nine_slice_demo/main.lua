...
if not luna.render.newNineSlice then
    function luna.load()
        luna.render.setBackgroundColor(0.15, 0.15, 0.25)
        luna.render.print("luna.render.newNineSlice not available", 180, 270)
        luna.render.print("Nine-Slice Demo  --  placeholder screenshot", 200, 310)
    end
    return
end
...
--
-- Demonstrates luna.render.newNineSlice / drawNineSlice for
...
function luna.load()
    ...
    panel = luna.render.newNineSlice(img, 32, 32, 32, 32)
    button = luna.render.newNineSlice(img, 16, 16, 16, 16)
end
...
function luna.draw()
    local screenW, screenH = luna.render.getDimensions()
    local size = sizes[current]

    -- Title
    luna.render.setColor(1, 1, 1)
    luna.render.print("Nine-Slice (9-patch) Demo", 20, 20, 2.5)
    ...
    luna.render.setColor(0.3, 0.5, 0.9, 0.8)
    luna.render.drawNineSlice(panel, px, py, size.w, size.h)

    -- Label inside the panel
    luna.render.setColor(1, 1, 1)
    luna.render.print(size.label .. " panel (" .. size.w .. "x" .. size.h .. ")",
        px + 40, py + 20, 2)

    -- Draw three buttons below the panel using the method syntax
    luna.render.setColor(0.9, 0.4, 0.2, 0.9)
    ...
    for i = 1, 3 do
        ...
        luna.render.setColor(1, 1, 1)
        luna.render.print("Btn " .. i, btnX + 15, btnY + 12, 1.5)
        luna.render.setColor(0.9, 0.4, 0.2, 0.9)
    end

    -- Info text
    luna.render.setColor(0.6, 0.6, 0.6)
    luna.render.print("Cycles every 2s — corners stay fixed, center stretches", 20, screenH - 40, 1.5)

    -- FPS
    luna.render.setColor(0.5, 0.5, 0.5)
    local fps = math.floor(luna.time.getFPS())
    luna.render.print("FPS: " .. tostring(fps), screenW - 120, 10, 1.5)
end
