...
- `luna.render.rectangle()` — tile map, soil plots, crop stems, and HUD panel rendering
- `luna.render.circle()` — ripe crop icons and growth-stage seedling indicators
- `luna.render.setColor()` — dynamic day/night factor applied to every colour each frame
- `luna.render.setBackgroundColor()` — season-tinted background that updates in `luna.draw()`
- `luna.render.print()` — day, season, money, seed inventory, and harvest count HUD
