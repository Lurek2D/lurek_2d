...
function luna.draw()
    luna.render.setBackgroundColor(0.1, 0.12, 0.08)

    -- UI header
    luna.render.setColor(1, 0.9, 0.4, 1)
    luna.render.print("Zoo Tycoon", 10, 5, 1.2)
    luna.render.setColor(1, 1, 0.6, 1)
    luna.render.print("Gold: " .. gold, 160, 8, 0.9)
    luna.render.print("Day: " .. day, 300, 8, 0.9)
    luna.render.print("Guests: " .. #guests, 400, 8, 0.9)
    luna.render.print("Happy: " .. math.floor(happiness) .. "%", 520, 8, 0.9)

    -- Tool bar
    luna.render.setColor(0.7, 0.7, 0.6, 1)
    luna.render.print("Tool: " .. selected_tool, 10, 30, 0.75)
    luna.render.print("Tab=cycle | 1-4=animals | Click=place", 150, 30, 0.65)
    ...
    for y = 1, GRID_H do
        for x = 1, GRID_W do
            ...
            luna.render.rectangle("fill", OX + (x - 1) * CELL, OY + (y - 1) * CELL, CELL - 1, CELL - 1)
        end
    end

    -- Gate label
    luna.render.setColor(0, 0, 0, 1)
    luna.render.print("G", OX + 6, OY + (GRID_H - 1) * CELL + 6, 0.7)
    ...
    for _, a in ipairs(animals) do
        ...
        if a.happy then
            ...
            luna.render.setColor(0.8, 0.2, 0.2, 0.4)
        end
        luna.render.rectangle("fill", ax, ay, CELL - 3, CELL - 3)
        -- Icon
        luna.render.setColor(at.color[1], at.color[2], at.color[3], 1)
        luna.render.print(at.icon, ax + 6, ay + 4, 1)
    end

    -- Guests
    luna.render.setColor(1, 0.9, 0.7, 1)
    for _, g in ipairs(guests) do
        luna.render.circle("fill", g.x, g.y, 4)
    end

    -- Day timer bar
    luna.render.setColor(0.2, 0.2, 0.2, 1)
    luna.render.rectangle("fill", OX, OY + GRID_H * CELL + 5, GRID_W * CELL, 8)
    luna.render.setColor(0.4, 0.7, 1, 1)
    luna.render.rectangle("fill", OX, OY + GRID_H * CELL + 5, GRID_W * CELL * (day_timer / DAY_LENGTH), 8)

    -- Legend
    local lx = OX + GRID_W * CELL + 10
    luna.render.setColor(0.9, 0.85, 0.6, 1)
    luna.render.print("Legend:", lx, OY, 0.8)
    ...
    for i, l in ipairs(legend) do
        luna.render.setColor(l[2], l[3], l[4], 1)
        luna.render.rectangle("fill", lx, OY + 18 + (i - 1) * 20, 12, 12)
        luna.render.setColor(0.8, 0.8, 0.8, 1)
        luna.render.print(l[1], lx + 16, OY + 16 + (i - 1) * 20, 0.65)
    end

    -- Animal key
    luna.render.setColor(0.9, 0.85, 0.6, 1)
    luna.render.print("Animals:", lx, OY + 150, 0.8)
    for i, at in ipairs(animal_types) do
        luna.render.setColor(at.color[1], at.color[2], at.color[3], 1)
        luna.render.print(i .. "=" .. at.name, lx, OY + 170 + (i - 1) * 18, 0.65)
    end

    -- Message
    if msg_timer > 0 then
        luna.render.setColor(1, 1, 0.7, clamp(msg_timer, 0, 1))
        luna.render.print(message, OX, OY + GRID_H * CELL + 20, 0.85)
    end

    -- Day report overlay
    if show_report then
        luna.render.setColor(0, 0, 0, 0.75)
        luna.render.rectangle("fill", 100, 100, 400, 220)
        luna.render.setColor(1, 0.9, 0.4, 1)
        luna.render.print(last_report, 120, 120, 1)
        luna.render.setColor(0.5, 1, 0.5, 1)
        luna.render.print("Press ENTER to continue", 180, 290, 0.9)
    end
