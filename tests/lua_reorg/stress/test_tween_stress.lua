...
describe("stress: many active tweens updated simultaneously", function()
    ...
    it("1000 tweens       100 updates each: <5s", function()
        ...
        expect_true(tweens[1]:getElapsed() > 0, "tween elapsed time should advance under load")
    end)

    -- @stress LTween:getProgress
    it("5000 instant tween seek calls: <5s", function()
        ...
        local progress = tw:getProgress()
        expect_true(progress >= 0.0 and progress <= 1.0, "seek keeps progress in range")
        expect_true(elapsed < 5.0, "tween seek budget: " .. elapsed .. "s")
    end)

    -- @stress LTween:onComplete
