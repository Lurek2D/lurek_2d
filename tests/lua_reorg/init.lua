...
package.preload["test_harness"] = function()
    return require("tests.lua_reorg.harness")
end
...
--- @return string path like "tests/artifacts/current/physics/"
function evidence_output_dir(category)
    return "tests/artifacts/current/" .. category .. "/"
end

--- Returns the committed golden baseline directory for a category.
--- @param category string  e.g. "physics", "animation"
--- @return string path like "tests/artifacts/baselines/physics/"
function golden_baseline_dir(category)
    return "tests/artifacts/baselines/" .. category .. "/"
end
...
local function _read_file_bytes(path)
    ...
    if lurek and lurek.filesystem then
        if lurek.filesystem.readBytes then
            local ok, content = pcall(function() return lurek.filesystem.readBytes(path) end)
            if ok then
                return content
            end
        end
        if lurek.filesystem.read then
