...
1. `dofile("tests/lua_reorg/demos/_common_checks.lua")` for shared helpers.
2. Static analysis of `content/games/.../main.lua` (lifecycle callbacks, config sanity).
3. `test_summary()` as the last line.

Colocated `content/games/**/test.lua` files are still discovered by `lua_reorg_demo_colocated_games` in `tests/lua_reorg_tests.rs`. This folder centralizes the screenshot-smoke demo set.
