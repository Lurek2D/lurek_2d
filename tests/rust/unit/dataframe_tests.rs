...
fn test_from_rows_builds_column_major_data() {
    ...
    assert_eq!(
        df.get_value(1, ColRef::Name("name".to_string())).unwrap(),
        CellValue::Text("Bob".to_string())
    );
}
...
fn test_with_eval_pivot_rolling_and_rank_paths() {
    ...
    assert_eq!(
        eval.get_value(2, ColRef::Name("double".to_string()))
            .unwrap(),
    ...
    let roll = base
        ...
        .unwrap();
    assert_eq!(
        roll.get_value(2, ColRef::Name("score_ma2".to_string()))
            .unwrap(),
    ...
    assert_eq!(
        ranked
            .get_value(0, ColRef::Name("rank".to_string()))
            .unwrap(),
        CellValue::Number(3.0)
    );
    assert_eq!(
        ranked
            .get_value(2, ColRef::Name("rank".to_string()))
            .unwrap(),
        CellValue::Number(1.0)
    );
}
...
fn test_query_sql_database_multi_table_join() {
    ...
    assert_eq!(
        out.get_value(0, ColRef::Name("name".to_string())).unwrap(),
        CellValue::Text("Bob".to_string())
    );
    assert_eq!(
        out.get_value(0, ColRef::Name("score".to_string())).unwrap(),
