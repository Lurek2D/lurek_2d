...
mod error_tests {
    ...
    #[test]
    fn error_snapshot_to_json_escapes_control_characters() {
        let err = EngineError::LuaError("line1\nline2\ttab".into());
    ...
    #[test]
    fn error_snapshot_to_json_uses_hint_field_name() {
        let err = EngineError::ConfigError("test".into());
        let snapshot = err.snapshot();
        let json = snapshot.to_json();
