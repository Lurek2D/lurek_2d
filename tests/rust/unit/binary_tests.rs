//! File: tests/rust/unit/binary_tests.rs
...
mod dataview_tests {
    ...
    use lurek2d::binary::DataView;

    #[test]
    fn new_slice_and_reads_reject_out_of_bounds_ranges() {
        let bytes = Arc::new(vec![1_u8, 2, 3, 4]);
        let view = DataView::new_slice(bytes.clone(), 1, 2).expect("slice");
        assert_eq!(view.get_u8(0).expect("u8"), 2);
        assert!(view.get_u16(1).is_err());

        let err = match DataView::new_slice(bytes, 3, 2) {
            Ok(_) => panic!("slice should be out of bounds"),
            Err(err) => err,
        };
        assert!(err.contains("slice out of bounds"));
    }
}
