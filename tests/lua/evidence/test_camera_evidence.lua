...
describe("evidence: camera", function()
    ...
    -- Does: Runs "exports zoom/rotation transform samples" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.camera.newCamera without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/camera/camera_transform_samples.json
    -- Why: This is meaningful only if the visible/text output comes from lurek.camera.newCamera; export helpers are just the container.
    ...
    -- Does: Runs "exports follow smoothing trace" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.camera.newCamera without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/camera/camera_follow_smoothing_trace.json
    -- Why: This is meaningful only if the visible/text output comes from lurek.camera.newCamera; export helpers are just the container.
    ...
    -- Does: Runs "exports shake response trace" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.camera.newCamera without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/camera/camera_shake_response_trace.json
    -- Why: This is meaningful only if the visible/text output comes from lurek.camera.newCamera; export helpers are just the container.
    ...
    -- Artifact: tests/artifacts/current/camera/camera_follow_effects_bounds_trace.txt
    -- Why: This is meaningful only if the visible/text output comes from LCamera:setBounds, LCamera:getBounds, and related owner calls; export helpers are just the container.
