...
describe("Evidence: lurek.ui layouts, widgets, and native charts", function()
    ...
    -- Does: Runs "line chart widget" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newLineChart without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newLineChart; export helpers are just the container.
    ...
    -- Does: Runs "bar chart widget" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newBarChart without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/chart_bar_quarter_kpi.png
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newBarChart; export helpers are just the container.
    ...
    -- Does: Runs "pie chart widget" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newPieChart without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newPieChart; export helpers are just the container.
    ...
    -- Does: Runs "area chart widget" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newAreaChart without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/chart_area_capacity_plan.png
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newAreaChart; export helpers are just the container.
    ...
    -- Does: Runs "scatter plot cluster points" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newScatterPlot without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newScatterPlot; export helpers are just the container.
    ...
    -- Does: Runs "PNG: dense line chart signal" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newLineChart and related owner calls.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the output is driven by lurek.ui.newLineChart and related owner calls rather than by helper-only drawing.
    ...
    -- Does: Runs "bar chart monthly production" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newBarChart without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newBarChart; export helpers are just the container.
    ...
    -- Does: Runs "PNG: pie chart revenue mix" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newPieChart and related owner calls.
    -- Artifact: tests/artifacts/current/ui/pie_chart_revenue_mix.png
    -- Why: This is meaningful only if the output is driven by lurek.ui.newPieChart and related owner calls rather than by helper-only drawing.
    ...
    -- Does: Runs "stacked area chart capacity plan" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newAreaChart without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newAreaChart; export helpers are just the container.
    ...
    -- Does: Runs "analytics chart contact sheet" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.ui.newLineChart, lurek.ui.newBarChart, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/ui/<artifact>
    -- Why: This is meaningful only if the visible/text output comes from lurek.ui.newLineChart, lurek.ui.newBarChart, and related owner calls; export helpers are just the container.
