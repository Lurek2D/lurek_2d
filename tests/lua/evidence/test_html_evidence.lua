...
describe("evidence: html", function()
    ...
    -- Does: Runs "exports markup snapshot" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.html.newDocument and LHtmlDocument:getHtml without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_document_markup_snapshot.txt
    -- Why: This is meaningful only if the visible/text output comes from lurek.html.newDocument and LHtmlDocument:getHtml; export helpers are just the container.
    ...
    -- Does: Runs "exports loaded document snapshot" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.html.loadDocument, LHtmlDocument:getElementById, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_loaded_document_snapshot.txt
    -- Why: This is meaningful only if the visible/text output comes from lurek.html.loadDocument, LHtmlDocument:getElementById, and related owner calls; export helpers are just the container.
    ...
    -- Does: Runs "exports support and render trace" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by lurek.html.supports, LHtmlDocument:render, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_support_render_trace.txt
    -- Why: This is meaningful only if the visible/text output comes from lurek.html.supports, LHtmlDocument:render, and related owner calls; export helpers are just the container.
    ...
    -- Does: Runs "exports element rect and class state" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by LHtmlDocument:setCss, LHtmlDocument:relayout, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_element_state.json
    -- Why: This is meaningful only if the visible/text output comes from LHtmlDocument:setCss, LHtmlDocument:relayout, and related owner calls; export helpers are just the container.
    ...
    -- Does: Runs "exports queryAll and viewport evidence" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by LHtmlDocument:queryAll, LHtmlDocument:setViewport, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_query_viewport_snapshot.json
    -- Why: This is meaningful only if the visible/text output comes from LHtmlDocument:queryAll, LHtmlDocument:setViewport, and related owner calls; export helpers are just the container.
    ...
    -- Does: Runs "exports HTML click event trace" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by LHtmlDocument:mousepressed, lurek.html.preventDefault, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_click_event_trace.txt
    -- Why: This is meaningful only if the visible/text output comes from LHtmlDocument:mousepressed, lurek.html.preventDefault, and related owner calls; export helpers are just the container.
    ...
    -- Does: Runs "exports HTML mutation snapshot" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by LHtmlElement:setHtml, LHtmlElement:setText, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_mutation_snapshot.txt
    -- Why: This is meaningful only if the visible/text output comes from LHtmlElement:setHtml, LHtmlElement:setText, and related owner calls; export helpers are just the container.
    ...
    -- Does: Runs "exports HTML lifecycle trace" and turns the owner-module result into an inspectable artifact.
    -- Shows: The artifact should expose the behavior produced by LHtmlDocument:addCss, LHtmlDocument:clearCss, and related owner calls without needing a special evidence-only renderer.
    -- Artifact: tests/artifacts/current/html/html_lifecycle_trace.txt
    -- Why: This is meaningful only if the visible/text output comes from LHtmlDocument:addCss, LHtmlDocument:clearCss, and related owner calls; export helpers are just the container.
