-- test_evidence_postfx_types.lua
-- Evidence test: proves all 8 new PostFxEffectType variants can be constructed,
-- configured, and introspected via the lurek.postfx Lua API.
...
describe("New PostFxEffectType construction evidence", function()
    for _, type_name in ipairs(NEW_TYPES) do
        it("newEffect('" .. type_name .. "') constructs without error", function()
            local e = lurek.postfx.newEffect(type_name)
...
local ok = lurek.fs.mkdir(output_dir)
if ok then
    local f = lurek.fs.open(output_path, "w")
    if f then
        f:write("# PostFx Types Evidence\n")
        f:write("# Generated by test_evidence_postfx_types.lua\n\n")
