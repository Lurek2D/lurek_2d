...
describe("evidence: tilefield", function()
    ...
    it("PNG: per-player visibility and action masks differ", function()
        ...
        local vis = lurek.visibility.newTileVisibility(field, { players = { "blue", "red" } })
    ...
    -- Why: This proves tilefield lighting is independent gameplay data, not raycaster-owned rendering state.
    it("PNG: multilevel point and global lighting", function()
        ...
        field:addPointLight({ x = 2, y = 4, z = 1, radius = 8, intensity = 1.0, color = { r = 1.0, g = 0.48, b = 0.18 } })
        field:addPointLight({ x = 11, y = 4, z = 1, radius = 7, intensity = 0.85, color = { r = 0.15, g = 0.35, b = 1.0 } })
        field:addPointLight({ x = 5, y = 2, z = 2, radius = 5, intensity = 0.65, color = { r = 0.25, g = 1.0, b = 0.35 } })
        field:addPointLight({ x = 10, y = 7, z = 3, radius = 5, intensity = 0.55, color = { r = 0.9, g = 0.2, b = 0.85 } })
        field:setGlobalLight({ intensity = 0.32, color = { r = 1.0, g = 0.58, b = 0.24 } })
        field:computeLight({ includePointLights = true, includeGlobalLight = true })

        local img = lurek.image.newImageData(12 * cell * 3, 8 * cell)
        img:fill(8, 9, 13, 255)
        for z = 1, 3 do
            local layer = field:exportLightLayer(z)
            for y = 1, 8 do
                for x = 1, 12 do
    ...
    it("TXT: tile lighting numeric calculations", function()
        ...
        blocked:addPointLight({ x = 1, y = 2, z = 1, radius = 5, intensity = 1.0, color = { r = 1, g = 1, b = 1 } })
        blocked:computeLight({ includePointLights = true, includeGlobalLight = false })
        local _, _, _, blocked_luma = blocked:getLight(5, 2, 1)
        ...
        partial:addPointLight({ x = 1, y = 2, z = 1, radius = 8, intensity = 1.0, color = { r = 1, g = 0.25, b = 0 } })
        partial:computeLight({ includePointLights = true, includeGlobalLight = false })
        local pr, pg, pb, pl = partial:getLight(6, 2, 1)
        ...
        radial:addPointLight({ x = 1, y = 1, z = 1, radius = 2, intensity = 1.0, color = { r = 1, g = 1, b = 1 } })
        radial:computeLight({ includePointLights = true, includeGlobalLight = false })
        local _, _, _, diagonal = radial:getLight(3, 3, 1)
        ...
        mixed:addPointLight({ x = 1, y = 2, z = 1, radius = 4, intensity = 1.0, color = { r = 1, g = 0, b = 0 } })
        mixed:addPointLight({ x = 5, y = 2, z = 1, radius = 4, intensity = 1.0, color = { r = 0, g = 0, b = 1 } })
        mixed:computeLight({ includePointLights = true, includeGlobalLight = false })
        local mr, mg, mb, ml = mixed:getLight(3, 2, 1)
        ...
        sky:setGlobalLight({ intensity = 0.4, color = { r = 1.0, g = 0.55, b = 0.25 } })
        sky:computeLight({ includePointLights = false, includeGlobalLight = true })
        local dr, dg, db, dl = sky:getLight(1, 1, 1)
        ...
        sky:setGlobalLight({ intensity = 0.12, color = { r = 0.22, g = 0.32, b = 1.0 } })
        sky:computeLight({ includePointLights = false, includeGlobalLight = true })
        local nr, ng, nb, nl = sky:getLight(1, 1, 1)
