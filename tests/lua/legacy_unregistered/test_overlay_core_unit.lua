...
describe("lurek.overlay", function()
    ...
    it("starts inactive", function()
        local ov = lurek.overlay.new(800, 600)
        expect_false(ov:isActive(), "new overlay should be inactive")
    end)
    ...
    it("clear deactivates all overlays", function()
        ...
        ov:clear()
        expect_false(ov:isActive(), "clear should deactivate all")
    end)
    ...
    it("update handles zero dt and completes active flash", function()
        local ov = lurek.overlay.new(800, 600)
        ov:update(0)
        ov:triggerFlash(1, 1, 1, 1, 0.1)
        ov:update(0.5)
        expect_false(ov:isActive(), "flash should complete after update past duration")
    end)
    ...
    it("creates a screen transition", function()
        ...
        expect_not_nil(t, "transition should be created")
    end)
end)
test_summary()
