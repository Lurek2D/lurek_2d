-- Integration test: library.quest × lurek.time.Scheduler.
--
-- Scope: Drives quest progression and failure from a `lurek.time.Scheduler`
...
-- NOT depend on `lurek.time` directly; the game loop creates a Scheduler and
-- the scheduler callback calls `QuestLog:failQuest(id)` or
-- `QuestLog:advanceObjective(...)`.
--
-- Fallback: none. `lurek.time.newScheduler` is gated by `modules.timer`
-- (default true) and works headless. The "lurek.time" name (NOT
...
describe("integration: library.quest × lurek.time.Scheduler", function()
