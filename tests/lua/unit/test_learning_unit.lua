...
describe("lurek.learning", function()
    ...
    it("adds layers and runs forward pass", function()
        ...
        local ok, err = pcall(function()
            return net:forward({0.5})
        end)
        expect_false(ok, "forward with wrong input length should error")
        expect_true(err ~= nil, "forward length error message not nil")
    end)
    ...
    it("creates a genetic algorithm", function()
        ...
        local ok, err = pcall(function()
            return lurek.learning.newGeneticAlgorithm(0, 5, 42)
        end)
        expect_false(ok, "zero-sized GA should error")
        expect_true(err ~= nil, "zero-sized GA error message not nil")
    end)
    ...
    it("creates a q-learner", function()
        ...
        local seeded = lurek.learning.newQLearner(5, 3, 17)
        expect_true(seeded ~= nil, "seeded qlearner should be created")
        local ok, err = pcall(function()
            return lurek.learning.newQLearner(5, 0)
        end)
        expect_false(ok, "qlearner with zero actions should error")
        expect_true(err ~= nil, "qlearner zero-action error message not nil")
    end)
    ...
    it("deserialize restores learner state", function()
        ...
        local ok, err = pcall(function()
            restored:deserialize("[[1, 'bad', 3]]")
        end)
        expect_false(ok, "invalid qlearner JSON should error")
        expect_true(err ~= nil, "invalid qlearner JSON error message not nil")
    end)
    ...
    it("newTensor creates LTensor with correct type", function()
        ...
        expect_false(ok, "shape/data mismatch should error")
        expect_true(err ~= nil, "shape/data mismatch error message not nil")
    end)
