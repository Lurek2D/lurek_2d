...
describe("lurek.dsp.newLevelDetector", function()
    ...
    it("get_rms returns a positive level after processing audio samples", function()
        local det = lurek.dsp.newLevelDetector()
        det:process_sample(0.5)
        det:process_sample(-0.5)
        expect_true(det:get_rms() > 0.0)
    end)

    -- @covers LLevelDetector:get_peak
    it("get_peak returns the processed peak level", function()
        local det = lurek.dsp.newLevelDetector()
        det:process_sample(0.25)
        det:process_sample(-0.75)
        expect_near(0.75, det:get_peak(), 0.01)
    end)
...
describe("lurek.dsp.newSpectrumAnalyzer", function()
    ...
    it("setSize affects the analyzer and still produces bins", function()
        local sa = lurek.dsp.newSpectrumAnalyzer()
        sa:setSize(32)
        local bins = sa:analyze(make_test_tone(4410, 0.5))
        expect_true(#bins > 0)
    end)
end)
...
describe("lurek.dsp.newAdsrEnvelope", function()
    ...
    it("next_sample returns a numeric envelope sample after trigger_on", function()
        local env = lurek.dsp.newAdsrEnvelope(0.2, 0.1, 0.8, 0.1)
        env:trigger_on()
        expect_type("number", env:next_sample())
    end)
    ...
    it("is_idle reports true after the envelope fully releases", function()
        local env = lurek.dsp.newAdsrEnvelope(0.0, 0.0, 1.0, 0.0)
        env:trigger_on()
        env:trigger_off()
        for _ = 1, 10 do
...
describe("lurek.dsp.newSynthesizer", function()
    ...
    it("render synthesizes sound data from the current waveform", function()
        local synth = lurek.dsp.newSynthesizer()
        local sd = synth:render(220, 0.01, 44100, 0.5)
        expect_type("userdata", sd)
    end)
    ...
    it("generate returns sound data as a render alias", function()
        local synth = lurek.dsp.newSynthesizer()
        local sd = synth:generate(330, 0.01, 44100, 0.25)
        expect_type("userdata", sd)
    end)
end)
...
describe("lurek.dsp.newNode", function()
    ...
    it("getParam reads back a previously stored node parameter", function()
        local node = lurek.dsp.newNode("gain")
        node:setParam("gain", 0.75)
        expect_near(0.75, node:getParam("gain"), 0.001)
    end)
end)
...
describe("lurek.dsp.newGraph", function()
    ...
    it("connect links two added graph nodes", function()
        local g = lurek.dsp.newGraph()
        local id1 = g:addNode(lurek.dsp.newNode("lowpass"))
        local id2 = g:addNode(lurek.dsp.newNode("gain"))
        expect_true(g:connect(id1, id2))
    end)

    -- @covers LDspGraph:disconnect
    it("disconnect removes an existing graph connection", function()
        local g = lurek.dsp.newGraph()
        local id1 = g:addNode(lurek.dsp.newNode("lowpass"))
        local id2 = g:addNode(lurek.dsp.newNode("gain"))
        expect_true(g:connect(id1, id2))
        expect_true(g:disconnect(id1, id2))
    end)
