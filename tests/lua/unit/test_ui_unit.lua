...
describe("ui extended animations", function()
    ...
    it("starts rotation animation with explicit and default easing", function()
        local panel = lurek.ui.newPanel()
        local result = lurek.ui.animateRotation(panel, 0.0, 3.14, 1.0, "bounce_out")
        expect_true(result, "animateRotation should return true for valid widget")
        local fallback = lurek.ui.animateRotation(panel, 0.0, 1.57, 1.0)
    ...
    it("starts color tint animation on widget", function()
        ...
        local result = lurek.ui.animateColor(panel, from, to, 0.8, "sine_in_out")
...
describe("ui utility widget owner coverage", function()
    ...
    it("menu bar removeMenu removes a stored menu widget index", function()
        ...
        local file_menu = lurek.ui.newMenuItem("File")
        menu_bar:addMenu(file_menu)
        expect_true(menu_bar:removeMenu(file_menu))
...
it("widget handles report stale state after clear", function()
    local widget = lurek.ui.newButton("stale")
    expect_true(widget:isValid())
    local forged = { _idx = widget._idx }
    expect_error(function()
        lurek.ui.setFocus(forged)
    end)
    lurek.ui.clear()
...
-- @covers lurek.ui.destroy
it("destroy invalidates a widget subtree and removes stale references", function()
    local parent = lurek.ui.newPanel()
...
it("widget destroy supports leaf destruction", function()
    local widget = lurek.ui.newButton("leaf")
