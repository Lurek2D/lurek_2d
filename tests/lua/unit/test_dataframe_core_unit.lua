...
describe("sort", function()
    ...
    -- @covers LDataFrame:sort
    -- @covers LDataFrame:sort.fromTable
...
describe("Database SQL", function()
    ...
    -- @covers LDatabase:addTable
    it("queryParams binds booleans and rejects placeholder mismatches [LDatabase:addTable]", function()
        local db = lurek.dataframe.newDatabase()
        local df = lurek.dataframe.fromRows(
            { "name", "active" },
    ...
    -- @covers LDatabase:queryParams
    it("queryParams binds booleans and rejects placeholder mismatches [LDatabase:queryParams]", function()
        local db = lurek.dataframe.newDatabase()
        local df = lurek.dataframe.fromRows(
            { "name", "active" },
            { { "Alice", true }, { "Bob", false } }
    ...
    -- @covers lurek.dataframe.newDatabase
    it("queryParams binds booleans and rejects placeholder mismatches [lurek.dataframe.newDatabase]", function()
        local db = lurek.dataframe.newDatabase()
        local df = lurek.dataframe.fromRows(
            { "name", "active" },
            { { "Alice", true }, { "Bob", false } }
...
describe("async dataframe tasks", function()
    ...
    -- @covers lurek.dataframe.fromCSVFileAsync
    it("fromCSVFileAsync reports malformed CSV errors [lurek.dataframe.fromCSVFileAsync]", function()
    ...
    -- @covers LDataFrameTask:getError
    it("fromCSVFileAsync reports malformed CSV errors [LDataFrameTask:getError]", function()
        local path = TMP .. "async_bad.csv"
        lurek.filesystem.write(path, "name,score\nAlice\n")
    ...
    -- @covers LDataFrameTask:result
    it("fromCSVFileAsync reports malformed CSV errors [LDataFrameTask:result]", function()
        local path = TMP .. "async_bad.csv"
        lurek.filesystem.write(path, "name,score\nAlice\n")
    ...
    -- @covers lurek.dataframe.fromJSONFileAsync
    it("fromJSONFileAsync reports malformed JSON errors [lurek.dataframe.fromJSONFileAsync]", function()
    ...
    -- @covers LDataFrameTask:getError
    it("fromJSONFileAsync reports malformed JSON errors [LDataFrameTask:getError]", function()
        local path = TMP .. "async_bad.json"
        lurek.filesystem.write(path, "{not valid json")
    ...
    -- @covers LDataFrameTask:result
    it("fromJSONFileAsync reports malformed JSON errors [LDataFrameTask:result]", function()
        local path = TMP .. "async_bad.json"
        lurek.filesystem.write(path, "{not valid json")
...
describe("lurek.dataframe.DataFrame analytics", function()
    ...
    -- @covers LDataFrame:addRow
    it("groupAgg errors on unknown aggregate name [LDataFrame:addRow]", function()
        local df = lurek.dataframe.newDataFrame()
        df_add_column(df, "cat", "")
        df_add_column(df, "val", 0)
        df:addRow({cat = "A", val = 10})
    ...
    -- @covers LDataFrame:groupAgg
    it("groupAgg errors on unknown aggregate name [LDataFrame:groupAgg]", function()
        local df = lurek.dataframe.newDataFrame()
        df_add_column(df, "cat", "")
        df_add_column(df, "val", 0)
        df:addRow({cat = "A", val = 10})
    ...
    -- @covers lurek.dataframe.newDataFrame
    it("groupAgg errors on unknown aggregate name [lurek.dataframe.newDataFrame]", function()
        local df = lurek.dataframe.newDataFrame()
        df_add_column(df, "cat", "")
        df_add_column(df, "val", 0)
        df:addRow({cat = "A", val = 10})
...
describe("VecFrame scalar column operations", function()
    ...
    -- @covers lurek.dataframe.fromVec
    it("colClamp clamps values to", function()
        local vf = make_vf()
    ...
    -- @covers LVecFrame:colAdd
    it("scalar ops on text columns error [LVecFrame:colAdd]", function()
    ...
    -- @covers lurek.dataframe.fromCSV
    it("scalar ops on text columns error [lurek.dataframe.fromCSV]", function()
        local df = lurek.dataframe.fromCSV("name\nAlice\nBob\n")
        local vf = lurek.dataframe.toVec(df)
        expect_error(function()
            vf:colAdd("name", 1)
    ...
    -- @covers lurek.dataframe.toVec
    it("scalar ops on text columns error [lurek.dataframe.toVec]", function()
        local df = lurek.dataframe.fromCSV("name\nAlice\nBob\n")
        local vf = lurek.dataframe.toVec(df)
        expect_error(function()
            vf:colAdd("name", 1)
...
describe("VecFrame reductions", function()
    ...
    -- @covers lurek.dataframe.fromCSV
    -- @covers LDataFrame:reduce.toVec
...
describe("DataFrame query and clone", function()
    ...
    -- @covers LDataFrame:nrows
    -- @covers LDataFrame:removeRow
    -- @covers LDataFrame:clone.fromCSV
...
describe("grouped:aggregate with Lua callback", function()
    ...
    -- @covers LDataFrame:groupByObj
    it("aggregate with sum callback produces correct result [LDataFrame:groupByObj]", function()
    ...
    it("aggregate with sum callback produces correct result [LGroupedFrame:aggregate]", function()
        local csv = "category,value\na,1\na,2\nb,10"
        local df = lurek.dataframe.fromCSV(csv)
        local g = df:groupByObj("category")
        if g then
    ...
    -- @covers lurek.dataframe.fromCSV
    it("aggregate with sum callback produces correct result [lurek.dataframe.fromCSV]", function()
        local csv = "category,value\na,1\na,2\nb,10"
        local df = lurek.dataframe.fromCSV(csv)
        local g = df:groupByObj("category")
        if g then
...
-- @describe lurek.dataframe.newDataFrame
describe("lurek.dataframe.newDataFrame", function()
    -- @covers lurek.dataframe.newDataFrame
    it("creates an empty dataframe", function()
        local df = lurek.dataframe.newDataFrame()
        expect_not_nil(df, "dataframe must not be nil")
        expect_equal(0, df:nrows())
    end)
end)

-- @describe lurek.dataframe.fromCSV
describe("lurek.dataframe.fromCSV", function()
    -- @covers lurek.dataframe.fromCSV
    it("parses CSV string into dataframe with correct row count", function()
        local df = lurek.dataframe.fromCSV("a,b,c\n1,2,3\n4,5,6\n7,8,9")
        expect_equal(3, df:nrows())
    end)

    -- @covers lurek.dataframe.fromCSV
    it("parses column names from header row", function()
        local df = lurek.dataframe.fromCSV("name,age\nAlice,30\nBob,25")
        local cols = df:columns()
        expect_equal(2, #cols)
    end)
end)

-- @describe lurek.dataframe.fromRows
describe("lurek.dataframe.fromRows", function()
    -- @covers lurek.dataframe.fromRows
    it("creates dataframe from column names and row data", function()
        local df = lurek.dataframe.fromRows({"x", "y"}, {{1, 2}, {3, 4}, {5, 6}})
        expect_equal(3, df:nrows())
        expect_equal(2, #df:columns())
    end)
end)

-- @describe lurek.dataframe.filter
describe("lurek.dataframe.filter", function()
    -- @covers lurek.dataframe.filter
    -- @covers lurek.dataframe.fromCSV
    it("filters rows matching equality condition", function()
        local df = lurek.dataframe.fromCSV("name,score\nAlice,90\nBob,70\nAlice,85")
        local result = df:filter("name", "==", "Alice")
        expect_equal(2, result:nrows())
    end)
    -- @covers lurek.dataframe.filter
    -- @covers lurek.dataframe.fromCSV
    it("filters rows with numeric comparison", function()
        local df = lurek.dataframe.fromCSV("item,price\nA,10\nB,25\nC,50")
        local result = df:filter("price", ">", 20)
        expect_equal(2, result:nrows())
    end)
end)

-- @describe lurek.dataframe.select
describe("lurek.dataframe.select", function()
    -- @covers lurek.dataframe.select
    -- @covers lurek.dataframe.fromCSV
    it("returns dataframe with only selected columns", function()
        local df = lurek.dataframe.fromCSV("a,b,c\n1,2,3\n4,5,6")
        local result = df:select("a", "c")
...
-- @describe lurek.dataframe.groupBy
describe("lurek.dataframe.groupBy", function()
    -- @covers lurek.dataframe.groupBy
    -- @covers lurek.dataframe.fromCSV
    it("groups rows by column and returns table of groups", function()
        local df = lurek.dataframe.fromCSV("team,score\nA,10\nB,20\nA,30\nB,40")
        local groups = df:groupBy("team")
        expect_not_nil(groups, "groupBy must return a result")
        local count = 0
        for _ in pairs(groups) do count = count + 1 end
        expect_true(count >= 2, "should have at least 2 groups")
    end)
end)

-- @describe lurek.dataframe.sort
describe("lurek.dataframe.sort", function()
    -- @covers lurek.dataframe.sort
    -- @covers lurek.dataframe.fromCSV
    it("sorts rows in ascending order by column", function()
        local df = lurek.dataframe.fromCSV("name,val\nC,3\nA,1\nB,2")
        local sorted = df:sort("val", true)
        expect_equal(3, sorted:nrows())
    end)
    -- @covers lurek.dataframe.sort
    -- @covers lurek.dataframe.fromCSV
    it("sorts rows in descending order", function()
        local df = lurek.dataframe.fromCSV("name,val\nA,1\nC,3\nB,2")
        local sorted = df:sort("val", false)
        expect_equal(3, sorted:nrows())
    end)
end)

-- @describe lurek.dataframe.join
describe("lurek.dataframe.join", function()
    -- @covers lurek.dataframe.join
    -- @covers lurek.dataframe.fromCSV
    it("joins two dataframes on a shared column", function()
        local left = lurek.dataframe.fromCSV("id,name\n1,Alice\n2,Bob")
        local right = lurek.dataframe.fromCSV("id,score\n1,90\n2,80")
        local joined = left:join(right, "id", "id")
        expect_not_nil(joined, "join result must not be nil")
        expect_equal(2, joined:nrows())
    end)
end)

-- @describe parallel operations
describe("parallel operations", function()
    -- @covers lurek.dataframe.parFilter
    it("parFilter works on small frame", function()
        local df = lurek.dataframe.fromCSV("name,score\nAlice,90\nBob,60\nCarol,85")
        local result = df:parFilter("score", ">", 70)
        expect_equal(2, result:nrows())
    end)

    -- @covers lurek.dataframe.parFilter
    it("parFilter with equality operator", function()
        local df = lurek.dataframe.fromCSV("name,score\nAlice,90\nBob,60\nCarol,90")
        local result = df:parFilter("score", "==", 90)
        expect_equal(2, result:nrows())
    end)

    -- @covers lurek.dataframe.parGroupAgg
    it("parGroupAgg computes sum aggregation", function()
        local df = lurek.dataframe.fromCSV("category,value\nA,10\nB,20\nA,30\nB,40")
        local result = df:parGroupAgg("category", "value", "sum")
        expect_equal(2, result:nrows())
    end)

    -- @covers lurek.dataframe.parGroupAgg
    it("parGroupAgg computes count aggregation", function()
        local df = lurek.dataframe.fromCSV("category,value\nA,10\nB,20\nA,30\nB,40")
        local result = df:parGroupAgg("category", "value", "count")
        expect_equal(2, result:nrows())
    end)
end)
test_summary()
