...
describe("draw()", function()
    local function make_playing_animation()
        local a = lurek.animation.new()
        a:addFrame(0, 0, 16, 16)
        a:addClip("idle", {0}, 12, true)
        a:play("idle")
        return a, lurek.render.newImage("assets/icon.png")
    end

    -- @covers LAnimation:draw
    -- @covers LAnimation:getCurrentFrame
    it("returns true for an active frame and does not advance playback", function()
        local a, img = make_playing_animation()
        local before = a:getCurrentFrame()
        local ok = a:draw(img, 10, 20, {
            rotation = 0.25,
            scale = 2.0,
            scaleX = 1.5,
            scaleY = 0.5,
    ...
    it("returns false when no current frame is active", function()
        ...
        expect_false(a:draw(img))
    end)

    -- @covers LAnimation:draw
    it("errors when image is not an LImage", function()
        local a = lurek.animation.new()
        a:addFrame(0, 0, 16, 16)
        a:addClip("idle", {0}, 12, true)
        a:play("idle")
    ...
    -- @covers LAnimation:draw
    it("errors when image handle has been released", function()
        local a, img = make_playing_animation()
        img:release()
        expect_error(function()
            a:draw(img)
    ...
    it("errors when opts is not a table", function()
        local a, img = make_playing_animation()
        expect_error(function()
            a:draw(img, 0, 0, "bad opts")
        end)
    end)

    -- @covers LAnimation:draw
    it("errors when an option field is not numeric", function()
        local a, img = make_playing_animation()
        expect_error(function()
            a:draw(img, 0, 0, { rotation = "bad" })
        end)
    end)
end)
...
describe("newStateMachine()", function()
    ...
    it("draw returns true for the active state-machine frame", function()
        local a = make_anim()
        a:play("idle")
        local fsm = lurek.animation.newStateMachine(a, "idle")
        local img = lurek.render.newImage("assets/icon.png")
        expect_true(fsm:draw(img, 5, 6, { scale = 1.25 }))
    end)
end)
...
describe("AnimCurve custom easing", function()
    ...
    it("setCustomEasing nil clears callback and reverts to linear", function()
        if lurek.animation.newCurve then
            ...
            local clear_chunk = assert(load_func("return function(curve) curve:setCustomEasing(nil) end"))()
            clear_chunk(c)
            local v = c:eval(0.5)
...
-- @describe setImage() / draw without explicit image (LAnimation)
describe("setImage() / draw without explicit image (LAnimation)", function()
    local function make_playing_animation()
        local a = lurek.animation.new()
        a:addFrame(0, 0, 16, 16)
        a:addClip("idle", {0}, 12, true)
    ...
    -- @covers LAnimation:draw
    it("setImage stores image; draw(x, y) succeeds without explicit image arg", function()
...
-- @describe setImage() / draw without explicit image (LAnimStateMachine)
describe("setImage() / draw without explicit image (LAnimStateMachine)", function()
    local function make_playing_statemachine()
        local a = lurek.animation.new()
        a:addFrame(0, 0, 16, 16)
        a:addClip("idle", {0}, 12, true)
        a:play("idle")
