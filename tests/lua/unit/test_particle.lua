...
describe("luna.particles module exists", function()
    it("luna.particles is a table", function()
        expect_type("table", luna.particles)
    end)
end)

describe("luna.particles.newSystem", function()
    ...
    it("newSystem with no config uses defaults", function()
        ...
        expect_true(luna.particles.isActive(ps), "default system should be active")
    end)

    it("newSystem with config table", function()
        local ps = luna.particles.newSystem({ emissionRate = 50, maxParticles = 100 })
        expect_type("userdata", ps)
    end)

    it("newSystem backward compat: sizeStart/sizeEnd", function()
        local ps = luna.particles.newSystem({ sizeStart = 8.0, sizeEnd = 2.0 })
        expect_type("userdata", ps)
    end)
...
describe("luna.particles lifecycle", function()
    it("isActive returns true for new system", function()
        local ps = luna.particles.newSystem()
        expect_true(luna.particles.isActive(ps), "new system should be active")
    end)

    it("isPaused returns false for new system", function()
        local ps = luna.particles.newSystem()
        expect_true(not luna.particles.isPaused(ps), "new system should not be paused")
    end)

    it("isStopped returns false for new (active) system", function()
        local ps = luna.particles.newSystem()
        expect_true(not luna.particles.isStopped(ps), "new system should not be stopped")
    end)

    it("stop sets isStopped", function()
        local ps = luna.particles.newSystem()
        luna.particles.stop(ps)
        expect_true(luna.particles.isStopped(ps), "stopped system should report isStopped")
    end)

    it("pause sets isPaused", function()
        local ps = luna.particles.newSystem()
        luna.particles.pause(ps)
        expect_true(luna.particles.isPaused(ps), "paused system should report isPaused")
    end)

    it("start after stop resumes active state", function()
        local ps = luna.particles.newSystem()
        luna.particles.stop(ps)
        luna.particles.start(ps)
        expect_true(luna.particles.isActive(ps), "started system should be active")
    end)

    it("reset clears particles and keeps active", function()
        local ps = luna.particles.newSystem({ emissionRate = 1000, maxParticles = 50 })
        luna.particles.update(ps, 1.0)
        luna.particles.reset(ps)
        expect_equal(0, luna.particles.getCount(ps), "count after reset")
    end)
end)

describe("luna.particles.getCount / isEmpty / isFull", function()
    it("getCount returns 0 before any update", function()
        local ps = luna.particles.newSystem()
        expect_equal(0, luna.particles.getCount(ps), "count before update")
    end)

    it("isEmpty returns true when count is 0", function()
        local ps = luna.particles.newSystem()
        expect_true(luna.particles.isEmpty(ps), "empty before update")
    end)

    it("isFull returns false for fresh system", function()
        local ps = luna.particles.newSystem({ maxParticles = 100 })
        expect_true(not luna.particles.isFull(ps), "not full before update")
    end)

    it("emit burst fills particles immediately", function()
        local ps = luna.particles.newSystem({ maxParticles = 100 })
        luna.particles.stop(ps)  -- stop continuous emission
        luna.particles.emit(ps, 10)
        expect_true(luna.particles.getCount(ps) > 0, "count should increase after emit")
    end)

    it("getCount increases after update with high emission rate", function()
        local ps = luna.particles.newSystem({ emissionRate = 500, maxParticles = 50 })
        luna.particles.update(ps, 0.1)
        expect_true(luna.particles.getCount(ps) > 0, "count should be positive after update")
    end)
end)

describe("luna.particles position", function()
    it("setPosition / getPosition round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setPosition(ps, 100, 200)
        local x, y = luna.particles.getPosition(ps)
    ...
    it("moveTo updates position", function()
        local ps = luna.particles.newSystem()
        luna.particles.moveTo(ps, 50, 75)
        local x, y = luna.particles.getPosition(ps)
...
describe("luna.particles emission settings", function()
    it("setEmissionRate / getEmissionRate round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setEmissionRate(ps, 99.0)
        local rate = luna.particles.getEmissionRate(ps)
    ...
    it("setParticleLifetime / getParticleLifetime round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setParticleLifetime(ps, 0.5, 2.5)
        local mn, mx = luna.particles.getParticleLifetime(ps)
    ...
    it("setEmitterLifetime / getEmitterLifetime round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setEmitterLifetime(ps, 5.0)
        local t = luna.particles.getEmitterLifetime(ps)
        expect_true(math.abs(t - 5.0) < 0.001, "emitter lifetime")
    end)

    it("setSpeed / getSpeed round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setSpeed(ps, 20.0, 80.0)
        local mn, mx = luna.particles.getSpeed(ps)
    ...
    it("setDirection / getDirection round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setDirection(ps, 1.23)
        local d = luna.particles.getDirection(ps)
        expect_true(math.abs(d - 1.23) < 0.001, "direction")
    end)

    it("setSpread / getSpread round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setSpread(ps, 0.5)
        local s = luna.particles.getSpread(ps)
        expect_true(math.abs(s - 0.5) < 0.001, "spread")
    end)
end)

describe("luna.particles acceleration settings", function()
    it("setLinearAcceleration / getLinearAcceleration round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setLinearAcceleration(ps, -10, -50, 10, 50)
        local xmin, ymin, xmax, ymax = luna.particles.getLinearAcceleration(ps)
    ...
    it("setRadialAcceleration / getRadialAcceleration round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setRadialAcceleration(ps, -5.0, 5.0)
        local mn, mx = luna.particles.getRadialAcceleration(ps)
    ...
    it("setTangentialAcceleration / getTangentialAcceleration round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setTangentialAcceleration(ps, 1.0, 3.0)
        local mn, mx = luna.particles.getTangentialAcceleration(ps)
    ...
    it("setLinearDamping / getLinearDamping round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setLinearDamping(ps, 0.1, 0.9)
        local mn, mx = luna.particles.getLinearDamping(ps)
        expect_true(math.abs(mn - 0.1) < 0.001, "damping min")
        expect_true(math.abs(mx - 0.9) < 0.001, "damping max")
    end)
end)

describe("luna.particles size settings", function()
    it("setSizes with multiple keyframes / getSizes round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setSizes(ps, 8.0, 4.0, 2.0, 1.0)
        local sizes = luna.particles.getSizes(ps)
    ...
    it("setSizeVariation / getSizeVariation round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setSizeVariation(ps, 0.5)
        local v = luna.particles.getSizeVariation(ps)
        expect_true(math.abs(v - 0.5) < 0.001, "size variation")
    end)
end)

describe("luna.particles rotation settings", function()
    it("setRotation / getRotation round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setRotation(ps, 0.1, 0.9)
        local mn, mx = luna.particles.getRotation(ps)
    ...
    it("setSpin / getSpin round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setSpin(ps, 0.2, 1.5)
        local mn, mx = luna.particles.getSpin(ps)
    ...
    it("setSpinVariation / getSpinVariation round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setSpinVariation(ps, 0.75)
        local v = luna.particles.getSpinVariation(ps)
        expect_true(math.abs(v - 0.75) < 0.001, "spin variation")
    end)

    it("setRelativeRotation / hasRelativeRotation round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setRelativeRotation(ps, true)
        expect_true(luna.particles.hasRelativeRotation(ps), "relative rotation enabled")
        luna.particles.setRelativeRotation(ps, false)
        expect_true(not luna.particles.hasRelativeRotation(ps), "relative rotation disabled")
    end)
end)

describe("luna.particles color settings", function()
    it("setColors / getColors round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setColors(ps, {1, 0, 0, 1}, {0, 0, 1, 0})
        -- getColors returns a sequence of color tables: {{r,g,b,a}, {r,g,b,a}, ...}
        local colors = luna.particles.getColors(ps)
...
describe("luna.particles rendering settings", function()
    it("setOffset / getOffset round-trip", function()
        local ps = luna.particles.newSystem()
        luna.particles.setOffset(ps, 4.0, 8.0)
        local ox, oy = luna.particles.getOffset(ps)
    ...
    it("setInsertMode / getInsertMode: top", function()
        local ps = luna.particles.newSystem()
        luna.particles.setInsertMode(ps, "top")
        expect_equal("top", luna.particles.getInsertMode(ps), "insert mode")
    end)

    it("setInsertMode / getInsertMode: bottom", function()
        local ps = luna.particles.newSystem()
        luna.particles.setInsertMode(ps, "bottom")
        expect_equal("bottom", luna.particles.getInsertMode(ps), "insert mode bottom")
    end)

    it("setInsertMode / getInsertMode: random", function()
        local ps = luna.particles.newSystem()
        luna.particles.setInsertMode(ps, "random")
        expect_equal("random", luna.particles.getInsertMode(ps), "insert mode random")
    end)

    it("setBufferSize / getBufferSize round-trip", function()
        local ps = luna.particles.newSystem({ maxParticles = 50 })
        luna.particles.setBufferSize(ps, 200)
        expect_equal(200, luna.particles.getBufferSize(ps), "buffer size")
    end)
end)

describe("luna.particles emission area", function()
    it("setEmissionArea / getEmissionArea: none", function()
        local ps = luna.particles.newSystem()
        luna.particles.setEmissionArea(ps, "none", 0, 0)
        local dist, w, h = luna.particles.getEmissionArea(ps)
        expect_equal("none", dist, "area distribution none")
    end)

    it("setEmissionArea: uniform rectangle", function()
        local ps = luna.particles.newSystem()
        luna.particles.setEmissionArea(ps, "uniform", 100, 50)
        local dist, w, h = luna.particles.getEmissionArea(ps)
    ...
    it("setEmissionArea: ellipse", function()
        local ps = luna.particles.newSystem()
        luna.particles.setEmissionArea(ps, "ellipse", 60, 30)
        local dist = luna.particles.getEmissionArea(ps)
        expect_equal("ellipse", dist, "area distribution ellipse")
    end)
end)

describe("luna.particles object-method syntax", function()
    ...
    it("ps:emit(n) / ps:getCount() work", function()
        local ps = luna.particles.newSystem({ maxParticles = 20 })
        ps:stop()
        ps:emit(5)
...
describe("luna.particles.clone", function()
    it("clone returns a different userdata handle", function()
        local ps = luna.particles.newSystem({ emissionRate = 77.0 })
        local ps2 = luna.particles.clone(ps)
        expect_type("userdata", ps2)
        -- Clones share config but are independent objects
        local r1 = luna.particles.getEmissionRate(ps)
        local r2 = luna.particles.getEmissionRate(ps2)
        expect_true(math.abs(r1 - r2) < 0.001, "clone has same emission rate")
    end)
end)

describe("luna.particles.release", function()
    it("release returns true for valid handle", function()
        local ps = luna.particles.newSystem()
        local ok = luna.particles.release(ps)
        expect_equal(true, ok, "release returns true")
    end)

    it("accessing released handle raises an error", function()
        local ps = luna.particles.newSystem()
        luna.particles.release(ps)
        local ok, err = pcall(function() luna.particles.getCount(ps) end)
...
describe("particle gravity", function()
    it("gravity_y keeps particle alive after update", function()
        ...
        luna.particles.emit(ps, 1)
        luna.particles.update(ps, 0.1)
        -- Particle should still be alive (lifetime=10s, only 0.1s elapsed)
        expect_equal(luna.particles.getCount(ps), 1)
        luna.particles.release(ps)
    end)

    it("gravityY config key is accepted", function()
        local ps = luna.particles.newSystem({ gravityY = 100 })
        local gx, gy = luna.particles.getGravity(ps)
