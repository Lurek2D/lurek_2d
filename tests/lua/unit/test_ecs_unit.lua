...
describe("LUniverse blueprints", function()
    ...
    it("spawnBulk returns ids for entities spawned from one blueprint", function()
        ...
        local version = world:getVersion()
        local ids = world:spawnBulk("bullet", 3)
        expect_equal(3, #ids)
        expect_true(world:isAlive(ids[1]))
        expect_equal(version + 1, world:getVersion())
        world:get(ids[1], "pos").x = 9
        expect_equal(0, world:get(ids[2], "pos").x)
    end)
end)
...
describe("LUniverse ChangeSet bridge", function()
    ...
    -- @covers LUniverse:prepareChangeSet
    it("prepares and commits one version-checked logical mutation", function()
        local world = new_world()
        local entity = world:spawn()
        local version = world:getVersion()
        local changes = lurek.event.newChangeSet({ schema = "ecs", revision = 1 })
        changes:append(entity, "hp", "set", 42)
        changes:append(entity, "speed", "set", 5)
        local batch = world:prepareChangeSet(changes:toTable(), version)
        local preview = batch:preview()
        expect_equal(version, preview.baseVersion)
        expect_equal(2, preview.recordCount)
        expect_true(batch:isPending())
        expect_nil(world:get(entity, "hp"))

        expect_equal(2, batch:commit())
    ...
    -- @covers LUniverse:getVersion
    it("reports a monotonic mutation version", function()
        local world = new_world()
        local version = world:getVersion()
        world:spawn()
        expect_equal(version + 1, world:getVersion())
    ...
    -- @covers LEcsBatch:preview
    it("previews staged records without mutating the universe", function()
        local world = new_world()
        local entity = world:spawn()
        local changes = lurek.event.newChangeSet({ schema = "ecs", revision = 1 })
        changes:append(entity, "hp", "set", 42)
        local preview = world:prepareChangeSet(changes:toTable()):preview()
        expect_equal(1, preview.recordCount)
        expect_nil(world:get(entity, "hp"))
    end)

    -- @covers LEcsBatch:isPending
    it("reports whether a prepared change set remains pending", function()
        local world = new_world()
        local changes = lurek.event.newChangeSet({ schema = "ecs", revision = 1 })
    ...
    -- @covers LEcsBatch:commit
    it("keeps a conflicted batch pending without partial changes", function()
        local world = new_world()
        local entity = world:spawn()
        local changes = lurek.event.newChangeSet({ schema = "ecs", revision = 1 })
        changes:append(entity, "hp", "set", 42)
        local batch = world:prepareChangeSet(changes:toTable())
        world:set(entity, "concurrent", true)
        expect_false(pcall(function() batch:commit() end))
        expect_true(batch:isPending())
        expect_nil(world:get(entity, "hp"))
    end)

    -- @covers LEcsBatch:discard
    it("supports explicit discard and runtime type checks", function()
        local world = new_world()
        local changes = lurek.event.newChangeSet({ schema = "ecs", revision = 1 })
        local batch = world:prepareChangeSet(changes:toTable())
        expect_equal("LEcsBatch", batch:type())
        expect_true(batch:typeOf("LEcsBatch"))
    ...
    -- @covers LEcsBatch:type
    it("reports its concrete userdata type", function()
        local world = new_world()
        local changes = lurek.event.newChangeSet({ schema = "ecs", revision = 1 })
        expect_equal("LEcsBatch", world:prepareChangeSet(changes:toTable()):type())
    end)

    -- @covers LEcsBatch:typeOf
    ...
    -- @covers LUniverse:readComponents
    it("reads selected component rows in stable request order", function()
        local world = new_world()
        local a = world:spawn()
        local b = world:spawn()
        world:set(a, "hp", 10)
        world:set(b, "hp", 20)
        world:set(b, "name", "worker")
        local rows = world:readComponents({
            { id = b, names = { "name", "hp" } },
            { id = a, names = { "hp", "name" } },
        })
        expect_equal(b, rows[1].id)
        expect_equal("worker", rows[1].components.name)
        expect_equal(20, rows[1].components.hp)
        expect_equal(a, rows[2].id)
        expect_equal(10, rows[2].components.hp)
        expect_nil(rows[2].components.name)
    end)
end)
