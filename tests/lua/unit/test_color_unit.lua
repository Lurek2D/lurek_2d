...
describe("lurek.color.fromHex", function()
    ...
    it("parses rgb and rgba hex strings and rejects invalid input", function()
        ...
        c = lurek.color.fromHex("#0F8")
        expect_not_nil(c, "rgb shorthand should parse")
        expect_near(0.0, c[1], 0.01)
        expect_near(1.0, c[2], 0.01)
        expect_near(0x88 / 255, c[3], 0.01)

        c = lurek.color.fromHex("#1A2C")
        expect_not_nil(c, "rgba shorthand should parse")
        expect_near(0x11 / 255, c[1], 0.01)
        expect_near(0xAA / 255, c[2], 0.01)
        expect_near(0x22 / 255, c[3], 0.01)
        expect_near(0xCC / 255, c[4], 0.01)
