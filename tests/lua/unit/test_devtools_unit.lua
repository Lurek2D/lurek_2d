...
-- BEGIN test_devtools_core_unit.lua
do
...
describe("lurek.devtools logger", function()
    ...
    it("returns the current log file path", function()
...
describe("lurek.devtools frame stats", function()
    ...
    it("adds one gpu frame sample to the gpu stats history", function()
        local before = lurek.devtools.getGpuFrameStats()
        lurek.devtools.recordGpuFrameTime(0.0125)
        local after = lurek.devtools.getGpuFrameStats()
        expect_equal(before.samples + 1, after.samples)
        expect_true(after.max >= 0.0125)
    end)
end)
...
describe("lurek.devtools profiler", function()
    ...
    it("records a pushed profiling zone into the next profiler frame", function()
        lurek.devtools.resetProfile()
        lurek.devtools.setProfilingEnabled(true)
        lurek.devtools.profilePush("push-zone")
        lurek.devtools.profilePop()
        lurek.devtools.profileFrame()

        local data = lurek.devtools.getProfileData(0)
        expect_equal(1, #data)
        expect_equal("push-zone", data[1].name)
    ...
    it("accepts an optional zone name and closes the active zone", function()
        lurek.devtools.resetProfile()
        lurek.devtools.setProfilingEnabled(true)
        lurek.devtools.profilePush("pop-zone")
        lurek.devtools.profilePop("ignored-name")
        lurek.devtools.profileFrame()

        local data = lurek.devtools.getProfileData(0)
        expect_equal(1, #data)
        expect_equal("pop-zone", data[1].name)
    ...
    it("stores the current profiling frame when requested", function()
        lurek.devtools.resetProfile()
        lurek.devtools.setProfilingEnabled(true)
        lurek.devtools.profilePush("frame-zone")
        lurek.devtools.profilePop()
        lurek.devtools.profileFrame()
        expect_equal(1, lurek.devtools.getProfileFrameCount())
        lurek.devtools.resetProfile()
        lurek.devtools.setProfilingEnabled(false)
    end)

    -- @covers lurek.devtools.getProfileFrameCount
    it("reports how many profiling frames are retained", function()
        lurek.devtools.resetProfile()
        lurek.devtools.setProfilingEnabled(true)
        expect_equal(0, lurek.devtools.getProfileFrameCount())
        lurek.devtools.profilePush("count-zone")
        lurek.devtools.profilePop()
        lurek.devtools.profileFrame()
        expect_equal(1, lurek.devtools.getProfileFrameCount())
        lurek.devtools.resetProfile()
...
describe("lurek.devtools file watcher", function()
    ...
    it("removes all module-level watched paths", function()
        lurek.devtools.clearWatches()
        lurek.devtools.watch("save/watch_a.txt")
        lurek.devtools.watch("save/watch_b.txt")
        expect_equal(2, #lurek.devtools.getWatchedPaths())
        lurek.devtools.clearWatches()
        expect_equal(0, #lurek.devtools.getWatchedPaths())
    end)
...
describe("devtools strict: LFileWatcher type / typeOf", function()
    ...
    it("LFileWatcher type returns the watcher userdata type name", function()
        local fw = lurek.devtools.newFileWatcher("save")
        expect_equal("LFileWatcher", fw:type())
    end)
...
describe("devtools strict: LReplConsole type / typeOf", function()
    ...
    it("LReplConsole type returns the repl userdata type name", function()
        local repl = lurek.devtools.newRepl(10)
        expect_equal("LReplConsole", repl:type())
    end)
...
-- END test_devtools_core_unit.lua
