...
-- BEGIN test_parallax_core_unit.lua
do
...
describe("lurek.parallax", function()
    ...
    it("getTint returns the current tint rgba values", function()
        local layer = make_layer()
        layer:setTint(0.5, 0.3, 0.8, 0.7)
        local r, g, b, a = layer:getTint()
        expect_near(0.5, r, 0.001)
    ...
    it("getStats reports current tile and effect telemetry", function()
        local layer = make_layer({ autoscroll_x = 40.0, scale_x = 1.5, scale_y = 1.25 })
        layer:addEffectPass("blur", { radius = 2.0 })
        layer:update(0.25)
        local stats = layer:getStats()
        expect_type("table", stats)
        expect_true(stats.visible_tile_count > 0)
        expect_true(stats.tile_width > 0)
    ...
    -- @covers LParallaxSet:getStats
    it("getStats aggregates layer telemetry", function()
        local set = make_set("analytics")
        local layer_a = make_layer({ z = -1, autoscroll_x = 30.0 })
        local layer_b = make_layer({ z = 2, scale_x = 1.2, scale_y = 1.2 })
        layer_b:addEffectPass("blur", { radius = 1.0 })
        set:addLayer(layer_a)
        set:addLayer(layer_b)
        set:update(0.2)
        local stats = set:getStats()
        expect_type("table", stats)
        expect_equal("analytics", stats.name)
        expect_equal(2, stats.layer_count)
        expect_true(stats.visible)
        expect_true(stats.visible_tile_count > 0)
        expect_true(stats.effect_pass_count >= 1)
    end)
