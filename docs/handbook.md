...
- Lurek2D is a desktop 2D game engine written in Rust that loads and executes Lua scripts via LuaJIT.


> Onboarding manual for contributors and game authors. New here? Start at Â§3.
>
> Companion docs: [../README.md](../README.md) (one-page intro) Â· [../CONTRIBUTING.md](../CONTRIBUTING.md) (PR contract) Â·
> [docs/architecture/](architecture/) (deep design) Â· [docs/specs/](specs/) (per-module reference) Â·
...
## 1. What Lurek2D is
 ...
 One binary, one scripting language, one afternoon to learn. Source under [src/](../src/) is organised into five responsibility groups â€” Foundations, Core Runtime, Platform Services, Feature Systems, and Edge/Integration â€” defined in [architecture/philosophy.md](architecture/philosophy.md) and inventoried in [architecture/engine-architecture.md](architecture/engine-architecture.md). Renderer is `wgpu` (Vulkan / DX12 / Metal); physics is `rapier2d`; audio is `rodio`. Engine version is **1.0.0** ([Cargo.toml](../Cargo.toml)). Licence is MIT. Desktop targets only â€” Windows, Linux, macOS on x86_64 and ARM (constraint **A-02**).
...
| ------------ | ------------------------------------------------- | ----------------------------------------- | ----------------------------------------------------------------- |
| **GameDev**  | Writing a game in Lua against `lurek.*`.          | Â§3 First 30 min Â· Â§6 First game           | Â§8 Docs Â· Â§9 Testing                                              |
| **EngDev**   | Modifying Rust source under `src/`.               | Â§3 Â· Â§7 First engine change               | Â§10 Quality gates Â· Â§8 Docs                                       |
| **Modder**   | Authoring a Lureksome library or sandboxed mod.   | Â§3 Â· Â§6 (Lua skeleton) Â· Â§11 (CAG skills) | [library-authoring](../.github/skills/library-authoring/SKILL.md) |
| **Player**   | Just running shipped games / demos.               | Â§3 (steps 1â€“4) only                       | â€”                                                                 |
| **GameTest** | Writing Lua BDD tests for game/library behaviour. | Â§3 Â· Â§9 Testing                           | [test-framework.md](architecture/test-framework.md)               |
| **EngTest**  | Writing Rust unit / integration / benches.        | Â§3 Â· Â§9 Â· Â§10                             | [test-framework.md](architecture/test-framework.md)               |

Personas are formalised in [architecture/cag-system.md Â§ 4](architecture/cag-system.md#4-six-persona-model).
...
1. **Install Rust â‰Ą 1.78 (stable)**. The repo pins `stable` via [rust-toolchain.toml](../rust-toolchain.toml); `rustup` will pick it up automatically. You also need Python â‰Ą 3.10 for the docs / validation tooling.
2. **Clone**:
   ```bash
...
3. **Build & run the splash screen**:
   ...
   cargo run
   ```
   The first build pulls and compiles the dependency tree (â‰3â€“8 minutes on a modern laptop). After that, incremental builds finish in seconds. The splash window opens with no game loaded.
...
5. **Open VS Code and accept the workspace extension recommendations** ([extension/vscode/](../extension/vscode/)). The Lurek2D Toolkit extension provides scaffolding, run/debug commands, the LuaCATS-driven IntelliSense for `lurek.*`, and the in-editor doc browser. The recommendations also pre-populate [.vscode/tasks.json](../.vscode/tasks.json) â€” every command in Â§5 is also a task.
...
| Folder                                      | Purpose                                                                                                                       |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| [src/](../src/)                             | Rust engine source â€” 49 modules across the five responsibility groups.                                                        |
...
| [content/](../content/)                     | Lua content â€” `games/` (showcase apps), `examples/` (one-file API demos), `layouts/`, `plugins/`, `snippets/`, and `zips/`. |
| [library/](../library/)                     | Reusable Lua game-logic modules (Lureksome).                                                                                  |
| [.github/](../.github/)                     | CAG layer â€” system prompt + agents/, skills/, prompts/. See Â§11.                                                              |
| [extension/vscode/](../extension/vscode/) | First-party VS Code extension.                                                                                                |
...
For canonical group-by-group module placement, see [architecture/engine-architecture.md Â§ Complete Module Inventory](architecture/engine-architecture.md#complete-module-inventory).
...
- `cargo build` â†’ `build/debug/lurek2d` â€” fast incremental, slow runtime, full debug symbols.
- `cargo build --release` â†’ `build/release/lurek2d` â€” slow build, optimised binary (target is â‰¤ 10 MB once the [plugin architecture](architecture/plugins.md) lands).
- `cargo check` â€” type-check only; the fastest "did I break the build?" loop.
...
- Default: `lua-jit` â€” vendored LuaJIT 2.1 via `mlua` (constraint **B-01**).
- Fallback: `cargo build --no-default-features --features lua54` â€” Lua 5.4 for environments where LuaJIT cannot vendor.

Common workspace tasks (see [.vscode/tasks.json](../.vscode/tasks.json)) â€” invoke from VS Code's *Tasks: Run Task*:
...
| `â–¶ Run: Debug (no rebuild)`        | Launch `build/debug/lurek2d` without rebuilding                                     |
| `â–¶ Run: Release (no rebuild)`      | Launch `build/release/lurek2d` without rebuilding                                   |
| `â–¶ Run: Examples Sweep (Debug)`    | Build debug, then run every `content/examples/*.lua` sequentially through `lurek2d` |
| `â–¶ Run: Examples Sweep (Release)`  | Build release, then run every `content/examples/*.lua` sequentially through `lurek2d` |
...
| `Test: Math / Physics / Graphics â€¦`| `python tools/dev/parallel_cargo.py test target <module>_tests`                     |
...
| `Quality: Gate`                    | format check â†’ clippy deny â†’ test all (the canonical pre-PR sweep)                  |
...
- **Windows**: `Dist: Package Windows` task â†’ `tools/dist/dist.ps1` builds release, copies into `dist/lurek2d-windows-x86_64/` and produces a zip. `Dist: NSIS Installer (Windows)` invokes `makensis tools/dist/installer.nsi`.
- **Linux / macOS**: `Dist: Package Linux / macOS` â†’ `tools/dist/dist.sh` produces a `.tar.gz`.
...
Smallest possible game â€” `content/games/showcase/my_game/main.lua`:
...
function lurek.draw()
    ...
    lurek.render.print("Move with â†’ key", 20, 20)
...
Assets are resolved relative to the game folder by `GameFS`. The sandbox prevents `..` escapes. Supported image formats include PNG (always), JPEG, GIF, and DDS (compressed). Audio formats are OGG Vorbis and WAV â€” see [Cargo.toml](../Cargo.toml) for the build-time format set. For more single-API examples, browse [content/examples/README.md](../content/examples/README.md).
...
2. **Read its spec** at [docs/specs/timer.md](specs/timer.md) â€” types, functions, Lua API surface, current test paths. The spec is the canonical contract.
3. **Make the Rust change**. Follow the file-structure rules in [architecture/engine-architecture.md Â§ Module Internal File Structure Standard](architecture/engine-architecture.md#module-internal-file-structure-standard): thin `mod.rs`, `<primary>.rs` for logic, no `impl LuaUserData` outside `src/lua_api/`, no `wgpu::*` outside `src/render/`. Every new `pub` item gets a `///` doc comment (constraint **Q-05**).
...
6. **Add tests** â€” Lua first. The Lua-first rule is binding: behaviour observable through `lurek.*` MUST be tested in Lua under [tests/lua/](../tests/lua/), and the new test file must be registered manually in [tests/lua/harness.rs](../tests/lua/harness.rs). Reach for [tests/rust/unit/](../tests/rust/unit/) only for internals not reachable from Lua.
7. **Run quality gates** (Â§10).
8. **Commit** with `type(scope): description` â€” see Â§10 for format.
...
The cross-artifact sync table in the system prompt ([.github/copilot-instructions.md Â§ Cross-Artifact Sync](../.github/copilot-instructions.md)) tells you which docs to update when source changes. Regenerate generated docs after any public-API change. Module-spec manual-vs-auto rules are documented in [docs/specs/SPEC_TEMPLATE.md](specs/SPEC_TEMPLATE.md) and checked by `python tools/validate/validate_module_coverage.py` plus `python tools/audit/doc_coverage.py`.

Snippet docs and workflow are tracked separately in [docs/snippets.md](snippets.md). Snippet source-of-truth files live in [content/snippets/](../content/snippets/) and generate [extension/vscode/data/snippets.json](../extension/vscode/data/snippets.json). Use `python tools/snippets/gen_vscode_snippets.py` after snippet edits, then run `python tools/validate/validate_snippets.py` and `python tools/audit/snippet_coverage.py`.

---

## 9. Testing

The Lua-first testing rule is binding (see [philosophy.md Â§ Testing Constraints](architecture/philosophy.md#testing-constraints), constraints **TST-01**..**TST-04**): behaviour observable through `lurek.*` is tested in Lua. Rust unit tests under [tests/rust/unit/](../tests/rust/unit/) are reserved for internals not reachable through Lua bindings.
...
- **Keep `src/lua_api/*_api.rs` and every `mod.rs` thin.** Business logic goes in `src/<module>/*.rs` sibling files. See **TST-03** and **TST-04** in [philosophy.md Â§ Testing Constraints](architecture/philosophy.md#testing-constraints).

Full decision tree and banned-patterns list: [test-framework.md Â§ Test placement](architecture/test-framework.md#test-placement).

Two test layers (full design in [architecture/test-framework.md](architecture/test-framework.md)):

- **Rust** â€” `cargo test`. Per-module suites under `tests/rust/unit/<module>_tests.rs`. Float comparisons use absolute tolerance, never `assert_eq!` on `f32`.
- **Lua BDD** â€” `cargo test lua_test_<category>_<name>`. Files under [tests/lua/](../tests/lua/) using `describe` / `it` / `expect_equal` from [tests/lua/init.lua](../tests/lua/init.lua). Every Lua test file must end with `test_summary()`.

Adding a new Lua test:

1. Create `tests/lua/<category>/test_<name>.lua`.
2. Add a matching `#[test] fn lua_test_<category>_<name>()` entry in [tests/lua/harness.rs](../tests/lua/harness.rs) â€” auto-discovery is intentionally not used.
3. Run: `cargo test lua_test_<category>_<name> -- --nocapture`.

Common test workspace tasks: `Test: All`, `Test: Math module`, `Test: Physics module`, `Test: Graphics module`, `Test: Audio module`, `Test: Input module`, `Test: Lua bindings`. See Â§5.
...
From the system prompt's *Quality Gates* section ([.github/copilot-instructions.md](../.github/copilot-instructions.md)) â€” run all of these before any commit:

- `cargo test && cargo clippy -- -D warnings` (constraints **Q-01**, **Q-02**).
- `python tools/validate/cag_validate.py` â€” gates the `.github/` CAG layer. Use `--baseline` to fail only on regressions.
- `python tools/audit/doc_coverage.py` and `python tools/audit/test_coverage.py` â€” coverage audits.
- `python tools/gen_all_docs.py` â€” regenerate generated API references after any public-API change.
...
- Confirm branch with `git rev-parse --abbrev-ref HEAD` and stage only the files you actually changed â€” never `git add .`.
- Commit format: `type(scope): description` where `type` â {`feat`, `fix`, `refactor`, `test`, `docs`, `chore`}. **One logical change per commit.**
...
The full `Quality Gate: Full` workspace task (Â§5) bundles fmt + clippy + test in one click.

---

## 11. Working with AI agents (CAG)

The **CAG layer** (Context Augmented Guidance) lives under [.github/](../.github/) and customises how Copilot â€” or any agent honouring Markdown system prompts â€” works in this repo. Five artifact types: the system prompt, agents, skills, prompts, and companion files.
...
- **Agents** under `.github/agents/` are workflow specialists (`Architect`, `Developer`, `Doc-Writer`, `Tester`, â€¦). Each declares which personas it serves.
...
Full deep dive in [architecture/cag-system.md](architecture/cag-system.md). When working with an agent on a multi-step change, route through `Manager` â€” it engages `Planner` for â‰Ą 3-agent or â‰Ą 5-file tasks before any implementation.

Common slash prompts â€” each one is a parameterised playbook under [.github/prompts/](../.github/prompts/):

| Prompt                  | What it does                                                              |
| ----------------------- | ------------------------------------------------------------------------- |
| `/run-quality-sweep`    | Runs the full audit â†’ diagnose â†’ fix â†’ verify cycle across the repo.      |
...
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Build fails on a fresh clone                 | Verify Rust â‰Ą 1.78; delete `build/` and retry. See [build-system](../.github/skills/build-system/SKILL.md).                                                                       |
...
| Don't know which module a feature belongs to | [architecture/engine-architecture.md Â§ Complete Module Inventory](architecture/engine-architecture.md#complete-module-inventory).                                                 |
| Need to design a new `lurek.*` API           | [lua-api-design](../.github/skills/lua-api-design/SKILL.md) skill.                                                                                                                |
| Need to add a new module                     | [architecture/engine-architecture.md Â§ Module Internal File Structure Standard](architecture/engine-architecture.md#module-internal-file-structure-standard) + `Architect` agent. |
| Need a new agent / skill / prompt            | [architecture/cag-system.md Â§ 6 Authoring Guides](architecture/cag-system.md#6-authoring-guides).                                                                                 |
...
| **CAG**                             | Context Augmented Guidance â€” the `.github/` layer customising AI workflows. [architecture/cag-system.md](architecture/cag-system.md).                                                                              |
| **CORE-KEEP**                       | Plugin tier for modules that always ship in the core binary. [architecture/plugins.md Â§ 4](architecture/plugins.md#4-plugin-tiers).                                                                                |
...
| **Persona**                         | One of six target user profiles (EngDev, GameDev, Modder, Player, GameTest, EngTest). [architecture/cag-system.md Â§ 4](architecture/cag-system.md#4-six-persona-model).                                            |
| **Plugin tier**                     | One of CORE-KEEP / TIER-1-PLUGIN / TIER-2-PLUGIN / THIRD-PARTY-PLUGIN. [architecture/plugins.md Â§ 4](architecture/plugins.md#4-plugin-tiers).                                                                      |
...
| **`SharedState`**                   | The `Rc<RefCell<...>>` hub holding every runtime resource pool. `src/runtime/shared_state.rs`. [architecture/engine-architecture.md Â§ State Architecture](architecture/engine-architecture.md#state-architecture). |
| **Skill**                           | On-demand domain knowledge under `.github/skills/<name>/SKILL.md`, loaded when the user task matches. [architecture/cag-system.md Â§ 2](architecture/cag-system.md#2-file-type-catalog).                            |
| **Sweep**                           | The end-of-session CAG-Architect review confirming `.github/` changes are valid. [architecture/cag-system.md Â§ 7](architecture/cag-system.md#7-end-of-session-cag-sweep-contract).                                 |
...
| **`wgpu`**                          | Cross-platform GPU abstraction (Vulkan / DX12 / Metal). Constraint **B-02** â€” only renderer backend in Lurek2D.                                                                                                    |
| **`rapier2d`**                      | The 2D rigid-body physics engine used by `src/physics/`. Heavy crate tree â€” part of the case for plugin tier-2.                                                                                                    |
| **Quality Gate**                    | The set of must-pass checks before a commit: tests, clippy, CAG validator, doc coverage. Â§10.                                                                                                                      |
