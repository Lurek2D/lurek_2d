...
- `LGraph` (43 methods) - Lua-side graph handle storing graph state and registered event callbacks.
...
- Source spec: [docs/specs/flownet.md](../blob/main/docs/specs/flownet.md)
...
Source: [flownet.lua](../blob/main/content/examples/flownet.lua)
...
---@param edge_type? string Edge type.
---@return LGraphEdge New edge handle.
function LGraph:addEdgeUnchecked(from_ud, to_ud, edge_type) end
```
...
---@return number[] Array of new edge IDs.
function LGraph:batchAddEdges(edges) end
```
...
- `edges` (`table`, required): Array of sub-tables with node IDs and optional edge type.

Returns: `integer[]` - Array of new edge IDs.
...
---@param count number Number of nodes to create.
---@param config? table Optional shared config: node_type (string?), capacity (integer?).
---@return number[] Array of new node IDs.
function LGraph:batchAddNodes(count, config) end
```
...
- `config` (`table`, optional): Optional shared config: node_type (string?), capacity (integer?).

Returns: `integer[]` - Array of new node IDs.
...
- `dt` (`number`, required): Delta time per step.
- `iterations` (`integer`, required): Number of steps to run.
