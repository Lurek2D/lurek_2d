...
LProvinceRegistry:setBorderPairStyle(a: integer, b: integer, style: table|Style table with optional fields: color={r,g,b,a}, thickness=number, flags=string) -> boolean -- Sets the style override for a specific adjacency pair, including optional color, thickness, and semantic fl...
...
lurek.scene.pushOverlay(scene: table, [transition]: string, [duration]: number, [easing]: string, [params]: table) -- Push a scene as an overlay on top of the current scene. Unlike `push`, the underlying scene is NOT paused —...
