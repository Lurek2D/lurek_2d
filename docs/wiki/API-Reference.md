...
LProvinceRegistry:setBorderPairStyle(a: integer, b: integer, style: table) -> boolean -- Sets the style override for a specific adjacency pair, including optional color, thickness, and semantic fl...
...
LProvinceRegistry:setFogState(id: integer, fog_state: integer) -> boolean -- Sets a fog-of-war byte for a province. This value is game-defined metadata and can be used by scripts/map m...
...
lurek.render.printRotatedWithFont(font: LFont, text: string, x: number, y: number, angle: number, [scale]: number) -- Draws text centered and rotated around its midpoint using a specific font without changing the global activ...
...
lurek.scene.getActiveScenes() -> table -- Returns a Lua array of all process-active scene tables ordered by their layer value (lowest layer first). I...
...
lurek.scene.preload(name: string, loader: function) -- Register a deferred-loading function for a scene. The loader function is NOT called immediately â€” it runs...
lurek.scene.process(dt: number) -- Call `ready(self)` once on newly-pushed scenes, then call `process(self, dt)` on every process-active scene...
...
lurek.scene.pushOverlay(scene: table, [transition]: string, [duration]: number, [easing]: string, [params]: table) -- Push a scene as an overlay on top of the current scene. Unlike `push`, the underlying scene is NOT paused â...
...
lurek.scene.unregisterScene(name: string) -- Remove a scene registration by name. Does not pop the scene if it is currently active on the stack â€” it o...
...
```lua
lurek.serial.applyDefaults(value: any, schema: table) -> table -- Merges a schema's default values into a data table, filling in any missing fields without overwriting exist...
...
lurek.serial.encode(value: any, format: string, [opts]: table) -> string -- Universal encoder that serializes a Lua value into the specified format. Supports JSON, TOML, CSV, and Mess...
...
lurek.serial.toJson(value: any, [pretty]: boolean) -> string -- Serializes a Lua value (table, string, number, boolean, or nil) into a JSON string. Useful for saving game...
...
lurek.serial.validate(value: any, schema: table) -> boolean -- Validates a Lua value against a schema table. The schema defines expected types, required fields, and const...
```
...
lurek.timer.sleep(seconds: number) -- Blocks the current thread for the given number of seconds. Use sparingly â€” this halts the entire game loo...
