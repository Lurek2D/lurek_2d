...
*Coverage: 5248/5248 functions documented (100.0%)*

---

## Contents

- [`lurek.render`](#render) — 129 fn, 12 classes
...
- [`lurek.agent`](#agent) — 2 fn, 2 classes
...
- [`lurek.learning`](#learning) — 5 fn, 5 classes
...
- [`lurek.procgen`](#procgen) — 30 fn, 2 classes
...
```lua
lurek.agent.new( config : table ) -> LAgent  -- Creates a new LLM Agent instance. The configuration table must specify at least the endpoint URL, model, format (json or text) and optional prompt parameters
...
```lua
LAgent:addSkill( name : string, prompt : string ) -> nil  -- Adds a skill to the agent's context
...
LAgent:update() -> nil  -- Polls the background client for completed LLM requests
```
...
LAgentManager:update() -> nil  -- Polls the manager's background client for completed tasks
```
