...
*Auto-generated by `tools/gen_docs_lua.py`. Version: `1.1.0` | Generated: 2026-07-27*
*Coverage: 7229/7351 functions documented (98.3%)*

---

## Contents

- [`lurek.render`](#render) — 131 fn, 15 classes
...
- [`lurek.province`](#province) — 16 fn, 1 class
...
*Coverage: 247/247 items documented (100%)*
...
lurek.render.getCanvas()  -- Returns the active canvas, or nil when drawing to the screen
lurek.render.getCanvasSize( ud : any )  -- Returns the pixel dimensions of a canvas
...
lurek.render.setCanvas( ud : any )  -- Redirects subsequent drawing to a canvas, or nil for the screen
...
lurek.render.setColorMask( ... : any )  -- Sets which color channels are written during draw calls. Call with no args to enable all
...
```lua
LVoxelModel:getBounds()  -- Returns local model bounds as `{minX, minY, minZ, maxX, maxY, maxZ}`
LVoxelModel:getVoxelCount()  -- Returns the number of occupied source voxels
```
...
lurek.input.startPlayback( opts : any )  -- Starts playback of the loaded recording. `opts.mode` may be `frame`, `fixed`, or `realtime`
...
LCombo:consume()  -- Marks the latest combo completion as consumed
...
LPlayerInputContext:clearActions()  -- Removes every local action
...
LPlayerInputContext:removeAction( name : string ) -> boolean  -- Removes one local action
...
*Coverage: 340/351 items documented (97%)*
...
```lua
LKinematicController2D:clearVerticalSpan()
LKinematicController2D:getLastResult()
LKinematicController2D:move( dx : number, dy : number, opts : table? ) -> table  -- Sweeps and wall-slides the controlled kinematic body
LKinematicController2D:recover( opts : any )
LKinematicController2D:release()
LKinematicController2D:setFilter( filter : any )
LKinematicController2D:setMaxSlides( max_slides : any )
LKinematicController2D:setRadius( radius : any )
LKinematicController2D:setSkin( skin : any )
LKinematicController2D:setVerticalSpan( z_min : any, z_max : any )
LKinematicController2D:testMove( dx : any, dy : any, opts : any )  -- Solves movement without mutating the controlled body
LKinematicController2D:type()
LKinematicController2D:typeOf( name : any )
```
...
LWorld:isBodyEnabled( body_id : any )  -- Returns whether one live body participates in simulation and queries
LWorld:isBodySleeping( id : integer ) -> boolean  -- Returns whether a body is currently in the sleeping (inactive) state
LWorld:isFixtureEnabled( body_id : any, fixture_index : any )  -- Returns whether one zero-based fixture participates in simulation and queries
...
LWorld:setBodyEnabled( body_id : any, enabled : any )  -- Enables or disables one body without destroying its stable id
...
LWorld:setFixtureEnabled( body_id : any, fixture_index : any, enabled : any )  -- Enables or disables one zero-based fixture without changing sensor state
...
LUnknown:deserializeShapeData( bytes : string ) -> LuaValue  -- Decodes serialized province shape data into span and segment tables
LUnknown:drawShapes( x : number?, y : number?, w : number?, h : number? ) -> integer  -- Queues filled polygon draw commands for province shapes, optionally culled to a viewport rect
LUnknown:getAt( x : integer, y : integer ) -> integer  -- Returns the province id stored at grid coordinates
...
LUnknown:typeOf( name : string ) -> boolean  -- Returns whether this province grid handle matches a supported type name
```
...
LLoadout:equip( slot : string, part : LPartDef )  -- Equips a part into a named slot after compatibility checks
...
```lua
LStatBlock:add( name : string, value : number )  -- Adds a numeric delta to one stat
LStatBlock:get( name : string ) -> number  -- Returns one stat value, or zero when the key is absent
LStatBlock:set( name : string, value : number )  -- Replaces one stat value
...
LMinimap:setCenterFromTileMapWorld( tilemap_ud : any, wx : any, wy : any )  -- Converts tilemap world coordinates into one-based tile coordinates and centers this minimap
...
LMinimap:setLayerStyle( layer : any, style : any )  -- Applies common raw-layer style fields: visible, alpha, blend, and colors
...
LMinimap:setViewportFromTileMapWorld( tilemap_ud : any, x : any, y : any, w : any, h : any )  -- Converts a tilemap world rectangle into a minimap viewport rectangle
...
LMinimap:syncTileAwarenessFog( awareness_ud : any, player : any, opts : any )  -- Copies explored/visible masks from `LTileAwareness` into minimap fog data
...
LMinimap:syncTileLightLayer( light_ud : any, layer : any, opts : any )  -- Copies computed tilelight luma into a minimap raw byte layer
...
LCursorManager:setVisible( visible : boolean )  -- Shows or hides the runtime cursor
...
*Coverage: 207/317 items documented (65%)*

```lua
lurek.progression.acquirePerk( this : any, name : any )  -- Acquire perk
lurek.progression.activeCount( this : any )  -- Active count
lurek.progression.activeIds( this : any )  -- Active ids
lurek.progression.addBuff()  -- Adds buff
lurek.progression.addJournalEntry( this : any, quest_id : any, text : any, tag : any )  -- Adds journal entry
lurek.progression.addQuest( this : any, quest : any )  -- Adds quest
lurek.progression.addXP( this : any, amount : any )  -- Adds xp
lurek.progression.adjustMorale( this : any, delta : any )  -- Adjust morale
lurek.progression.advanceObjective()  -- Advance objective
lurek.progression.applyDamage( this : any, stat : any, amount : any, dtype : any )  -- Apply damage
lurek.progression.applyTraitBuffs( this : any, trait_name : any )  -- Apply trait buffs
lurek.progression.beginTurn( this : any )  -- Begin turn
lurek.progression.checkMorale( this : any )  -- Check morale
lurek.progression.clearBuffs( this : any, stat : any )  -- Clears buffs
lurek.progression.clearFlag( this : any, name : any )  -- Clears flag
lurek.progression.completeQuest( this : any, id : any )  -- Complete quest
lurek.progression.completedCount( this : any )  -- Completed count
lurek.progression.completedIds( this : any )  -- Completed ids
lurek.progression.createLegacyQuestAdapter( store : any, profile : any, options : any )  -- Create legacy quest adapter
lurek.progression.createLegacyStatsAdapter( store : any, profile : any, options : any )  -- Create legacy stats adapter
lurek.progression.define( this : any, name : any, base : any, opts : any )  -- Define
lurek.progression.definePerk( this : any, name : any, opts : any )  -- Define perk
lurek.progression.defineSkill( this : any, name : any, opts : any )  -- Define skill
lurek.progression.failQuest( this : any, id : any )  -- Fail quest
lurek.progression.failedIds( this : any )  -- Failed ids
lurek.progression.get( this : any, name : any )  -- Returns a value
lurek.progression.getActionPoints( this : any ) -> number, number  -- Returns the action points
lurek.progression.getActiveTraits( this : any )  -- Returns the active traits
lurek.progression.getBase( this : any, name : any )  -- Returns the base
lurek.progression.getBuffCount( this : any, stat : any )  -- Returns the buff count
lurek.progression.getBuffs( this : any, stat : any )  -- Returns the buffs
lurek.progression.getCooldownRemaining( this : any, name : any )  -- Returns the cooldown remaining
lurek.progression.getEncumbrance( this : any )  -- Returns the encumbrance
lurek.progression.getFlags( this : any )  -- Returns the flags
lurek.progression.getInitiative( this : any )  -- Returns the initiative
lurek.progression.getLevel( this : any )  -- Returns the level
lurek.progression.getMax( this : any, name : any )  -- Returns the max
lurek.progression.getMin( this : any, name : any )  -- Returns the min
lurek.progression.getMorale( this : any ) -> number, number  -- Returns the morale
lurek.progression.getQuest( this : any, id : any )  -- Returns the quest
lurek.progression.getQuestReward( this : any, id : any )  -- Returns the quest reward
lurek.progression.getRegen( this : any, name : any )  -- Returns the regen
lurek.progression.getResistance( this : any, dtype : any )  -- Returns the resistance
lurek.progression.getSkillLevel( this : any, name : any )  -- Returns the skill level
lurek.progression.getStatNames( this : any )  -- Returns the stat names
lurek.progression.getUseCount( this : any, name : any )  -- Returns the use count
lurek.progression.getXP( this : any )  -- Returns the xp
lurek.progression.hasFlag( this : any, name : any )  -- Returns true if flag
lurek.progression.hasPerk( this : any, name : any )  -- Returns true if perk
lurek.progression.hasTrait( this : any, trait_name : any )  -- Returns true if trait
lurek.progression.importLegacyQuestSnapshot( snapshot : any )  -- Import legacy quest snapshot
lurek.progression.importLegacyStatsSnapshot( snapshot : any )  -- Import legacy stats snapshot
lurek.progression.isEncumbered( this : any )  -- Returns true if encumbered
lurek.progression.learnSkill( this : any, name : any )  -- Learn skill
lurek.progression.loadStore( snapshot : any )  -- Load store
lurek.progression.newStatusTracker() -> LStatusTracker  -- Creates an isolated deterministic status lifecycle tracker
lurek.progression.newStore( options : any )  -- New store
lurek.progression.questCount( this : any )  -- Quest count
lurek.progression.questIds( this : any )  -- Quest ids
lurek.progression.questsWithStatus( this : any, wanted : any )  -- Quests with status
lurek.progression.recordUse( this : any, name : any )  -- Record use
lurek.progression.recoverActionPoints( this : any, amount : any )  -- Recover action points
lurek.progression.removeBuff( this : any, handle : any )  -- Removes buff
lurek.progression.removeQuest( this : any, id : any )  -- Removes quest
lurek.progression.removeTraitBuffs( this : any, trait_name : any )  -- Removes trait buffs
lurek.progression.resetQuest( this : any, id : any )  -- Clears quest
lurek.progression.restore( this : any, snap : any )  -- Restore
lurek.progression.setActionPoints( this : any, max_val : any )  -- Sets the action points
lurek.progression.setBase( this : any, name : any, value : any )  -- Sets the base
lurek.progression.setBerserkThreshold( this : any, value : any )  -- Sets the berserk threshold
lurek.progression.setEncumbrance( this : any, cur : any, max_val : any )  -- Sets the encumbrance
lurek.progression.setFlag( this : any, name : any )  -- Sets the flag
lurek.progression.setInitiative( this : any, value : any )  -- Sets the initiative
lurek.progression.setLevel( this : any, value : any )  -- Sets the level
lurek.progression.setLevelThresholds( this : any, thresholds : any )  -- Sets the level thresholds
lurek.progression.setMax( this : any, name : any, value : any )  -- Sets the max
lurek.progression.setMin( this : any, name : any, value : any )  -- Sets the min
lurek.progression.setMorale( this : any, max_val : any )  -- Sets the morale
lurek.progression.setPanicThreshold( this : any, value : any )  -- Sets the panic threshold
lurek.progression.setQuestReward( this : any, id : any, reward : any )  -- Sets the quest reward
lurek.progression.setRegen( this : any, name : any, value : any )  -- Sets the regen
lurek.progression.setResistance( this : any, dtype : any, value : any )  -- Sets the resistance
lurek.progression.setXP( this : any, value : any )  -- Sets the xp
lurek.progression.snapshot( this : any )  -- Snapshot
lurek.progression.spendActionPoints( this : any, amount : any )  -- Spend action points
lurek.progression.startQuest( this : any, id : any )  -- Start quest
lurek.progression.type()  -- Type
lurek.progression.typeOf( name : any )  -- Type of
lurek.progression.update( this : any, dt : any )  -- Update
lurek.progression.useSkill( this : any, name : any ) -> boolean, string?  -- Use skill
```
...
```lua
LProgressionProfile:getId()  -- Returns the id
LProgressionProfile:getPendingRewards() -> table  -- Returns this profile's pending reward records as typed reward handles
LProgressionProfile:type()  -- Type
LProgressionProfile:typeOf( name : any )  -- Type of
```

### `LProgressionStore`

```lua
LProgressionStore:acceptQuest( profile : any, quest_id : any )  -- Returns the pending rewards
LProgressionStore:ackChangesThrough( revision : any )  -- Ack changes through
LProgressionStore:acquirePerk( profile : any, perk_id : any )
LProgressionStore:activateChallenge( profile : any, challenge_id : any, options : any )
LProgressionStore:addAttributeBase( profile : any, attribute_id : any, amount : any )
LProgressionStore:addCounter( profile : any, counter_id : any, amount : any )
LProgressionStore:addExperience( profile : any, track_id : any, amount : any )
LProgressionStore:addModifier( profile : any, target_id : any, opts : any )
LProgressionStore:addProfileTag( id : any, tag : any )  -- Adds profile tag
LProgressionStore:addResource( profile : any, resource_id : any, amount : any )
LProgressionStore:advanceTime( seconds : any )  -- Advance time
LProgressionStore:applyChangeset( changeset : any )  -- Apply changeset
LProgressionStore:applyChangesetEnvelope( changeset : any, options : any )
LProgressionStore:applyPrestige( profile : any, prestige_id : any )
LProgressionStore:applyProfileTemplate( profile : any, template_id : any )
LProgressionStore:applyTrait( profile : any, trait_id : any )
LProgressionStore:beginTransaction( options : any )
LProgressionStore:canPrestige( profile : any, prestige_id : any )
LProgressionStore:canSpendResource( profile : any, resource_id : any, amount : any )
LProgressionStore:clear()  -- Clears the state
LProgressionStore:clearEvents()  -- Clears events
LProgressionStore:collectCollectionItem( profile : any, collection_id : any, item_id : any )
LProgressionStore:compactChanges( max_records : any )  -- Compact changes
LProgressionStore:compileCondition( condition : any )  -- Compile condition
LProgressionStore:completeQuest( profile : any, quest_id : any )
LProgressionStore:countProfiles(  : any )  -- Returns the number of items
LProgressionStore:createProfile( id : any, options : any )
LProgressionStore:debugSnapshot()  -- Debug snapshot
LProgressionStore:defineAchievement( id : any, definition : any )
LProgressionStore:defineAttribute( id : any, definition : any )
LProgressionStore:defineChallengeTemplate( id : any, definition : any )
LProgressionStore:defineCollection( id : any, definition : any )
LProgressionStore:defineCounter( id : any, definition : any )
LProgressionStore:defineDerivedValue( id : any, definition : any )
LProgressionStore:defineLeaderboard( id : any, definition : any )
LProgressionStore:defineLevelTrack( id : any, definition : any )
LProgressionStore:definePerk( id : any, definition : any )
LProgressionStore:definePopulationTemplate( id : any, definition : any )
LProgressionStore:definePrestige( id : any, definition : any )
LProgressionStore:defineProfileTemplate( id : any, definition : any )
LProgressionStore:defineQuest( id : any, definition : any )
LProgressionStore:defineResource( id : any, definition : any )
LProgressionStore:defineSeason( id : any, definition : any )
LProgressionStore:defineSkill( id : any, definition : any )
LProgressionStore:defineTrait( id : any, definition : any )
LProgressionStore:dematerializePopulationProfile( profile_id : any, options : any )
LProgressionStore:drainEvents()  -- Drain events
LProgressionStore:endSeason( id : any, options : any )
LProgressionStore:ensureProfile( id : any, options : any )
LProgressionStore:evaluateCondition( profile : any, condition : any )
LProgressionStore:explainAttribute( profile : any, attribute_id : any )
LProgressionStore:explainCondition( profile : any, condition : any )
LProgressionStore:explainDerivedValue( profile : any, id : any )
LProgressionStore:exportChangesSince( revision : any )  -- Export changes since
LProgressionStore:exportChangeset( revision : any, options : any )
LProgressionStore:exportSnapshot()  -- Export snapshot
LProgressionStore:failQuest( profile : any, quest_id : any )
LProgressionStore:generatePopulation( template_id : any, options : any )
LProgressionStore:getAchievement( profile : any, achievement_id : any )
LProgressionStore:getActivityFeed( query : any )  -- Returns one typed activity-feed selection object
LProgressionStore:getAttribute( profile : any, attribute_id : any, mode : any )
LProgressionStore:getAttributeState( profile : any, attribute_id : any )
LProgressionStore:getChallenge( profile : any, challenge_id : any )
LProgressionStore:getCollection( profile : any, collection_id : any )
LProgressionStore:getCounter( profile : any, counter_id : any )
LProgressionStore:getCounterState( profile : any, counter_id : any )
LProgressionStore:getDefinitionHash()  -- Returns the definition hash
LProgressionStore:getDerivedValue( profile : any, id : any )
LProgressionStore:getExperience( profile : any, track_id : any )
LProgressionStore:getExperienceToNextLevel( profile : any, track_id : any )
LProgressionStore:getId()  -- Returns the id
LProgressionStore:getLeaderboardEntry( profile : any, leaderboard_id : any )
LProgressionStore:getLevel( profile : any, track_id : any )
LProgressionStore:getPopulation( handle_or_id : any )  -- Returns the population
LProgressionStore:getPopulationStatistics( handle_or_id : any, query : any )
LProgressionStore:getPrestige( profile : any, prestige_id : any )
LProgressionStore:getProfile( id : any )  -- Returns the profile
LProgressionStore:getQuestState( profile : any, quest_id : any )
LProgressionStore:getResource( profile : any, resource_id : any )
LProgressionStore:getRevision()  -- Returns the revision
LProgressionStore:getRival( profile : any, rival_profile : any )
LProgressionStore:getRivalDelta( profile : any, rival_profile : any )
LProgressionStore:getSchemaVersion()  -- Returns the schema version
LProgressionStore:getSeason( id : any )  -- Returns the season
LProgressionStore:getSeasonArchive( id : any, query : any )
LProgressionStore:getSkillCooldown( profile : any, skill_id : any )
LProgressionStore:getSkillLevel( profile : any, skill_id : any )
LProgressionStore:getTime()  -- Returns the time
LProgressionStore:hasPerk( profile : any, perk_id : any )
LProgressionStore:hasProfile( id : any )  -- Returns true if profile
LProgressionStore:hasTrait( profile : any, trait_id : any )
LProgressionStore:learnSkill( profile : any, skill_id : any )
LProgressionStore:listAchievements( profile : any )  -- List achievements
LProgressionStore:listChallenges( profile : any, options : any )
LProgressionStore:listCollections( profile : any )  -- List collections
LProgressionStore:listCounters( profile : any )  -- List counters
LProgressionStore:listLeaderboardAroundProfile()
LProgressionStore:listLeaderboardRange( leaderboard_id : any, start_rank : any, limit : any )
LProgressionStore:listLeaderboardTop( leaderboard_id : any, limit : any )
LProgressionStore:listModifiers( profile : any )  -- List modifiers
LProgressionStore:listPopulationProfiles( handle_or_id : any, query : any )
LProgressionStore:listPrestiges( profile : any )  -- List prestiges
LProgressionStore:listProfiles(  : any )  -- List profiles
LProgressionStore:listRivals( profile : any )  -- List rivals
LProgressionStore:listSeasons( query : any )  -- List seasons
LProgressionStore:listTraits( profile : any )  -- List traits
LProgressionStore:loadSnapshot( snapshot : any )  -- Load snapshot
LProgressionStore:materializePopulationProfile( profile_id : any )
LProgressionStore:pausePopulation( handle_or_id : any )  -- Pause population
LProgressionStore:pinRival( profile : any, rival_profile : any, options : any )
LProgressionStore:refillResource( profile : any, resource_id : any, amount : any )
LProgressionStore:refreshQuestLifecycle( profile : any )  -- Refresh quest lifecycle
LProgressionStore:regeneratePopulation( handle_or_id : any, options : any )
LProgressionStore:removeDerivedValue( id : any )  -- Removes derived value
LProgressionStore:removeModifier( profile : any, handle : any )
LProgressionStore:removePopulation( handle_or_id : any, options : any )
LProgressionStore:removeProfile( id : any, opts : any )
LProgressionStore:removeProfileMetadata( id : any, key : any )
LProgressionStore:removeProfileTag( id : any, tag : any )
LProgressionStore:removeTrait( profile : any, trait_id : any )
LProgressionStore:resumePopulation( handle_or_id : any )  -- Resume population
LProgressionStore:revealQuest( profile : any, quest_id : any )
LProgressionStore:setAttributeBase( profile : any, attribute_id : any, value : any )
LProgressionStore:setChallengeProgress( profile : any, challenge_id : any, value : any )
LProgressionStore:setCounter( profile : any, counter_id : any, value : any )
LProgressionStore:setExperience( profile : any, track_id : any, value : any )
LProgressionStore:setLevel( profile : any, track_id : any, level : any )
LProgressionStore:setProfileMetadata( id : any, key : any, value : any )
LProgressionStore:setQuestObjective( profile : any, quest_id : any, objective_id : any, value : any )
LProgressionStore:setQuestObjectiveStatus( profile : any, quest_id : any, objective_id : any, status : any )
LProgressionStore:setQuestObjectiveVisibility( profile : any, quest_id : any, objective_id : any, visible : any )
LProgressionStore:setResource( profile : any, resource_id : any, value : any )
LProgressionStore:setTime( seconds : any )  -- Sets the time
LProgressionStore:simulatePopulationUntil( handle_or_id : any, logical_time : any, options : any )
LProgressionStore:spendResource( profile : any, resource_id : any, amount : any )
LProgressionStore:startSeason( id : any, options : any )
LProgressionStore:stats()  -- Stats
LProgressionStore:submitScore( profile : any, leaderboard_id : any, score : any )
LProgressionStore:type()  -- Type
LProgressionStore:typeOf( name : any )  -- Type of
LProgressionStore:unlockAchievement( profile : any, achievement_id : any )
LProgressionStore:update( dt : any, opts : any )
LProgressionStore:updatePopulation( handle_or_id : any, dt : any, options : any )
LProgressionStore:updateProfile( id : any, patch : any )
LProgressionStore:useSkill( profile : any, skill_id : any )
LProgressionStore:validate()  -- Validate
LProgressionStore:validateCondition( condition : any )  -- Validate condition
LProgressionStore:validateDerivedValues()  -- Validate derived values
LProgressionStore:validatePopulationTemplate( id : any )  -- Validate population template
```

### `LProgressionTransaction`

```lua
LProgressionTransaction:addCounter( profile : any, counter_id : any, amount : any )
LProgressionTransaction:addExperience( profile : any, track_id : any, amount : any )
LProgressionTransaction:addModifier( profile : any, target_id : any, opts : any )
LProgressionTransaction:commit()  -- Commit
LProgressionTransaction:rollback()  -- Rollback
LProgressionTransaction:setAttributeBase( profile : any, attribute_id : any, value : any )
LProgressionTransaction:setCounter( profile : any, counter_id : any, value : any )
LProgressionTransaction:setQuestObjective( profile : any, quest_id : any, objective_id : any, value : any )
LProgressionTransaction:setResource( profile : any, resource_id : any, value : any )
LProgressionTransaction:type()  -- Type
LProgressionTransaction:typeOf( name : any )  -- Type of
```
...
```lua
LQuestJournal:addEntry( text : string, tag : string? ) -> LQuestJournalEntry  -- Appends one entry to the live quest journal and returns the stored entry object
...
LReward:markApplied( external_receipt : string? ) -> LReward  -- Marks this claimed reward as applied and returns the updated reward object
LReward:reject( reason : string? ) -> LReward  -- Rejects this reward and returns the updated reward object
```
...
```lua
LStatusTracker:apply( subjectId : integer, definitionId : string, sourceId : integer?, stacks : integer? ) -> integer  -- Applies a status to a subject and returns its stable runtime instance id
LStatusTracker:clear()  -- Removes all definitions, instances, and queued events
LStatusTracker:define( definition : table )  -- Registers or replaces one status definition
LStatusTracker:drainEvents() -> table  -- Takes and clears neutral apply/refresh/stack/tick/expired events
LStatusTracker:get( instanceId : integer ) -> table?  -- Returns one active status instance by runtime id
LStatusTracker:has( subjectId : integer, definitionOrTag : string ) -> boolean  -- Checks whether a subject has a status with the requested definition id or tag
LStatusTracker:list( subjectId : integer, filter : table? ) -> table  -- Lists active status instances attached to one subject and matching all optional filters
LStatusTracker:remove( instanceId : integer ) -> boolean  -- Removes one active status instance
LStatusTracker:removeByDefinition( subjectId : integer, definitionId : string ) -> integer  -- Removes every matching definition instance from one subject
LStatusTracker:removeByTag( subjectId : integer, tag : string ) -> integer  -- Removes every instance carrying a copied tag from one subject
LStatusTracker:restore( snapshot : table )  -- Restores definitions, active instances, and ID allocation from a snapshot
LStatusTracker:setPaused( instanceId : integer, paused : boolean ) -> boolean  -- Pauses or resumes one status instance's lifecycle timers
LStatusTracker:setRemaining( instanceId : integer, seconds : number? ) -> boolean  -- Sets one status instance's remaining duration; nil makes it infinite
...
LStatusTracker:typeOf( name : string ) -> boolean  -- Checks whether this handle matches `LStatusTracker` or `LObject`
LStatusTracker:update( dt : number ) -> integer  -- Advances finite durations and periodic tick timers by dt seconds
```
...
LMultiLevelGrid:clearPickAttr( x : any, y : any, surface : any, key : any )  -- Clears one arbitrary pick attribute or the whole surface channel from one active-level cell
...
LMultiLevelGrid:getPickAttr( x : any, y : any, surface : any, key : any )  -- Reads one arbitrary pick attribute from one active-level surface cell
...
LMultiLevelGrid:setPickAttr( x : any, y : any, surface : any, key : any, value : any )  -- Sets one arbitrary pick attribute on one active-level surface cell
...
LRaycaster:clearPickAttr( x : any, y : any, surface : any, key : any )  -- Clears one arbitrary pick attribute or the whole channel from a raycaster surface cell
...
LRaycaster:getPickAttr( x : any, y : any, surface : any, key : any )  -- Reads one arbitrary pick attribute from a raycaster surface cell
...
LRaycaster:setPickAttr( x : any, y : any, surface : any, key : any, value : any )  -- Sets one arbitrary pick attribute on a raycaster surface cell
...
LRaycasterView:typeOf( name : string ) -> boolean  -- Checks the handle type
```
...
LSpriteManager:setAttr( id : any, key : any, value : any )  -- Sets one arbitrary string attribute on the sprite
...
*Coverage: 79/80 items documented (99%)*
...
lurek.sprite.newBatch( texture : any, max : any )
...
LUiContext:focusNext()  -- Moves focus forward
LUiContext:focusPrev()  -- Moves focus backward
...
LUiContext:typeOf( name : string ) -> boolean  -- Checks this handle type
