...
*Coverage: 5415/5425 functions documented (99.8%)*
...
- [`lurek.math`](#math) — 94 fn, 12 classes
- [`lurek.audio`](#audio) — 83 fn, 6 classes
...
- [`lurek.learning`](#learning) — 12 fn, 10 classes
...
- [`lurek.visibility`](#visibility) — 1 fn, 1 class
...
*Coverage: 234/234 items documented (100%)*
...
LAgentChat:clear() -> nil  -- Clears the chat history
...
LSemanticMemory:recall( key : string )  -- Returns the fact for `key`, or `nil` if not found
```
...
LWorkingMemory:get( key : string )  -- Returns the value for `key`, or `nil` if not found
...
*Coverage: 15/25 items documented (60%)*
...
```lua
LModel:predict( input : any )  -- Runs the wrapped model's prediction. Delegates to `chooseAction`, `forward`, or `select`
