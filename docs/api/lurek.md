...
*Auto-generated by `tools/gen_docs_lua.py`. Version: `1.1.0` | Generated: 2026-08-05*
*Coverage: 7379/7379 functions documented (100.0%)*

---

## Contents

- [`lurek.render`](#render) — 134 fn, 15 classes
- [`lurek.window`](#window) — 55 fn
- [`lurek.input`](#input) — 108 fn, 4 classes
- [`lurek.time`](#timer) — 21 fn, 1 class
- [`lurek.math`](#math) — 100 fn, 15 classes
- [`lurek.audio`](#audio) — 96 fn, 6 classes
- [`lurek.physics`](#physics) — 28 fn, 9 classes
- [`lurek.filesystem`](#filesystem) — 47 fn, 2 classes
- [`lurek.particle`](#particle) — 5 fn, 2 classes
- [`lurek.signal`](#event) — 18 fn, 2 classes
- [`lurek.runtime`](#system) — 30 fn
- [`lurek.thread`](#thread) — 7 fn, 4 classes
- [`lurek.ai`](#ai) — 29 fn, 21 classes
- [`lurek.compute`](#compute) — 13 fn, 1 class
- [`lurek.dataframe`](#dataframe) — 15 fn, 6 classes
- [`lurek.image`](#image) — 16 fn, 7 classes
- [`lurek.tilemap`](#tilemap) — 14 fn, 6 classes
- [`lurek.dialog`](#dialog) — 12 fn, 5 classes
- [`lurek.ecs`](#ecs) — 22 fn, 8 classes
- [`lurek.scene`](#scene) — 64 fn, 2 classes
- [`lurek.pathfind`](#pathfind) — 35 fn, 14 classes
- [`lurek.minimap`](#minimap) — 1 fn, 1 class
- [`lurek.save`](#save) — 2 fn, 1 class
- [`lurek.mods`](#mods) — 4 fn, 3 classes
- [`lurek.i18n`](#i18n) — 36 fn
- [`lurek.log`](#log) — 18 fn
- [`lurek.debugbridge`](#debugbridge) — 16 fn
- [`lurek.docs`](#docs) — 26 fn, 5 classes
- [`lurek.patterns`](#patterns) — 30 fn, 25 classes
- [`lurek.agent`](#agent) — 21 fn, 10 classes
- [`lurek.animation`](#animation) — 10 fn, 5 classes
- [`lurek.asset`](#asset) — 29 fn, 1 class
- [`lurek.simulator`](#automation) — 32 fn
- [`lurek.awareness`](#awareness) — 5 fn, 3 classes
- [`lurek.binary`](#binary) — 18 fn, 4 classes
- [`lurek.camera`](#camera) — 4 fn, 3 classes
- [`lurek.charts`](#charts) — 14 fn, 12 classes
- [`lurek.cinematic`](#cinematic) — 2 fn, 2 classes
- [`lurek.color`](#color) — 19 fn
- [`lurek.cursor`](#cursor) — 4 fn, 3 classes
- [`lurek.devtools`](#devtools) — 50 fn, 2 classes
- [`lurek.dsp`](#dsp) — 29 fn, 7 classes
- [`lurek.effect`](#effect) — 10 fn, 3 classes
- [`lurek.engine`](#engine) — 13 fn
- [`lurek.flownet`](#flownet) — 1 fn, 5 classes
- [`lurek.font`](#font) — 11 fn, 1 class
- [`lurek.globe`](#globe) — 12 fn, 2 classes
- [`lurek.grep`](#grep) — 7 fn, 2 classes
- [`lurek.layout`](#layout) — 10 fn
- [`lurek.learning`](#learning) — 20 fn, 18 classes
- [`lurek.light`](#light) — 23 fn, 2 classes
- [`lurek.mapblock`](#mapblock) — 10 fn, 10 classes
- [`lurek.network`](#network) — 29 fn, 3 classes
- [`lurek.overlay`](#overlay) — 2 fn, 2 classes
- [`lurek.parallax`](#parallax) — 4 fn, 2 classes
- [`lurek.pipeline`](#pipeline) — 3 fn, 2 classes
- [`lurek.procgen`](#procgen) — 46 fn, 5 classes
- [`lurek.progression`](#progression) — 92 fn, 21 classes
- [`lurek.province`](#province) — 17 fn, 1 class
- [`lurek.raycaster`](#raycaster) — 20 fn, 7 classes
- [`lurek.repl`](#repl) — 1 fn, 1 class
- [`lurek.serialize`](#serialize) — 20 fn
- [`lurek.spine`](#spine) — 4 fn, 2 classes
- [`lurek.sprite`](#sprite) — 13 fn, 6 classes
- [`lurek.svg`](#svg) — 1 fn, 1 class
- [`lurek.terminal`](#terminal) — 31 fn, 2 classes
- [`lurek.tilefield`](#tilefield) — 6 fn, 3 classes
- [`lurek.tilelight`](#tilelight) — 2 fn, 1 class
- [`lurek.tileset`](#tileset) — 3 fn, 2 classes
- [`lurek.tween`](#tween) — 15 fn, 6 classes
- [`lurek.ui`](#ui) — 116 fn, 43 classes
- [`lurek.validator`](#validator) — 3 fn, 1 class
...
function lurek.mousepressed(x, y, button, clicks)  -- Called when a mouse button is pressed and UI did not consume it.
function lurek.mousereleased(x, y, button, clicks)  -- Called when a mouse button is released and UI did not consume it.
...
*Coverage: 270/270 items documented (100%)*

```lua
lurek.render.applyEffectToCanvas( sourceCanvas : LCanvas, targetCanvas : LCanvas, effectOrStack : LPostFxEffect|LPostFxStack ) -> LCanvas  -- Applies a post-processing effect or stack from one canvas into another canvas
lurek.render.applyShaderToCanvas( canvas : LCanvas, shader : LShader, opts : table? ) -> LCanvas  -- Queues a postfx shader pass that mutates a canvas after queued draws in the current frame
...
lurek.render.drawBatch( batch : LSpriteBatch ) -> nil  -- Draws a SpriteBatch using the same queued DrawBatch command as lurek.render.draw(batch)
...
lurek.render.drawPath( path : table, modeOrOpts : string/table?, close : boolean? )  -- Draws a vector path composed of moveTo, lineTo, quadTo, cubicTo, and close segments
...
lurek.render.drawText( text : string, x : number, y : number, rotation : number?, sx : number?, sy : number?, ox : number?, oy : number? )  -- Draws text using the active font with image-like transform parameters on the GPU
lurek.render.drawTextWithFont( font : LFont, text : string, x : number, y : number, rotation : number?, sx : number?, sy : number?, ox : number?, oy : number? )  -- Draws text using a specific font with image-like transform parameters on the GPU
...
lurek.render.getBuiltinShapeInfo( id : string ) -> table  -- Returns metadata for one native shape template
lurek.render.getCanvas() -> LCanvas  -- Returns the active canvas, or nil when drawing to the screen
lurek.render.getCanvasSize( ud : userdata ) -> number, number  -- Returns the pixel dimensions of a canvas
...
lurek.render.getDefaultFont( pointSize : integer?, bold : boolean? ) -> LFont  -- Returns a built-in default font at the nearest available bundled point size
...
lurek.render.listBuiltinShapes( filter : table|string? ) -> table  -- Lists the 96 native primitive-based shape templates
lurek.render.loadBuiltinShape( id : string, opts : table? ) -> LShape  -- Creates and compiles one native shape template into the shape registry
...
lurek.render.loadVoxel( path : string, voxelSize : number? ) -> LVoxelModel  -- Loads a MagicaVoxel `.vox` static prop with palette colours and a configurable world-space voxel size
...
lurek.render.newFont( pathOrSize : any, size : number? ) -> LFont  -- Compatibility alias for the canonical `lurek.font.load` and built-in font APIs
lurek.render.newImage( pathOrData : string|LImageData, colorSpace : string? ) -> LImage  -- Compatibility alias for `lurek.render.newTexture`
...
lurek.render.newShader( code : string, opts : table? ) -> LShader  -- Compiles a target-aware WGSL fragment shader through the render module and returns a shader handle
lurek.render.newShape() -> LShape  -- Creates a new retained compound shape for accumulating draw commands
lurek.render.newSpriteBatch( image : LImage, max : integer? ) -> LSpriteBatch  -- Compatibility alias for `lurek.sprite.newBatch`
lurek.render.newTexture( pathOrData : string|LImageData, colorSpace : string? ) -> LImage  -- Creates a render texture from a GameFS path or CPU-owned ImageData
...
lurek.render.prewarmShaders( shaders : table ) -> LShaderPrewarmRequest  -- Queues up to 64 live shaders for bounded frame-boundary cache preparation
lurek.render.print( text : string, x : number?, y : number?, scale : number? )  -- Draws text using the active font at the given position
lurek.render.printRich( spans : table, x : number, y : number, rotation : number?, sx : number?, sy : number?, ox : number?, oy : number? )  -- Draws rich text composed of individually styled spans at the given position
lurek.render.printRichWithFont( font : LFont, spans : table, x : number, y : number, rotation : number?, sx : number?, sy : number?, ox : number?, oy : number? )  -- Draws rich text using a specific font without changing the global active font
...
lurek.render.printRotatedWithFont( font : LFont, text : string, x : number, y : number, angle : number, scale : number? )  -- Draws text centered and rotated around its midpoint using a specific font without changing the global active font
lurek.render.printWithFont( font : LFont, text : string, x : number?, y : number?, scale : number? )  -- Draws text using a specific font without changing the global active font
...
lurek.render.printfWithFont( font : LFont, text : string, x : number, y : number, limit : number, align : string? )  -- Draws word-wrapped and aligned text with a specific font without changing the global active font
...
lurek.render.resetCanvas( canvas : LCanvas ) -> nil  -- Marks a canvas as needing a full clear before its next render pass
...
lurek.render.setBold( bold : boolean )  -- Sets whether subsequent font size lookups use the bold Courier New variant
lurek.render.setCanvas( ud : userdata? )  -- Redirects subsequent drawing to a canvas, or nil for the screen
...
lurek.render.setColorMask( args : any )  -- Sets which color channels are written during draw calls. Call with no args to enable all
lurek.render.setDebugShader( shader : LShader? )  -- Activates a debugviz-target shader, or restores the normal draw shader with nil
...
lurek.render.setDefaultFont( pointSize : integer?, bold : boolean? ) -> LFont  -- Selects a built-in default font by bundled point size and makes it the active render font
...
lurek.render.setShader( shader : LShader? )  -- Activates a draw-target shader for subsequent draw calls, or restores the default with nil
...
lurek.render.setTextShader( shader : LShader? )  -- Activates a text-target shader for font-atlas text, or restores default text rendering with nil
...
```lua
LCanvas:applyShader( shader : LShader, opts : table? ) -> LCanvas  -- Queues a postfx shader pass that mutates this canvas render target after queued canvas draws in the current frame
...
LReadbackRequest:typeOf( name : string ) -> boolean  -- Returns whether this object matches a supported type name
```
...
LShaderPrewarmRequest:typeOf( name : string ) -> boolean  -- Returns whether this object matches a supported type name
```
...
```lua
LShape:addShape( child : LShape, opts : table? )  -- Snapshots another shape's IR under an optional local transform
...
LShape:arrow( mode : string, x1 : number, y1 : number, x2 : number, y2 : number, width : number?, head : number? )  -- Adds an arrow from `(x1,y1)` to `(x2,y2)` with a triangular head
LShape:capsule( mode : string, x : number, y : number, w : number, h : number, radius : number? )  -- Adds a rounded capsule (horizontal or vertical according to its dimensions)
...
LShape:compile( opts : table? ) -> boolean  -- Compiles the shape once on the CPU; the next frame uploads one static GPU mesh
...
LShape:drawMany( instances : table )  -- Queues up to 250,000 instances of one compiled shape for a single compatible draw
...
LShape:path( segments : table, opts : table? )  -- Adds a native path with move, line, quadratic, cubic, and close verbs
LShape:point( x : number, y : number, size : number? )  -- Adds one point or disc primitive to the shape
LShape:points()  -- Adds many point/disc primitives from flat coordinates or `{x,y}` pairs
...
LShape:regularPolygon( mode : string, x : number, y : number, radius : number, sides : integer, rotation : number? )  -- Adds a regular polygon, useful for icons and data-viz marks
LShape:release() -> boolean  -- Releases the shape slot and makes this handle stale
LShape:ring( mode : string, x : number, y : number, outer : number, inner : number, segments : integer? )  -- Adds a ring or annulus approximation from two circles
...
LShape:sector( mode : string, x : number, y : number, radius : number, angle1 : number, angle2 : number, segments : integer? )  -- Adds a filled sector or stroked arc
LShape:setColor( r : number, g : number, b : number, a : number? )  -- Sets the drawing color for subsequent shape commands
LShape:setColorRole( role : string )  -- Selects a semantic palette role for subsequent commands
LShape:setLineWidth( w : number )  -- Sets the line width for subsequent line-mode shape commands
LShape:setPalette( palette : table )  -- Replaces one or more semantic palette roles with normalized RGBA colors
LShape:setStrokeStyle( opts : table )  -- Sets cap, join, miter and dash parameters for subsequent stroke commands
LShape:star( mode : string, x : number, y : number, outer : number, inner : number, points : integer, rotation : number? )  -- Adds a regular star with alternating outer/inner radii
LShape:symbol( name : string, x : number, y : number, size : number, mode : string? )  -- Adds a data-viz marker symbol (circle, square, diamond, triangle, cross, plus, times, asterisk, wye)
LShape:trail( coords : table, width_table : table )  -- Adds a variable-width trail as a strip between two point lists
...
LSpriteBatch:addComposite( parts : table ) -> number  -- Adds multiple part entries to the batch for one modular composite visual
LSpriteBatch:addMany( entries : table ) -> integer  -- Atomically appends an array of sprite entries after validating the whole input
...
LSpriteBatch:removeEntries( indices : table ) -> integer  -- Atomically removes selected one-based sprite entries
LSpriteBatch:setEntries( entries : table ) -> integer  -- Atomically replaces all sprite entries after validating the whole input
...
LSpriteBatch:updateEntries( updates : table ) -> integer  -- Atomically replaces selected one-based sprite entries
```
...
*Coverage: 153/153 items documented (100%)*

```lua
lurek.input.advancePlayback() -> table  -- Advances playback by one frame and returns events for that frame
lurek.input.assignPlayer( player : integer, gamepad_id : integer )  -- Assigns a gamepad slot to a positive player number
...
lurek.input.defineActions( defs : table, defaultCategory : string ) -> integer  -- Defines multiple named actions at once, replacing prior definitions
...
lurek.input.getAssignedPlayer( id : integer ) -> integer  -- Returns the player assigned to a gamepad, or nil when unassigned
...
lurek.input.getDeadzone( id : integer, stick : string ) -> number  -- Returns a configured per-gamepad deadzone, defaulting to 0.0
lurek.input.getDelta() -> number  -- Returns raw pointer movement accumulated during the current frame
...
lurek.input.getPlayerGamepad( player : integer ) -> integer  -- Returns the gamepad assigned to a player, or nil
...
lurek.input.getStandardAxis( id : integer, name : string ) -> number  -- Returns a standard named Xbox axis value
lurek.input.getStandardButton( id : integer, name : string ) -> boolean  -- Returns whether a standard named Xbox button is currently down
lurek.input.getSystemCursor( name : string ) -> LCursor  -- Creates a system cursor handle from a cursor name
lurek.input.getTextInput() -> string[]  -- Returns committed text segments received during the current frame
...
lurek.input.isContextEnabled( name : string ) -> boolean  -- Returns whether an action input context is enabled
...
lurek.input.newPlayerContext( player_id : integer ) -> LPlayerInputContext  -- Creates an isolated player-scoped input context with local action names
...
lurek.input.setContextEnabled( name : string, enabled : boolean )  -- Enables or disables an action input context
lurek.input.setCursor( cursor : any )  -- Sets the active cursor from a cursor handle, system cursor name, or nil for arrow
lurek.input.setDeadzone( id : integer, stick : string, value : number )  -- Sets a per-gamepad deadzone for a named stick or axis group
...
lurek.input.startPlayback( opts : table? )  -- Starts playback of the loaded recording. `opts.mode` may be `frame`, `fixed`, or `realtime`
...
lurek.input.wasPressed( key : string ) -> boolean  -- Returns whether a logical key transitioned to pressed this frame
lurek.input.wasPressed( button : integer ) -> boolean  -- Returns whether a one-based mouse button index transitioned to pressed this frame
...
lurek.input.wasReleased( key : string ) -> boolean  -- Returns whether a logical key transitioned to released this frame
lurek.input.wasReleased( button : integer ) -> boolean  -- Returns whether a one-based mouse button index transitioned to released this frame
...
lurek.input.wasScancodePressed( scancode : string ) -> boolean  -- Returns whether a physical keyboard scancode transitioned to pressed this frame
lurek.input.wasScancodeReleased( scancode : string ) -> boolean  -- Returns whether a physical keyboard scancode transitioned to released this frame
```
...
```lua
LCombo:completedWithin( ms : integer ) -> boolean  -- Returns whether the combo completed within a recent millisecond window
...
```lua
LPlayerInputContext:assignGamepad( id : integer, opts : table? )  -- Assigns a persistent gamepad slot to this context
LPlayerInputContext:assignKeyboardMouse( opts : table? )  -- Assigns keyboard and mouse to this context
LPlayerInputContext:clearActions()  -- Removes every local action from this player input context
LPlayerInputContext:defineAxis1D( name : string, opts : table )  -- Defines or replaces a continuous one-dimensional action
LPlayerInputContext:defineAxis2D( name : string, opts : table )  -- Defines or replaces a continuous radial two-dimensional action
LPlayerInputContext:defineButton( name : string, opts : table )  -- Defines or replaces a player-local digital action
LPlayerInputContext:getAssignedDevices() -> table  -- Returns keyboard/mouse ownership and deterministic gamepad slot records
LPlayerInputContext:getAxis1D( name : string ) -> number  -- Returns a shaped continuous one-dimensional value
LPlayerInputContext:getAxis2D( name : string ) -> number  -- Returns a shaped continuous vector with length no greater than one
LPlayerInputContext:getConflicts() -> table  -- Returns local binding conflicts in deterministic order
LPlayerInputContext:isDown( name : string ) -> boolean  -- Returns whether a button or thresholded analog action is currently active
LPlayerInputContext:isEnabled() -> boolean  -- Returns whether this context is enabled
LPlayerInputContext:removeAction( name : string ) -> boolean  -- Removes one local action from this player input context
LPlayerInputContext:restoreBindings( json : string )  -- Atomically restores definitions and enabled state from JSON
LPlayerInputContext:serializeBindings() -> string  -- Serializes local definitions and enabled state as versioned JSON
LPlayerInputContext:setEnabled( enabled : boolean )  -- Enables or disables all queries for this context
LPlayerInputContext:type() -> string  -- Returns the Lua-visible handle type
LPlayerInputContext:typeOf( name : string ) -> boolean  -- Checks this handle against `LPlayerInputContext` or `LObject`
LPlayerInputContext:unassignGamepad( id : integer? )  -- Releases one gamepad slot, or every assigned slot when id is nil
LPlayerInputContext:unassignKeyboardMouse()  -- Releases keyboard and mouse ownership
LPlayerInputContext:vibrate( low : number, high : number, duration_ms : number ) -> integer  -- Queues vibration for every assigned connected gamepad that supports it
LPlayerInputContext:wasPressed( name : string ) -> boolean  -- Returns the current-frame press transition, including analog hysteresis transitions
LPlayerInputContext:wasReleased( name : string ) -> boolean  -- Returns the current-frame release transition, including analog hysteresis transitions
```
...
lurek.time.sleep( seconds : number )  -- Blocks the current thread for the given number of seconds. Use sparingly â€” this halts the entire game loop. Intended for loading screens or synchronization
...
*Coverage: 269/269 items documented (100%)*
...
lurek.math.lootFromList( entries : table ) -> LLootTable  -- Creates a loot table from a Lua list of entry tables
lurek.math.lootFromToml( path : string ) -> LLootTable  -- Loads a loot table from a TOML file path
...
lurek.math.newLootTable( opts : any ) -> LLootTable  -- Creates a Walker-Vose alias-method loot table for O(1) weighted random sampling
...
lurek.math.newWrapSpace( min_x : number, min_y : number, width : number, height : number ) -> LWrapSpace  -- Creates a pure toroidal-coordinate helper without changing world, ECS, or physics state
...
lurek.math.sampleWithPity( loot_table : LLootTable, pity : LPityTracker ) -> string  -- Samples loot table with pity behavior: forced target when tracker is primed
...
LLootTable:merge( other : LLootTable )  -- Merges entries from another loot table into this one
LLootTable:remove( id : string ) -> boolean  -- Removes an entry by id. Returns true when found
LLootTable:restore( blob : string )  -- Restores loot table state from a blob produced by `save`
...
LPityTracker:import( blob : string )  -- Compatibility alias for `restore`
...
LPityTracker:restore( blob : string )  -- Restores pity state from a blob produced by `save`
...
```lua
LWrapSpace:delta( ax : number, ay : number, bx : number, by : number ) -> number  -- Returns the shortest signed toroidal displacement from point A to point B
LWrapSpace:distance( ax : number, ay : number, bx : number, by : number ) -> number  -- Returns the shortest toroidal distance between two points
LWrapSpace:type() -> string  -- Returns this helper's runtime type name for Lua introspection
LWrapSpace:typeOf( name : string ) -> boolean  -- Checks this helper against its public type names
LWrapSpace:wrap( x : number, y : number ) -> number  -- Wraps a point into this half-open toroidal domain
```
...
```lua
lurek.audio.beatClockFromSource( source : LSource|integer, bpm : number, opts : table? ) -> LBeatClock  -- Creates a new beat clock and synchronizes it to an audio source position
...
lurek.audio.getSourceSpatialResult( source : LSource|integer ) -> table  -- Returns the current bounded spatial result for one source
...
lurek.audio.judgeBeat( clock : LBeatClock, division : integer?, hit_offset : number? ) -> string  -- Judges timing against the nearest beat grid for a beat clock
...
lurek.audio.newBeatClock( bpm : number, beats_per_bar_or_opts : any, opts : table? ) -> LBeatClock  -- Creates a musical beat clock for rhythm-game timing, tap-tempo, and beat scheduling
...
lurek.audio.playMusic( path : string, opts : table? ) -> LSource  -- Starts a looping streaming track and routes it through a named music group
...
lurek.audio.setGroupVolume( group : string, volume : number )  -- Sets the volume multiplier for one named music/SFX group
...
lurek.audio.setJudgementWindows( windows : table )  -- Sets global default timing windows used by beat-clock judgement
...
lurek.audio.setListeners( listeners : table, opts : table? )  -- Atomically replaces the neutral spatial listener set
...
lurek.audio.setSourceListenerMask( source : LSource|integer, listenerIds : table? )  -- Sets or clears the listener allow-list for one source
...
```lua
LBeatClock:at( beat : number, fn : function ) -> table  -- Registers a one-shot callback fired when `beat` is crossed
LBeatClock:beatTimeRemaining( division : integer? ) -> number  -- Returns seconds until the next division boundary
...
LBeatClock:cancel( handle : any ) -> boolean  -- Cancels a scheduled callback handle
...
LBeatClock:every( division : integer, fn : function ) -> table  -- Registers a callback fired on each crossed step of `division`
...
LBeatClock:getPhase( division : integer? ) -> number  -- Returns phase within the current division in [0, 1)
LBeatClock:isOnBeat( division : integer?, tolerance : number? ) -> boolean  -- Returns true when the clock is near a beat boundary
LBeatClock:isRunning() -> boolean  -- Returns true when the clock is running
LBeatClock:judge( division : integer?, hit_offset : number?, windows : table? ) -> table  -- Judges timing against the nearest beat grid and returns a detailed result table
LBeatClock:nearestBeat( division : integer? ) -> number, number  -- Returns nearest beat and signed timing error in seconds
LBeatClock:pattern( pattern : string, fn : function ) -> table  -- Registers a repeating pattern callback where `x` triggers and `.` skips
...
LBeatClock:rampBpm( target : number, seconds : number )  -- Ramps BPM linearly to a target value over time
...
LBeatClock:setSwing( amount : number )  -- Sets rhythmic swing amount in `[0.0, 0.5]` for off-beat timing feel
...
LBeatClock:syncToSource( source : LSource|integer )  -- Synchronizes beat position to an audio source playback position
...
LBeatClock:update( dt : number ) -> table  -- Advances the clock by `dt` seconds and returns beat/bar transitions
```
...
*Coverage: 351/351 items documented (100%)*
...
lurek.physics.createBodiesFromTilefield( field : LTileField, slot : string, tileset : LTileSet, world : LWorld, opts : table? ) -> LBody[]  -- Creates physics bodies from `lurek.tilefield` refs and tileset object metadata
...
lurek.physics.newAltitudeLayer( opts : table ) -> LAltitudeLayer  -- Creates a deterministic altitude-layer grid for 2.5D terrain height and clearance sampling
lurek.physics.newBody( world : LWorld, x : number, y : number, bodyType : string, opts : table ) -> LBody  -- Creates a new body in a world (free-function variant)
...
lurek.physics.newLiquidMap( width : integer, height : integer, cellSize : number, world : LWorld, terrain : LTerrain? ) -> LLiquidMap  -- Creates a grid-based liquid map linked to a physics world and optionally to a terrain blocker grid
lurek.physics.newMaterial( opts : table ) -> table  -- Validates and canonicalizes a reusable physics material table
...
lurek.physics.reflectVelocity( vx : number, vy : number, nx : number, ny : number, coefficient : number ) -> number  -- Reflects a velocity vector around a surface normal without mutating any body
...
lurek.physics.shapeFromImage( image : LImageData, opts : table? ) -> LPhysicsShape  -- Builds an approximate collision shape from an image alpha mask
...
```lua
LAltitudeLayer:getCellHeight( cx : integer, cy : integer ) -> number  -- Returns one terrain-height cell from the altitude layer
LAltitudeLayer:load( data : table )  -- Replaces this altitude-layer payload from serialized data
LAltitudeLayer:sampleClearance( x : number, y : number ) -> number  -- Samples gameplay clearance at world coordinates using the layer's current sampling mode
LAltitudeLayer:sampleHeight( x : number, y : number ) -> number  -- Samples terrain height at world coordinates using the layer's current sampling mode
LAltitudeLayer:serialize() -> table  -- Serializes the full altitude-layer payload for save/load and inspection
LAltitudeLayer:setCellClearance( cx : integer, cy : integer, clearance : number )  -- Sets one gameplay-clearance cell in the altitude layer
LAltitudeLayer:setCellHeight( cx : integer, cy : integer, height : number )  -- Sets one terrain-height cell in the altitude layer
LAltitudeLayer:type() -> string  -- Returns the type name of this object ("LAltitudeLayer")
LAltitudeLayer:typeOf( name : string ) -> boolean  -- Checks whether this object matches a given type name
```
...
LBody:applyThrust( amount : number )  -- Applies force in the body's current forward direction for top-down inertial movement
LBody:applyTorque( torque : number )  -- Applies a rotational torque to the body
LBody:applyTurn( torque : number )  -- Applies torque to the body for top-down turning
...
LBody:setAirScale( scale : number )  -- Sets the extra multiplier used only for `air` flow fields
LBody:setAltitude( z : number )  -- Sets this body's terrain-relative or fixed-world altitude value
LBody:setAltitudeCollision( opts : table )  -- Replaces this body's altitude-collision flags
LBody:setAltitudeMode( mode : string )  -- Sets how this body's altitude is interpreted: ground, airborne, ballistic, or fixed
...
LBody:setBeamReflectivity( reflectivity : number )  -- Sets the energy multiplier used when a reflective beam bounces from this body
LBody:setBullet( bullet : boolean )  -- Enables or disables continuous collision detection to prevent fast-moving tunneling. Use it for small, fast bodies such as bullets and shrapnel, not every body in the scene
LBody:setClearanceClass( className : string )  -- Sets this body's authored clearance class for higher-level RTS filtering
LBody:setCollisionGroup( group : integer )  -- Assigns the body to one collision group and opens its local mask to the 16 group bits
LBody:setFixedRotation( fixed : boolean )  -- Locks or unlocks the body's rotation. Useful for player characters
LBody:setFlowCrossSection( crossSection : number )  -- Sets the drag cross-section factor used by drag-style flow application
LBody:setFlowScale( scale : number )  -- Sets the global multiplier applied to all flow-field influences on this body
...
LBody:setHeightExtent( height : number )  -- Sets this body's targetable vertical extent for 2.5D overlap tests
...
LBody:setMaterial( material : table )  -- Applies a validated material table to this body's primary collider and body-level solver properties
LBody:setMirror( mirror : boolean )  -- Enables or disables mirror-style beam reflection on this body
LBody:setPosition( x : number, y : number )  -- Teleports the body to a new world-space position (does not apply physics forces)
LBody:setProjectileReflectivity( reflectivity : number )  -- Sets the gameplay projectile reflectivity hint stored on this body
...
LBody:setVerticalGravity( gravity : number )  -- Sets this body's per-step vertical gravity
LBody:setVerticalVelocity( vz : number )  -- Sets this body's vertical velocity used by airborne and ballistic altitude modes
LBody:setWaterScale( scale : number )  -- Sets the extra multiplier used only for `water` flow fields
...
```lua
LFlowStream:destroy()  -- Disables and removes this flow field from the world
...
LFlowStream:setApplication( mode : string )  -- Sets the body-application mode used during stepping
LFlowStream:setCombine( mode : string )  -- Sets how this field combines with overlapping fields
LFlowStream:setEnabled( enabled : boolean )  -- Enables or disables this flow field
LFlowStream:setLayerMask( mask : integer )  -- Sets the body-layer mask that this field affects
LFlowStream:setPoints( points : table )  -- Replaces the polyline points of a path-shaped flow field
LFlowStream:setStrength( strength : number )  -- Sets the current movement strength for this flow field
LFlowStream:setWidth( width : number )  -- Sets the width of a path-shaped flow field
LFlowStream:type() -> string  -- Returns the type name of this object
LFlowStream:typeOf( name : string ) -> boolean  -- Returns whether this object matches the requested type name
```
...
LKinematicController2D:move( dx : number, dy : number, opts : table? ) -> table  -- Sweeps and wall-slides the controlled kinematic body
LKinematicController2D:recover( opts : table? ) -> table  -- Applies penetration recovery and updates the controlled body position
LKinematicController2D:release() -> nil  -- Releases this controller so further operations fail predictably
LKinematicController2D:setFilter( filter : any ) -> nil  -- Replaces the query filter used by movement sweeps
LKinematicController2D:setMaxSlides( max_slides : integer ) -> nil  -- Sets the maximum number of wall slides permitted for one movement
LKinematicController2D:setRadius( radius : number ) -> nil  -- Sets the positive sweep radius used by subsequent movement queries
LKinematicController2D:setSkin( skin : number ) -> nil  -- Sets the non-negative collision skin used by subsequent movement queries
LKinematicController2D:setVerticalSpan( z_min : number, z_max : number ) -> nil  -- Constrains movement sweeps to a strict vertical span
LKinematicController2D:testMove( dx : number, dy : number, opts : table? ) -> table  -- Solves movement without mutating the controlled body
LKinematicController2D:type() -> string  -- Returns this controller's runtime type name
LKinematicController2D:typeOf( name : string ) -> boolean  -- Checks whether a type name is implemented by this controller
```
...
```lua
LLiquidMap:applyBuoyancy( opts : table? ) -> table  -- Applies sampled buoyancy and linear drag to matching dynamic bodies in the linked world
LLiquidMap:drainRect( x : integer, y : integer, width : integer, height : integer, amount : number )  -- Removes up to the requested amount from every cell in a rectangular region
LLiquidMap:fillRect( x : integer, y : integer, width : integer, height : integer, amount : number, kind : any )  -- Sets every liquid cell in a rectangular region to the same amount and kind
LLiquidMap:getAmountAt( worldX : number, worldY : number ) -> number  -- Samples liquid fill amount at one world-space point
LLiquidMap:getCell( cx : integer, cy : integer ) -> number  -- Returns the amount and kind stored in one liquid cell
LLiquidMap:getDirtyChunks() -> table  -- Returns liquid chunks changed by the most recent liquid edit or simulation step
LLiquidMap:getLevelAt( worldX : number, worldY : number ) -> number  -- Returns the top liquid surface level for the sampled column
LLiquidMap:loadFromBytes( data : string ) -> boolean  -- Restores liquid grid state from binary data previously produced by `toBytes`
LLiquidMap:setCell( cx : integer, cy : integer, amount : number, kind : any )  -- Sets one liquid cell amount and kind
LLiquidMap:step( opts : table? ) -> table  -- Advances the liquid simulation with deterministic per-cell flow
...
LLiquidMap:typeOf( name : string ) -> boolean  -- Checks whether this object matches a given type name
```
...
```lua
LTerrain:addCircle( wx : number, wy : number, radius : number )  -- Adds solid terrain inside a circular region
LTerrain:addRect( wx : number, wy : number, w : number, h : number )  -- Adds solid terrain across a rectangular region
LTerrain:carveCircle( wx : number, wy : number, radius : number )  -- Carves a circular hole by clearing terrain cells inside the given radius
LTerrain:carveRect( wx : number, wy : number, w : number, h : number )  -- Carves a rectangular hole by clearing all overlapping terrain cells
LTerrain:collapseColumns() -> integer  -- Removes isolated single-cell overhangs that have no support below or beside them
LTerrain:collapseUnsupported( opts : table ) -> table  -- Collapses unsupported connected terrain components using the requested support rule and collapse mode
LTerrain:damageCircle( wx : number, wy : number, radius : number, opts : table? ) -> table  -- Carves a circular hole and can immediately collapse unsupported terrain with policy options
...
LTerrain:flush( maxDirtyChunks : integer? ) -> table  -- Regenerates physics colliders from the current terrain grid state and returns rebuild diagnostics
...
LTerrain:setColliderStrategy( strategy : string )  -- Selects static terrain collider generation. `rowRuns` is the fast filled default; `contourEdges` emits only exposed cell boundaries
...
LWorld:addFan( opts : table ) -> LFlowStream  -- Creates a directional fan helper around the flow-field system
...
LWorld:addFlowField( opts : table ) -> LFlowStream  -- Creates one authored flow field and returns a handle for later mutation
...
LWorld:addGravityVector( gx : number, gy : number, layerMask : integer? ) -> integer  -- Adds an extra directional gravity vector that is summed with world gravity when no non-additive zone override is active
...
LWorld:applyForces( forces : table )  -- Applies many continuous forces through one validated world borrow
LWorld:beamAll( x : number, y : number, dx : number, dy : number, range : number, filter : table? ) -> table  -- Returns all instant beam hits in deterministic distance order
LWorld:beamClosest( x : number, y : number, dx : number, dy : number, range : number, filter : table? ) -> table  -- Returns only the closest instant beam hit, or nil if nothing blocks the beam
LWorld:castBallisticArc( opts : table ) -> table  -- Traces a deterministic ballistic arc without spawning a persistent projectile
LWorld:castBeam( x : number, y : number, dx : number, dy : number, range : number, opts : table? ) -> table  -- Casts an instant beam and returns hit plus segment data for gameplay or rendering
LWorld:castCircle( x : number, y : number, radius : number, dx : number, dy : number, maxDist : number, filter : table? ) -> table  -- Sweeps a circle along a direction and returns the first collider hit
LWorld:castCircle2_5d( opts : table ) -> table  -- Sweeps a 2.5D circle and vertical interval, returning the earliest body or terrain hit
LWorld:castProjectile( opts : table ) -> table  -- Sweeps a projectile circle and returns a movement result with final position and hit data
...
LWorld:clearFlowFields()  -- Disables every authored flow field in the world
LWorld:clearGravityVectors()  -- Removes all additive gravity vectors from the world
LWorld:configureCollisionGroups( spec : table, opts : table ) -> table  -- Configures named 0..15 collision-group roles and returns their layer/mask profile
...
LWorld:drawAltitudeDebug( target : LImageData, opts : table? )  -- Draws altitude-layer cells, body vertical ranges, and ballistic arcs into an ImageData target
...
LWorld:drawFlowDebug( target : LImageData, opts : table? )  -- Draws flow-field centerlines and sampled arrows into an ImageData target
...
LWorld:getBallisticProjectile( id : integer ) -> table  -- Returns one active engine-owned ballistic projectile by id, or nil when inactive
...
LWorld:getBodyCCD( id : integer ) -> boolean  -- Returns whether continuous collision detection is enabled on a body. This is the world-level alias for `LBody:isBullet`
...
LWorld:getBodyStates( ids : integer[] ) -> table  -- Reads compact position, angle, and velocity state for many bodies in one Lua boundary crossing
...
LWorld:getCollisionGroupMask( group : integer ) -> integer  -- Returns one row of the 16-group collision matrix
LWorld:getCollisionPair( groupA : integer, groupB : integer ) -> boolean  -- Returns whether collisions are enabled between two world-level collision groups
...
LWorld:getFixtureMaterial( bodyId : integer, fixtureIndex : integer ) -> table  -- Returns the current material table for one fixture
LWorld:getFlowField( id : integer ) -> table  -- Returns one authored flow field table by id, or nil when missing
LWorld:getGravity() -> number  -- Returns the current world gravity vector
LWorld:getGravityVector( id : integer ) -> table  -- Returns an additive gravity vector by ID, or nil when no active vector exists
...
LWorld:isBodyEnabled( body_id : integer )  -- Returns whether one live body participates in simulation and queries
LWorld:isBodySleeping( id : integer ) -> boolean  -- Returns whether a body is currently in the sleeping (inactive) state
LWorld:isFixtureEnabled( body_id : integer, fixture_index : integer )  -- Returns whether one zero-based fixture participates in simulation and queries
LWorld:jointCount() -> integer  -- Returns the total number of joints in the world
LWorld:newBodies( specs : table ) -> integer[]  -- Batch-creates multiple bodies at once for better performance. Each entry is {x, y, w, h, type} or {x, y, type}
LWorld:newBody( x : number, y : number, bodyType : string, opts : table ) -> LBody  -- Creates a new physics body at the given position with the specified type and dimensions
LWorld:newChainBody( x : number, y : number, vertices : table, closed : boolean, bodyType : string, opts : table ) -> LBody  -- Creates a new body with a chain (polyline) collider. Useful for terrain edges
LWorld:newCircleBody( x : number, y : number, radius : number, bodyType : string, opts : table ) -> LBody  -- Creates a new body with a circle collider already attached
LWorld:newEdgeBody( x : number, y : number, x1 : number, y1 : number, x2 : number, y2 : number, bodyType : string, opts : table ) -> LBody  -- Creates a new body with an edge (line segment) collider between two local points
LWorld:newKinematicController( body : LBody, opts : table ) -> LKinematicController2D  -- Creates a bounded circle-sweep controller for one kinematic body
LWorld:newPolygonBody( x : number, y : number, vertices : table, bodyType : string, opts : table ) -> LBody  -- Creates a new body with a convex polygon collider defined by vertex pairs
LWorld:newProjectileBody( opts : table ) -> LBody  -- Creates a small circle body with shooter-friendly projectile defaults
...
LWorld:queryAltitudeOverlap( x : number, y : number, radius : number, zMin : number, zMax : number, filter : table? ) -> table  -- Returns all 2.5D overlaps whose XY footprint and world-space Z interval match the query
LWorld:querySector( x : number, y : number, radius : number, angle : number, halfAngle : number, filter : table? ) -> table  -- Returns body centers in a radius and angular sector with stable ordering
...
LWorld:reflectBodyVelocity( bodyId : integer, normalX : number, normalY : number, coefficient : number ) -> boolean  -- Reflects a body's current velocity around a supplied world-space surface normal
LWorld:removeBallisticProjectile( id : integer ) -> boolean  -- Removes one active engine-owned ballistic projectile by id
LWorld:removeFlowField( id : integer ) -> boolean  -- Disables and removes one authored flow field by id
LWorld:removeGravityVector( id : integer ) -> boolean  -- Removes one additive gravity vector so it no longer affects future steps
LWorld:resetCollisionGroups()  -- Restores all 16 collision groups so every group can collide with every other group
LWorld:resetWorld()  -- Fully resets the world to its post-construction state
LWorld:sampleFlow( x : number, y : number, opts : table? ) -> table  -- Samples combined flow at a world position
LWorld:setAltitudeLayer( layer : LAltitudeLayer )  -- Attaches or replaces the world's 2.5D altitude layer from an `LAltitudeLayer` snapshot
LWorld:setBeginContact( callback : function )  -- Registers a callback function invoked whenever two bodies begin touching
LWorld:setBodyCCD( id : integer, enabled : boolean )  -- Enables or disables continuous collision detection (bullet mode) on a body to prevent tunneling. This is the world-level alias for `LBody:setBullet`
...
LWorld:setBodyEnabled( body_id : integer, enabled : boolean )  -- Enables or disables one body without destroying its stable id
...
LWorld:setBodyStates( states : table )  -- Applies complete transform and velocity updates atomically after validating every state record
LWorld:setBodyType( id : integer, bodyType : string )  -- Changes the type of an existing body (e.g. from "dynamic" to "static")
LWorld:setCcdSubsteps( n : integer )  -- Sets the maximum number of CCD substeps. Increase this when fast bullet bodies still need more reliable thin-wall resolution
LWorld:setCollisionGroupMask( group : integer, mask : integer )  -- Replaces one row of the 16-group collision matrix
LWorld:setCollisionPair( groupA : integer, groupB : integer, enabled : boolean )  -- Enables or disables collisions between two world-level collision groups
LWorld:setEndContact( callback : function )  -- Registers a callback function invoked whenever two bodies stop touching
LWorld:setFixtureEnabled( body_id : integer, fixture_index : integer, enabled : boolean )  -- Enables or disables one zero-based fixture without changing sensor state
...
LWorld:setFixtureMaterial( bodyId : integer, fixtureIndex : integer, material : table )  -- Assigns a reusable material table to one fixture
...
LWorld:setGravityVector( id : integer, gx : number, gy : number, layerMask : integer? )  -- Replaces the direction, strength, and optional layer mask of an existing additive gravity vector
...
LWorld:setTopDownDamping( linear : number, angular : number )  -- Sets default linear and angular damping for top-down inertial bodies and applies it to existing bodies
LWorld:setWrapBounds( minX : number?, minY : number?, maxX : number?, maxY : number? )  -- Sets or clears toroidal wrap bounds for top-down arenas
LWorld:sleepBody( id : integer )  -- Forces a body into the sleeping state, pausing its simulation until disturbed
LWorld:spawnBallisticProjectile( opts : table ) -> integer  -- Spawns a deterministic engine-owned ballistic projectile and returns its stable id
LWorld:step( dt : number )  -- Advances the physics simulation by a time delta and fires any registered contact callbacks
LWorld:stepFixed( accumulator : number, stepDt : number, maxSteps : integer ) -> number  -- Performs fixed-timestep physics stepping, consuming accumulated time. Use this for frame pacing; bullet CCD still matters for thin barriers
...
LWorld:wrapBody( bodyId : integer ) -> number  -- Wraps one body through the current toroidal bounds and returns its final position
```
...
LZone:setGravityAdditive( additive : boolean )  -- Controls whether this zone adds gravity to other fields instead of overriding world gravity by priority
LZone:setGravityDirectional( gx : number, gy : number )  -- Sets the zone to apply a constant directional gravity to bodies inside
LZone:setGravityFalloff( mode : string )  -- Sets point/repulsor gravity falloff. Accepted modes: inverseSquare, inverse, linear, constant
LZone:setGravityLimits( minAccel : number?, maxAccel : number? )  -- Sets optional minimum and maximum acceleration clamps for point/repulsor gravity
...
LZone:setGravityRadius( innerRadius : number, outerRadius : number? )  -- Sets the inner radius and optional outer radius used by point/repulsor falloff
...
LZone:setLinearDrag( value : number? )  -- Sets or clears area drag proportional to velocity for bodies inside this zone
LZone:setPriority( priority : integer )  -- Sets the priority of this zone. Higher-priority zones take precedence when overlapping
LZone:setQuadraticDrag( value : number? )  -- Sets or clears area drag proportional to speed times velocity for bodies inside this zone
...
lurek.filesystem.mountWorkspace( src : string, mp : string ) -> boolean  -- Mounts a user-selected workspace directory at a non-empty virtual mount point
...
lurek.filesystem.unwatchPath( path : string )  -- Removes a GameFS path from the module-local file watcher
lurek.filesystem.watchPath( path : string )  -- Adds a GameFS path to the module-local file watcher
...
lurek.filesystem.writeWorkspace( path : string, content : string ) -> nil  -- Writes UTF-8 text inside a previously mounted user workspace
lurek.filesystem.writeWorkspaceAtomic( path : string, content : string ) -> nil  -- Atomically writes UTF-8 text inside a previously mounted user workspace
```
...
*Coverage: 115/115 items documented (100%)*
...
LParticleSystem:emitAt( x : number, y : number, count : integer, direction : number )  -- Moves the emitter, optionally changes direction, and emits particles immediately
...
LParticleSystem:setShader( shader : LShader? )  -- Sets or clears the render-time shader for this particle system
LParticleSystem:setShaderUniform( name : string, value : number|boolean|table )  -- Sends a uniform value to the shader bound to this particle system
...
lurek.signal.fromChangeSetTable( value : table, maxChanges : integer? ) -> LChangeSet  -- Creates a ChangeSet from a table produced by `LChangeSet:toTable()`
lurek.signal.getHistory() -> table  -- Returns retained pushed event history entries
lurek.signal.newChangeSet( options : table? ) -> LChangeSet  -- Creates an empty bounded ChangeSet for neutral state replication, save, or Lua-side module integration
...
```lua
LChangeSet:append( objectId : integer, component : string, operation : string, payload : any ) -> integer  -- Appends one neutral object/component mutation and returns the new record count
...
LChangeSet:restore( snapshot : table )  -- Restores records from a table produced by `snapshot` or `toTable`
...
LChangeSet:typeOf( name : string ) -> boolean  -- Checks whether this handle matches `LChangeSet` or `LObject`
```
...
lurek.runtime.parseArgs( args : table? ) -> table  -- Parses command-line arguments into structured flags, options, and positional values. Supports `--key=value`, `--key value`, `-flag`, and `--` end-of-options
...
*Coverage: 271/271 items documented (100%)*
...
LAIWorld:queryAgentsInRadius( x : number, y : number, radius : number, opts : table? ) -> table  -- Returns nearby agents by using the world's persistent spatial index instead of a full Lua scan
LAIWorld:removeAgent( agent : LBot )  -- Removes an agent from this world by using an existing agent handle
LAIWorld:setAutoAcquireBudget( budget : integer )  -- Sets the maximum number of stance-driven hostile-acquisition queries attempted in one update
LAIWorld:setOrderArrivalRadius( radius : number )  -- Sets the move-order arrival threshold used by world update when completing queued move orders
LAIWorld:setSpatialCellSize( size : number )  -- Sets the spatial-hash cell size used by nearby-agent queries in this world
...
```lua
LBot:acquireTarget( opts : table? ) -> LuaValue  -- Returns the nearest target selected from this agent's stance-driven hostile-acquisition query
LBot:addTag( tag : string )  -- Adds a tag string to this agent when the agent still exists in its world
LBot:addTraitModifier( trait_name : string, delta : number, duration : number?, source : string )  -- Adds a temporary or permanent modifier to one trait on this agent
LBot:clearOrders( reason : string? ) -> integer  -- Clears every queued order owned by this agent
LBot:drainCommandEvents() -> table  -- Returns and clears queued order lifecycle events for this agent
LBot:findHostilesInRange( radius : number?, opts : table? ) -> table  -- Returns nearby hostile agents by using the world's spatial index and this agent's team as the hostile reference
...
LBot:getTrait( name : string ) -> number  -- Returns one effective trait value from this agent's profile
...
LBot:setStance( stance : string, opts : table? )  -- Sets this agent's built-in RTS stance and optionally overrides its acquisition settings
LBot:setTeam( team : integer )  -- Sets this agent's integer team identifier used by hostile-acquisition queries
LBot:setTrait( name : string, value : number )  -- Sets one trait on this agent, creating an empty profile first when needed
LBot:setTraitProfile( profile : LTraitProfile )  -- Copies a trait profile onto this agent so future agent decisions can read commander personality values
...
```lua
LCommandQueue:cancelByTag( tag : string ) -> integer  -- Cancels all queued orders whose kind matches `tag`
LCommandQueue:cancelCurrent( reason : string? ) -> boolean  -- Cancels the currently active command when one exists
LCommandQueue:clear( reason : string? ) -> integer  -- Removes every queued command. This method is available to Lua scripts
LCommandQueue:completeCurrent( reason : string? ) -> LuaValue  -- Marks the current command as completed and advances the queue
LCommandQueue:drainEvents() -> table  -- Returns and clears queued lifecycle events
LCommandQueue:enqueue( kind : string, callback : function, opts : table? ) -> integer  -- Adds a command callback to the back of the queue
LCommandQueue:failCurrent( reason : string? ) -> boolean  -- Marks the current command as failed and advances the queue
...
LCommandQueue:pushFront( kind : string, callback : function, opts : table? ) -> integer  -- Adds a command callback to the front of the queue
LCommandQueue:pushOrder( order : table ) -> integer  -- Adds a data-only RTS order to the back of the queue
LCommandQueue:replace( kind : string, callback : function, opts : table? ) -> integer  -- Replaces the queue contents with one command callback
LCommandQueue:replaceOrders( orders : table ) -> integer  -- Replaces the queue with an array of data-only orders
...
```lua
LDecisionBiasSet:addRule( trait_name : string, decision_key : string, weight : number, mode : string? )  -- Adds one rule that adjusts a named decision score using one trait
LDecisionBiasSet:ruleCount() -> integer  -- Returns the number of stored bias rules
LDecisionBiasSet:score( profile : LTraitProfile, decision_key : string, base_score : number ) -> number  -- Scores one decision using a profile and this bias set
LDecisionBiasSet:type() -> string  -- Returns the Lua-visible type name for this decision bias handle
LDecisionBiasSet:typeOf( name : string ) -> boolean  -- Returns whether this decision bias handle matches a supported type name
```
...
LSquad:assignFormationMove( world : LAIWorld, leader_x : number, leader_y : number, opts : table?|Optional table with `laneWidth`, `positions`, `mode = replace|push_front ) -> table  -- Resolves formation slots and applies queued `move` orders to matching agents in the supplied world
...
LSquad:getFormationSlots( leader_x : number, leader_y : number, opts : table? ) -> table  -- Returns resolved formation slot assignments for every member, optionally using current member positions and lane width
LSquad:getFormationSpacing() -> number  -- Returns the spacing used by squad formation positioning
LSquad:getFormationSummary( leader_x : number, leader_y : number, opts : table? ) -> table  -- Returns formation layout metadata after slot assignment and fallback policy are resolved
...
LSquad:getMemberProfile( name : string ) -> table  -- Returns the stored footprint and subgroup metadata for one member
...
LSquad:setFormationBehavior( sort_mode : string, fallback_mode : string?, preserve_subgroups : boolean? )  -- Sets formation assignment behavior knobs used for slot ordering and chokepoint fallback
LSquad:setLeader( name : string )  -- Sets the squad leader name. This method is available to Lua scripts
LSquad:setMemberProfile( name : string, opts : table )  -- Stores footprint and subgroup metadata used during formation slot assignment
LSquad:submitFormationPaths( world : LAIWorld, grid : LNavGrid, leader_x : number, leader_y : number, opts : table ) -> table  -- Resolves formation slots, converts world positions into navigation cells, and submits one async paired path batch
...
LTraitArchetypes:createProfile( name : string, variance : number? ) -> LuaValue  -- Creates a trait profile from a registered archetype and optional deterministic variance
LTraitArchetypes:names() -> table  -- Returns registered archetype names
LTraitArchetypes:register( name : string, traits : table )  -- Registers or replaces one named archetype from a table of trait values
LTraitArchetypes:type() -> string  -- Returns the Lua-visible type name for this archetype registry handle
LTraitArchetypes:typeOf( name : string ) -> boolean  -- Returns whether this archetype registry handle matches a supported type name
```
...
LTraitProfile:scoreDecision( biases : LDecisionBiasSet, decision_key : string, base_score : number ) -> number  -- Scores one decision by applying a decision bias set to this profile
...
LUtilityAI:evaluateWithProfile( profile : LTraitProfile, biases : LDecisionBiasSet ) -> LuaValue  -- Evaluates all actions after applying trait-profile decision bias rules to each action score
...
lurek.dataframe.fromCSVFile( path : string, opts : table? ) -> LDataFrame  -- Reads CSV text from GameFS and parses it into a dataframe
lurek.dataframe.fromCSVFileAsync( path : string, opts : table? ) -> LDataFrameTask  -- Starts a Rust worker task that reads CSV text from GameFS and parses it into a dataframe
lurek.dataframe.fromJSON( s : string ) -> LDataFrame  -- Parses a dataframe from JSON text. This function is exposed to Lua scripts
lurek.dataframe.fromJSONFile( path : string, opts : table? ) -> LDataFrame  -- Reads JSON text from GameFS and parses it into a dataframe
lurek.dataframe.fromJSONFileAsync( path : string, opts : table? ) -> LDataFrameTask  -- Starts a Rust worker task that reads JSON text from GameFS and parses it into a dataframe
...
lurek.dataframe.loadDatabase( path : string, opts : table? ) -> LDatabase  -- Reads a JSON database file from GameFS and parses it into a database
...
LDataFrame:dateParts( date_col : any, prefix : string? ) -> LDataFrame  -- Returns a new dataframe with year, month, and day columns extracted from ISO `yyyy-mm-dd` text
...
LDataFrame:max( col : any ) -> number|string|boolean|nil  -- Returns the maximum value of a column
...
LDataFrame:min( col : any ) -> number|string|boolean|nil  -- Returns the minimum value of a column
LDataFrame:missingReport( opts : table? ) -> LDataFrame  -- Reports missing and non-missing cell counts for every column
...
LDataFrame:parFilter( col : any, op : string, val : any ) -> LDataFrame  -- Parallel filter â€” automatically parallelizes when frame has 10,000+ rows
LDataFrame:parGroupAgg( group_col : any, agg_col : any, fn_name : string ) -> LDataFrame  -- Parallel group-by aggregation â€” partitions and aggregates in parallel
...
LDataFrame:queryAsync( sql_str : string ) -> LDataFrameTask  -- Runs a SQL-style query against this dataframe on a Rust worker thread
...
LDataFrame:toBinaryFile( path : string, opts : table? ) -> boolean  -- Serializes this dataframe to LVDF binary data and writes it through GameFS
LDataFrame:toCSV() -> string  -- Serializes this dataframe to CSV text
LDataFrame:toCSVFile( path : string, opts : table? ) -> boolean  -- Serializes this dataframe to CSV text and writes it through GameFS
LDataFrame:toJSON() -> string  -- Serializes this dataframe to JSON text
LDataFrame:toJSONFile( path : string, opts : table? ) -> boolean  -- Serializes this dataframe to JSON text and writes it through GameFS
...
LDataFrame:valueCounts( col : any, opts : table? ) -> LDataFrame  -- Counts occurrences of each value in a column with optional percentage output
...
LDataFrameTask:typeOf( name : string ) -> boolean  -- Returns whether this dataframe task handle matches a supported type name
...
LDatabase:queryAsync( sql_str : string ) -> LDataFrameTask  -- Runs a SQL-style query against a snapshot of the database tables on a Rust worker thread
LDatabase:queryParams( sql_str : string, params : table ) -> LDataFrame  -- Runs a SQL-style query against the database tables with positional parameters
LDatabase:queryParamsAsync( sql_str : string, params : table ) -> LDataFrameTask  -- Runs a parameterized SQL query against a snapshot of the database tables on a Rust worker thread
LDatabase:removeTable( name : string )  -- Removes a named table from the database
LDatabase:save( path : string, opts : table? ) -> boolean  -- Serializes the database to the JSON database file format and writes it through GameFS
...
*Coverage: 122/122 items documented (100%)*
...
lurek.image.loadAnimated( source : string ) -> LAnimatedImage  -- Loads an animated GIF from GameFS path or decodes animated GIF bytes
...
lurek.image.newCompressedData( filename : string ) -> LCompressedImageData  -- Attempts to load DDS compressed image data from GameFS
...
lurek.image.newProvinceGrid( filename : string ) -> LProvinceGrid  -- Loads a province id grid from an image file under the current game directory. This is a compatibility facade over the province subsystem
lurek.image.requestShader( image : LImageData, shader : LShader, opts : table? ) -> LImageShaderJob  -- Starts an offline image shader request and returns a completed job handle
...
lurek.image.savePNGWorkspace( img_ud : LImageData, filename : string )  -- Encodes image data as PNG and atomically writes it to a user-authorized workspace mount
```
...
LAnimatedImage:getDuration( index : integer ) -> integer  -- Returns a frame duration in milliseconds by one-based index
LAnimatedImage:getDurations() -> table  -- Returns all frame durations in milliseconds
LAnimatedImage:getFrame( index : integer ) -> LImageData  -- Returns a decoded frame by one-based index
...
LAnimatedImage:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
```
...
LImageData:applyEffect( name : string, opts : table? ) -> LImageData|nil  -- Applies a named image effect in place, or returns a new image when the effect changes size
LImageData:applyEffects( effects : table, opts : table? ) -> LImageData|nil  -- Applies a sequence of named effects in order
LImageData:applyMask( mask : LImageData )  -- Multiplies this image alpha by another image's alpha channel
LImageData:applyPaletteLut( lut_ud : LPaletteLUT )  -- Applies a palette lookup table to this image in place
LImageData:applyShader( shader : LShader, opts : table? ) -> LImageData  -- Applies an offline image shader and returns the processed image
...
LImageData:copyRegion( x : integer, y : integer, w : integer, h : integer ) -> LImageData  -- Copies a rectangular region into a new image
...
LImageData:transform( opts : table ) -> LImageData  -- Returns a transformed image, currently supporting high-quality resize through `width`, `height`, and `filter`
...
```lua
LImageShaderJob:cancel()  -- Cancels this pending offline image shader job
LImageShaderJob:poll() -> LImageData  -- Returns the shader output image when the job has completed, or nil if pending/cancelled
LImageShaderJob:wait( timeoutMs : integer? ) -> LImageData  -- Waits for the offline image shader job and returns its output image
```
...
*Coverage: 148/148 items documented (100%)*
...
lurek.tilemap.fromProvider( provider : table, opts : table? ) -> LTileMap  -- Builds a native tilemap from a Lua provider table with tileWidth, tileHeight, layers, optional tilesets, and optional getTile(layer,x,y)
...
LChunkMap:chunkToBytes( cx : integer, cy : integer ) -> string  -- Serializes one loaded chunk into binary bytes for save workflows
LChunkMap:clearDirtyChunks()  -- Clears chunk dirty tracking without changing tile contents
...
LChunkMap:hashRegion( x0 : integer, y0 : integer, x1 : integer, y1 : integer ) -> string  -- Returns a deterministic hash of one bounded half-open tile rectangle
LChunkMap:loadChunk( cx : integer, cy : integer )  -- Loads a chunk into memory at the given chunk coordinates
LChunkMap:loadChunkFromBytes( cx : integer, cy : integer, data : string )  -- Loads one chunk from bytes previously returned by `chunkToBytes`
LChunkMap:prepareBatch( edits : table, expectedVersion : integer? ) -> LChunkMapBatch  -- Validates and stages tile edits without mutating the live map
LChunkMap:readTiles( coords : table ) -> table  -- Reads many tile coordinates while crossing the Lua boundary only once
LChunkMap:setTile( x : integer, y : integer, gid : integer )  -- Sets the tile GID at the given world-tile coordinate
LChunkMap:setTiles( edits : table ) -> table  -- Applies multiple `{x, y, gid}` tile edits and returns the chunks dirtied by this batch
LChunkMap:snapshotRegion( x0 : integer, y0 : integer, x1 : integer, y1 : integer ) -> table  -- Captures a bounded half-open rectangle as a deterministic row-major tile array
...
LChunkMapBatch:typeOf( name : string ) -> boolean  -- Checks whether this object matches the requested type
```
...
LLargeMapRenderer:setTiles( edits : table ) -> integer  -- Applies a bounded list of tile edits atomically and increments the data version once
...
LLargeMapRenderer:setViewport( w : number, h : number )  -- Sets the viewport rectangle used for render-command culling
...
LTileMap:getLayerShader( layer : integer ) -> LShader  -- Returns the shader override bound to one layer, or nil when the layer has no override
...
LTileMap:renderFieldCatalogSlot( field : LTileField|table, catalog : LTileCatalog, opts : table )  -- Renders typed refs from a tilefield slot through a tileset catalog
LTileMap:renderFieldSlot( field : LTileField|table, tileset : LTileSet|table, opts : table )  -- Renders objects referenced from a tilefield slot using tileset object visuals
...
LTileMap:setLayerShader( layer : integer, shader : LShader? )  -- Binds a tilemap-target shader override to one layer. Pass nil to clear the layer override
...
LTileMap:setShader( shader : LShader? )  -- Binds a tilemap-target shader to this map's generated render commands. Pass nil to clear
...
LTileMap:tryWorldToTile( wx : number, wy : number ) -> integer  -- Converts world-space pixel coordinates to tile-grid coordinates, returning nils for negative or non-finite input
...
lurek.dialog.compileStory( source : string, opts : table? ) -> LDialogStory  -- Compiles a safe Ink-subset story source into an `LDialogStory`
...
lurek.dialog.label( name : string ) -> table  -- Creates a Label node used as a jump target marker
...
LDialogSequencer:clearHistory()  -- Clears accumulated spoken-line history
...
LDialogSequencer:restore( snapshot : table )  -- Restores sequencer runtime state from a prior snapshot table
...
LDialogStory:setVariable( name : string, value : any )  -- Sets or replaces one story variable using a nil, boolean, number, or string value
...
LDialogueState:setVariable( key : string, value : string )  -- Sets a conversation variable for this object
LDialogueState:start( node_id : string )  -- Starts a conversation at the given node
LDialogueState:type() -> string  -- Returns the Lua-visible type name
LDialogueState:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
...
lurek.ecs.clearClasses()  -- Removes every global ECS class definition
lurek.ecs.clearObjects()  -- Removes every live ECS object while keeping class definitions
lurek.ecs.defineClass( name : string, def : table )  -- Defines or replaces a global ECS class for Lua object instances
lurek.ecs.destroyObject( id : integer ) -> boolean  -- Removes a live ECS object from the global object registry
lurek.ecs.getClass( name : string ) -> table  -- Returns metadata for a global ECS class
lurek.ecs.getObject( id : integer ) -> table  -- Returns a live ECS object table by object id
lurek.ecs.getProperty( self : LObject, name : string ) -> LuaValue  -- Returns the current value of one object property, honoring any registered getter override first
lurek.ecs.hasClass( name : string ) -> boolean  -- Returns whether a global ECS class name is defined
lurek.ecs.hasObject( id : integer ) -> boolean  -- Returns whether a live ECS object id exists
lurek.ecs.isA( self : LObject, candidate : string ) -> boolean  -- Returns whether this object inherits from or matches the supplied ECS class name
lurek.ecs.newLoadout( opts : table? ) -> LLoadout  -- Creates a modular loadout from optional slot definitions and base stats
lurek.ecs.newObject( className : string, props : table? ) -> table  -- Creates a Lua table object from a registered ECS class
lurek.ecs.newPartDef( opts : table ) -> LPartDef  -- Creates a modular loadout part definition
...
lurek.ecs.newSlotDef( name : string, opts : table? ) -> LSlotDef  -- Creates a modular loadout slot definition
lurek.ecs.newStatBlock( stats : table? ) -> LStatBlock  -- Creates a standalone stat block from a plain table
...
lurek.ecs.setProperty( self : LObject, name : string, value : any )  -- Writes one object property, delegating to a registered setter override when the property defines one
lurek.ecs.type( self : LObject ) -> string  -- Returns the registered ECS class name for this object table
lurek.ecs.typeOf( self : LObject, candidate : string ) -> boolean  -- Returns whether this object matches a supported Lua-visible type or ECS class name
```
...
LEcsBatch:typeOf( name : string ) -> boolean  -- Checks whether this userdata matches a requested type
```
...
```lua
LLoadout:addSlot( slot : LSlotDef )  -- Adds or replaces one slot definition on this loadout
LLoadout:computeStats() -> LStatBlock  -- Computes final additive stats from base stats and equipped parts
LLoadout:equip( slot : string, part : LPartDef ) -> boolean  -- Equips a part into a named slot after compatibility checks
...
LLoadout:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
LLoadout:unequip( slot : string ) -> boolean  -- Removes the part currently equipped in one slot
...
LPartDef:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
```
...
LRelationshipManager:getLevel( a : integer, b : integer, type_name : string ) -> string  -- Returns the effective named level for one relationship type on a pair, falling back to the type default when no explicit level exists
...
LRelationshipManager:setLevel( a : integer, b : integer, type_name : string, level : string ) -> boolean  -- Assigns a named level for one relationship type between two entity ids and reports whether the type-level pair was accepted
...
LSlotDef:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
```
...
```lua
LStatBlock:add( name : string, value : number )  -- Adds a numeric delta to one stat in this additive stat block
LStatBlock:get( name : string ) -> number  -- Returns one stat value, or zero when the key is absent
LStatBlock:set( name : string, value : number )  -- Replaces one stat value in this additive stat block
...
LStatBlock:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
```
...
LUniverse:applyChangeSet( changeset : table ) -> integer  -- Applies a transport-neutral ChangeSet table to explicit ECS component operations
LUniverse:applySnapshot( snapshot : table )  -- Replaces this universe state from a Lua table snapshot
LUniverse:attachObject( entityId : integer, obj : table )  -- Attaches an existing ECS object table to an entity as the `object` component
...
LUniverse:prepareChangeSet( changeset : table, expectedVersion : integer? ) -> LEcsBatch  -- Validates and stores an ECS ChangeSet without mutating this universe
...
LUniverse:readComponents( requests : table ) -> table  -- Reads many entity/component selections with one Lua-to-Rust boundary crossing
...
LUniverse:spawnObject( className : string, props : table? ) -> integer  -- Creates an ECS object instance from a registered class and attaches it to a new entity
...
lurek.scene.isLateEnabled( target : any? ) -> boolean  -- Returns whether `process_late` is enabled for a selected scene
...
lurek.scene.isPhysicsEnabled( target : any? ) -> boolean  -- Returns whether `process_physics` is enabled for a selected scene
...
lurek.scene.isProcessEnabled( target : any? ) -> boolean  -- Returns whether `process` is enabled for a selected scene
lurek.scene.isSceneActive( target : any? ) -> boolean  -- Returns whether the selected scene is globally active
...
lurek.scene.process( dt : number )  -- Call `ready(self)` once on newly-pushed scenes, then call `process(self, dt)` on every process-active scene ordered by layer (lowest first)
lurek.scene.processLate( dt : number )  -- Call `process_late(self, dt)` on every process-active scene after all other processing
lurek.scene.processPhysics( dt : number )  -- Call `process_physics(self, dt)` on every process-active scene ordered by layer
...
lurek.scene.pushOverlay( scene : table, transition : string?, duration : number?, easing : string?, params : table? )  -- Push a scene as an overlay on top of the current scene. Unlike `push`, the underlying scene is NOT paused â€” it can continue to receive `process` callbacks unless frozen. Rendering remains single-scene (top scene only) at engine level
...
lurek.scene.pushRegistered( name : string, transition : string?, duration : number?, easing : string?, params : table? ) -> boolean  -- Push a registered scene by name, honoring its persistence policy
...
lurek.scene.registerScene( name : string, sceneOrFactory : any, opts : table? )  -- Register a scene table or scene factory under a unique name for later retrieval, pushRegistered navigation, or deferred push via `pushPreloaded`. Registering does not push the scene onto the stack
...
lurek.scene.restoreScene( snapshot : table, opts : table? ) -> integer  -- Fully restore scene shared data and rebuild the scene stack from registered scene names captured by `serializeScene`
...
lurek.scene.setData( key : string, value : any )  -- Store an arbitrary Lua value in the scene module's shared data map, keyed by a string name. Scenes can use this to pass information between each other without direct references â€” for example, passing a selected level index from a menu scene to a gameplay scene
lurek.scene.setLateEnabled( target : any?, enabled : boolean ) -> boolean  -- Enable or disable `process_late(self, dt)` execution for a selected scene
lurek.scene.setPhysicsEnabled( target : any?, enabled : boolean ) -> boolean  -- Enable or disable `process_physics(self, dt)` execution for a selected scene
lurek.scene.setProcessEnabled( target : any?, enabled : boolean ) -> boolean  -- Enable or disable `process(self, dt)` execution for a selected scene
lurek.scene.setSceneActive( target : any?, enabled : boolean ) -> boolean  -- Enable or disable all update, process, physics, late, and render callbacks for a selected scene
lurek.scene.setUpdateEnabled( target : any?, enabled : boolean ) -> boolean  -- Enable or disable `update(self, dt)` execution for a selected scene
...
lurek.scene.switchTo( scene : table, transition : string?, duration : number?, easing : string?, params : table? )  -- Replace the current top scene with a different one without changing stack depth. The old scene receives `leave()` and the new scene receives `enter(self, params)`. Unlike `push`, no scene is added to the stack â€” the old scene is removed and the new one takes its slot. Ideal for transitioning between peer-level game states (e.g. level 1 â†’ level 2)
lurek.scene.unregisterScene( name : string )  -- Remove a scene registration by name. Does not pop the scene if it is currently active on the stack â€” it only removes the name mapping
lurek.scene.update( dt : number )  -- Advance any active transition animation and call `update(self, dt)` on the current top scene
...
LSceneObjectContainer:defineGroup( name : string ) -> integer  -- Define an object group and return its 0-based bit index
...
LSceneObjectContainer:getGroupBit( name : string ) -> integer  -- Return the bit index assigned to a group name
...
LSceneObjectContainer:isGroupEnabled( group : any, pass : string ) -> boolean  -- Return whether one object group is enabled for one pass
LSceneObjectContainer:processPhysics( dt : number )  -- Call process_physics(dt) or physics(dt) on all physics-pass-enabled objects
LSceneObjectContainer:remove( obj : table )  -- Remove an object from the container (identity comparison)
LSceneObjectContainer:setGroupEnabled( group : any, pass : string, enabled : boolean ) -> boolean  -- Enable or disable one object group for one pass
...
*Coverage: 245/245 items documented (100%)*
...
lurek.pathfind.graphConnected( edges : table, from : integer, to : integer, opts : table? ) -> boolean  -- Returns true when a target node is reachable from a start node in an integer-id graph
lurek.pathfind.graphConnectedComponents( edges : table, nodes : table?, opts : table? ) -> table  -- Returns connected components for an integer-id graph. Pass `nodes` to include isolated node ids
lurek.pathfind.graphRoute( edges : table, from : integer, to : integer, opts : table? ) -> integer[]  -- Finds a route through an integer-id graph. Edges may be `{from,to}`, `{a,b}`, `{province_a,province_b}`, or `{from_id,to_id}` arrays. Options: `directed`, `algorithm` ("bfs"|"dijkstra"), and optional `cost(from, to)`
lurek.pathfind.graphRoutes( edges : table, requests : table, opts : table? ) -> table  -- Finds routes for a batch of graph `{from, to}` requests using the same edge table and options as `graphRoute`
lurek.pathfind.newContextSteering( slots : integer ) -> LContextSteering  -- Creates a context steering model with the requested directional slot count
...
lurek.pathfind.newHexGridFromField( field_ud : LTileField, opts : table? ) -> LHexGrid  -- Creates a hex navigation grid from a hex tilefield level and movement category
lurek.pathfind.newInfluenceMap( w : integer, h : integer, cs : number ) -> LInfluenceMap  -- Creates a grid influence map with the supplied cell dimensions and world cell size
lurek.pathfind.newIsoGrid( width : integer, height : integer ) -> LIsoGrid  -- Creates an isometric grid with the given dimensions
lurek.pathfind.newIsoGridFromField( field_ud : LTileField, opts : table? ) -> LIsoGrid  -- Creates an isometric navigation grid from an iso-square tilefield level and movement category
...
lurek.pathfind.newNavGridFromField( field_ud : LTileField, opts : table? ) -> LNavGrid  -- Creates a navigation grid from a tilefield level and movement category
lurek.pathfind.newNavGridFromProvider( provider : table ) -> LNavGrid  -- Builds a navigation grid from a Lua provider table with width, height, optional costs/blocked arrays, or getCost/isBlocked callbacks
...
lurek.pathfind.newORCASolver( time_horizon : number ) -> LORCASolver  -- Creates an ORCA avoidance solver with the supplied prediction horizon
...
lurek.pathfind.newPathGridFromProvider( provider : table ) -> LPathGrid  -- Builds a path grid from a Lua provider table with width, height, optional cellSize, costs/walkable arrays, or getCost/isWalkable callbacks
...
lurek.pathfind.rangeMapFromField( field_ud : LTileField, opts : table ) -> table  -- Computes reachable cells from a tilefield level and movement category
...
lurek.pathfind.submitAsyncPathPairs( grid_ud : LNavGrid, opts : table ) -> integer  -- Queues one async paired batch query against a navigation grid snapshot
lurek.pathfind.submitAsyncPathsToGoal( grid_ud : LNavGrid, opts : table ) -> integer  -- Queues one async shared-goal batch query against a navigation grid snapshot
```
...
LFlowField:calculateFor( name : string, tx : integer, ty : integer )  -- Calculates a flow field toward one target cell using a named navigation footprint
LFlowField:calculateMulti( targets : table, unit_size : integer? )  -- Calculates a flow field toward multiple target cells
LFlowField:calculateMultiFor( name : string, targets : table )  -- Calculates a flow field toward multiple target cells using a named navigation footprint
...
LFlowField:pathFrom( x : integer, y : integer, max_steps : integer? ) -> table  -- Reconstructs a downhill route from one start cell to the nearest active target in the current flow field
LFlowField:pathsFrom( starts : table, max_steps : integer? ) -> table  -- Reconstructs downhill routes from many start cells to the nearest active target using one shared flow field
...
```lua
LIsoGrid:findPath( fx : integer, fy : integer, tx : integer, ty : integer ) -> table  -- Finds a path between one-based isometric cells
LIsoGrid:getCost( x : integer, y : integer ) -> number  -- Returns movement cost for a one-based isometric grid cell
LIsoGrid:isBlocked( x : integer, y : integer ) -> boolean  -- Returns whether a one-based isometric grid cell is blocked
LIsoGrid:setBlocked( x : integer, y : integer, blocked : boolean )  -- Sets blocked state for a one-based isometric grid cell
LIsoGrid:setCost( x : integer, y : integer, cost : number )  -- Sets movement cost for a one-based isometric grid cell
LIsoGrid:type() -> string  -- Returns the Lua-visible type name for this isometric grid handle
LIsoGrid:typeOf( name : string ) -> boolean  -- Returns whether this isometric grid handle matches a supported type name
```
...
```lua
LNavGrid:beginUpdate()  -- Starts a batched navigation edit that is finalized by `commitUpdate`
LNavGrid:clearDirty()  -- Clears all dirty region markers from the grid
LNavGrid:commitUpdate( opts : table?|Optional table with `rebuild = "none"|"dirty_chunks" ) -> integer  -- Finalizes a batched navigation edit and refreshes clearance caches according to `opts.rebuild`
LNavGrid:defineFootprint( name : string, footprint : table )  -- Stores or replaces a named rectangular footprint for clearance caching
...
LNavGrid:findHpaPathsToGoal( starts : table, gx : integer, gy : integer, unit_size : integer? ) -> table  -- Finds hierarchical paths from many one-based start cells to one goal while sharing one abstract-goal search setup
...
LNavGrid:getFootprint( name : string ) -> table  -- Returns the stored width and height for a named footprint when it exists
...
LNavGrid:isWalkableFor( name : string, x : integer, y : integer ) -> boolean  -- Returns whether a one-based grid cell is walkable for a named footprint
LNavGrid:loadFromString( data : string )  -- Loads grid data from a serialized binary string
LNavGrid:patchCells( patches : table, opts : table?|Optional `{rebuild="none"|"dirty_chunks" ) -> table  -- Atomically patches navigation-owned cost or blocked state
LNavGrid:rebuildAbstract()  -- Rebuilds the cached abstract graph for this grid
LNavGrid:rebuildClearance( opts : table? ) -> integer  -- Rebuilds clearance caches for all defined footprints or the supplied named subset
...
LNavGrid:setBlockedRect( x : integer, y : integer, w : integer, h : integer, blocked : boolean, opts : table? )  -- Applies one blocked or passable rectangle in batch-edit style
...
LNavGrid:setCostRect( x : integer, y : integer, w : integer, h : integer, cost : integer, opts : table? )  -- Applies one cost rectangle in batch-edit style
...
LORCASolver:compute( dt_or_opts : number|table )  -- Computes safe velocities for all ORCA agents, optionally under a time budget
LORCASolver:getSafeVelocity( idx : integer ) -> number, number  -- Returns the computed safe velocity for an ORCA agent addressed by zero-based index or stable key
LORCASolver:getStats() -> table  -- Returns statistics from the most recent ORCA compute step
LORCASolver:removeAgent( idx : integer ) -> boolean  -- Removes an ORCA agent addressed by zero-based index or stable key
LORCASolver:setAgent( key : integer, opts : table )  -- Inserts or updates an ORCA avoidance agent under a stable caller-provided key
LORCASolver:setCellSize( size : number )  -- Sets the spatial-hash cell size used when grouping ORCA agents
LORCASolver:setMaxNeighbors( count : integer )  -- Sets the maximum retained neighbor count used during one ORCA solve step
LORCASolver:setNeighborRadius( radius : number )  -- Sets an explicit neighbor-query radius in world units; zero restores dynamic per-agent radius
LORCASolver:setPosition( idx : integer, x : number, y : number )  -- Sets the position for an ORCA agent addressed by zero-based index or stable key
LORCASolver:setPreferredVelocity( idx : integer, pvx : number, pvy : number )  -- Sets the preferred velocity for an ORCA agent addressed by zero-based index or stable key
LORCASolver:setVelocity( idx : integer, vx : number, vy : number )  -- Sets the current velocity for an ORCA agent addressed by zero-based index or stable key
...
LUnitPathfinder:clearSharedGoalCache()  -- Clears all cached shared-goal fields on this object
LUnitPathfinder:findAttackMovePaths( starts : table, gx : integer, gy : integer, unit_size : integer?, max_steps : integer? ) -> table  -- Finds one path per start toward a shared attack-move goal
LUnitPathfinder:findFormationPaths( starts : table, gx : integer, gy : integer, unit_size : integer?, spacing : integer? ) -> table  -- Finds one path per start toward formation slots around a shared goal
...
LUnitPathfinder:findPathsToGoal( starts : table, gx : integer, gy : integer, unit_size : integer?, max_steps : integer? ) -> table  -- Finds routes from many one-based start cells to one goal cell using one shared-goal field
LUnitPathfinder:findPathsToGoalFor( name : string, starts : table, gx : integer, gy : integer, max_steps : integer? ) -> table  -- Finds routes from many one-based start cells to one goal cell using one named-footprint shared-goal field
...
LUnitPathfinder:getSharedFlowField( gx : integer, gy : integer, unit_size : integer? ) -> LFlowField  -- Returns a cached shared-goal flow field handle for one target cell and unit footprint size
LUnitPathfinder:getSharedFlowFieldFor( name : string, gx : integer, gy : integer ) -> LFlowField  -- Returns a cached shared-goal flow field handle for one target cell and one named footprint
LUnitPathfinder:getSharedFlowFieldMulti( targets : table, unit_size : integer? ) -> LFlowField  -- Returns a cached shared-goal flow field handle for many target cells and one unit footprint size
LUnitPathfinder:getSharedFlowFieldMultiFor( name : string, targets : table ) -> LFlowField  -- Returns a cached shared-goal flow field handle for many target cells and one named footprint
...
LUnitPathfinder:reserveCells( cells : table ) -> integer  -- Records caller-owned cell reservations for batch planning
LUnitPathfinder:setCacheEnabled( enabled : boolean )  -- Enables or disables the pathfinder's internal route and shared-goal caches on this object
...
*Coverage: 108/108 items documented (100%)*
...
LMinimap:getLayerAlpha( layer : integer ) -> number  -- Returns the opacity multiplier for a minimap data layer
LMinimap:getLayerBlendMode( layer : integer ) -> string  -- Returns how a minimap data layer is blended over the base terrain
LMinimap:getLayerColor( layer : integer, value : integer ) -> number  -- Returns the palette color for one raw value in a minimap data layer
...
LMinimap:isLayerVisible( layer : integer ) -> boolean  -- Returns whether a minimap data layer is drawn when it is not active
...
LMinimap:setCenterFromTileMapWorld( tilemap : LTileMap, wx : number, wy : number ) -> number, number  -- Converts tilemap world coordinates into one-based tile coordinates and centers this minimap
...
LMinimap:setLayerAlpha( layer : integer, alpha : number )  -- Sets the opacity multiplier for a minimap data layer
LMinimap:setLayerBlendMode( layer : integer, mode : string )  -- Sets how a minimap data layer is blended over the base terrain
LMinimap:setLayerColor( layer : integer, value : integer, r : number, g : number, b : number, a : number? )  -- Sets a palette color for one raw value in a minimap data layer
LMinimap:setLayerData( layer : integer, data_tbl : table )  -- Sets raw cell data for a minimap layer
LMinimap:setLayerStyle( layer : integer, style : table )  -- Applies common raw-layer style fields: visible, alpha, blend, and colors
LMinimap:setLayerVisible( layer : integer, visible : boolean )  -- Sets whether a minimap data layer is drawn even when it is not the active layer
...
LMinimap:setObjects( updates : table ) -> boolean  -- Applies object icon updates atomically for one Lua-owned minimap refresh
LMinimap:setOwnerColor( owner : integer, r : number, g : number, b : number, a : number? )  -- Sets the RGBA display color for an owner id
LMinimap:setShader( shader : LShader? )  -- Binds or clears a `mapviz` shader for command-rendered minimap visualization
...
LMinimap:setViewportFromTileMapWorld( tilemap : LTileMap, x : number, y : number, w : number, h : number ) -> number, number, number, number  -- Converts a tilemap world rectangle into a minimap viewport rectangle
...
LMinimap:syncProvinceRegistry( registry : LProvinceRegistry, opts : table? )  -- Copies province registry terrain, visibility, and palette data into this minimap
LMinimap:syncTileAwarenessFog( awareness : LTileAwareness, player : string, opts : table? ) -> table  -- Copies explored/visible masks from `LTileAwareness` into minimap fog data
LMinimap:syncTileAwarenessLayer()  -- Copies visible or action masks from `LTileAwareness` into a minimap raw layer
LMinimap:syncTileFieldBlockLayer()  -- Copies one `LTileField` blocker channel layer into a minimap raw data layer
LMinimap:syncTileFieldCostLayer()  -- Copies one `LTileField` cost channel layer into a minimap raw byte layer
LMinimap:syncTileLightLayer( light : LTileLightMap, layer : integer, opts : table? ) -> table  -- Copies computed tilelight luma into a minimap raw byte layer
LMinimap:syncTileMapTerrain( tilemap : LTileMap, opts : table? )  -- Copies tile GIDs from an `LTileMap` layer into minimap terrain cells
...
LSaveManager:registerSchema( name : string, version : integer, migrator : function? )  -- Register a named schema section and optionally attach a migration into the requested version
...
LSaveManager:setFormat( format : string )  -- Set the payload serialization format for future saves and loads
...
LSaveManager:setSummary( summary : string )  -- Set a human-readable summary string stored alongside save metadata (e.g. "Level 5 â€“ Forest")
...
```lua
LContentRegistry:defineType( type_name : string, schema : table )  -- Registers a content type and its field-validation schema
...
LContentRegistry:getSchema( type_name : string ) -> table  -- Returns a registered type schema, or nil for an untyped content type
...
LContentRegistry:restore( snapshot : table )  -- Restores schemas and values from a previous snapshot and applies its frozen flag
...
LContentRegistry:unregisterType( type_name : string ) -> boolean  -- Removes a content type and all values registered under it
LContentRegistry:validate( type_name : string, value : table ) -> boolean  -- Validates a value against a registered type schema without storing it
```
...
lurek.patterns.newDeck( cards : table? ) -> LDeck  -- Create a reusable deck/card collection with shuffle, draw, discard, and reset operations
...
LBehaviorTree:addInverter( label : string? ) -> number  -- Create a decorator node that inverts its child's result (success â†” failure)
...
```lua
LDeck:add( card : any ) -> integer  -- Add a card payload to the bottom of the deck's draw pile
LDeck:count() -> integer  -- Return the number of cards left in the draw pile
LDeck:discard( card : any ) -> boolean  -- Move a card into the discard pile by card table or stable id
LDeck:discardCount() -> integer  -- Return the number of cards in the discard pile
LDeck:draw( count : integer? ) -> LuaValue  -- Draw one or more cards from the top of the draw pile
LDeck:isEmpty() -> boolean  -- Return true when no cards remain in the draw pile
LDeck:peek( count : integer? ) -> LuaValue  -- Inspect one or more cards from the top without removing them
LDeck:reset()  -- Restore the draw pile to original insertion order and clear discard
LDeck:shuffle( seed : integer? )  -- Shuffle the current draw pile with a deterministic optional seed
...
```lua
lurek.agent.cancel( callback_id : integer ) -> nil  -- Cancels a module-level asynchronous completion by callback ID
...
lurek.agent.configure( config : table ) -> nil  -- Configures the global LLM provider settings used by module-level functions
...
```lua
LAISystem:addAgent( name : string, agent : LAgent ) -> nil  -- Registers a named agent in the system
LAISystem:addInstruction( key : string, text : string ) -> nil  -- Adds a named instruction block the user can explicitly include per prompt
LAISystem:addSkill( name : string, keywords : table, prompt : string ) -> nil  -- Adds a keyword-gated system skill that Lurek auto-injects when the prompt overlaps with its keywords
...
```lua
LAgent:addSkill( name : string, prompt : string ) -> nil  -- Appends a named skill prompt to the agent's context block
LAgent:cancel( callback_id : integer ) -> nil  -- Cancels an in-flight or pending request by callback ID
...
LAgent:setContextSize( n : integer ) -> nil  -- Sets the token context window size forwarded to the LLM backend
LAgent:setDescription( description : string ) -> nil  -- Sets the agent's role description injected after the system prompt when routed through an AISystem
LAgent:setFormat( format : string ) -> nil  -- Changes the response format for future prompts
LAgent:setMaxRetries( n : integer ) -> nil  -- Sets the maximum retry count on transient network or timeout errors
LAgent:setModel( model : string ) -> nil  -- Changes the model identifier for future prompts
LAgent:setName( name : string ) -> nil  -- Sets the agent's name identifier used when added to an AISystem
LAgent:setOption( key : string, value : any ) -> nil  -- Sets a single model option forwarded to the LLM backend
LAgent:setTemperature( t : number ) -> nil  -- Sets the sampling temperature forwarded to the LLM backend
LAgent:setTimeout( secs : integer ) -> nil  -- Sets the per-request timeout in seconds (0 uses the default 60 s)
LAgent:setUrl( url : string ) -> nil  -- Changes the LLM endpoint URL for future prompts after safe-mode validation
...
```lua
LAgentChat:addMessage( role : string, content : string ) -> nil  -- Appends a message to the chat history without sending a completion
...
LAgentChat:setSystemPrompt( prompt : string ) -> nil  -- Sets the system prompt used for all completions in this session
```
...
```lua
LEpisodicMemory:forgetBefore( cutoff : integer ) -> nil  -- Removes all episodes with tick < `cutoff`
...
LEpisodicMemory:record( tick : integer, data : table ) -> nil  -- Records a new episode at `tick` with `data`
```
...
LSemanticMemory:learn( key : string, value : any ) -> nil  -- Inserts or replaces a fact at `key`
...
LWorkingMemory:push( key : string, value : any ) -> nil  -- Inserts or updates a key-value entry; evicts the oldest entry if capacity is exceeded
```
...
lurek.animation.fromAnimatedImage( animated : LAnimatedImage, opts : table? ) -> LAnimation  -- Creates an animation from decoded frames returned by `lurek.image.loadAnimated`
lurek.animation.fromAseprite( json_str : string ) -> LuaValue  -- Loads an animation from an Aseprite JSON export string
lurek.animation.fromFrames( frames : table, opts : table? ) -> LAnimation  -- Creates an animation from explicit frame rectangle DTOs
lurek.animation.fromSpriteSheet( sheet : LSpriteSheet, opts : table? ) -> LAnimation  -- Creates an animation from a `LSpriteSheet`, optionally using a named group
...
LAnimStateMachine:draw( image : LImage?, x : number?, y : number?, opts : table? ) -> boolean  -- Draws the current state-machine animation frame without advancing playback
...
LAnimStateMachine:setImage( image : LImage )  -- Stores a spritesheet image on this state machine so draw can be called without an explicit image argument
...
LAnimation:draw( image : LImage?, x : number?, y : number?, opts : table? ) -> boolean  -- Draws the current animation frame without advancing playback
...
LAnimation:seek( index : integer )  -- Seeks to a frame index in the current clip
...
LAnimation:setImage( image : LImage )  -- Stores a spritesheet image on this animation so draw can be called without an explicit image argument
...
```lua
lurek.asset.addTag( handle : LAssetHandle, tag : string ) -> nil  -- Adds a tag to the tag set of an asset handle
...
lurek.asset.getRevision( handle : LAssetHandle ) -> integer  -- Returns the current reload revision for an asset handle
...
lurek.asset.loadManifest( path : string ) -> table  -- Loads a TOML asset manifest and registers listed assets without transforming them
lurek.asset.onReload( handle : LAssetHandle, callback : function ) -> nil  -- Registers a callback fired by `lurek.asset.reload(handle)`
lurek.asset.preload( paths : table, callback : any ) -> nil  -- Synchronously loads a batch of assets and fires `callback(loaded, total)` after each item
...
lurek.asset.reload( handle : LAssetHandle ) -> integer  -- Reloads the cached asset metadata/content and increments its revision
lurek.asset.removeTag( handle : LAssetHandle, tag : string ) -> boolean  -- Removes a tag from the tag set of an asset handle
lurek.asset.resolve( handle : LAssetHandle ) -> table  -- Returns a metadata snapshot for an asset handle without transforming the asset data
lurek.asset.setGroup( handle : LAssetHandle, group : string ) -> nil  -- Assigns an asset handle to a named group
lurek.asset.setName( handle : LAssetHandle, name : string ) -> nil  -- Sets the display name for an asset handle
lurek.asset.stats() -> table  -- Returns a snapshot table describing the current cache state
lurek.asset.unload( handle : LAssetHandle ) -> nil  -- Decrements the ref count for a cached asset; removes the entry when it reaches zero
lurek.asset.watch( handle_or_path : LAssetHandle|string, asset_type : string? ) -> LAssetHandle  -- Marks an asset handle or path as watched for live reload
```
...
```lua
lurek.awareness.lineOfAction( field : LTileField, from : table, to : table, opts : table? ) -> boolean  -- Returns whether two tilefield cells have a clear action line
lurek.awareness.lineOfSight( field : LTileField, from : table, to : table, opts : table? ) -> boolean  -- Returns whether two tilefield cells have a clear sight line
lurek.awareness.new( config : table ) -> LAwarenessGrid  -- Create a new visibility grid for shadow-cast computation
lurek.awareness.newFov( opts : table ) -> LFov  -- Creates a new tile-grid shadowcasting FOV for roguelike and stealth games
lurek.awareness.newTileAwareness( field : LTileField, opts : table ) -> LTileAwareness  -- Creates per-player tile visibility/action masks backed by a tilefield
```
...
LAwarenessGrid:getCost( region_id : integer ) -> number  -- Gets the discovery cost for a region
LAwarenessGrid:getFogIntensity( player_id : integer, region_id : integer ) -> number  -- Gets the fog intensity for a region from a player's perspective
LAwarenessGrid:getState( player_id : integer, region_id : integer ) -> string  -- Gets the visibility state for a player at a region
LAwarenessGrid:hasFlag( region_id : integer, bit : integer ) -> boolean  -- Checks if a visibility flag bit is set on a region
LAwarenessGrid:hide( player_id : integer, region_id : integer )  -- Hides a region for a player (moves from Visible to Discovered)
...
LAwarenessGrid:reset( player_id : integer )  -- Resets all visibility to Hidden for a player
LAwarenessGrid:reveal( player_id : integer, region_id : integer, flags : integer? )  -- Reveals a region for a player (and their allies). Optional flags argument
LAwarenessGrid:revealAll( player_id : integer )  -- Reveals all regions for a player (debug/cheat)
LAwarenessGrid:setCost( region_id : integer, cost : number )  -- Sets the discovery cost for a region
LAwarenessGrid:setFlag( region_id : integer, bit : integer, value : boolean )  -- Sets a visibility flag bit on a region
LAwarenessGrid:setGroup( players : table ) -> integer  -- Sets an alliance group for a list of players (shared visibility)
LAwarenessGrid:sharesVisibility( player_a : integer, player_b : integer ) -> boolean  -- Checks if two players share visibility (same alliance group or same player)
```
...
```lua
LTileAwareness:actionCells( player : string, z : integer? ) -> table  -- Returns all currently actionable cells for a player, optionally filtered to a level
LTileAwareness:canActOn( player : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell is currently actionable for a player
LTileAwareness:clearAll()  -- Clears current, explored, and action masks for all players
LTileAwareness:clearPlayer( player : string )  -- Clears current, explored, and action masks for one player
LTileAwareness:clearShares()  -- Clears all directed awareness share edges
LTileAwareness:computeAction( player : string, opts : table )  -- Computes one player's current action mask from a tilefield origin
LTileAwareness:computeVisible( player : string, opts : table )  -- Computes one player's current visible mask from a tilefield origin
LTileAwareness:defineCategory( name : string, opts : table? )  -- Defines or replaces one awareness category
LTileAwareness:getCategories() -> table  -- Returns known awareness category names
LTileAwareness:getCategory( name : string ) -> table  -- Returns awareness category metadata
LTileAwareness:isAware( player : string, category : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell is visible for a specific awareness category
LTileAwareness:isExplored( player : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell has been explored for a player
LTileAwareness:isVisible( player : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell is currently visible for a player
LTileAwareness:setTeam( players : table, categories : table? )  -- Creates directed share edges between all listed players for selected categories
LTileAwareness:share( from : string, to : string, category : string, opts : table? )  -- Adds a directed awareness share edge for one category
LTileAwareness:type() -> string  -- Returns the Lua-visible type name for this tile visibility handle
LTileAwareness:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
LTileAwareness:updateSightSources( player : string, sources : table ) -> integer  -- Computes one player's current visible mask from multiple tilefield sight sources
LTileAwareness:visibleCells( player : string, categoryOrZ : string|integer?, z : integer? ) -> table  -- Returns all currently visible cells for a player, optionally filtered to a level
```
...
LCamera:presetHorizontalFollow( opts : table )  -- Applies a horizontal side-scroller follow preset with optional overrides
...
*Coverage: 229/229 items documented (100%)*
...
lurek.charts.newBoxPlot( config : table? ) -> LBoxPlotChart  -- Creates a new boxplot chart userdata instance
lurek.charts.newBubble( config : table? ) -> LBubbleChart  -- Creates a new bubble chart userdata instance
lurek.charts.newCandlestick( config : table? ) -> LCandlestickChart  -- Creates a new candlestick chart userdata instance
...
lurek.charts.newRadar( config : table? ) -> LRadarChart  -- Creates a new radar chart userdata instance
lurek.charts.newScatter( config : table? ) -> LScatterPlot  -- Creates a new scatter plot userdata instance
lurek.charts.newTreemap( config : table? ) -> LTreemapChart  -- Creates a new treemap chart userdata instance
...
```lua
LAreaChart:addLayer( name : string, values : table, color : table? ) -> nil  -- Adds one filled area layer from a numeric value list
LAreaChart:addLayerFromDataFrame( name : string, df : userdata, value_col : string, color : table?, opts : table? ) -> nil  -- Builds one filled area layer from a dataframe value column
LAreaChart:addSeries( name : string, data : table, color : table? ) -> nil  -- Adds a named area series from an array-style Lua table of points
LAreaChart:appendPoint( name : string, x : number, y : number, color : table? ) -> nil  -- Appends one finite point to a named area series
LAreaChart:clear()  -- Clears all series and cached chart state
LAreaChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the area chart at world or screen coordinates using optional transform options
...
```lua
LBarChart:addCategoriesFromDataFrame( df : userdata, label_col : string, value_cols : table, opts : table? ) -> nil  -- Adds grouped bar categories by reading one label column and one or more value columns from a dataframe
LBarChart:addCategory( label : string, values : table ) -> nil  -- Adds one category label with a numeric value list for grouped bars
LBarChart:addSeries( name : string, data : table, color : table? ) -> nil  -- Adds a named bar series from an array-style Lua table of values or points
LBarChart:clear()  -- Clears all series and cached chart state
LBarChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the bar chart at world or screen coordinates using optional transform options
...
```lua
LBoxPlotChart:addSeries( name : string, values : table, color : table? )  -- Adds or replaces a named distribution sample series
LBoxPlotChart:appendValue( name : string, value : number, color : table? )  -- Appends one numeric sample to a named distribution
LBoxPlotChart:clear()  -- Clears all chart data and cached chart state
LBoxPlotChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the chart at world or screen coordinates using optional transform options
LBoxPlotChart:drawToImage( target : userdata )  -- Draws the rendered chart into an existing image
...
LBoxPlotChart:renderImage()  -- Renders the chart into a new LImage userdata
LBoxPlotChart:setShowLegend( value : boolean )  -- Controls whether the chart legend is rendered
LBoxPlotChart:setTitle( title : string )  -- Sets the chart title text shown in rendered output
LBoxPlotChart:type() -> string  -- Returns the runtime userdata type name for this chart
LBoxPlotChart:typeOf( name : string ) -> boolean  -- Checks whether a type name matches this chart userdata
```
...
```lua
LBubbleChart:addSeries( name : string, data : table, color : table? )  -- Adds or replaces a weighted point series from `{x, y, size}` rows
LBubbleChart:appendPoint( name : string, x : number, y : number, size : number, color : table? )  -- Appends one weighted point to a named bubble series
LBubbleChart:clear()  -- Clears all chart data and cached chart state
LBubbleChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the chart at world or screen coordinates using optional transform options
LBubbleChart:drawToImage( target : userdata )  -- Draws the rendered chart into an existing image
...
LBubbleChart:renderImage()  -- Renders the chart into a new LImage userdata
LBubbleChart:setRadiusRange( min : number, max : number )  -- Sets the minimum and maximum bubble radius in pixels
LBubbleChart:setShowLegend( value : boolean )  -- Controls whether the chart legend is rendered
LBubbleChart:setTitle( title : string )  -- Sets the chart title text shown in rendered output
LBubbleChart:type() -> string  -- Returns the runtime userdata type name for this chart
LBubbleChart:typeOf( name : string ) -> boolean  -- Checks whether a type name matches this chart userdata
```
...
```lua
LCandlestickChart:appendCandle( label : string, open : number, high : number, low : number, close : number )  -- Appends one labeled OHLC candle to the end of the current candlestick stream
LCandlestickChart:clear()  -- Clears all chart data and cached chart state
LCandlestickChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the chart at world or screen coordinates using optional transform options
LCandlestickChart:drawToImage( target : userdata )  -- Draws the rendered chart into an existing image
...
LCandlestickChart:renderImage()  -- Renders the chart into a new LImage userdata
LCandlestickChart:setCandles( candles : table )  -- Replaces all OHLC candles from table rows with open/high/low/close fields or values 1..4
LCandlestickChart:setColors( up : table, down : table )  -- Sets the rising and falling candle colors used by the candlestick renderer
LCandlestickChart:setShowLegend( value : boolean )  -- Controls whether the chart legend is rendered
LCandlestickChart:setTitle( title : string )  -- Sets the chart title text shown in rendered output
LCandlestickChart:type() -> string  -- Returns the runtime userdata type name for this chart
LCandlestickChart:typeOf( name : string ) -> boolean  -- Checks whether a type name matches this chart userdata
```
...
LHeatmapChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the heatmap at world or screen coordinates using optional transform options
...
LHeatmapChart:setColorRange( low : table, high : table ) -> nil  -- Sets the low and high RGBA colors used for the heatmap gradient
LHeatmapChart:setColumnLabels( labels : table )  -- Sets labels displayed for heatmap columns
LHeatmapChart:setMatrix( matrix : table, row_labels : table?, col_labels : table? ) -> nil  -- Replaces the heatmap contents from a numeric matrix with optional row and column labels
LHeatmapChart:setMatrixFromDataFrame( df : userdata, row_col : string, col_col : string, value_col : string, opts : table? ) -> nil  -- Builds the heatmap contents from dataframe row, column, and value fields
...
```lua
LHistogramChart:addSeries( name : string, data : table, color : table? ) -> nil  -- Adds a named histogram sample series from a numeric value list
LHistogramChart:addSeriesFromDataFrame( name : string, df : userdata, x_col : string, y_col : string, color : table?, opts : table? ) -> nil  -- Builds a named histogram sample series from one dataframe value column
LHistogramChart:appendValue( name : string, value : number, color : table? ) -> nil  -- Appends one finite numeric sample to a named histogram series
...
LHistogramChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the histogram at world or screen coordinates using optional transform options
...
LHistogramChart:replaceSeries( name : string, data : table, color : table? ) -> nil  -- Replaces a named histogram sample series with a new numeric value list
...
```lua
LLineChart:addSeries( name : string, data : table, color : table? ) -> nil  -- Adds a named line series from an array-style Lua table of points
LLineChart:addSeriesFromDataFrame( name : string, df : userdata, x_col : string, y_col : string, color : table?, opts : table? ) -> nil  -- Builds a named line series from x and y columns in a dataframe
LLineChart:appendPoint( name : string, x : number, y : number, color : table? ) -> nil  -- Appends one finite point to a named line series
LLineChart:clear()  -- Clears all series and cached chart state
LLineChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the line chart at world or screen coordinates using optional transform options
...
LLineChart:replaceSeries( name : string, data : table, color : table? ) -> nil  -- Replaces a named line series with a new array-style Lua table of points
...
```lua
LPieChart:addSegment( label : string, value : number, color : table? ) -> nil  -- Adds one pie segment with a non-negative value
LPieChart:addSegmentsFromDataFrame( df : userdata, label_col : string, value_col : string, opts : table? ) -> nil  -- Adds pie segments by reading label and value columns from a dataframe
LPieChart:addSlice( label : string, value : number, color : table? ) -> nil  -- Legacy alias that adds one pie slice with a non-negative value
LPieChart:clear()  -- Clears all series and cached chart state
LPieChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the pie chart at world or screen coordinates using optional transform options
...
```lua
LRadarChart:addSeries( name : string, values : table, color : table? )  -- Adds or replaces a named radar series
LRadarChart:clear()  -- Clears all chart data and cached chart state
LRadarChart:clearMaxValue()  -- Clears the explicit maximum radial value
LRadarChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the chart at world or screen coordinates using optional transform options
LRadarChart:drawToImage( target : userdata )  -- Draws the rendered chart into an existing image
...
LRadarChart:renderImage()  -- Renders the chart into a new LImage userdata
LRadarChart:setAxes( axes : table )  -- Replaces the radar axis labels used for each radial spoke
LRadarChart:setMaxValue( value : number )  -- Sets the explicit maximum radial value
LRadarChart:setShowLegend( value : boolean )  -- Controls whether the chart legend is rendered
LRadarChart:setTitle( title : string )  -- Sets the chart title text shown in rendered output
LRadarChart:type() -> string  -- Returns the runtime userdata type name for this chart
LRadarChart:typeOf( name : string ) -> boolean  -- Checks whether a type name matches this chart userdata
```
...
```lua
LScatterPlot:addSeries( name : string, data : table, color : table? ) -> nil  -- Adds a named scatter series from an array-style Lua table of points
LScatterPlot:addSeriesFromDataFrame( name : string, df : userdata, x_col : string, y_col : string, color : table?, opts : table? ) -> nil  -- Builds a named scatter series from x and y columns in a dataframe
LScatterPlot:appendPoint( name : string, x : number, y : number, color : table? ) -> nil  -- Appends one finite point to a named scatter series
LScatterPlot:clear()  -- Clears all series and cached chart state
LScatterPlot:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the scatter plot at world or screen coordinates using optional transform options
...
LScatterPlot:replaceSeries( name : string, data : table, color : table? ) -> nil  -- Replaces a named scatter series with a new array-style Lua table of points
...
```lua
LTreemapChart:addItem( label : string, value : number, color : table? )  -- Adds one weighted treemap item to the current rectangle layout
LTreemapChart:clear()  -- Clears all chart data and cached chart state
LTreemapChart:draw( x : number, y : number, opts : table? ) -> nil  -- Draws the chart at world or screen coordinates using optional transform options
LTreemapChart:drawToImage( target : userdata )  -- Draws the rendered chart into an existing image
...
LTreemapChart:renderImage()  -- Renders the chart into a new LImage userdata
LTreemapChart:setItems( items : table )  -- Replaces weighted treemap items from label/value rows or fields
LTreemapChart:setShowLegend( value : boolean )  -- Controls whether the chart legend is rendered
LTreemapChart:setTitle( title : string )  -- Sets the chart title text shown in rendered output
LTreemapChart:type() -> string  -- Returns the runtime userdata type name for this chart
LTreemapChart:typeOf( name : string ) -> boolean  -- Checks whether a type name matches this chart userdata
```

---

## `lurek.cinematic` {#cinematic}
...
LCinematicTimeline:addTrack( name : string ) -> nil  -- Adds a named track to this cinematic timeline
...
lurek.color.gammaToLinear( c : number ) -> number  -- Converts a single sRGB gamma-encoded component to linear space
lurek.color.invert( r : number, g : number, b : number, a : number? ) -> table  -- Inverts the RGB channels of a color, keeping alpha unchanged
lurek.color.lerp( c1 : table, c2 : table, t : number ) -> table  -- Linearly interpolates between two color tables by factor t (clamped to 0â€“1)
lurek.color.linearToGamma( c : number ) -> number  -- Converts a single linear component to sRGB gamma-encoded space
lurek.color.multiply( c1 : table, c2 : table ) -> table  -- Channel-wise multiply blend of two colors
lurek.color.new( r : number, g : number, b : number, a : number? ) -> table  -- Creates an RGBA color from 0â€“1 float components. Alpha defaults to 1.0
lurek.color.overlay( base : table, blend : table ) -> table  -- Overlay blend of two colors exposed by the lurek engine
lurek.color.palette( name : string ) -> table  -- Returns a named retro palette as a table of color tables. Supported: "pico8", "gameboy", "nes"
lurek.color.screen( c1 : table, c2 : table ) -> table  -- Apply screen blend mode to combine two color values
lurek.color.toHex( r : number, g : number, b : number, a : number? ) -> string  -- Converts RGBA components to a hex string ("#RRGGBB" or "#RRGGBBAA" if alpha < 1)
lurek.color.toHsl( r : number, g : number, b : number ) -> number, number, number  -- Convert RGB color components to HSL color representation
lurek.color.withAlpha( r : number, g : number, b : number, a : number, newAlpha : number ) -> table  -- Returns a color with the alpha channel replaced
```
...
```lua
lurek.cursor.newAnimated( looping : boolean ) -> LAnimatedCursor  -- Creates an animated cursor that can cycle through custom cursor frames
lurek.cursor.newCustom( w : integer, h : integer, hx : integer, hy : integer ) -> LCustomCursor  -- Creates a custom RGBA cursor image with an explicit hotspot
...
```lua
LAnimatedCursor:addFrame( cursor : LCustomCursor, duration_ms : integer )  -- Appends one frame to the animated cursor sequence
LAnimatedCursor:clearPulse()  -- Disables pulse scaling for the animated cursor
...
LAnimatedCursor:reset()  -- Resets playback to the first frame and clears accumulated animation time
LAnimatedCursor:setPulse( min_scale : number, max_scale : number, speed : number )  -- Enables pulse scaling for the animated cursor
LAnimatedCursor:update( dt : number )  -- Advances animated cursor playback and pulse state
```
...
```lua
LCursorManager:addRule( context_or_rule : string|table, cursor_name : string ) -> integer  -- Registers a legacy context rule or a v2 runtime rule table for hover, click, release, leave, wheel, or context state resolution
LCursorManager:addSource( source_tbl : table ) -> integer  -- Registers a hover source that feeds semantic cursor hits into the shared runtime resolver
LCursorManager:defineEffect( name : string, spec : table )  -- Defines a reusable cursor-local burst effect preset for hover or click rules
LCursorManager:defineState( name : string, spec : table )  -- Defines a reusable named cursor state for rule-driven runtime selection
LCursorManager:disableTrail()  -- Disables the current cursor trail
LCursorManager:disableZoom()  -- Disables the live cursor zoom lens
LCursorManager:enableLineTrail( r : number, g : number, b : number, width : number )  -- Enables a simple connected line trail behind the cursor
LCursorManager:enableTrail( r : number, g : number, b : number, lifetime : number )  -- Enables a simple fading point trail behind the cursor
LCursorManager:enableZoom( mag : number, radius : number )  -- Enables the live zoom lens centered on the runtime cursor
...
LCursorManager:removeRule( ctx : string )  -- Removes a legacy context rule that was registered with the `(context, cursor_name)` shorthand
LCursorManager:removeSource( id : integer ) -> boolean  -- Removes a previously registered hover source
LCursorManager:setAnimated( cursor : LAnimatedCursor )  -- Switches the active runtime cursor to an animated cursor immediately
LCursorManager:setContext( ctx : string )  -- Sets the named cursor context used by legacy rules and context-sensitive state resolution
LCursorManager:setCustom( cursor : LCustomCursor )  -- Switches the active runtime cursor to a custom RGBA cursor immediately
LCursorManager:setLocked( locked : boolean )  -- Locks or unlocks the runtime cursor according to the active platform policy
LCursorManager:setSystem( name : string )  -- Switches the active runtime cursor to a named system cursor immediately
LCursorManager:setVisible( visible : boolean )  -- Shows or hides the runtime cursor for the active application window
LCursorManager:type() -> string  -- Returns the Lua handle type name for this cursor manager userdata
LCursorManager:update( x : number, y : number, dt : number )  -- Overrides the runtime cursor position and advances cursor-local effects for one frame
```
...
LCustomCursor:getPixel( x : integer, y : integer ) -> integer  -- Reads one RGBA pixel from the custom cursor image
LCustomCursor:getSize() -> integer  -- Returns the custom cursor image size
LCustomCursor:setPixel( x : integer, y : integer, r : integer, g : integer, b : integer, a : integer )  -- Writes one RGBA pixel into the custom cursor image
```
...
lurek.dsp.newAdsrEnvelope( attack : number, decay : number, sustain : number, release : number ) -> LAdsrEnvelope  -- Creates an ADSR envelope object for procedural synthesis and buffer shaping workflows
...
lurek.dsp.newLevelDetector( options : table? ) -> LLevelDetector  -- Creates a level detector object that tracks RMS, peak, and clipping state over samples
lurek.dsp.newNode( kind : string, options : table? ) -> LDspNode  -- Creates a DSP graph node object with a node kind and optional initial options
lurek.dsp.newSawtoothWave( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Generates a sawtooth wave as a `SoundData` buffer
lurek.dsp.newSineWave( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Generates a sine wave as a `SoundData` buffer
lurek.dsp.newSpectrumAnalyzer( options : table? ) -> LSpectrumAnalyzer  -- Creates a spectrum analyzer object for bounded frequency-bin analysis on SoundData
lurek.dsp.newSquareWave( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Generates a square wave as a `SoundData` buffer
lurek.dsp.newSynthWave( waveform : string, freq : number, duration : number, sample_rate : integer, amplitude : number, adsr : table? ) -> LSoundData  -- Generates a synthesized waveform with optional ADSR
lurek.dsp.newSynthesizer( options : table? ) -> LSynthesizer  -- Creates a synthesizer object that combines waveform selection and optional ADSR shaping
lurek.dsp.newTriangleWave( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Generates a triangle wave as a `SoundData` buffer
lurek.dsp.newWaveform( kind : string, options : table? ) -> LWaveform  -- Creates a waveform descriptor object that can render repeated procedural tones
lurek.dsp.newWhiteNoise( duration : number, sample_rate : integer, amplitude : number, seed : integer ) -> LSoundData  -- Generates deterministic white noise as a `SoundData` buffer
lurek.dsp.normalize( input : string, output : string, target : number ) -> boolean  -- Normalizes an audio file to a target peak amplitude and saves the result
lurek.dsp.processOffline( input : string, output : string, effects : table ) -> boolean  -- Processes an audio file offline through a chain of effects and writes the result to an output file
...
lurek.dsp.spectrogramToPng( input : string, output : string, width : integer, height : integer, options : table? ) -> boolean  -- Renders a spectrogram visualization of an audio file and saves it as a PNG image
lurek.dsp.waveformToPng( input : string, output : string, width : integer, height : integer ) -> boolean  -- Renders a waveform visualization of an audio file and saves it as a PNG image
```
...
```lua
LAdsrEnvelope:apply( sound_data_ud : LSoundData )  -- Applies this ADSR envelope across an entire sound buffer in place
...
```lua
LDspGraph:addNode( node_ud : LDspNode ) -> integer  -- Adds a DSP node object to the graph and returns its stable node ID
LDspGraph:clear()  -- Clears all graph nodes and edges from this graph
LDspGraph:connect( from : integer, to : integer, options : table? ) -> boolean  -- Connects two node IDs in this graph object
LDspGraph:disconnect( from : integer, to : integer ) -> boolean  -- Removes a connection between two node IDs
LDspGraph:process( sound_data_ud : LSoundData ) -> LSoundData  -- Processes a sound buffer through the graph and returns transformed data
```
...
```lua
LDspNode:getParam( name : string ) -> number  -- Returns one named numeric parameter from the node
LDspNode:setParam( name : string, value : number )  -- Sets one named numeric parameter on the node
...
LLevelDetector:process( sound_data_ud : LSoundData ) -> table  -- Processes all samples in a sound buffer and returns aggregate level statistics
LLevelDetector:process_sample( sample : number )  -- Processes one audio sample and updates detector statistics incrementally
LLevelDetector:reset()  -- Resets detector state so a new measurement window can begin
LLevelDetector:to_db( value : number ) -> number  -- Converts a linear amplitude value to decibels full scale
```
...
```lua
LSpectrumAnalyzer:analyze( sound_data_ud : LSoundData ) -> table  -- Analyzes one sound buffer and returns `(frequency, magnitude)` rows
LSpectrumAnalyzer:setSize( size : integer )  -- Sets the frequency-bin count used by subsequent spectrum analysis calls
```
...
```lua
LSynthesizer:generate( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Generates a SoundData buffer; alias of `render` for compatibility
LSynthesizer:render( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Renders a SoundData buffer using current synthesizer settings
LSynthesizer:setEnvelope( envelope_ud : LAdsrEnvelope )  -- Attaches an ADSR envelope used by future render calls
LSynthesizer:setWaveform( value : any )  -- Sets the oscillator waveform using a kind string or waveform object
```
...
```lua
LWaveform:render( freq : number, duration : number, sample_rate : integer, amplitude : number ) -> LSoundData  -- Renders this waveform to a new SoundData buffer
...
lurek.effect.newCustomEffect( shader : LShader|integer ) -> LPostFxEffect  -- Creates a custom post-processing effect from a postfx-target shader
...
lurek.effect.newPass( shader : LShader|integer ) -> LPostFxEffect  -- Creates a custom post-processing pass from a postfx-target shader
...
LPostFxEffect:setBrightness( v : number )  -- Sets the brightness shader parameter on this effect
LPostFxEffect:setContrast( v : number )  -- Sets the contrast shader parameter on this effect
LPostFxEffect:setEnabled( enabled : boolean )  -- Enables or disables this effect. This method is available to Lua scripts
LPostFxEffect:setIntensity( v : number )  -- Sets the intensity shader parameter on this effect
LPostFxEffect:setOffset( v : number )  -- Sets the offset shader parameter on this effect
LPostFxEffect:setParameter( name : string, value : number )  -- Sets a numeric shader parameter by name
LPostFxEffect:setRadius( v : number )  -- Sets the radius shader parameter on this effect
LPostFxEffect:setSaturation( v : number )  -- Sets the saturation shader parameter on this effect
LPostFxEffect:setScanlineStrength( v : number )  -- Sets the scanline strength shader parameter on this effect
LPostFxEffect:setStrength( v : number )  -- Sets the strength shader parameter on this effect
LPostFxEffect:setThreshold( v : number )  -- Sets the threshold shader parameter on this effect
...
LPostFxStack:restore( snapshot : table )  -- Restores a stack snapshot and rebuilds its effect handles without entering capture mode
...
lurek.engine.setResourceBudget( budget_bytes : integer )  -- Sets the hard resource budget used by public resource allocation and reporting
...
LGraph:addEdgeUnchecked( from_ud : LGraphNode, to_ud : LGraphNode, edge_type : string? ) -> LGraphEdge  -- Adds an edge without validating endpoint nodes exist. Faster for batch construction
...
LGraph:batchAddEdges( edges : table ) -> integer[]  -- Creates multiple edges from a table of {from_id, to_id} or {from_id, to_id, edge_type} entries
LGraph:batchAddNodes( count : integer, config : table? ) -> integer[]  -- Creates multiple nodes at once, returning their IDs as a table
LGraph:batchStep( dt : number, iterations : integer )  -- Runs multiple simulation steps in sequence. More efficient than calling step() in a loop from Lua
...
LGraph:drainEvents( maxCount : integer? ) -> table  -- Removes and returns queued graph events in deterministic emission order
...
LGraph:getEdgeById( id : integer ) -> LGraphEdge  -- Resolves a stable numeric edge id to a graph-local handle
...
LGraph:getItemById( id : integer ) -> LGraphItem  -- Resolves a stable numeric item id to a graph-local handle
...
LGraph:getNodeById( id : integer ) -> LGraphNode  -- Resolves a stable numeric node id to a graph-local handle
...
LGraph:prepareBatch( edits : table, expectedVersion : integer? ) -> LGraphTopologyBatch  -- Validates and stages graph topology edits without mutating the live graph
...
LGraph:restoreSnapshot( snapshot : string, expectedVersion : integer? )  -- Replaces graph state from a versioned snapshot without changing Lua callbacks
LGraph:sendItem( item_ud : LGraphItem, edge_ud : LGraphEdge )  -- Starts moving an item along an edge
LGraph:setEventMode( mode : string )  -- Selects callback delivery, pull-queue delivery, both, or no delivery
LGraph:setEventQueueLimit( limit : integer )  -- Sets the bounded pull-event queue capacity, dropping oldest queued records if needed
LGraph:snapshot() -> string  -- Serializes complete graph state to deterministic compact JSON
LGraph:spawnItems( node : LGraphNode, itemType : string, count : integer, decayTime : number? ) -> integer[]  -- Creates many same-type items directly in one node inventory
...
LGraphNode:removeRecipe( name : string ) -> boolean  -- Removes one named explicit recipe
...
LGraphNode:runRecipe( name : string, maxRuns : integer? ) -> table  -- Executes a stored recipe as a bounded inventory operation
...
LGraphNode:setRecipe( name : string, inputs : table, outputs : table )  -- Stores a named multi-input, multi-output recipe without scheduling it
...
LGraphTopologyBatch:typeOf( name : string ) -> boolean  -- Checks whether this userdata matches a requested type
```
...
```lua
LFont:containsGlyph( char : string ) -> boolean  -- Returns whether the font contains a glyph for the given character. This method is available to Lua scripts
...
LFont:measure( text : string, scale : number ) -> number, number  -- Measures the pixel dimensions of a text string at the given scale. This method is available to Lua scripts
LFont:wrapText( text : string, maxWidth : number, scale : number ) -> table  -- Wraps text into lines fitting within the given max width. This method is available to Lua scripts
```
...
LGlobe:addMarkerEx( marker_tbl : table ) -> integer  -- Adds a marker on one named orbit shell using a table-based configuration
LGlobe:addOrbit( orbit_tbl : table ) -> boolean  -- Adds or replaces one named orbit shell above the globe surface
LGlobe:addProvince( p : table ) -> boolean  -- Adds a province described by id, centroid, polygon vertices or multipart geometry, neighbors, and optional base color
LGlobe:addRegion( p : table ) -> boolean  -- Adds a region described by id, centroid, polygon vertices or multipart geometry, neighbors, and optional base color
LGlobe:addTerrainPatch( p : table ) -> boolean  -- Adds a base terrain polygon patch described by id, centroid, polygon vertices or multipart geometry, optional attrs, and optional base color
...
LGlobe:clearOrbitShader( orbit : string ) -> boolean  -- Clears one orbit shell shader without affecting the globe-wide shader
LGlobe:clearProvinceTexture( id : integer ) -> boolean  -- Removes texture metadata from a province
LGlobe:clearTerrainPatchTexture( id : integer ) -> boolean  -- Removes texture metadata from a terrain patch
...
LGlobe:getMarkerInfo( id : integer ) -> table  -- Returns a snapshot table describing one marker
LGlobe:getMarkerOrbit( id : integer ) -> string  -- Returns the named orbit shell assigned to one marker
...
LGlobe:getOrbit( name : string ) -> table  -- Returns a snapshot table for one orbit shell
LGlobe:getOrbitAttr( name : string, key : string ) -> string  -- Reads one string attribute from an orbit shell
...
LGlobe:getTerrainPatchAttr( id : integer, key : string ) -> string  -- Reads a string attribute from a terrain patch
...
LGlobe:pickAllObjects( sx : number, sy : number, opts : table? ) -> table[]  -- Resolves all shell-aware hits at one screen position using the supplied ordering policy
...
LGlobe:pickObject( sx : number, sy : number, opts : table? ) -> table  -- Resolves the highest-priority shell-aware hit at one screen position
...
LGlobe:removeOrbit( name : string ) -> boolean  -- Removes one named orbit shell and moves any markers on it back to `surface`
LGlobe:removeProvince( id : integer ) -> boolean  -- Removes a region by id. This method is available to Lua scripts
LGlobe:removeRegion( id : integer ) -> boolean  -- Removes a region by id. This method is available to Lua scripts
LGlobe:removeTerrainPatch( id : integer ) -> boolean  -- Removes a stored terrain patch by its numeric id
...
LGlobe:screenToOrbitLatLon( sx : number, sy : number, orbit : string ) -> number  -- Converts a visible screen position into latitude and longitude on one named visible pickable orbit shell
LGlobe:screenToShells( sx : number, sy : number ) -> table  -- Resolves shell hits for every visible pickable orbit at one screen-space position
...
LGlobe:setOrbitAttr( name : string, key : string, value : string ) -> boolean  -- Sets one string attribute on an orbit shell
LGlobe:setOrbitShader( orbit : string, shader : LShader ) -> boolean  -- Binds a `mapviz` shader to one orbit shell and restores the globe shader outside that shell scope
...
LGlobe:validateTerrainCoverage( opts : table? ) -> table  -- Samples terrain coverage over equirectangular latitude-longitude space
...
```lua
lurek.layout.centerInArea( result : table, width : number, height : number ) -> table  -- Centers the layout within a given area
lurek.layout.circular( nodes : table, config : table|nil ) -> table  -- Lays out nodes around a chord-safe circle for cycle-heavy graphs and overviews
lurek.layout.dag( nodes : table, edges : table, config : table|nil ) -> table  -- Lays out a size-aware DAG with centered layers, stable barycenter ordering, and height-aware ranks
lurek.layout.force( nodes : table, edges : table, config : table|nil ) -> table  -- Lays out a graph using bounded size-aware force simulation with final overlap cleanup
lurek.layout.grid( nodes : table, config : table|nil ) -> table  -- Lays out nodes in a compact variable-size grid for dense or disconnected graphs
lurek.layout.radial( nodes : table, edges : table, root : integer, config : table|nil ) -> table  -- Lays out a graph on size-aware breadth-first concentric rings from a root node
lurek.layout.snapToGrid( result : table, gridSize : number ) -> table  -- Snaps all node positions to the nearest grid point
lurek.layout.spiral( nodes : table, config : table|nil ) -> table  -- Lays out nodes on an expanding overlap-checked spiral for fast large-graph refreshes
lurek.layout.stress( nodes : table, edges : table, config : table|nil ) -> table  -- Lays out a graph by preserving graph-distance relationships with bounded relaxation and cleanup
lurek.layout.tree( nodes : table, children : table, root : integer, config : table|nil ) -> table  -- Lays out a size-aware tree, centering parents over child spans and keeping disconnected nodes
```
...
*Coverage: 152/152 items documented (100%)*
...
lurek.learning.newMaxPool2D( kernel_h : integer, kernel_w : integer, stride_h : integer, stride_w : integer ) -> LMaxPool2D  -- Creates a MaxPool2D layer wrapper
lurek.learning.newMultiHeadAttention( d_model : integer, num_heads : integer ) -> LMultiHeadAttention  -- Creates a multi-head attention block
...
```lua
LConv2D:forward( input : LTensor ) -> LTensor  -- Runs convolution over an input tensor shaped as `[channels,height,width]`
...
LConv2D:setWeights( weights : table ) -> boolean  -- Loads flattened convolution weights and biases into this layer
LConv2D:type() -> string  -- Returns the Lua-visible type name for this wrapper
LConv2D:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
```lua
LGRU:forward( input : table ) -> table  -- Runs one GRU recurrent step on input data and returns next hidden state values
...
LGRU:setWeights( weights : table ) -> boolean  -- Loads flattened layer weights and biases into the wrapped GRU layer
LGRU:type() -> string  -- Returns the Lua-visible type name for this wrapper
LGRU:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
LGeneticAlgorithm:getGenes( idx : integer ) -> number[]  -- Returns the genes for a chromosome by zero-based index
...
```lua
LLSTM:forward( input : table ) -> table  -- Runs one LSTM recurrent step on input data and returns next hidden state values
...
LLSTM:setWeights( weights : table ) -> boolean  -- Loads flattened layer weights and biases into the wrapped LSTM layer
LLSTM:type() -> string  -- Returns the Lua-visible type name for this wrapper
LLSTM:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
```lua
LMaxPool2D:forward( input : LTensor ) -> LTensor  -- Runs max-pooling over an input tensor shaped as `[channels,height,width]`
LMaxPool2D:type() -> string  -- Returns the Lua-visible type name for this wrapper
LMaxPool2D:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
```lua
LMultiHeadAttention:forward( input : LTensor ) -> LTensor  -- Runs multi-head self-attention over an input tensor shaped as `[seq_len,d_model]`
...
```lua
LPositionalEncoding:apply( input : LTensor ) -> LTensor  -- Applies sinusoidal positional encoding values to a `[seq_len,d_model]` tensor
LPositionalEncoding:type() -> string  -- Returns the Lua-visible type name for this wrapper
LPositionalEncoding:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
```lua
LTransformerDecoder:forward( input : LTensor, encoder_out : LTensor ) -> LTensor  -- Runs one transformer decoder block over input and encoder-output tensors
...
LTransformerDecoder:setWeights( weights : table ) -> boolean  -- Loads flattened trainable parameters for this decoder block
LTransformerDecoder:type() -> string  -- Returns the Lua-visible type name for this wrapper
LTransformerDecoder:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
```lua
LTransformerEncoder:forward( input : LTensor ) -> LTensor  -- Runs one transformer encoder block over an input `[seq_len,d_model]` tensor
...
LTransformerEncoder:setWeights( weights : table ) -> boolean  -- Loads flattened trainable parameters for this encoder block
LTransformerEncoder:type() -> string  -- Returns the Lua-visible type name for this wrapper
LTransformerEncoder:typeOf( name : string ) -> boolean  -- Returns whether this userdata matches the requested type string
```
...
lurek.light.createLightsFromTilefield( field : LTileField, slot : string, tileset : LTileSet, opts : table? ) -> table  -- Creates render lights and occluders from `lurek.tilefield` refs and tileset object metadata
lurek.light.drawToImage( width : integer, height : integer ) -> LImageData  -- Renders an approximate light-map preview of this world into an ImageData
...
lurek.light.setMaxLights( n : integer )  -- Sets the renderer selection count; values must be 1 through 256
lurek.light.setShader( shader : LShader? )  -- Sets or clears the default custom light shader for the light world
...
LLight:clearCookie()  -- Clears the cookie resource path from the authoritative light state
...
LLight:setCookie( path : string )  -- Stores a cookie resource path on the authoritative light state. It is not sampled until a renderer supports cookies
...
LLight:setShader( shader : LShader? )  -- Sets or clears the custom light-contribution shader for this light
...
*Coverage: 79/79 items documented (100%)*

```lua
lurek.mapblock.newBlock( width : integer, height : integer, layers : integer, config : MapBlockConfig ) -> MapBlock  -- Create a new map block exposed by the lurek engine
...
lurek.mapblock.newGenerator( config : MapBlockConfig ) -> MapBlockGenerator  -- Create a new procedural map block generator instance
...
```lua
LMapBlockConfig:addSlot( name : string, required : boolean, default_gid : integer )  -- Add a slot definition â€” Lua userdata object exposed by the engine
LMapBlockConfig:getSlotCount() -> integer  -- Get the number of slots for this object
LMapBlockConfig:removeSlot( name : string ) -> boolean  -- Remove a slot by name for this object
LMapBlockConfig:setDefaultSegmentSize( size : integer )  -- Set default segment size for this object
LMapBlockConfig:setMaxLayers( max : integer )  -- Set maximum layers per block for this object
```
...
LMapBlockResult:getGid( level : integer, layer : integer, x : integer, y : integer, slot : integer ) -> integer  -- Get tile GID at position for this object
...
LMapBlockResult:toTileField( opts : table ) -> LTileField  -- Converts one mapblock result layer and slot into a shared tilefield ref layer
LMapBlockResult:writeTileField( field : LTileField, opts : table )  -- Writes one mapblock result layer and slot into an existing tilefield ref layer
```
...
```lua
LMapGroup:addBlock( block : MapBlock )  -- Add a block to this group for this object
LMapGroup:addScript( script : MapScript )  -- Add a script to this group for this object
...
```lua
LMapScript:addStep( step_type : string, opts : table )  -- Add a generation step â€” Lua userdata object exposed by the engine
...
```lua
LNeighborRules:addCompatible( type_a : integer, type_b : integer )  -- Add bidirectional compatibility between two edge types
LNeighborRules:addCompatibleOneWay( type_a : integer, type_b : integer )  -- Add one-way compatibility for this object
LNeighborRules:clear()  -- Clear all neighbor placement rules from this rule set
LNeighborRules:isCompatible( type_a : integer, type_b : integer ) -> boolean  -- Check if two edge types are compatible
```
...
```lua
LPlacementGrid:addPosition( x : integer, y : integer )  -- Add a position to the grid â€” Lua userdata object exposed by the engine
...
LTilesetRef:setImagePath( path : string )  -- Set the image file path for this tileset reference
```
...
lurek.network.newInputBuffer( opts : table? ) -> LNetworkInputBuffer  -- Creates a bounded input-reordering buffer for Lua-owned fixed-step simulation
lurek.network.newNetState( host : LNetworkHost?, opts : table? ) -> LNetworkState  -- Creates a transport-neutral replicated state manager
...
lurek.network.newRpc( host : LNetworkHost?, channel : integer?, timeout_seconds : number? ) -> LNetworkRpc  -- Creates a transport-neutral RPC protocol manager
lurek.network.newServer( opts : table ) -> LNetworkHost  -- Creates a server host from an options table
lurek.network.newSnapshotStore( opts : table? ) -> LNetworkSnapshotStore  -- Creates a bounded resolved snapshot history for explicit Lua interpolation and correction
...
```lua
LNetworkInputBuffer:drainThrough( tick : integer ) -> table  -- Drains accepted inputs through a simulation tick in stable tick, peer, and sequence order
LNetworkInputBuffer:getStats() -> table  -- Reports the buffer's finite capacity and current deterministic drain state
LNetworkInputBuffer:push( peer_id : integer, tick : integer, sequence : integer, payload : any ) -> boolean  -- Stores one serializable remote input and reports its deterministic acceptance result
LNetworkInputBuffer:type() -> string  -- Returns the Lua-visible type name of this bounded input-buffer handle
LNetworkInputBuffer:typeOf( name : string ) -> boolean  -- Checks this userdata against the public input-buffer type names
```
...
```lua
LNetworkSnapshotStore:get( tick : integer ) -> table  -- Returns one resolved full snapshot frame, or nil when its tick is not retained
LNetworkSnapshotStore:getStats() -> table  -- Returns bounded history capacity, retained ticks, and current frame count
LNetworkSnapshotStore:interpolate( id : integer, from_tick : integer, to_tick : integer, alpha : number ) -> table  -- Interpolates one entity between two retained resolved frames
LNetworkSnapshotStore:latest() -> table  -- Returns the latest resolved full snapshot frame, or nil when no frame is retained
LNetworkSnapshotStore:push( snapshot : table )  -- Resolves and retains one full, corrective, or delta snapshot
LNetworkSnapshotStore:type() -> string  -- Returns the Lua-visible type name of this resolved snapshot-store handle
LNetworkSnapshotStore:typeOf( name : string ) -> boolean  -- Checks this userdata against its public type names
```
...
*Coverage: 106/106 items documented (100%)*

```lua
lurek.overlay.new( w : integer?, h : integer? ) -> LOverlay  -- Creates an overlay controller for screen effects using optional dimensions
lurek.overlay.newTransition( kind : string?, duration : number?, color_tbl : table? ) -> LScreenTransition  -- Creates a timed screen transition with optional kind, duration, and color
```
...
LOverlay:clearStatusEffect( kind : string, opts : table? )  -- Starts fading out one status layer
...
LOverlay:getShaderLayer( layer : string ) -> LShader  -- Returns a shader bound to one overlay layer, if present
...
LOverlay:getStatusEffect( kind : string ) -> table  -- Returns one status layer table or nil
...
LOverlay:render( opts : table? )  -- Queues renderer commands for the overlay's current visual state
...
LOverlay:setShader( shader : LShader? )  -- Sets or clears the shader used for custom overlay rendering
LOverlay:setShaderLayer( layer : string, shader : LShader? )  -- Sets or clears an overlay-layer shader binding
LOverlay:setStatusEffect( kind : string, opts : table )  -- Creates or updates one overlay-owned status layer such as `frozen`, `poison`, or `lowHealth`
LOverlay:setStatusIntensity( kind : string, intensity : number )  -- Updates one existing status intensity or creates a preset-backed layer when it is missing
...
lurek.parallax.newLayerSet( name : string, layerDefs : table|Array of `newLayer` option tables; each may include `preset = "far"|"mid", opts : table ) -> LParallaxSet  -- Creates a parallax layer set from an array of layer definition tables
...
LParallaxLayer:setShader( shader : LShader? )  -- Binds a draw-target shader to this parallax layer's generated render commands. Pass nil to clear
...
```lua
LPipelineStep:connectOutput( outputSlot : integer, target : string|LPipelineStep, inputSlot : integer?, condition : function?, signal : boolean? ) -> LPipelineStep  -- Connects one output slot (1..5) to a target step input slot (1..5), optionally gated by a Lua predicate
...
LPipelineStep:setState( state : table? )  -- Stores a Lua table as local state for this step. The table is retained by registry reference
...
lurek.procgen.cellularAutomataGrid( width : integer, height : integer, opts : table? ) -> LProcgenGrid  -- Generate a cave or organic map and return a typed grid result
lurek.procgen.fbm( x : number, y : number, seed : integer?, octaves : integer?, lac : number?, gain : number? ) -> number  -- Samples stateless fractal Brownian motion noise
...
lurek.procgen.heightmapFromCellularGrid( width : integer, height : integer, cells : table, floorValue : number? ) -> LProcgenScalarGrid  -- Convert a cellular automata grid into a typed heightmap scalar grid
lurek.procgen.heightmapGrid( opts : table? ) -> LProcgenScalarGrid  -- Generate a fractal heightmap and return a typed scalar grid result
...
lurek.procgen.newBiomeClassifier( opts : table? ) -> LBiomeClassifier  -- Create a BiomeClassifier object with custom threshold rules for mapping height/moisture/temperature to biome types
lurek.procgen.newCellular( width : integer, height : integer ) -> LCellular  -- Performs the 'procgen' operation
lurek.procgen.newGridResult( width : integer, height : integer, cells : table, opts : table? ) -> LProcgenGrid  -- Wrap a flat grid table as a typed procgen grid result
lurek.procgen.newNoiseGenerator( seed : integer? ) -> LNoiseGenerator  -- Creates a procedural noise generator with an optional seed
lurek.procgen.newScalarGridResult( width : integer, height : integer, cells : table, opts : table? ) -> LProcgenScalarGrid  -- Wrap a flat numeric table as a typed procgen scalar grid result
...
lurek.procgen.noiseMapGrid( width : integer, height : integer, opts : table? ) -> LProcgenScalarGrid  -- Generate a typed scalar noise grid using the optional seed in opts
...
lurek.procgen.noiseMapParallelGrid( width : integer, height : integer, opts : table? ) -> LProcgenScalarGrid  -- Generate a typed scalar noise grid using the parallel backend and seed 0
...
lurek.procgen.noiseMapParallelSeededGrid( width : integer, height : integer, opts : table? ) -> LProcgenScalarGrid  -- Generate a typed scalar noise grid using the parallel backend and explicit seed
lurek.procgen.perlin2d( x : number, y : number, seed : integer? ) -> number  -- Samples stateless 2D Perlin noise
lurek.procgen.perlin3d( x : number, y : number, z : number, seed : integer? ) -> number  -- Samples stateless 3D Perlin noise
lurek.procgen.perlin4d( x : number, y : number, z : number, w : number, seed : integer? ) -> number  -- Samples stateless 4D Perlin noise
...
lurek.procgen.placeConstrained( candidates : table, rules : table, opts : table? ) -> table, table  -- Selects a deterministic weighted subset satisfying neutral spatial and tag constraints
...
lurek.procgen.roomsDungeonGrid( opts : table? ) -> LProcgenGrid  -- Generate a rooms dungeon and return only its tile grid as a typed procgen result
...
lurek.procgen.roomsDungeonWithPrefabsGrid( opts : table?, prefabs : table, stampValue : number? ) -> LProcgenGrid  -- Generate a rooms dungeon with prefabs and return only its tile grid as a typed procgen result
...
lurek.procgen.validateConnectivity( grid : LProcgenGrid|table, opts : table? ) -> table  -- Reports deterministic connectivity, unreachable goals, isolated regions, and safe-radius failures
...
lurek.procgen.wfcGenerateGrid( opts : table ) -> LProcgenGrid  -- Run WFC and return a typed procgen grid result
...
```lua
LBiomeClassifier:classify( height : number, moisture : number, temperature : number ) -> string  -- Classify a single point into a biome type based on its environmental parameters
...
LNoiseGenerator:generateMapComputeGrid( w : integer, h : integer, opts : table? ) -> LProcgenScalarGrid  -- Generates a compute-style noise map and returns it as a typed scalar grid
LNoiseGenerator:generateMapGrid( w : integer, h : integer, opts : table? ) -> LProcgenScalarGrid  -- Generates a noise map and returns it as a typed scalar grid
...
```lua
LProcgenGrid:getCell( x : integer, y : integer ) -> integer  -- Returns one cell value using one-based Lua coordinates
...
LProcgenGrid:toTileField( opts : table ) -> LTileField  -- Converts this generated grid into a tilefield by writing each value as a named ref
LProcgenGrid:type() -> string  -- Returns the type name of this object
LProcgenGrid:typeOf( name : string ) -> boolean  -- Check whether this object matches a given type name
LProcgenGrid:writeTileField( field : LTileField, opts : table )  -- Writes this generated grid into an existing tilefield ref layer
```
...
```lua
LProcgenScalarGrid:getCell( x : integer, y : integer ) -> number  -- Returns one scalar cell value using one-based Lua coordinates
...
LProcgenScalarGrid:toTileField( opts : table ) -> LTileField  -- Converts this scalar field into a new tilefield channel layer
LProcgenScalarGrid:type() -> string  -- Returns the type name of this object
LProcgenScalarGrid:typeOf( name : string ) -> boolean  -- Check whether this object matches a given type name
LProcgenScalarGrid:writeTileField( field : LTileField, opts : table )  -- Writes this scalar field into an existing tilefield channel layer
```

---

## `lurek.progression` {#progression}

> Registers the public `lurek.progression` Lua API for deterministic offline progression state and owns its typed userdata wrappers.

*Coverage: 317/317 items documented (100%)*

```lua
lurek.progression.acquirePerk( this : table, name : string ) -> boolean  -- Performs the `acquirePerk` progression operation for Lua callers
lurek.progression.activeCount( this : table ) -> integer  -- Performs the `activeCount` progression operation for Lua callers
lurek.progression.activeIds( this : table ) -> nil  -- Performs the `activeIds` progression operation for Lua callers
lurek.progression.addBuff() -> string  -- Adds buff to the progression store for Lua callers
lurek.progression.addJournalEntry( this : table, quest_id : string, text : string, tag : string? ) -> integer  -- Adds journal entry to the progression store for Lua callers
lurek.progression.addQuest( this : table, quest : table ) -> nil  -- Adds quest to the progression store for Lua callers
lurek.progression.addXP( this : table, amount : number ) -> nil  -- Adds x p to the progression store for Lua callers
lurek.progression.adjustMorale( this : table, delta : number ) -> nil  -- Performs the `adjustMorale` progression operation for Lua callers
lurek.progression.advanceObjective() -> boolean  -- Performs the `advanceObjective` progression operation for Lua callers
lurek.progression.applyDamage( this : table, stat : string, amount : number, dtype : string? ) -> number  -- Applies damage in the progression store for Lua callers
lurek.progression.applyTraitBuffs( this : table, trait_name : string ) -> nil  -- Applies trait buffs in the progression store for Lua callers
lurek.progression.beginTurn( this : table ) -> nil  -- Begins turn in the progression store for Lua callers
lurek.progression.checkMorale( this : table ) -> string  -- Performs the `checkMorale` progression operation for Lua callers
lurek.progression.clearBuffs( this : table, stat : string? ) -> nil  -- Clears buffs in the progression store for Lua callers
lurek.progression.clearFlag( this : table, name : string ) -> nil  -- Clears flag in the progression store for Lua callers
lurek.progression.completeQuest( this : table, id : string ) -> boolean  -- Performs the `completeQuest` progression operation for Lua callers
lurek.progression.completedCount( this : table ) -> integer  -- Performs the `completedCount` progression operation for Lua callers
lurek.progression.completedIds( this : table ) -> nil  -- Performs the `completedIds` progression operation for Lua callers
lurek.progression.createLegacyQuestAdapter( store : AnyUserData, profile : any, options : table? ) -> nil  -- Creates legacy quest adapter in the progression store for Lua callers
lurek.progression.createLegacyStatsAdapter( store : AnyUserData, profile : any, options : table? ) -> nil  -- Creates legacy stats adapter in the progression store for Lua callers
lurek.progression.define( this : table, name : string, base : number, opts : table? ) -> nil  -- Defines this operation in the progression store for Lua callers
lurek.progression.definePerk( this : table, name : string, opts : table? ) -> nil  -- Defines perk in the progression store for Lua callers
lurek.progression.defineSkill( this : table, name : string, opts : table? ) -> nil  -- Defines skill in the progression store for Lua callers
lurek.progression.failQuest( this : table, id : string ) -> boolean  -- Performs the `failQuest` progression operation for Lua callers
lurek.progression.failedIds( this : table ) -> nil  -- Performs the `failedIds` progression operation for Lua callers
lurek.progression.get( this : table, name : string ) -> number  -- Returns this operation from the progression store for Lua callers
lurek.progression.getActionPoints( this : table ) -> table | number, number | Current action points followed by the configured maximum  -- Returns action points from the progression store for Lua callers
lurek.progression.getActiveTraits( this : table ) -> table  -- Returns active traits from the progression store for Lua callers
lurek.progression.getBase( this : table, name : string ) -> number  -- Returns base from the progression store for Lua callers
lurek.progression.getBuffCount( this : table, stat : string? ) -> integer  -- Returns buff count from the progression store for Lua callers
lurek.progression.getBuffs( this : table, stat : string? ) -> table  -- Returns buffs from the progression store for Lua callers
lurek.progression.getCooldownRemaining( this : table, name : string ) -> number  -- Returns cooldown remaining from the progression store for Lua callers
lurek.progression.getEncumbrance( this : table ) -> table  -- Returns encumbrance from the progression store for Lua callers
lurek.progression.getFlags( this : table ) -> table  -- Returns flags from the progression store for Lua callers
lurek.progression.getInitiative( this : table ) -> table  -- Returns initiative from the progression store for Lua callers
lurek.progression.getLevel( this : table ) -> integer  -- Returns level from the progression store for Lua callers
lurek.progression.getMax( this : table, name : string ) -> table  -- Returns max from the progression store for Lua callers
lurek.progression.getMin( this : table, name : string ) -> table  -- Returns min from the progression store for Lua callers
lurek.progression.getMorale( this : table ) -> table | number, number | Current morale followed by the configured maximum  -- Returns morale from the progression store for Lua callers
lurek.progression.getQuest( this : table, id : string ) -> table  -- Returns quest from the progression store for Lua callers
lurek.progression.getQuestReward( this : table, id : string ) -> table  -- Returns quest reward from the progression store for Lua callers
lurek.progression.getRegen( this : table, name : string ) -> table  -- Returns regen from the progression store for Lua callers
lurek.progression.getResistance( this : table, dtype : string ) -> number  -- Returns resistance from the progression store for Lua callers
lurek.progression.getSkillLevel( this : table, name : string ) -> integer  -- Returns skill level from the progression store for Lua callers
lurek.progression.getStatNames( this : table ) -> table  -- Returns stat names from the progression store for Lua callers
lurek.progression.getUseCount( this : table, name : string ) -> integer  -- Returns use count from the progression store for Lua callers
lurek.progression.getXP( this : table ) -> number  -- Returns x p from the progression store for Lua callers
lurek.progression.hasFlag( this : table, name : string ) -> boolean  -- Checks whether flag exists in the progression store for Lua callers
lurek.progression.hasPerk( this : table, name : string ) -> boolean  -- Checks whether perk exists in the progression store for Lua callers
lurek.progression.hasTrait( this : table, trait_name : string ) -> boolean  -- Checks whether trait exists in the progression store for Lua callers
lurek.progression.importLegacyQuestSnapshot( snapshot : table ) -> nil  -- Imports legacy quest snapshot into the progression store for Lua callers
lurek.progression.importLegacyStatsSnapshot( snapshot : table ) -> nil  -- Imports legacy stats snapshot into the progression store for Lua callers
lurek.progression.isEncumbered( this : table ) -> boolean  -- Checks whether encumbered is true for this progression object
lurek.progression.learnSkill( this : table, name : string ) -> boolean  -- Performs the `learnSkill` progression operation for Lua callers
lurek.progression.loadStore( snapshot : any ) -> nil  -- Performs the `loadStore` progression operation for Lua callers
...
lurek.progression.newStore( options : table? ) -> nil  -- Performs the `newStore` progression operation for Lua callers
lurek.progression.questCount( this : table ) -> integer  -- Performs the `questCount` progression operation for Lua callers
lurek.progression.questIds( this : table ) -> table  -- Performs the `questIds` progression operation for Lua callers
lurek.progression.questsWithStatus( this : table, wanted : string ) -> table  -- Performs the `questsWithStatus` progression operation for Lua callers
lurek.progression.recordUse( this : table, name : string ) -> nil  -- Performs the `recordUse` progression operation for Lua callers
lurek.progression.recoverActionPoints( this : table, amount : number ) -> nil  -- Performs the `recoverActionPoints` progression operation for Lua callers
lurek.progression.removeBuff( this : table, handle : string ) -> boolean  -- Removes buff from the progression store for Lua callers
lurek.progression.removeQuest( this : table, id : string ) -> boolean  -- Removes quest from the progression store for Lua callers
lurek.progression.removeTraitBuffs( this : table, trait_name : string ) -> boolean  -- Removes trait buffs from the progression store for Lua callers
lurek.progression.resetQuest( this : table, id : string ) -> boolean  -- Performs the `resetQuest` progression operation for Lua callers
lurek.progression.restore( this : table, snap : table ) -> nil  -- Performs the `restore` progression operation for Lua callers
lurek.progression.setActionPoints( this : table, max_val : number ) -> nil  -- Sets action points in the progression store for Lua callers
lurek.progression.setBase( this : table, name : string, value : number ) -> boolean  -- Sets base in the progression store for Lua callers
lurek.progression.setBerserkThreshold( this : table, value : number ) -> nil  -- Sets berserk threshold in the progression store for Lua callers
lurek.progression.setEncumbrance( this : table, cur : number, max_val : number ) -> nil  -- Sets encumbrance in the progression store for Lua callers
lurek.progression.setFlag( this : table, name : string ) -> nil  -- Sets flag in the progression store for Lua callers
lurek.progression.setInitiative( this : table, value : number ) -> nil  -- Sets initiative in the progression store for Lua callers
lurek.progression.setLevel( this : table, value : integer ) -> nil  -- Sets level in the progression store for Lua callers
lurek.progression.setLevelThresholds( this : table, thresholds : table ) -> nil  -- Sets level thresholds in the progression store for Lua callers
lurek.progression.setMax( this : table, name : string, value : number ) -> nil  -- Sets max in the progression store for Lua callers
lurek.progression.setMin( this : table, name : string, value : number ) -> nil  -- Sets min in the progression store for Lua callers
lurek.progression.setMorale( this : table, max_val : number ) -> nil  -- Sets morale in the progression store for Lua callers
lurek.progression.setPanicThreshold( this : table, value : number ) -> nil  -- Sets panic threshold in the progression store for Lua callers
lurek.progression.setQuestReward( this : table, id : string, reward : string ) -> nil  -- Sets quest reward in the progression store for Lua callers
lurek.progression.setRegen( this : table, name : string, value : number ) -> nil  -- Sets regen in the progression store for Lua callers
lurek.progression.setResistance( this : table, dtype : string, value : number ) -> nil  -- Sets resistance in the progression store for Lua callers
lurek.progression.setXP( this : table, value : number ) -> nil  -- Sets x p in the progression store for Lua callers
lurek.progression.snapshot( this : table ) -> table  -- Performs the `snapshot` progression operation for Lua callers
lurek.progression.spendActionPoints( this : table, amount : number ) -> boolean  -- Performs the `spendActionPoints` progression operation for Lua callers
lurek.progression.startQuest( this : table, id : string ) -> boolean  -- Performs the `startQuest` progression operation for Lua callers
lurek.progression.type() -> string  -- Returns the runtime type name exposed by this progression object to Lua callers
lurek.progression.typeOf( name : string ) -> boolean  -- Returns the runtime type name exposed by this progression object to Lua callers
lurek.progression.update( this : table, dt : number ) -> nil  -- Updates this operation in the progression store for Lua callers
lurek.progression.useSkill( this : table, name : string ) -> nil | boolean, string? | Success flag followed by an optional failure reason  -- Performs the `useSkill` progression operation for Lua callers
```
...
LProgressionProfile:typeOf( name : string ) -> boolean  -- Returns the runtime type name exposed by this progression object to Lua callers
```
...
```lua
LProgressionStore:acceptQuest( profile : any, quest_id : any ) -> boolean  -- Performs the `acceptQuest` progression operation for Lua callers
LProgressionStore:ackChangesThrough( revision : integer ) -> nil  -- Performs the `ackChangesThrough` progression operation for Lua callers
LProgressionStore:acquirePerk( profile : any, perk_id : any ) -> nil  -- Performs the `acquirePerk` progression operation for Lua callers
LProgressionStore:activateChallenge( profile : any, challenge_id : any, options : any ) -> nil  -- Performs the `activateChallenge` progression operation for Lua callers
LProgressionStore:addAttributeBase( profile : any, attribute_id : any, amount : any ) -> nil  -- Adds attribute base to the progression store for Lua callers
LProgressionStore:addCounter( profile : any, counter_id : any, amount : any ) -> nil  -- Adds counter to the progression store for Lua callers
LProgressionStore:addExperience( profile : any, track_id : any, amount : any ) -> nil  -- Adds experience to the progression store for Lua callers
LProgressionStore:addModifier( profile : any, target_id : any, opts : any ) -> nil  -- Adds modifier to the progression store for Lua callers
LProgressionStore:addProfileTag( id : string, tag : string ) -> nil  -- Adds profile tag to the progression store for Lua callers
LProgressionStore:addResource( profile : any, resource_id : any, amount : any ) -> nil  -- Adds resource to the progression store for Lua callers
LProgressionStore:advanceTime( seconds : number ) -> nil  -- Performs the `advanceTime` progression operation for Lua callers
LProgressionStore:applyChangeset( changeset : any ) -> nil  -- Applies changeset in the progression store for Lua callers
LProgressionStore:applyChangesetEnvelope( changeset : any, options : any ) -> nil  -- Applies changeset envelope in the progression store for Lua callers
LProgressionStore:applyPrestige( profile : any, prestige_id : any ) -> nil  -- Applies prestige in the progression store for Lua callers
LProgressionStore:applyProfileTemplate( profile : any, template_id : any ) -> nil  -- Applies profile template in the progression store for Lua callers
LProgressionStore:applyTrait( profile : any, trait_id : any ) -> boolean  -- Applies trait in the progression store for Lua callers
LProgressionStore:beginTransaction( options : any ) -> nil  -- Begins transaction in the progression store for Lua callers
LProgressionStore:canPrestige( profile : any, prestige_id : any ) -> boolean  -- Checks whether Lua callers can prestige in the progression store
LProgressionStore:canSpendResource( profile : any, resource_id : any, amount : any ) -> boolean  -- Checks whether Lua callers can spend resource in the progression store
...
LProgressionStore:collectCollectionItem( profile : any, collection_id : any, item_id : any ) -> nil  -- Performs the `collectCollectionItem` progression operation for Lua callers
LProgressionStore:compactChanges( max_records : integer ) -> nil  -- Performs the `compactChanges` progression operation for Lua callers
LProgressionStore:compileCondition( condition : table ) -> table  -- Performs the `compileCondition` progression operation for Lua callers
LProgressionStore:completeQuest( profile : any, quest_id : any ) -> boolean  -- Performs the `completeQuest` progression operation for Lua callers
LProgressionStore:countProfiles( arg1 : any ) -> number  -- Performs the `countProfiles` progression operation for Lua callers
LProgressionStore:createProfile( id : any, options : any ) -> nil  -- Creates profile in the progression store for Lua callers
LProgressionStore:debugSnapshot() -> table  -- Performs the `debugSnapshot` progression operation for Lua callers
LProgressionStore:defineAchievement( id : any, definition : any ) -> nil  -- Defines achievement in the progression store for Lua callers
LProgressionStore:defineAttribute( id : any, definition : any ) -> nil  -- Defines attribute in the progression store for Lua callers
LProgressionStore:defineChallengeTemplate( id : any, definition : any ) -> nil  -- Defines challenge template in the progression store for Lua callers
LProgressionStore:defineCollection( id : any, definition : any ) -> nil  -- Defines collection in the progression store for Lua callers
LProgressionStore:defineCounter( id : any, definition : any ) -> nil  -- Defines counter in the progression store for Lua callers
LProgressionStore:defineDerivedValue( id : any, definition : any ) -> nil  -- Defines derived value in the progression store for Lua callers
LProgressionStore:defineLeaderboard( id : any, definition : any ) -> nil  -- Defines leaderboard in the progression store for Lua callers
LProgressionStore:defineLevelTrack( id : any, definition : any ) -> nil  -- Defines level track in the progression store for Lua callers
LProgressionStore:definePerk( id : any, definition : any ) -> nil  -- Defines perk in the progression store for Lua callers
LProgressionStore:definePopulationTemplate( id : any, definition : any ) -> nil  -- Defines population template in the progression store for Lua callers
LProgressionStore:definePrestige( id : any, definition : any ) -> nil  -- Defines prestige in the progression store for Lua callers
LProgressionStore:defineProfileTemplate( id : any, definition : any ) -> nil  -- Defines profile template in the progression store for Lua callers
LProgressionStore:defineQuest( id : any, definition : any ) -> nil  -- Defines quest in the progression store for Lua callers
LProgressionStore:defineResource( id : any, definition : any ) -> nil  -- Defines resource in the progression store for Lua callers
LProgressionStore:defineSeason( id : any, definition : any ) -> nil  -- Defines season in the progression store for Lua callers
LProgressionStore:defineSkill( id : any, definition : any ) -> nil  -- Defines skill in the progression store for Lua callers
LProgressionStore:defineTrait( id : any, definition : any ) -> nil  -- Defines trait in the progression store for Lua callers
LProgressionStore:dematerializePopulationProfile( profile_id : any, options : any ) -> nil  -- Performs the `dematerializePopulationProfile` progression operation for Lua callers
LProgressionStore:drainEvents() -> table  -- Performs the `drainEvents` progression operation for Lua callers
LProgressionStore:endSeason( id : any, options : any ) -> nil  -- Performs the `endSeason` progression operation for Lua callers
LProgressionStore:ensureProfile( id : any, options : any ) -> nil  -- Performs the `ensureProfile` progression operation for Lua callers
LProgressionStore:evaluateCondition( profile : any, condition : any ) -> table  -- Performs the `evaluateCondition` progression operation for Lua callers
LProgressionStore:explainAttribute( profile : any, attribute_id : any ) -> table  -- Performs the `explainAttribute` progression operation for Lua callers
LProgressionStore:explainCondition( profile : any, condition : any ) -> table  -- Performs the `explainCondition` progression operation for Lua callers
LProgressionStore:explainDerivedValue( profile : any, id : any ) -> table  -- Performs the `explainDerivedValue` progression operation for Lua callers
LProgressionStore:exportChangesSince( revision : integer ) -> table  -- Exports changes since from the progression store for Lua callers
LProgressionStore:exportChangeset( revision : any, options : any ) -> table  -- Exports changeset from the progression store for Lua callers
LProgressionStore:exportSnapshot() -> table  -- Exports snapshot from the progression store for Lua callers
LProgressionStore:failQuest( profile : any, quest_id : any ) -> boolean  -- Performs the `failQuest` progression operation for Lua callers
LProgressionStore:generatePopulation( template_id : any, options : any ) -> nil  -- Performs the `generatePopulation` progression operation for Lua callers
LProgressionStore:getAchievement( profile : any, achievement_id : any ) -> table  -- Returns achievement from the progression store for Lua callers
LProgressionStore:getActivityFeed( query : table? ) -> table  -- Returns one typed activity-feed selection object
LProgressionStore:getAttribute( profile : any, attribute_id : any, mode : any ) -> table  -- Returns attribute from the progression store for Lua callers
LProgressionStore:getAttributeState( profile : any, attribute_id : any ) -> table  -- Returns attribute state from the progression store for Lua callers
LProgressionStore:getChallenge( profile : any, challenge_id : any ) -> table  -- Returns challenge from the progression store for Lua callers
LProgressionStore:getCollection( profile : any, collection_id : any ) -> table  -- Returns collection from the progression store for Lua callers
LProgressionStore:getCounter( profile : any, counter_id : any ) -> table  -- Returns counter from the progression store for Lua callers
LProgressionStore:getCounterState( profile : any, counter_id : any ) -> table  -- Returns counter state from the progression store for Lua callers
LProgressionStore:getDefinitionHash() -> string  -- Returns definition hash from the progression store for Lua callers
LProgressionStore:getDerivedValue( profile : any, id : any ) -> table  -- Returns derived value from the progression store for Lua callers
LProgressionStore:getExperience( profile : any, track_id : any ) -> number  -- Returns experience from the progression store for Lua callers
LProgressionStore:getExperienceToNextLevel( profile : any, track_id : any ) -> number  -- Returns experience to next level from the progression store for Lua callers
LProgressionStore:getId() -> string  -- Returns id from the progression store for Lua callers
LProgressionStore:getLeaderboardEntry( profile : any, leaderboard_id : any ) -> table  -- Returns leaderboard entry from the progression store for Lua callers
LProgressionStore:getLevel( profile : any, track_id : any ) -> number  -- Returns level from the progression store for Lua callers
LProgressionStore:getPopulation( handle_or_id : string ) -> table  -- Returns population from the progression store for Lua callers
LProgressionStore:getPopulationStatistics( handle_or_id : any, query : any ) -> table  -- Returns population statistics from the progression store for Lua callers
LProgressionStore:getPrestige( profile : any, prestige_id : any ) -> table  -- Returns prestige from the progression store for Lua callers
LProgressionStore:getProfile( id : string ) -> table  -- Returns profile from the progression store for Lua callers
LProgressionStore:getQuestState( profile : any, quest_id : any ) -> table  -- Returns quest state from the progression store for Lua callers
LProgressionStore:getResource( profile : any, resource_id : any ) -> table  -- Returns resource from the progression store for Lua callers
LProgressionStore:getRevision() -> integer  -- Returns revision from the progression store for Lua callers
LProgressionStore:getRival( profile : any, rival_profile : any ) -> table  -- Returns rival from the progression store for Lua callers
LProgressionStore:getRivalDelta( profile : any, rival_profile : any ) -> table  -- Returns rival delta from the progression store for Lua callers
LProgressionStore:getSchemaVersion() -> integer  -- Returns schema version from the progression store for Lua callers
LProgressionStore:getSeason( id : string ) -> table  -- Returns season from the progression store for Lua callers
LProgressionStore:getSeasonArchive( id : any, query : any ) -> table  -- Returns season archive from the progression store for Lua callers
LProgressionStore:getSkillCooldown( profile : any, skill_id : any ) -> table  -- Returns skill cooldown from the progression store for Lua callers
LProgressionStore:getSkillLevel( profile : any, skill_id : any ) -> table  -- Returns skill level from the progression store for Lua callers
LProgressionStore:getTime() -> number  -- Returns time from the progression store for Lua callers
LProgressionStore:hasPerk( profile : any, perk_id : any ) -> boolean  -- Checks whether perk exists in the progression store for Lua callers
LProgressionStore:hasProfile( id : string ) -> boolean  -- Checks whether profile exists in the progression store for Lua callers
LProgressionStore:hasTrait( profile : any, trait_id : any ) -> boolean  -- Checks whether trait exists in the progression store for Lua callers
LProgressionStore:learnSkill( profile : any, skill_id : any ) -> nil  -- Performs the `learnSkill` progression operation for Lua callers
LProgressionStore:listAchievements( profile : any ) -> table  -- Lists achievements from the progression store for Lua callers
LProgressionStore:listChallenges( profile : any, options : any ) -> table  -- Lists challenges from the progression store for Lua callers
LProgressionStore:listCollections( profile : any ) -> table  -- Lists collections from the progression store for Lua callers
LProgressionStore:listCounters( profile : any ) -> table  -- Lists counters from the progression store for Lua callers
...
LProgressionStore:listLeaderboardRange( leaderboard_id : any, start_rank : any, limit : any ) -> table  -- Lists leaderboard range from the progression store for Lua callers
LProgressionStore:listLeaderboardTop( leaderboard_id : any, limit : any ) -> table  -- Lists leaderboard top from the progression store for Lua callers
LProgressionStore:listModifiers( profile : any ) -> table  -- Lists modifiers from the progression store for Lua callers
LProgressionStore:listPopulationProfiles( handle_or_id : any, query : any ) -> table  -- Lists population profiles from the progression store for Lua callers
LProgressionStore:listPrestiges( profile : any ) -> table  -- Lists prestiges from the progression store for Lua callers
LProgressionStore:listProfiles( arg1 : any ) -> table  -- Lists profiles from the progression store for Lua callers
LProgressionStore:listRivals( profile : any ) -> table  -- Lists rivals from the progression store for Lua callers
LProgressionStore:listSeasons( query : table? ) -> table  -- Lists seasons from the progression store for Lua callers
LProgressionStore:listTraits( profile : any ) -> table  -- Lists traits from the progression store for Lua callers
LProgressionStore:loadSnapshot( snapshot : any ) -> nil  -- Performs the `loadSnapshot` progression operation for Lua callers
LProgressionStore:materializePopulationProfile( profile_id : any ) -> nil  -- Performs the `materializePopulationProfile` progression operation for Lua callers
LProgressionStore:pausePopulation( handle_or_id : string ) -> nil  -- Performs the `pausePopulation` progression operation for Lua callers
LProgressionStore:pinRival( profile : any, rival_profile : any, options : any ) -> nil  -- Performs the `pinRival` progression operation for Lua callers
LProgressionStore:refillResource( profile : any, resource_id : any, amount : any ) -> nil  -- Performs the `refillResource` progression operation for Lua callers
LProgressionStore:refreshQuestLifecycle( profile : any ) -> nil  -- Performs the `refreshQuestLifecycle` progression operation for Lua callers
LProgressionStore:regeneratePopulation( handle_or_id : any, options : any ) -> nil  -- Performs the `regeneratePopulation` progression operation for Lua callers
LProgressionStore:removeDerivedValue( id : string ) -> boolean  -- Removes derived value from the progression store for Lua callers
LProgressionStore:removeModifier( profile : any, handle : any ) -> nil  -- Removes modifier from the progression store for Lua callers
LProgressionStore:removePopulation( handle_or_id : any, options : any ) -> nil  -- Removes population from the progression store for Lua callers
LProgressionStore:removeProfile( id : any, opts : any ) -> nil  -- Removes profile from the progression store for Lua callers
LProgressionStore:removeProfileMetadata( id : any, key : any ) -> nil  -- Removes profile metadata from the progression store for Lua callers
LProgressionStore:removeProfileTag( id : any, tag : any ) -> nil  -- Removes profile tag from the progression store for Lua callers
LProgressionStore:removeTrait( profile : any, trait_id : any ) -> boolean  -- Removes trait from the progression store for Lua callers
LProgressionStore:resumePopulation( handle_or_id : string ) -> nil  -- Performs the `resumePopulation` progression operation for Lua callers
LProgressionStore:revealQuest( profile : any, quest_id : any ) -> nil  -- Performs the `revealQuest` progression operation for Lua callers
LProgressionStore:setAttributeBase( profile : any, attribute_id : any, value : any ) -> nil  -- Sets attribute base in the progression store for Lua callers
LProgressionStore:setChallengeProgress( profile : any, challenge_id : any, value : any ) -> nil  -- Sets challenge progress in the progression store for Lua callers
LProgressionStore:setCounter( profile : any, counter_id : any, value : any ) -> nil  -- Sets counter in the progression store for Lua callers
LProgressionStore:setExperience( profile : any, track_id : any, value : any ) -> nil  -- Sets experience in the progression store for Lua callers
LProgressionStore:setLevel( profile : any, track_id : any, level : any ) -> nil  -- Sets level in the progression store for Lua callers
LProgressionStore:setProfileMetadata( id : any, key : any, value : any ) -> nil  -- Sets profile metadata in the progression store for Lua callers
LProgressionStore:setQuestObjective( profile : any, quest_id : any, objective_id : any, value : any ) -> nil  -- Sets quest objective in the progression store for Lua callers
LProgressionStore:setQuestObjectiveStatus( profile : any, quest_id : any, objective_id : any, status : any ) -> nil  -- Sets quest objective status in the progression store for Lua callers
LProgressionStore:setQuestObjectiveVisibility( profile : any, quest_id : any, objective_id : any, visible : any ) -> nil  -- Sets quest objective visibility in the progression store for Lua callers
LProgressionStore:setResource( profile : any, resource_id : any, value : any ) -> nil  -- Sets resource in the progression store for Lua callers
LProgressionStore:setTime( seconds : number ) -> nil  -- Sets time in the progression store for Lua callers
LProgressionStore:simulatePopulationUntil( handle_or_id : any, logical_time : any, options : any ) -> nil  -- Performs the `simulatePopulationUntil` progression operation for Lua callers
LProgressionStore:spendResource( profile : any, resource_id : any, amount : any ) -> nil  -- Performs the `spendResource` progression operation for Lua callers
LProgressionStore:startSeason( id : any, options : any ) -> nil  -- Performs the `startSeason` progression operation for Lua callers
LProgressionStore:stats() -> table  -- Performs the `stats` progression operation for Lua callers
LProgressionStore:submitScore( profile : any, leaderboard_id : any, score : any ) -> nil  -- Performs the `submitScore` progression operation for Lua callers
LProgressionStore:type() -> string  -- Returns the runtime type name exposed by this progression object to Lua callers
LProgressionStore:typeOf( name : string ) -> boolean  -- Returns the runtime type name exposed by this progression object to Lua callers
LProgressionStore:unlockAchievement( profile : any, achievement_id : any ) -> nil  -- Performs the `unlockAchievement` progression operation for Lua callers
LProgressionStore:update( dt : any, opts : any ) -> nil  -- Updates this operation in the progression store for Lua callers
LProgressionStore:updatePopulation( handle_or_id : any, dt : any, options : any ) -> nil  -- Updates population in the progression store for Lua callers
LProgressionStore:updateProfile( id : any, patch : any ) -> nil  -- Updates profile in the progression store for Lua callers
LProgressionStore:useSkill( profile : any, skill_id : any ) -> nil  -- Performs the `useSkill` progression operation for Lua callers
LProgressionStore:validate() -> boolean  -- Validates this operation using the progression store rules for Lua callers
LProgressionStore:validateCondition( condition : table ) -> boolean  -- Validates condition using the progression store rules for Lua callers
...
LProgressionStore:validatePopulationTemplate( id : string ) -> boolean  -- Validates population template using the progression store rules for Lua callers
```
...
```lua
LProgressionTransaction:addCounter( profile : any, counter_id : any, amount : any ) -> nil  -- Adds counter to the progression store for Lua callers
LProgressionTransaction:addExperience( profile : any, track_id : any, amount : any ) -> nil  -- Adds experience to the progression store for Lua callers
LProgressionTransaction:addModifier( profile : any, target_id : any, opts : any ) -> nil  -- Adds modifier to the progression store for Lua callers
...
LProgressionTransaction:setAttributeBase( profile : any, attribute_id : any, value : any ) -> nil  -- Sets attribute base in the progression store for Lua callers
LProgressionTransaction:setCounter( profile : any, counter_id : any, value : any ) -> nil  -- Sets counter in the progression store for Lua callers
LProgressionTransaction:setQuestObjective( profile : any, quest_id : any, objective_id : any, value : any ) -> nil  -- Sets quest objective in the progression store for Lua callers
LProgressionTransaction:setResource( profile : any, resource_id : any, value : any ) -> nil  -- Sets resource in the progression store for Lua callers
LProgressionTransaction:type() -> string  -- Returns the runtime type name exposed by this progression object to Lua callers
LProgressionTransaction:typeOf( name : string ) -> boolean  -- Returns the runtime type name exposed by this progression object to Lua callers
```
...
```lua
LQuestJournal:addEntry( text : string, tag : string? ) -> | LQuestJournalEntry | Retained journal entry after store-side indexing and trimming  -- Appends one entry to the live quest journal and returns the stored entry object
...
LReward:markApplied( external_receipt : string? ) -> | LReward | Updated reward handle after the apply transition  -- Marks this claimed reward as applied and returns the updated reward object
LReward:reject( reason : string? ) -> | LReward | Updated reward handle after the rejection transition  -- Rejects this reward and returns the updated reward object
```
...
```lua
LStatusTracker:apply( subjectId : integer, definitionId : string, sourceId : integer?, stacks : integer? ) -> | integer | Status instance id  -- Applies a status to a subject and returns its stable runtime instance id
LStatusTracker:clear() -> nil  -- Removes all definitions, instances, and queued events
LStatusTracker:define( definition : table ) -> nil  -- Registers or replaces one status definition
...
LStatusTracker:get( instanceId : integer ) -> | table? | Status instance record, or nil when missing  -- Returns one active status instance by runtime id
LStatusTracker:has( subjectId : integer, definitionOrTag : string ) -> | boolean | Whether a matching instance exists  -- Checks whether a subject has a status with the requested definition id or tag
LStatusTracker:list( subjectId : integer, filter : table? ) -> | table | Status instance records  -- Lists active status instances attached to one subject and matching all optional filters
LStatusTracker:remove( instanceId : integer ) -> | boolean | True when an instance was removed  -- Removes one active status instance
LStatusTracker:removeByDefinition( subjectId : integer, definitionId : string ) -> | integer | Number of removed instances  -- Removes every matching definition instance from one subject
LStatusTracker:removeByTag( subjectId : integer, tag : string ) -> | integer | Number of removed instances  -- Removes every instance carrying a copied tag from one subject
LStatusTracker:restore( snapshot : table ) -> nil  -- Restores definitions, active instances, and ID allocation from a snapshot
LStatusTracker:setPaused( instanceId : integer, paused : boolean ) -> | boolean | True when the instance exists  -- Pauses or resumes one status instance's lifecycle timers
LStatusTracker:setRemaining( instanceId : integer, seconds : number? ) -> | boolean | True when the instance exists  -- Sets one status instance's remaining duration; nil makes it infinite
...
LStatusTracker:typeOf( name : string ) -> | boolean | Whether the name matches  -- Checks whether this handle matches `LStatusTracker` or `LObject`
LStatusTracker:update( dt : number ) -> | integer | Number of events currently queued after the update  -- Advances finite durations and periodic tick timers by dt seconds
```
...
```lua
lurek.province.clearProperties( id : integer )  -- Removes all properties, attributes, and flags for a province
...
lurek.province.getAttr( id : integer, key : string ) -> string  -- Gets a string attribute from a province. Returns nil if not set
lurek.province.getProperty( id : integer, key : string ) -> number  -- Gets a numeric property from a province. Returns nil if not set
lurek.province.hasFlag( id : integer, bit : integer ) -> boolean  -- Checks whether a flag bit is set on a province
...
lurek.province.newFromTiled( name : string, filename : string, opts : table? ) -> LProvinceRegistry  -- Creates a polygon-backed province registry from a Tiled TMX, TMJ, or JSON object map
lurek.province.newGrid( filename : string ) -> LProvinceGrid  -- Loads a province id grid from a GameFS-authorized encoded image
...
lurek.province.setAttr( id : integer, key : string, value : string )  -- Sets a string attribute on a province
lurek.province.setFlag( id : integer, bit : integer, value : boolean )  -- Sets a single flag bit (0â€“63) on a province
lurek.province.setProperty( id : integer, key : string, value : number )  -- Sets a numeric property on a province. Game logic defines the semantics of each key
...
LProvinceRegistry:borderSegmentsWhere( opts : table? ) -> table  -- Returns border segments filtered by province id and/or border type
LProvinceRegistry:drawCapitalPath( route : integer[], opts : table?|Draw options: mode="line" ) -> integer  -- Emits render commands for a route by connecting consecutive province capitals. Pass the route table returned by `findRoute`; pathfinding itself stays in the routing helpers. Options: mode ("line"|"bezier"), color ({r,g,b,a?} in 0..1), width, pixel_size, curve_offset, and segments
...
LProvinceRegistry:findRoute( from_id : integer, to_id : integer, cost_fn : function? ) -> table  -- Finds a route between two provinces by adapting registry adjacency to pathfind graph routing. Uses BFS by default or Dijkstra when `cost_fn` is supplied
LProvinceRegistry:findRoutes( pairs : table, cost_fn : function? ) -> table  -- Finds routes for a batch of `{from, to}` pairs by adapting registry adjacency to pathfind graph routing
...
LProvinceRegistry:getAt( x : integer, y : integer ) -> integer  -- Returns the province ID at the given grid cell coordinates. Raster maps sample the cell directly; polygon maps sample the cell center `(x + 0.5, y + 0.5)`. Returns 0 if unowned
...
LProvinceRegistry:getPolygonCount( province_id : integer? ) -> integer  -- Returns the total polygon component count, or the component count for one province
LProvinceRegistry:getProvince( id : integer ) -> table  -- Returns a snapshot table describing a single province: its ID, revision, style (political_color, terrain_type, border_style, fog_state, visibility_state, visual_state), centroid, capital marker, and custom attributes
LProvinceRegistry:getProvincePolygons( province_id : integer ) -> table  -- Returns read-only polygon component geometry for one province
...
LProvinceRegistry:isConnected( from_id : integer, to_id : integer ) -> boolean  -- Returns true when there is at least one pathfind graph route between two provinces
LProvinceRegistry:pickProvince( map_x : number, map_y : number ) -> integer  -- Returns the province under a floating-point map coordinate, or nil for a gap/outside
...
LProvinceRegistry:render( opts : table?|Render options: backend ("commands"|"gpu"|"segments"?), map_mode (string?), x/y/zoom/pixel_size/screen_w/screen_h (number?), tint ({r,g,b,a?}?), province_tints (table<integer,{r,g,b,a?}>?), segment_reuse_cache (boolean?), terrain_texture (LImage?), terrain_texture_scale/terrain_texture_strength (number?), edge_gradient_radius (output pixels?), edge_gradient_strength/edge_gradient_softness (number?), edge_gradient_color ({r,g,b,a?}?), border_palette ({province_color|land_color,coast_color,country_color,sea_darken}?), province_border_color/coast_border_color/country_border_color ({r,g,b,a?}?), sea_border_darken (number?), visual_effects ({ enabled, border_noise, water, weather, fog, climate }?), draw_fills/draw_borders/draw_labels/draw_capitals/draw_roads (boolean?), border_width (number?), zoom_mode ("auto"|"strategic" )  -- Renders the province map to the screen using the current camera and style settings. Generates draw commands for fills, borders, labels, and capitals based on the provided options. Optional `tint` multiplies all province fill colours for this render only, while `province_tints` supplies render-time fill colour overrides keyed by province id without mutating the registry
LProvinceRegistry:resolveMapModeColors( modeOrName : string|nil, opts : table? ) -> table  -- Resolves effective province fill colors for a map mode without rendering
...
LProvinceRegistry:setBorderClass( a : integer, b : integer, border_type : integer )  -- Backward-compatible alias for setBorderType. Sets the border type ID
LProvinceRegistry:setBorderPairStyle( a : integer, b : integer, style : table ) -> boolean  -- Sets the style override for a specific adjacency pair, including optional color, thickness, and semantic flags
...
LProvinceRegistry:setBorderType( a : integer, b : integer, border_type : integer )  -- Sets the border type ID between two adjacent provinces. Register types first with registerBorderType
...
LProvinceRegistry:setMapMode( name : string ) -> boolean  -- Switches the active map mode to a previously registered mode name
...
LProvinceRegistry:setShader( shader : LShader? )  -- Binds or clears a `mapviz` shader for command-rendered province visualization
...
LProvinceRegistry:setVisualState( id : integer, state : table ) -> boolean  -- Sets climate, weather, and shader-effect metadata for a province without moving simulation rules into `province`
...
LProvinceRegistry:viewportRect( opts : table? ) -> table  -- Computes the province-space viewport rectangle used by province rendering and culling. The returned table can be passed to minimap:setViewportRect(rect.x, rect.y, rect.w, rect.h)
```
...
*Coverage: 163/163 items documented (100%)*
...
lurek.raycaster.buildMultiLevelSceneFromField( params : table, field : LTileField, opts : table?, lights : table?, sprites : table|LSpriteManager?, wallTextures : table? ) -> integer  -- Builds a multilevel raycaster scene from tilefield blockers, slots, holes, surfaces, and tile light emitters
...
lurek.raycaster.drawLastScene( width : integer, height : integer ) -> LImageData  -- Rasterizes the most recently built raycaster scene to raw image data
...
lurek.raycaster.newView( opts : table? ) -> LRaycasterView  -- Creates an isolated camera projection and picking view
...
lurek.raycaster.setShader( shader : LShader? )  -- Binds a draw-target shader to the most recently built raycaster scene when it is presented by the renderer. Pass nil to clear
```
...
LMultiLevelGrid:clearPickAttr( x : integer, y : integer, surface : string, key : string? )  -- Clears one arbitrary pick attribute or the whole surface channel from one active-level cell
...
LMultiLevelGrid:getPickAttr( x : integer, y : integer, surface : string, key : string ) -> string  -- Reads one arbitrary pick attribute from one active-level surface cell
...
LMultiLevelGrid:patchCells( patches : table )  -- Atomically patches render cells and surface overrides across persistent levels
...
LMultiLevelGrid:setPickAttr( x : integer, y : integer, surface : string, key : string, value : string )  -- Sets one arbitrary pick attribute on one active-level surface cell
LMultiLevelGrid:setWallFeatureCell( x : integer, y : integer, feature : table|Feature table {kind="half"|"window" )  -- Attaches a render-only wall feature descriptor to a blocking cell on the active level
...
```lua
LRaycaster:addParticleEmitter( emitter : table )  -- Adds a projected raycaster particle emitter that spawns during scene builds
...
LRaycaster:castFloorRow( camX : number, camY : number, dirX : number, dirY : number, planeX : number, planeY : number, row : integer, screenWidth : integer?, screenHeight : integer? ) -> table  -- Computes floor/ceiling texture UV coordinates for a single scanline row
...
LRaycaster:clearParticleEmitters()  -- Removes all projected particle emitters from this map
LRaycaster:clearPickAttr( x : integer, y : integer, surface : string, key : string? )  -- Clears one arbitrary pick attribute or the whole channel from a raycaster surface cell
...
LRaycaster:getCeilingMaterialCell( x : integer, y : integer ) -> table  -- Returns the ceiling material override for one cell, or nil when none is set
...
LRaycaster:getFloorMaterialCell( x : integer, y : integer ) -> table  -- Returns the floor material override for one cell, or nil when none is set
...
LRaycaster:getPickAttr( x : integer, y : integer, surface : string, key : string ) -> string  -- Reads one arbitrary pick attribute from a raycaster surface cell
...
LRaycaster:getWallMaterial( cellValue : integer ) -> table  -- Returns the material override for a wall tile type, or nil when none is set
...
LRaycaster:patchCells( patches : table )  -- Atomically patches render cells, wall features, and per-cell surface overrides
...
LRaycaster:setCeilingMaterialCell( x : integer, y : integer, material : table? )  -- Assigns a render material override to one ceiling cell. Pass nil to clear
...
LRaycaster:setFloorMaterialCell( x : integer, y : integer, material : table? )  -- Assigns a render material override to one floor cell. Pass nil to clear
...
LRaycaster:setPickAttr( x : integer, y : integer, surface : string, key : string, value : string )  -- Sets one arbitrary pick attribute on a raycaster surface cell
...
LRaycaster:setWallFeatureCell( x : integer, y : integer, feature : table|Feature table {kind="half"|"window" )  -- Attaches a render-only wall feature descriptor to a blocking cell
LRaycaster:setWallMaterial( cellValue : integer, material : table? )  -- Assigns a render material override to a wall tile type. Pass nil to clear
...
```lua
LRaycasterView:build( source : any, inputs : table? ) -> integer  -- Builds an isolated projection from `LRaycaster` or `LMultiLevelGrid`
LRaycasterView:buildFromAdapter( source : any, adapter : LSceneAdapter, opts : table? ) -> integer  -- Builds using inputs resolved from an `LSceneAdapter`
LRaycasterView:clear()  -- Clears the last projection without changing view configuration
LRaycasterView:getCameraState() -> table  -- Returns the current camera values
LRaycasterView:getDepthAt( screen_x : number ) -> number  -- Returns the last-built wall depth for a screen X coordinate
...
LRaycasterView:pick( screen_x : number, screen_y : number ) -> table  -- Picks against the exact source and depth snapshot retained by the last build
LRaycasterView:queue( opts : table? ) -> integer  -- Queues this view without changing the legacy global raycaster output
LRaycasterView:setCameraState( camera : table )  -- Replaces the camera used by subsequent builds
LRaycasterView:setQuality( quality : table )  -- Sets ray count and maximum distance for subsequent builds
LRaycasterView:setShader( shader : LShader? )  -- Sets or clears the per-view raycaster shader
LRaycasterView:setViewport( viewport : table )  -- Replaces the screen composition rectangle
LRaycasterView:type() -> string  -- Returns the runtime type name for this raycaster projection view
LRaycasterView:typeOf( name : string ) -> boolean  -- Checks whether this handle matches a supported raycaster view type
```
...
LSceneAdapter:addModel( model : LObjModel|LVoxelModel, x : number, y : number, opts : table? )  -- Adds a static OBJ or MagicaVoxel model instance entry
...
LSceneAdapter:bindBodyModel( body : LBody, model : LObjModel|LVoxelModel, opts : table? )  -- Binds an OBJ or MagicaVoxel model instance to a live physics body
...
LSpriteManager:clearAttr( id : integer, key : string? )  -- Clears one arbitrary string attribute or all attrs from the sprite
LSpriteManager:getAttr( id : integer, key : string ) -> string  -- Reads one arbitrary string attribute from the sprite
LSpriteManager:remove( id : integer )  -- Removes a sprite by its id. This method is available to Lua scripts
LSpriteManager:setAttr( id : integer, key : string, value : string )  -- Sets one arbitrary string attribute on the sprite
...
```lua
lurek.serialize.applyDefaults( value : any, schema : table ) -> table  -- Merges a schema's default values into a data table, filling in any missing fields without overwriting existing ones. Use this to ensure game config or save data always has complete fields even when the user provides only partial overrides
lurek.serialize.canonicalEncode( value : any, opts : table? ) -> string  -- Encodes any supported Lua value as compact canonical JSON with recursively sorted map keys
lurek.serialize.canonicalHash( value : any, opts : table? ) -> string  -- Returns a deterministic 64-bit FNV-1a hash of the canonical value encoding
lurek.serialize.decode( payload : string, format : string?, opts : table? ) -> table  -- Universal decoder that parses a string payload into a Lua table using the specified format. If no format is given, auto-detects from the content. Supports JSON, TOML, CSV, XML, INI, and MessagePack. Use this as a single entry point when handling files of varying or unknown formats
lurek.serialize.decodeChangeSet( payload : string, format : string?, opts : table? ) -> table  -- Decodes and validates a ChangeSet transport payload into a Lua table
lurek.serialize.decodeMsgPack( bytes : string ) -> table  -- Decodes a binary MessagePack string back into a Lua table. Use this to read save files, network packets, or any data previously encoded with encodeMsgPack
lurek.serialize.decodeXml( text : string ) -> table  -- Parses an XML string into a Lua table structure. Elements become nested tables with tag names as keys. Useful for loading Tiled map exports, SVG data, UI layout definitions, or other XML-based game assets
...
lurek.serialize.encode( value : any, format : string, opts : table? ) -> string  -- Universal encoder that serializes a Lua value into the specified format. Supports JSON, TOML, CSV, and MessagePack. Returns a string (text for JSON/TOML/CSV, binary for MessagePack). Use this as a single entry point for all serialization needs
lurek.serialize.encodeChangeSet( value : table, format : string, opts : table? ) -> string  -- Encodes a validated `lurek.event` ChangeSet table for save or network transport
lurek.serialize.encodeMsgPack( value : table ) -> string  -- Encodes a Lua table into a compact binary MessagePack string. MessagePack is faster and smaller than JSON, making it ideal for save files, network packets, or any scenario where performance matters more than human readability. The argument must be a table
lurek.serialize.fromCsv( text : string, delimiter : string?, hasHeaders : boolean? ) -> table  -- Parses a CSV string into a Lua table (array of rows). Each row is either a keyed table (when headers are present) or an indexed array of field values. Useful for loading spreadsheet exports, leaderboard data, or tabular game data
lurek.serialize.fromIni( text : string ) -> table  -- Parses an INI-format string into a Lua table. Sections become nested tables, and key-value pairs become string fields. Useful for legacy config files or simple settings
lurek.serialize.fromJson( text : string ) -> table  -- Parses a JSON string into a Lua table. Use this to load configuration files, network responses, or any structured data stored as JSON
lurek.serialize.fromToml( text : string ) -> table  -- Parses a TOML string into a Lua table. Ideal for loading game configuration files, level definitions, and engine settings stored in TOML format
lurek.serialize.toCsv( value : table, delimiter : string?, hasHeaders : boolean? ) -> string  -- Serializes a Lua table (array of row tables) into a CSV-formatted string. Each row table should have consistent keys or be an indexed array. Use this to export leaderboards, save tabular data, or generate spreadsheet-compatible output
lurek.serialize.toJson( value : any, pretty : boolean? ) -> string  -- Serializes a Lua value (table, string, number, boolean, or nil) into a JSON string. Useful for saving game state, writing config files, or preparing network payloads
lurek.serialize.toToml( value : table ) -> string  -- Serializes a Lua table into a TOML-formatted string. Use this to write configuration files, save structured settings, or export data in a human-readable format
lurek.serialize.validate( value : any, schema : table ) -> boolean  -- Validates a Lua value against a schema table. The schema defines expected types, required fields, and constraints. Returns a success boolean and an optional error message string describing the first validation failure. Use this to verify save data integrity or user-provided configuration before processing
```
...
lurek.spine.skeletonFromJson( json : string ) -> LSkeleton  -- Parses a Spine or DragonBones JSON string into a full runtime skeleton
```
...
LSkeleton:applyLoadoutVisuals( loadout : LLoadout ) -> integer  -- Applies visual attachment mappings from an ECS loadout to matching skeleton slots
LSkeleton:bindAtlas( atlas : LSpriteAtlas ) -> integer  -- Binds all atlas entries as sprite-region attachment sources by name
LSkeleton:bindPhysics( world : LWorld, parts : table, opts : table? ) -> table  -- Creates physics bodies for skeleton parts and connects child parts to parent parts with joints
...
LSkeleton:buildAnimation( name : string, duration : number, tracks : table|Array of `{bone=<name ) -> LSkeletonAnimation  -- Builds a full skeleton animation from bone tracks keyed by bone name or index
...
LSkeleton:getAttachmentSource( slot : string ) -> table|nil  -- Returns the neutral visual source assigned to a slot/source key
...
LSkeleton:setAttachmentSource( slot : string, source : table )  -- Assigns a neutral visual source to a slot name, attachment name, or `slot:attachment` key
...
```lua
LSkeletonAnimation:addBoneTrack( bone_idx : integer, keys : table )  -- Adds many keyframes for one bone from an array of key tables
...
lurek.sprite.newAtlasFromImage( image : LImageData, atlas_json : string ) -> LSpriteAtlas  -- Parses atlas JSON for an existing `LImageData` source
...
lurek.sprite.newAutoTileSheet( image : LImageData, layout : string, opts : table ) -> LSpriteAutoTileSheet  -- Creates an autotile sheet descriptor from an image source, layout, and tile options
lurek.sprite.newBatch( texture : LImage, max : integer? ) -> LSpriteBatch  -- Creates a sprite-owned batch that draws many instances of one render texture
lurek.sprite.newNineSlice( image : LImage, top : number, right : number, bottom : number, left : number ) -> LNineSlice  -- Creates a 9-slice definition from an image and four border insets for scalable UI rendering
lurek.sprite.newRPGMakerSheet( tw : integer, th : integer ) -> LSpriteSheet  -- Creates a sprite sheet using RPG Maker's standard character layout (3 columns by 4 rows per character block)
...
lurek.sprite.newSheetFromImage( image : LImageData, opts : table ) -> LSpriteSheet  -- Creates a sprite sheet from an existing `LImageData` source and frame options
lurek.sprite.newSprite( texture_id : integer, x : number, y : number ) -> LSprite  -- Creates a lightweight sprite record with transform and optional normal-map metadata
...
```lua
LAtlasPacker:clear()  -- Removes all packed regions and resets packing shelves
LAtlasPacker:getDimensions() -> integer  -- Returns the current width and height of this atlas packer
LAtlasPacker:getRegion( name : string ) -> table  -- Returns the named packed atlas region, or nil if not found
LAtlasPacker:pack( name : string, w : integer, h : integer ) -> boolean  -- Packs a named region into this atlas and returns success plus an optional failure code
LAtlasPacker:regionCount() -> integer  -- Returns the number of currently packed regions
LAtlasPacker:setNineSlice( name : string, left : integer, right : integer, top : integer, bottom : integer ) -> boolean  -- Sets nine-slice insets for a previously packed region
LAtlasPacker:type() -> string  -- Returns the type name of this object
LAtlasPacker:typeOf( name : string ) -> boolean  -- Checks whether this object matches the given type name
```
...
```lua
LSprite:clearNormalMap()  -- Removes the assigned normal map from this sprite
...
LSprite:setNormalIntensity( intensity : number )  -- Sets the normal-map intensity used by lit sprite workflows
LSprite:setNormalMap( texture_id : integer )  -- Assigns the texture used as this sprite's normal map for lit sprite workflows
LSprite:setPosition( x : number, y : number )  -- Sets the sprite anchor position in pixels
LSprite:setShader( shader : LShader? )  -- Sets or clears the render-owned sprite material shader
LSprite:setShaderUniform( name : string, value : number|boolean|table )  -- Sends a uniform value to the shader bound to this sprite
...
LSpriteAnimator:play( name : string, restart : boolean? ) -> boolean  -- Plays or restarts a named animation clip
...
```lua
LSpriteAutoTileSheet:getBitmaskForTile( tile_id : integer ) -> integer  -- Returns the bitmask for a one-based tile id
...
LSpriteAutoTileSheet:getQuad( tile_id : integer ) -> table  -- Returns a one-based tile source rectangle
LSpriteAutoTileSheet:getTileCount() -> integer  -- Returns the number of logical tiles in the sheet
LSpriteAutoTileSheet:getTileForBitmask( bitmask : integer ) -> integer|nil  -- Returns a one-based tile id for a bitmask, or nil when missing
...
LSpriteAutoTileSheet:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
```
...
LSpriteSheet:toAnimationClip( opts : table? ) -> table  -- Builds an animation clip DTO from this sheet without creating playback state
LSpriteSheet:toFrames( group : string? ) -> table  -- Returns frame rectangle DTOs for all frames or a named group
...
LSvgImage:containsPoint( id : string, x : number, y : number ) -> boolean  -- Returns whether a visible SVG element contains the given document-space point
...
LSvgImage:getElementAtPoint( prefix : string, x : number, y : number ) -> string  -- Returns the first visible element matching `prefix` that contains the document-space point
...
*Coverage: 99/99 items documented (100%)*
...
LTerminal:setShader( shader : LShader? )  -- Binds or clears a render-owned UI shader for this terminal's generated render commands
...
LTerminal:trySet( col : integer, row : integer, ch : string|number, fr : number?, fg : number?, fb : number?, fa : number?, br : number?, bg : number?, bb : number?, ba : number? ) -> boolean, string  -- Strictly writes a character with colors to a specific cell and returns an explicit error string on invalid input
...
## `lurek.tilefield` {#tilefield}

> Registers `lurek.tilefield`, converting Lua tables and one-based coordinates while Rust owns field behavior.

*Coverage: 114/114 items documented (100%)*

```lua
lurek.tilefield.createLightsFromTileset( field : LTileField, slot : string, tileset : LTileSet, opts : table? ) -> table  -- Compatibility alias: creates normal render lights and occluders from tilefield refs whose tileset objects define `renderLight` or `occluder`
lurek.tilefield.createPhysicsFromTileset( field : LTileField, slot : string, tileset : LTileSet, world : LWorld, opts : table? ) -> LBody[]  -- Compatibility alias: creates physics bodies from tilefield refs whose tileset objects define `physics`
lurek.tilefield.fromProvider( provider : table ) -> LTileField  -- Builds a native tilefield from a Lua provider table with width, height, optional levels/topology, slots, modifiers, regions, and optional getCell(x,y,z)
lurek.tilefield.fromTileMap( tilemap : LTileMap, opts : table? ) -> LTileField  -- Copies a tilemap layer into a tilefield, optionally applying tileset object defaults and a ref slot
lurek.tilefield.new( opts : table ) -> LTileField  -- Creates a multi-level tilefield with explicit dimensions and topology
lurek.tilefield.newFieldMap( opts : table ) -> LTileFieldMap  -- Creates a 2D or layered map of shared tilefields
```
...
```lua
LTileField:applyModifier( x : integer, y : integer, z : integer?, modifier : string )  -- Applies a named modifier to one cell
LTileField:applyProfile( x : integer, y : integer, z : integer?, profile : string )  -- Applies a legacy profile to one cell
LTileField:applyTilesetObject( x : integer, y : integer, z : integer?, slot : string, tileset : LTileSet, opts : table? ) -> boolean  -- Applies the object archetype defaults for a tileset tile referenced from one cell
LTileField:applyTilesetObjectLayer( slot : string, tileset : LTileSet, opts : table? ) -> integer  -- Applies tileset object defaults for every referenced cell on one tilefield level
LTileField:beginEdit()  -- Starts a nested grouped edit without discarding pending dirty rectangles
LTileField:blocks( x : integer, y : integer, z : integer?, channel : string ) -> boolean  -- Returns whether a cell blocks a channel
LTileField:blocksCategory( x : integer, y : integer, z : integer?, category : string ) -> boolean  -- Returns whether one cell blocks a category
LTileField:clear()  -- Clears cells, regions, occupants, resources, buildability, and active cell data while retaining category, modifier, and slot definitions
LTileField:clearCell( x : integer, y : integer, z : integer? )  -- Clears gameplay state for one addressed cell
LTileField:clearLine( from_tbl : table, to_tbl : table, channel : string, opts : table? ) -> boolean  -- Returns true when the line between two cell tables has no blocker for a channel
LTileField:clearModifier( x : integer, y : integer, z : integer?, modifier : string ) -> boolean  -- Removes one modifier from one cell
LTileField:clearOccupant( x : integer, y : integer, z : integer? ) -> boolean  -- Clears any occupant id stored on one tile cell
LTileField:clearRef( x : integer, y : integer, z : integer?, slot : string )  -- Clears a named object/tile reference from one cell
LTileField:commitEdit( chunkSize : integer? ) -> table  -- Closes the outermost grouped edit and clears/returns its coalesced dirty rectangles
...
LTileField:defineCategory( name : string, opts : table?|Options: kind='movement|awareness|light|sun )  -- Defines or replaces a user category used by movement, awareness, light, sun, or custom systems
LTileField:defineSlot( slot : string )  -- Defines a named object slot that cells may reference
LTileField:drainDirtyRects( chunkSize : integer? ) -> table  -- Clears and returns pending dirty cell rectangles
LTileField:exportBlockLayer( channel : string, z : integer? ) -> table  -- Exports one blocker channel and level as a row-major boolean array
LTileField:exportCostLayer( channel : string, z : integer? ) -> table  -- Exports one cost channel and level as a row-major number array
LTileField:exportRefLayer( slot : string, z : integer? )  -- Exports one named object/tile reference slot and level as a row-major array
LTileField:firstBlocker( from_tbl : table, to_tbl : table, channel : string, opts : table? ) -> table|nil  -- Returns the first one-based blocking cell table between two cells, or nil
LTileField:footprintPassable( x : integer, y : integer, z : integer?, w : integer, h : integer, category : string ) -> boolean  -- Returns whether a rectangular footprint can occupy a cell anchor for a category
LTileField:getCategories() -> string[]  -- Returns the sorted names of all known cell categories
LTileField:getCategory( name : string ) -> table|nil  -- Returns category metadata, or nil when the category is unknown
LTileField:getCategoryCost( x : integer, y : integer, z : integer?, category : string ) -> number  -- Returns one effective category cost
LTileField:getCategoryFilter( x : integer, y : integer, z : integer?, category : string ) -> table  -- Returns one effective RGB category filter
LTileField:getCategoryTransmission( x : integer, y : integer, z : integer?, category : string ) -> number  -- Returns one effective category transmission multiplier
LTileField:getCell( x : integer, y : integer, z : integer? ) -> table  -- Returns a table with blockers, costs, sun occlusion, refs, and modifiers
LTileField:getCost( x : integer, y : integer, z : integer?, channel : string ) -> number  -- Returns the cost for one cell/channel
LTileField:getDirtyRects( chunkSize : integer? ) -> table  -- Returns pending dirty cell rectangles without clearing them
LTileField:getModifier( name : string ) -> table|nil  -- Returns a named tile modifier table, or nil
LTileField:getModifiers( x : integer, y : integer, z : integer? ) -> string[]  -- Returns active modifier names on one cell
LTileField:getNeighbors( x : integer, y : integer, z : integer? ) -> table  -- Returns topology-aware same-level neighbours for one cell
LTileField:getOccupant( x : integer, y : integer, z : integer? ) -> integer  -- Returns the occupant id stored on one tile cell
LTileField:getProfile( name : string ) -> table|nil  -- Returns a legacy profile table, or nil
LTileField:getRef( x : integer, y : integer, z : integer?, slot : string ) -> integer|table|nil  -- Returns a named object/tile reference from one cell, or nil
LTileField:getRefProperties( x : integer, y : integer, z : integer?, slot : string, tileset : LTileSet, opts : table? ) -> table|nil  -- Reads all tileset properties for a tile referenced from one cell
LTileField:getRefProperty( x : integer, y : integer, z : integer?, slot : string, tileset : LTileSet, property : string, opts : table? ) -> string|nil  -- Reads a tileset property for a tile referenced from one cell
LTileField:getRefPropertyBool( x : integer, y : integer, z : integer?, slot : string, tileset : LTileSet, property : string, opts : table? ) -> boolean|nil  -- Reads a tileset property for a tile referenced from one cell and parses it as a boolean
LTileField:getRefPropertyNumber( x : integer, y : integer, z : integer?, slot : string, tileset : LTileSet, property : string, opts : table? ) -> number|nil  -- Reads a tileset property for a tile referenced from one cell and parses it as a number
LTileField:getRefSlots() -> string[]  -- Returns the sorted names of every declared reference slot
LTileField:getRegionCells( name : string ) -> table  -- Returns one-based cells for a named region, or nil when it does not exist
LTileField:getRegionNames() -> table  -- Returns all region names in stable order
LTileField:getRegionProperties( name : string ) -> table  -- Returns all properties for a named region, or nil when the region does not exist
LTileField:getRegionProperty( name : string, key : string ) -> string  -- Returns one string property from a named region, or nil when absent
LTileField:getResource( x : integer, y : integer, z : integer? ) -> string  -- Returns a resource label stored on one tile cell
LTileField:getSize() -> integer  -- Returns field width, height, and level count
LTileField:getSunOcclusion( x : integer, y : integer, z : integer? ) -> number  -- Returns top-light occlusion in the inclusive range 0..1
...
LTileField:hasSlot( slot : string ) -> boolean  -- Returns true when a named object slot is declared
LTileField:hashRegion( opts : table|`{x, y, z?, w|width, h ) -> string  -- Returns a deterministic hash of gameplay facts inside one rectangular region
LTileField:inBounds( x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether one-based coordinates are inside the field
LTileField:inspectCells( coords : table ) -> table  -- Reads many one-based cells and common placement facts in one boundary crossing
LTileField:inspectFootprint( opts : table|`{x, y, z?, w|width, h ) -> table  -- Summarizes occupancy, buildability, resources, and blockers for one rectangle
LTileField:isBuildable( x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether one tile cell accepts build placement
LTileField:line( opts : table )  -- Returns topology-aware one-based cells between `from` and `to` tables
LTileField:patchCells( patches : table ) -> table  -- Atomically applies cell/profile/modifier/reference patches and returns stable dirty rectangles
LTileField:preparePatch( patches : table, expectedVersion : integer? ) -> LTileFieldBatch  -- Validates and stages cell/profile/modifier/reference patches without mutating the field
LTileField:regionContains( name : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a named region contains a one-based tile cell
LTileField:regionsAt( x : integer, y : integer, z : integer? ) -> table  -- Returns all region names that contain the addressed one-based tile cell
LTileField:removeCategory( name : string ) -> boolean  -- Removes a custom category and clears dependent cell/modifier data
LTileField:removeModifier( name : string ) -> boolean  -- Removes a named modifier and clears it from all cells
LTileField:removeProfile( name : string ) -> boolean  -- Removes a legacy profile and clears it from all cells
LTileField:removeRegion( name : string ) -> boolean  -- Removes a named region definition and its stored cell membership from this field
LTileField:removeSlot( slot : string ) -> boolean  -- Removes a named object slot and clears its references from the field
LTileField:restore( snapshot : table )  -- Transactionally replaces this tilefield state from a snapshot returned by `snapshot`; failures preserve the original state
LTileField:setBlock( x : integer, y : integer, z : integer?, channel : string, blocked : boolean )  -- Sets whether a cell blocks a channel
LTileField:setBuildable( x : integer, y : integer, z : integer?, buildable : boolean )  -- Sets whether one tile cell accepts build placement
LTileField:setCategoryBlock( x : integer, y : integer, z : integer?, category : string, blocked : boolean )  -- Sets one category blocker on one cell
LTileField:setCategoryCost( x : integer, y : integer, z : integer?, category : string, cost : number )  -- Sets one movement-cost override for a category on one cell
LTileField:setCategoryFilter( x : integer, y : integer, z : integer?, category : string, filter : table )  -- Sets one RGB category filter on one cell
LTileField:setCategoryTransmission( x : integer, y : integer, z : integer?, category : string, value : number )  -- Sets one category transmission multiplier on one cell
LTileField:setCell( x : integer, y : integer, z : integer?, cell : table )  -- Sets cell state from a table with optional `blocks`, `costs`, `sunOcclusion`, `refs`, and `modifiers`
LTileField:setCost( x : integer, y : integer, z : integer?, channel : string, cost : number )  -- Sets the cost for one cell/channel
LTileField:setFootprintOccupant( opts : table ) -> integer  -- Atomically assigns one occupant id to every cell in a rectangular footprint
LTileField:setModifier( name : string, modifier : table )  -- Registers or replaces a named tile modifier
LTileField:setOccupant( x : integer, y : integer, z : integer?, occupant : integer )  -- Stores an occupant id on one tile cell
LTileField:setProfile( name : string, profile : table )  -- Registers or replaces a legacy tilefield profile
LTileField:setRef( x : integer, y : integer, z : integer?, slot : string, value : integer|table )  -- Sets a named object/tile reference on one cell
LTileField:setRegionCells( name : string, cells : table )  -- Defines or replaces a named region from explicit one-based tile cells
LTileField:setRegionProperty( name : string, key : string, value : any )  -- Sets or clears one string property on a named region. Numbers and booleans are stringified; nil removes the property
LTileField:setRegionRect( name : string, x1 : integer, y1 : integer, x2 : integer, y2 : integer, z : integer? )  -- Defines or replaces a named region from an inclusive one-based tile rectangle
LTileField:setResource( x : integer, y : integer, z : integer?, resource : string? )  -- Sets or clears a resource label on one tile cell
LTileField:setSunOcclusion( x : integer, y : integer, z : integer?, value : number )  -- Sets top-light occlusion in the inclusive range 0..1
LTileField:snapshot() -> table  -- Captures block/cost/ref layers plus resource, buildable, and occupant cell facts
LTileField:snapshotRegion( opts : table|`{x, y, z?, w|width, h ) -> table  -- Captures a deterministic, bounded region snapshot for Lua-side save or diff logic
LTileField:summarizeResources( opts : table|`{x, y, z?, w|width, h ) -> table  -- Counts resource labels inside one rectangular footprint
LTileField:type() -> string  -- Returns the Lua-visible type name for this tilefield handle
LTileField:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
LTileField:writeBlockLayer( channel : string, z : integer?, values : table )  -- Writes one full blocker channel layer from a row-major boolean array
LTileField:writeCostLayer( channel : string, z : integer?, values : table )  -- Writes one full cost channel layer from a row-major number array
LTileField:writeRefLayer( slot : string, z : integer?, values : table )  -- Writes one full named ref layer from a row-major integer-or-nil array
```
...
LTileFieldBatch:typeOf( name : string ) -> boolean  -- Checks this userdata against `LTileFieldBatch` or `LObject`
```
...
```lua
LTileFieldMap:getField( mapX : integer, mapY : integer, mapZ : integer? ) -> LTileField  -- Returns the shared tilefield at one field-map coordinate
...
LTileFieldMap:inBounds( mapX : integer, mapY : integer, mapZ : integer? ) -> boolean  -- Returns whether one-based field-map coordinates are inside the field map
LTileFieldMap:setField( mapX : integer, mapY : integer, mapZ : integer?, field : LTileField )  -- Replaces one field-map slot with an existing compatible tilefield handle
LTileFieldMap:type() -> string  -- Returns the Lua-visible type name for this tilefield map handle
LTileFieldMap:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
```

---

## `lurek.tilelight` {#tilelight}
...
```lua
lurek.tilelight.compute( field : LTileField|table, opts : table? ) -> LTileLightMap  -- Creates and computes a tile light map for a shared tilefield or Lua tilefield provider table
lurek.tilelight.new( field : LTileField|table ) -> LTileLightMap  -- Creates a tile light map attached to a shared tilefield or copied from a Lua tilefield provider table
```
...
```lua
LTileLightMap:addAreaLight( opts : table|`{x,y,z?,width|w,height )  -- Adds a rectangular area light and returns its stable id
LTileLightMap:addLineLight( opts : table )  -- Adds a tile line light and returns its stable id
LTileLightMap:addPointLight( opts : table )  -- Adds a point light and returns its stable id
LTileLightMap:addRectLight( opts : table|`{x,y,z?,width|w,height ) -> integer  -- Creates a rectangular area light via `addAreaLight`
LTileLightMap:clearAreaLights()  -- Removes all rectangular area lights currently stored on this tile light map
LTileLightMap:clearLineLights()  -- Removes all line lights currently stored on this tile light map
LTileLightMap:clearPointLights()  -- Removes all point lights currently stored on this tile light map
LTileLightMap:clearRectLights()  -- Clears all rectangular area lights via `clearAreaLights`
LTileLightMap:compute( opts : table? )  -- Computes tile light from ambient, point lights, line lights, and sun light
LTileLightMap:exportLayer( z : integer? ) -> table  -- Exports one level of computed light as row-major `{r,g,b,luma}` tables
LTileLightMap:exportVolume() -> table  -- Exports all computed light levels as nested row-major tables
LTileLightMap:getLight( x : integer, y : integer, z : integer? ) -> number, number, number, number  -- Returns r, g, b, and luma for one cell
LTileLightMap:getSize() -> integer, integer, integer  -- Returns light-map width, height, and level count
LTileLightMap:removeAreaLight( id : integer ) -> boolean  -- Removes a rectangular area light by id and returns whether it existed
LTileLightMap:removeLineLight( id : integer ) -> boolean  -- Removes a line light by id and returns whether it existed
LTileLightMap:removePointLight( id : integer ) -> boolean  -- Removes a point light by id and returns whether it existed
LTileLightMap:removeRectLight( id : integer ) -> boolean  -- Removes a rectangular area light via `removeAreaLight`
LTileLightMap:setAmbient( color : table )  -- Sets ambient tile light stored on this light map
LTileLightMap:setGlobalLight( opts : table )  -- Compatibility alias for top sun light parameters used during light computation
LTileLightMap:setSunLight( opts : table|`{kind='top' )  -- Sets tile sun light parameters used during light computation
LTileLightMap:type() -> string  -- Returns the Lua-visible type name for this tile light map handle
LTileLightMap:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
LTileLightMap:updateAreaLight( id : integer, opts : table )  -- Updates an existing rectangular area light by id
LTileLightMap:updateLineLight( id : integer, opts : table )  -- Updates an existing tile line light by id
LTileLightMap:updatePointLight( id : integer, opts : table )  -- Updates an existing point light by id
LTileLightMap:updateRectLight( id : integer, opts : table )  -- Updates a rectangular area light via `updateAreaLight`
```

---

## `lurek.tileset` {#tileset}
...
```lua
lurek.tileset.fromProvider( provider : table ) -> LTileSet  -- Builds a native tileset from a Lua provider table with atlas fields, objects, tileObjects, properties, and animations
lurek.tileset.newCatalog( entries : table ) -> LTileCatalog  -- Creates a catalog that resolves typed references across named tilesets
lurek.tileset.newTileSet( first_gid : integer, tile_count : integer, columns : integer, tile_width : integer, tile_height : integer, spacing : integer?, margin : integer? ) -> LTileSet  -- Creates a native tileset from atlas dimensions
```
...
LTileCatalog:getObject( reference : table ) -> table|nil  -- Resolves object archetype metadata from a typed tile or object reference
LTileCatalog:getTileset( id : string ) -> LTileSet|nil  -- Returns the tileset stored under a catalog id
LTileCatalog:getVisual( reference : table ) -> table|nil  -- Resolves render visual metadata from a typed tile or object reference
LTileCatalog:type() -> string  -- Returns the Lua-visible userdata type name for this tile catalog
LTileCatalog:typeOf( name : string ) -> boolean  -- Checks whether this catalog matches a type name
```
...
```lua
LTileSet:getAnimation( tile_id : integer ) -> table|nil  -- Returns the animation frames for one tile
LTileSet:getAutoTileId( type_name : string, bitmask : integer ) -> integer|nil  -- Resolves a four-neighbor autotile bitmask to a tile id
LTileSet:getAutoTileId8( type_name : string, bitmask : integer ) -> integer|nil  -- Resolves an eight-neighbor autotile bitmask to a tile id
LTileSet:getAutoTileMode( type_name : string ) -> string  -- Returns the autotile matching mode for a tile type
...
LTileSet:getObject( name : string ) -> table|nil  -- Returns object archetype metadata by name
LTileSet:getObjectNames() -> table  -- Returns all object archetype names in this tileset
LTileSet:getPhysicsShape( tile_id : integer ) -> string|nil  -- Returns the physics shape label for one tile
LTileSet:getProfile( tile_id : integer ) -> string|nil  -- Returns the named gameplay profile for one tile
LTileSet:getProperties( tile_id : integer ) -> table  -- Returns all custom properties for one tile
LTileSet:getProperty( tile_id : integer, name : string ) -> string|nil  -- Returns a custom tile property as a string
LTileSet:getPropertyBool( tile_id : integer, name : string ) -> boolean|nil  -- Returns a custom tile property parsed as a boolean
LTileSet:getPropertyNumber( tile_id : integer, name : string ) -> number|nil  -- Returns a custom tile property parsed as a number
LTileSet:getQuad( tile_id : integer ) -> table  -- Returns the atlas rectangle for one tile id
LTileSet:getSpacing() -> integer  -- Returns the spacing between atlas tiles in pixels
LTileSet:getTerrainProfile( name : string ) -> table|nil  -- Returns a Godot-style terrain-set profile
...
LTileSet:getTileObject( tile_id : integer ) -> string|nil  -- Returns the object archetype name mapped to one tile
LTileSet:getTileWidth() -> integer  -- Returns the width of each tile in pixels
LTileSet:removeObject( name : string ) -> boolean  -- Removes an object archetype by name
LTileSet:setAnimation( tile_id : integer, frames : table )  -- Replaces the animation frames for one tile
LTileSet:setAutoTileMode( type_name : string, mode : string )  -- Sets the autotile matching mode for a tile type
LTileSet:setAutoTileRule( type_name : string, bitmask : integer, tile_id : integer )  -- Sets a four-neighbor autotile bitmask rule for a tile type
LTileSet:setAutoTileRule8( type_name : string, bitmask : integer, tile_id : integer )  -- Sets an eight-neighbor autotile bitmask rule for a tile type
LTileSet:setObject( name : string, object : table )  -- Stores an object archetype and its visual, pathing, lighting, and custom metadata
LTileSet:setPhysicsShape( tile_id : integer, shape : string? )  -- Sets or clears the physics shape label for one tile
LTileSet:setProfile( tile_id : integer, profile : string? )  -- Sets or clears the named gameplay profile for one tile
LTileSet:setProperty( tile_id : integer, name : string, value : any )  -- Sets or clears a custom string-convertible tile property
LTileSet:setTerrainProfile( name : string, profile : table )  -- Sets a Godot-style terrain-set profile for autotile authoring
LTileSet:setTileObject( tile_id : integer, object_name : string? )  -- Assigns or clears the object archetype mapped to one tile
LTileSet:type() -> string  -- Returns the Lua-visible userdata type name for this tileset
LTileSet:typeOf( name : string ) -> boolean  -- Checks whether this tileset matches a type name
```
...
LTween:onComplete( f : function ) -> LTween  -- Sets a callback to fire when the tween completes. Returns the tween for chaining
...
```lua
LTweenChain:call( fn : function ) -> LTweenChain  -- Adds a fluent callback step that executes once at this point in the chain
...
LTweenChain:jumpTo( step : integer )  -- Jumps legacy chain cursor to given one-based step
LTweenChain:len() -> integer  -- Returns legacy step count currently stored in this tween chain
LTweenChain:loop( n : integer ) -> LTweenChain  -- Sets fluent loop count where `0` means infinite looping behavior
LTweenChain:onComplete( fn : function ) -> LTweenChain  -- Sets callback fired after the final fluent pass fully completes
LTweenChain:onLoop( fn : function ) -> LTweenChain  -- Sets callback fired when entering the next fluent loop iteration
LTweenChain:pause() -> LTweenChain  -- Pauses fluent playback while preserving timeline progress and cursor state
LTweenChain:push( opts : table ) -> integer  -- Appends a legacy scalar step to the compatibility chain
...
LTweenChain:setLooping( looping : boolean )  -- Enables/disables infinite loop compatibility mode
...
LTweenChain:tick( dt : number ) -> table  -- Advances the legacy scalar chain and returns completion events
LTweenChain:to( target : table, fields : table, dur : number, easing : string? ) -> LTweenChain  -- Adds a fluent tween step to this chain
...
LTweenChain:wait( seconds : number, callback : function? ) -> LTweenChain  -- Adds a fluent delay step to the chain timeline and keeps fluent chaining enabled
```
...
```lua
LTweenParallel:add( tw_ud : LTween )  -- Adds an existing tween handle to this parallel group. The tween becomes owned by the group
...
LTweenParallel:onComplete( f : function ) -> LTweenParallel  -- Sets a callback to fire when all tweens in this parallel group have finished. Returns the group for chaining
...
LTweenSequence:delay( seconds : number, cb : function? ) -> LTweenSequence  -- Appends a delay step to this sequence. Optionally fires a callback when the delay elapses
...
LTweenSequence:onComplete( f : function ) -> LTweenSequence  -- Sets a callback to fire when the sequence finishes all steps. Returns the sequence for chaining
...
*Coverage: 534/534 items documented (100%)*

```lua
lurek.ui.addToast( toast_table : table )  -- Adds a toast notification to the queue
lurek.ui.animateColor( widget : LUiWidget, from : table, to : table, duration : number, easing : string? ) -> nil  -- Animate widget color tint from one RGBA value to another
lurek.ui.animateRotation( widget : LUiWidget, from : number, to : number, duration : number, easing : string? ) -> nil  -- Animate widget rotation from one angle to another (in radians)
lurek.ui.animateScale( widget : LUiWidget, from_sx : number, from_sy : number, to_sx : number, to_sy : number, duration : number, easing : string? ) -> nil  -- Animate widget scale from one value to another
lurek.ui.beginDrag( widget : LUiWidget ) -> boolean  -- Begins a drag operation on a live widget handle
...
lurek.ui.destroy( widget : LUiWidget, recursive : boolean? ) -> integer  -- Destroys a widget handle and, by default, its retained descendant subtree
lurek.ui.draw()  -- Queues retained UI render commands, then invokes custom draw callbacks for widgets that registered one
lurek.ui.drawToImage( w : integer, h : integer ) -> LImageData  -- Renders the entire UI to an image buffer
lurek.ui.dropOn( target : LUiWidget ) -> boolean  -- Drops the currently dragged widget onto a target widget
...
lurek.ui.focusDirection( dx : number, dy : number ) -> boolean  -- Move focus in a spatial direction. Uses geometry to find nearest focusable widget
...
lurek.ui.getIconGlyph( name : string ) -> string|nil  -- Returns the built-in text glyph for an icon name, or nil when missing
...
lurek.ui.getStyleToken( name : string ) -> number  -- Returns the value of a named semantic style token from the active theme
...
lurek.ui.hasIcon( name : string ) -> boolean  -- Returns whether a built-in UI icon name exists
lurek.ui.keypressed( key : string ) -> boolean  -- Delivers a key press event to the UI
lurek.ui.loadLayout( def : table ) -> LUiWidget  -- Loads a UI layout from a Lua table definition
lurek.ui.loadLayoutFile( path : string ) -> LUiWidget  -- Loads a UI layout from a TOML layout file
lurek.ui.loadLayoutGameFile( path : string ) -> LUiWidget  -- Loads a UI layout from a TOML file resolved through GameFS
...
lurek.ui.newComparisonBar( label : string, current : number, target : number ) -> LLayout  -- Creates a horizontal comparison row from a label, current value, and target value
lurek.ui.newContext( opts : table? ) -> LUiContext  -- Creates an explicitly driven UI context with independent widgets and interaction state
...
lurek.ui.newGridContainer( columns : integer? ) -> LLayout  -- Creates a grid container with an optional column count
lurek.ui.newHBoxContainer() -> LLayout  -- Creates a horizontal box container that stacks children left-to-right
lurek.ui.newIcon( icon : string ) -> LLabel|nil  -- Creates a label-like widget that displays only a built-in UI icon
...
lurek.ui.newMarginContainer( top : number?, right : number?, bottom : number?, left : number? ) -> LLayout  -- Creates a padding container around one or more child widgets using CSS-style shorthand
...
lurek.ui.newRichLabel( text : string? ) -> LRichLabel  -- Creates a new rich label with simple inline formatting spans
...
lurek.ui.newSlotGrid( slots : table, columns : integer? ) -> LLayout  -- Creates a grid layout for unit slots or equipped parts
...
lurek.ui.newSplitContainer( orientation : string? ) -> LSplitPanel  -- Creates a split container alias for `newSplitPanel`
...
lurek.ui.newStatPanel( stats : table ) -> LLayout  -- Creates a compact stat panel from a table of numeric values
...
lurek.ui.renderToImage( pathOrWidth : any, widthOrHeight : integer, heightOrPath : any )  -- Renders the entire UI to a PNG image file. The canonical form is `(width, height, path)`
...
lurek.ui.setFocus( widget : LUiWidget? )  -- Sets keyboard focus to a widget, or clears focus if nil
lurek.ui.setFont( font : LFont )  -- Sets the global UI font by applying it to the root widget
lurek.ui.setSafeArea( top : number, right : number, bottom : number, left : number )  -- Supplies normalized window safe-area insets in pixels. Window discovery remains app-owned
...
lurek.ui.visibleRange( widget : LUiWidget, item_count : integer, item_height : number ) -> integer  -- Calculate the visible item range for a scrollable list widget
...
```lua
LAccordion:addSection( title : string, content : LUiWidget? )  -- Adds a collapsible section to this accordion
...
LAspectRatioContainer:setFit( fit : string )  -- Sets how the child is fit into the aspect rectangle
LAspectRatioContainer:setRatio( ratio : number )  -- Sets the child aspect ratio used by this container
```
...
LDialog:setContent( content : LUiWidget? ) -> nil  -- Sets the widget index rendered as this dialog's content
...
LDialog:setFooter( footer : LUiWidget? )  -- Assigns an optional footer content root for this dialog
...
LDialog:setModal( v : boolean ) -> nil  -- Sets whether this dialog blocks outside interaction
LDialog:setOnClose( f : function ) -> nil  -- Registers a callback fired when this dialog closes
LDialog:setResizable( value : boolean )  -- Enables or disables edge and corner resizing for this dialog
LDialog:setTitle( title : string ) -> nil  -- Sets the current title text for this dialog
```
...
```lua
LDockPanel:dock( child : LUiWidget, side : string )  -- Docks a child widget to the specified side of this dock panel
...
LDockPanel:undock( child : LUiWidget )  -- Removes a child widget from this dock panel
```
...
LGuiTable:addRow( cells : table )  -- Adds a row of data to this table widget
...
LGuiTable:setDataFrame( df : LDataFrame, opts : table? ) -> integer  -- Replaces columns and rows from a dataframe, stringifying cell values for display
LGuiTable:setOnSelect( f : function )  -- Registers a callback invoked when a table row is selected
LGuiTable:setRows( rows : table ) -> integer  -- Replaces all rows with an array of row arrays
...
LLayout:setAlign( align : string ) -> boolean  -- Sets the cross-axis alignment for children (e.g. "start", "center", "end", "stretch")
...
LLayout:setJustify( justify : string ) -> boolean  -- Sets the main-axis justification for children (e.g. "start", "center", "end", "space-between")
...
```lua
LMenuBar:addMenu( menu : LMenuItem )  -- Adds a menu (by its widget index) to this menu bar
...
LMenuBar:removeMenu( menu : LMenuItem ) -> boolean  -- Removes a menu from this menu bar by its widget index
```
...
```lua
LMenuItem:addSubItem( child : LMenuItem )  -- Adds a sub-item to this menu item for building nested menus
...
```lua
LPropertyWidget:addGroup( title : string, collapsed : boolean? ) -> integer  -- Adds a collapsible property group and returns its 1-based index
LPropertyWidget:addProperty( group : integer, name : string, value : any, valueType : string?, options : table?, readOnly : boolean? ) -> integer  -- Adds a labeled property row to the selected group
...
LPropertyWidget:getPropertyCount( group : integer ) -> integer  -- Returns the row count for a property group
LPropertyWidget:getPropertyOptions( name : string ) -> table  -- Returns the select options of the first property with `name`
LPropertyWidget:getPropertyType( name : string ) -> string  -- Returns the canonical editor type of the first property with `name`
LPropertyWidget:getPropertyValue( name : string ) -> string  -- Returns the stringified value of the first property with `name`
LPropertyWidget:isGroupCollapsed( group : integer ) -> boolean  -- Returns whether a property group is collapsed
LPropertyWidget:setLabelWidth( width : number )  -- Sets the left label column width in pixels
LPropertyWidget:setPropertyValue( name : string, value : any ) -> boolean  -- Updates the first property with `name`
LPropertyWidget:toggleGroup( group : integer ) -> boolean  -- Toggles a property group collapsed/expanded state
```
...
LRichLabel:setText( text : string )  -- Sets the rich label source text with simple `[b]` and `[color=...]` spans
```
...
LSplitPanel:setFirstChild( child : LUiWidget )  -- Sets the widget index for the first (left/top) panel
...
LSplitPanel:setSecondChild( child : LUiWidget )  -- Sets the widget index for the second (right/bottom) panel
...
```lua
LStackContainer:addTab( label : string )  -- Adds a tab/page label to this stack or tab container
...
LStackContainer:getTab( index : integer ) -> string|nil  -- Returns a tab/page label by 1-based index
LStackContainer:getTabCount() -> integer  -- Returns the number of tab/page labels in this stack or tab container
LStackContainer:setActiveIndex( index : integer ) -> boolean  -- Sets the active child page by 1-based child index
```
...
```lua
LStatusBar:addSection( text : string, width : number? )  -- Adds a labeled section to this status bar
...
LStatusBar:setSectionWidget( section_idx : integer, widget : LUiWidget? )  -- Assigns a live widget to a status bar section. The widget is reparented into the bar and clipped to that section
```
...
```lua
LTabContainer:addTab( label : string )  -- Adds a tab label to this tab container
...
LTabContainer:getTab( index : integer ) -> string|nil  -- Returns a tab label by 1-based index
LTabContainer:getTabCount() -> integer  -- Returns the number of tab labels in this tab container
LTabContainer:setActiveIndex( index : integer ) -> boolean  -- Sets the active tab page by 1-based child index
```
...
LTextArea:setMaxLength( n : integer )  -- Sets the maximum number of characters allowed in this text area
LTextArea:setPlaceholder( text : string )  -- Sets the placeholder text shown when the text area is empty
LTextArea:setText( text : string )  -- Sets the text content of this multi-line text area and moves the cursor to the end
```
...
```lua
LTheme:setStyle( widget_type : string, state : string, styleOrClass : any, styleTable : table? ) -> boolean  -- Sets a style entry for the given widget type and state, optionally restricted to a style class
...
```lua
LToolbar:addButton( id : string, tooltip : string? ) -> integer  -- Adds a new button to this toolbar and returns its 1-based index
LToolbar:addSeparator()  -- Adds a visual separator to this toolbar
LToolbar:addSpacer( size : number? )  -- Adds a flexible spacer to this toolbar
...
LTooltipPanel:setTarget( target : LUiWidget? )  -- Sets the widget index that this tooltip is attached to
LTooltipPanel:setText( text : string )  -- Sets the tooltip panel display text content
```
...
```lua
LTreeView:addNode( text : string, parent_index : integer? ) -> integer  -- Adds a new node to this tree view, optionally under a parent node
...
LTreeView:expandNode( index : integer ) -> boolean  -- Expands the node at the given 1-based index to show its children
LTreeView:getChildNodes( index : integer ) -> integer[]  -- Returns a table of 1-based child node indices for the node at the given index
LTreeView:getNodeCount() -> integer  -- Returns the total number of nodes in this tree view
LTreeView:getNodeDepth( index : integer ) -> integer  -- Returns the nesting depth of the node at the given index (0 for root nodes)
LTreeView:getNodeText( index : integer ) -> string  -- Returns the text of the node at the given 1-based index
LTreeView:getParentNode( index : integer ) -> integer  -- Returns the 1-based index of the parent of the node at the given index
LTreeView:getSelectedNode() -> integer  -- Returns the 1-based index of the currently selected node
LTreeView:isExpanded( index : integer ) -> boolean  -- Returns whether the node at the given 1-based index is currently expanded
...
LTreeView:removeNode( index : integer ) -> boolean  -- Removes the node at the given 1-based index from this tree view
LTreeView:setNodeIcon( index : integer, icon : string ) -> boolean  -- Sets the icon of the node at the given 1-based index
LTreeView:setNodeText( index : integer, text : string ) -> boolean  -- Sets the text of the node at the given 1-based index
LTreeView:setSelectedNode( index : integer ) -> boolean  -- Sets the selected node by 1-based index
LTreeView:toggleNode( index : integer ) -> boolean  -- Toggles the expanded/collapsed state of the node at the given 1-based index
```
...
LUiContext:create( widget_type : string, opts : table? ) -> LUiWidget  -- Creates a widget of `widget_type` from optional layout fields
LUiContext:destroy( widget : LUiWidget, recursive : boolean? ) -> integer  -- Destroys a live widget from this context
LUiContext:dispatchAction( action : string, value : any? ) -> boolean  -- Dispatches a gameplay-neutral UI action
LUiContext:dispatchKey( key : string ) -> boolean  -- Explicitly dispatches a key press
LUiContext:dispatchPointer( event : table|`{type="move"|"press" ) -> boolean  -- Explicitly dispatches a pointer event
LUiContext:dispatchText( text : string ) -> boolean  -- Explicitly dispatches text input
LUiContext:dispatchWheel( x : number, y : number ) -> boolean  -- Explicitly dispatches a wheel delta
LUiContext:focusDirection( direction : string ) -> boolean  -- Moves focus in a named spatial direction
LUiContext:focusNext()  -- Moves focus forward to the next focusable widget in this UI context
LUiContext:focusPrev()  -- Moves focus backward to the previous focusable widget in this UI context
LUiContext:getById( id : string ) -> LUiWidget  -- Finds a widget by id inside this context
...
LUiContext:loadLayout( path : string, opts : table? ) -> LUiWidget  -- Loads and atomically attaches a TOML layout through GameFS
LUiContext:queueDraw( opts : table? ) -> integer  -- Queues this context to its viewport or an explicit canvas and restores render state
LUiContext:renderToImage( opts : table? ) -> LImageData  -- Renders this context into CPU image data
LUiContext:setFocus( widget : LUiWidget? )  -- Sets focus to a context-owned widget or clears it with nil
LUiContext:setViewport( viewport : table )  -- Sets this context's screen rectangle
LUiContext:type() -> string  -- Returns the runtime type name for this explicit UI context handle
LUiContext:typeOf( name : string ) -> boolean  -- Checks whether this handle matches a supported UI context type
LUiContext:update( dt : number )  -- Advances this context and dispatches only its callbacks
```
...
```lua
LUiWidget:addChild( child : LUiWidget )  -- Adds a child widget to this widget's hierarchy
...
LUiWidget:clearIcon()  -- Clears this widget's assigned built-in icon
...
LUiWidget:destroy( recursive : boolean? ) -> integer  -- Destroys this widget and, by default, its retained descendant subtree
...
LUiWidget:removeChild( child : LUiWidget )  -- Removes a child widget from this widget's hierarchy
...
LUiWidget:setAriaName( name : string )  -- Sets the accessible name metadata for this widget
LUiWidget:setBindKey( key : string ) -> boolean  -- Binds this widget to a data key and reports whether the widget exists
LUiWidget:setDragEnabled( enabled : boolean ) -> boolean  -- Enables or disables pointer-initiated drag-and-drop for this widget. Defaults to disabled
LUiWidget:setDropEnabled( enabled : boolean ) -> boolean  -- Enables or disables this widget as a pointer drag-and-drop target. Defaults to disabled; enabling promotes an ignored mouse filter to `"stop"`
...
LUiWidget:setFocusGroup( group : string )  -- Sets the focus traversal group for this widget
LUiWidget:setFocusNeighbor( direction : string, target : LUiWidget? ) -> boolean  -- Sets an explicit directional focus neighbor for this widget
LUiWidget:setFocusable( value : boolean )  -- Sets whether this widget participates in keyboard focus traversal
...
LUiWidget:setIcon( icon : string ) -> boolean  -- Sets this widget's built-in UI icon by semantic name
LUiWidget:setIconPosition( position : string ) -> boolean  -- Sets where this widget's icon is placed relative to its text
LUiWidget:setIconSize( size : number ) -> boolean  -- Sets this widget's requested icon size in pixels
LUiWidget:setId( id : string )  -- Assigns a string identifier to this widget for lookup with findById
LUiWidget:setLabelFor( target : LUiWidget? )  -- Associates this label widget with another widget for accessibility naming
LUiWidget:setMargin( top : number, right : number?, bottom : number?, left : number? )  -- Sets the outer margin of this widget. Accepts 1 to 4 values (top, right?, bottom?, left?) following CSS shorthand rules
...
LUiWidget:setMouseFilter( filter : string ) -> boolean  -- Sets the mouse filter for this widget ("stop", "pass", "ignore")
...
LUiWidget:setOnDragEnd( f : function )  -- Registers a callback invoked when a drag from this widget ends
LUiWidget:setOnDragEnter( f : function )  -- Registers a callback invoked when a dragged widget enters this drop target
LUiWidget:setOnDragLeave( f : function )  -- Registers a callback invoked when a dragged widget leaves this drop target
LUiWidget:setOnDragStart( f : function )  -- Registers a callback invoked when a drag starts from this widget
LUiWidget:setOnDraw( f : function )  -- Registers a custom draw callback for this widget, invoked each frame during the draw pass
LUiWidget:setOnDrop( f : function )  -- Registers a callback invoked after a dragged widget is dropped onto this target
LUiWidget:setPadding( top : number, right : number?, bottom : number?, left : number? )  -- Sets the inner padding of this widget. Accepts 1 to 4 values (top, right?, bottom?, left?) following CSS shorthand rules
LUiWidget:setPosition( x : number, y : number )  -- Sets the local position of this widget relative to its parent
LUiWidget:setRole( role : string )  -- Sets a semantic role string for this widget
LUiWidget:setShader( shader : LShader?, opts : table? )  -- Binds or clears a render-owned UI shader for this widget subtree
LUiWidget:setShaderLayer( name : string, shader : LShader?, opts : table? )  -- Binds or clears a named render-owned UI shader layer for this widget subtree
...
LValidationEngine:addRequiredRule( id : string, pattern : string, message : string )  -- Add a required pattern rule (violation if pattern NOT found)
LValidationEngine:loadTomlRules( path : string )  -- Load validation rules from a TOML-formatted rule file
...
LValidationEngine:runFile( path : string ) -> table  -- Run validation against a single file
```
