...
*Coverage: 6160/6160 functions documented (100.0%)*
...
- [`lurek.tilemap`](#tilemap) — 30 fn, 10 classes
...
*Coverage: 203/203 items documented (100%)*
...
```lua
lurek.tilefield.fromTileMap( tilemap : LTileMap, opts : table? ) -> LTileField  -- Copies a tilemap layer into a tilefield using solid and empty profiles
...
```lua
LTileField:addPointLight( opts : table )  -- Adds a point light and returns its stable id
...
LTileField:clear()  -- Clears all cell gameplay state and computed light values
LTileField:clearCell( x : integer, y : integer, z : integer? )  -- Clears gameplay state and light values for one addressed cell
...
LTileField:clearPointLights()  -- Removes all point lights currently stored on this tilefield
LTileField:computeLight( opts : table? )  -- Computes tile light from ambient, point lights, and global top light
...
LTileField:exportLightLayer( z : integer? ) -> table  -- Exports one level of computed light as row-major `{r,g,b,luma}` tables
...
LTileField:getLight( x : integer, y : integer, z : integer? ) -> number  -- Returns r, g, b, and luma for one cell
...
LTileField:removePointLight( id : integer ) -> boolean  -- Removes a point light by id and returns whether it existed
...
LTileField:setGlobalLight( opts : table )  -- Sets top-down global light parameters used during light computation
...
LTileField:updatePointLight( id : integer, opts : table )  -- Updates an existing point light by id
```
...
```lua
lurek.visibility.lineOfAction( field : LTileField, from : table, to : table, opts : table? ) -> boolean  -- Returns whether two tilefield cells have a clear action line
lurek.visibility.lineOfSight( field : LTileField, from : table, to : table, opts : table? ) -> boolean  -- Returns whether two tilefield cells have a clear sight line
...
lurek.visibility.newTileVisibility( field : LTileField, opts : table ) -> LTileVisibility  -- Creates per-player tile visibility/action masks backed by a tilefield
```
...
```lua
LTileVisibility:actionCells( player : string, z : integer? ) -> table  -- Returns all currently actionable cells for a player, optionally filtered to a level
LTileVisibility:canActOn( player : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell is currently actionable for a player
LTileVisibility:clearAll()  -- Clears current, explored, and action masks for all players
LTileVisibility:clearPlayer( player : string )  -- Clears current, explored, and action masks for one player
LTileVisibility:computeAction( player : string, opts : table )  -- Computes one player's current action mask from a tilefield origin
LTileVisibility:computeVisible( player : string, opts : table )  -- Computes one player's current visible mask from a tilefield origin
LTileVisibility:isExplored( player : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell has been explored for a player
LTileVisibility:isVisible( player : string, x : integer, y : integer, z : integer? ) -> boolean  -- Returns whether a one-based cell is currently visible for a player
LTileVisibility:type() -> string  -- Returns the Lua-visible type name for this tile visibility handle
LTileVisibility:typeOf( name : string ) -> boolean  -- Returns whether this handle matches a supported type name
LTileVisibility:visibleCells( player : string, z : integer? ) -> table  -- Returns all currently visible cells for a player, optionally filtered to a level
```
