...
- The callbacks spec is the canonical inventory of runtime `lurek.*` hooks.
- It tells users which engine events exist for lifecycle, input, update, rendering, and shutdown behavior.
- The list makes callback names easy to discover without searching scattered source files.
- It stays synchronized with generated API metadata so the inventory does not drift.
- Full signatures live in the generated API docs, while this spec owns presence and scope.
