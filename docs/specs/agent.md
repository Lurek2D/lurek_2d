...
- `chat.rs`: Global LLM config, synchronous completion helpers, `LlmChat`, and `LlmTemplate`.
- `memory.rs`: `WorkingMemory`, `EpisodicMemory`, `SemanticMemory`, and `AgentMemory`.
...
### `chat.rs`
- `GlobalLlmConfig` is a process-wide default stored in a `Mutex`; set once via `configure()` and read by every module-level function.
- `ollama_generate` / `ollama_generate_json` call `/api/generate` synchronously using `crate::network::http::execute_request`.
- `ollama_embed` calls `/api/embeddings`; `ollama_list_models` queries `/api/tags`; `ollama_is_available` probes the base URL.
- `LlmTemplate::render` replaces `{key}` placeholders with values from a `HashMap<String, String>`, returning `Err` on missing keys.
- `LlmChat` maintains a `Vec<ChatMessage>` history; `complete()` POSTs the full history to `/api/chat` and appends the assistant reply.
...
### `memory.rs`
- `WorkingMemory` is a bounded FIFO `VecDeque<(String, serde_json::Value)>`; `push()` evicts the oldest entry when capacity is exceeded.
- `EpisodicMemory` records `Episode { tick, data }` entries; `query(filter)` does key-equality matching; `forget_before(cutoff)` prunes by tick.
- `SemanticMemory` is a `HashMap<String, serde_json::Value>` fact store; `query(filter)` matches facts that contain all filter key-value pairs.
- `AgentMemory` bundles all three; `save()` serialises to JSON at `persist_path`; `load()` restores from the same file.

### `mod.rs`
- LLM agent runtime: state, request types, error variants, background HTTP client, Ollama lifecycle management, direct LLM API, and memory primitives.
...
- `LlmTemplate::new` (`chat.rs`): Creates a template from a pattern string.
- `LlmTemplate::render` (`chat.rs`): Renders the template with a `HashMap<String, String>` substitution map.
- `LlmChat::new` (`chat.rs`): Creates an empty chat session.
- `LlmChat::set_system_prompt` (`chat.rs`): Sets the system prompt.
- `LlmChat::add_message` (`chat.rs`): Appends a message to history.
- `LlmChat::complete` (`chat.rs`): Sends history to `/api/chat` and returns the assistant reply.
- `LlmChat::clear` (`chat.rs`): Clears all history entries.
- `LlmChat::history` (`chat.rs`): Returns the full message history.
- `WorkingMemory::new` (`memory.rs`): Creates a new working memory with the given capacity.
- `WorkingMemory::push` (`memory.rs`): Inserts or updates an entry; evicts oldest on overflow.
- `WorkingMemory::get` (`memory.rs`): Returns the value for a key, or `None`.
- `WorkingMemory::forget` (`memory.rs`): Removes an entry; returns `true` if it existed.
- `WorkingMemory::get_recent` (`memory.rs`): Returns the `n` most recently inserted entries.
- `EpisodicMemory::new` (`memory.rs`): Creates an empty episodic memory.
- `EpisodicMemory::record` (`memory.rs`): Records an episode at `tick` with `data`.
- `EpisodicMemory::query` (`memory.rs`): Returns episodes matching all filter key-value pairs.
- `EpisodicMemory::forget_before` (`memory.rs`): Removes all episodes with `tick < cutoff`.
- `SemanticMemory::new` (`memory.rs`): Creates an empty semantic memory.
- `SemanticMemory::learn` (`memory.rs`): Inserts or replaces a fact.
- `SemanticMemory::recall` (`memory.rs`): Returns the fact for a key, or `None`.
- `SemanticMemory::forget` (`memory.rs`): Removes a fact; returns `true` if it existed.
- `SemanticMemory::query` (`memory.rs`): Returns facts whose value matches all filter pairs.
- `AgentMemory::new` (`memory.rs`): Creates a bundled memory with the given working capacity and optional persist path.
- `AgentMemory::save` (`memory.rs`): Serialises all banks to the persist path as JSON.
- `AgentMemory::load` (`memory.rs`): Deserialises all banks from the persist path.
...
- `LuaAISystemRuntime::run_all` (`lua_runtime.rs`): Dispatches named-agent batch tasks.
...
- `lurek.agent.configure(config)`: Sets the global LLM config. Keys: `provider`, `base_url`, `model`, `timeout_ms`, `api_key`.
- `lurek.agent.complete(prompt)`: Synchronous LLM completion; returns response text.
- `lurek.agent.completeAsync(prompt, callback)`: Async completion; calls `callback(text, err)` on completion.
- `lurek.agent.newChat()`: Returns a new `LAgentChat` stateful session.
- `lurek.agent.newTemplate(pattern)`: Returns a new `LAgentTemplate` from a `{key}` pattern string.
- `lurek.agent.completeJson(prompt)`: Synchronous JSON-format completion; returns parsed Lua table.
- `lurek.agent.embed(text)`: Returns a float embedding vector for `text`.
- `lurek.agent.isAvailable()`: Returns `true` if the global LLM server responds within 5 s.
- `lurek.agent.listModels()`: Returns a string array of available model names.
- `lurek.agent.newWorkingMemory(capacity)`: Returns a new `LWorkingMemory` with the given FIFO capacity.
- `lurek.agent.newEpisodicMemory()`: Returns a new `LEpisodicMemory`.
- `lurek.agent.newSemanticMemory()`: Returns a new `LSemanticMemory`.
- `lurek.agent.newAgentMemory(config?)`: Returns a new `LAgentMemory`. Config keys: `working_capacity`, `persist_path`.
...
### `LAgentChat` Methods
- `LAgentChat:setSystemPrompt(prompt)`: Sets the system prompt used for all completions in this session.
- `LAgentChat:addMessage(role, content)`: Appends a message to the chat history without sending a completion.
- `LAgentChat:complete()`: Sends the current history to the LLM and returns the assistant reply; appends reply to history.
- `LAgentChat:clear()`: Clears the chat history.
- `LAgentChat:getHistory()`: Returns history as an array of `{ role, content }` tables.

### `LAgentTemplate` Methods
- `LAgentTemplate:render(values)`: Substitutes `{key}` placeholders with `values` (table of string values); raises on missing keys.

### `LWorkingMemory` Methods
- `LWorkingMemory:push(key, value)`: Inserts or updates a key-value entry; evicts oldest if capacity exceeded.
- `LWorkingMemory:get(key)`: Returns the value for `key`, or `nil`.
- `LWorkingMemory:forget(key)`: Removes an entry; returns `true` if it existed.
- `LWorkingMemory:getRecent(n)`: Returns the `n` most recently inserted entries as `{ key, value }` tables.
...
- `LEpisodicMemory:record(tick, data)`: Records a new episode at `tick` with `data` table.
- `LEpisodicMemory:query(filter)`: Returns matching episodes as `{ tick, data }` tables; empty filter returns all.
- `LEpisodicMemory:forgetBefore(cutoff)`: Removes episodes with `tick < cutoff`.
- `LEpisodicMemory:len()`: Returns the number of stored episodes.

### `LSemanticMemory` Methods
- `LSemanticMemory:learn(key, value)`: Inserts or replaces a fact at `key`.
- `LSemanticMemory:recall(key)`: Returns the fact for `key`, or `nil`.
- `LSemanticMemory:forget(key)`: Removes the fact at `key`; returns `true` if it existed.
- `LSemanticMemory:query(filter)`: Returns matching facts as `{ key, value }` tables; empty filter returns all.
- `LSemanticMemory:len()`: Returns the number of stored facts.

### `LAgentMemory` Methods
- `LAgentMemory:working()`: Returns the working memory component as a `LWorkingMemory`.
- `LAgentMemory:episodic()`: Returns the episodic memory component as a `LEpisodicMemory`.
- `LAgentMemory:semantic()`: Returns the semantic memory component as a `LSemanticMemory`.
- `LAgentMemory:save()`: Serialises all memory banks to the configured `persist_path` as JSON.
- `LAgentMemory:load()`: Deserialises all memory banks from the configured `persist_path`.
