...
- `PickResult` (`struct`, `picking.rs`): Result of a successful region pick operation.
...
- `Layer` (`struct`, `types.rs`): A named rendering layer that sits above the base region map.
...
- `FogMask::state` (`fog.rs`): Return the stored fog state for a region id.
- `FogMask::set_state` (`fog.rs`): Set the fog state for a region id when it is in range.
- `FogMask::reveal` (`fog.rs`): Mark a region as visible.
- `FogMask::hide` (`fog.rs`): Mark a region as hidden.
- `FogMask::explore` (`fog.rs`): Mark a region as explored.
- `FogMask::toggle` (`fog.rs`): Toggle a region between visible and hidden.
- `FogMask::reveal_batch` (`fog.rs`): Reveal every region in the supplied iterator.
- `FogMask::visible_ids` (`fog.rs`): Return all visible region ids in ascending order.
- `FogMask::explored_ids` (`fog.rs`): Return all explored region ids in ascending order.
- `FogMask::from_visible_ids` (`fog.rs`): Build a mask that reveals the supplied region ids.
- `FogMask::count_visible` (`fog.rs`): Count visible regions in the mask.
- `FogMask::count_explored` (`fog.rs`): Count explored regions in the mask.
...
- `FogStore::explored_ids` (`fog.rs`): Return the explored region ids for a viewer when the viewer exists.
- `FogStore::state` (`fog.rs`): Return the state for a viewer or visible when the viewer has no mask.
- `FogStore::set_state` (`fog.rs`): Set the fog state for a viewer and region.
...
- `FogStore::load` (`fog.rs`): Replace a viewer mask with one built from visible region ids.
...
- `LayerStore::set_province_color` (`layer.rs`): Set a region color override for a layer and return true when the layer exists.
- `LayerStore::clear_province_colors` (`layer.rs`): Clear all region color overrides from a layer.
...
- `LayerStore::effective_color` (`layer.rs`): Resolve the effective region color by applying visible layers in z-order.
...
- `province_intensity` (`lighting.rs`): Compute the lighting intensity for a region centroid.
- `compute_intensities` (`lighting.rs`): Batch-compute light intensities for all regions.
- `terminator_alpha` (`lighting.rs`): Compute a day/night terminator alpha for a region for a soft edge.
- `load_from_toml_str` (`loader.rs`): Parse a TOML region file from a string.
- `load_from_toml_file` (`loader.rs`): Load region data from the filesystem (synchronous).
- `load_from_png_file` (`loader.rs`): Load regions from a color-indexed PNG.
- `generate_voronoi_provinces` (`loader.rs`): Generate approximate regions from Voronoi input points.
...
- `pick` (`picking.rs`): Pick the region at screen coordinate `(sx, sy)`.
