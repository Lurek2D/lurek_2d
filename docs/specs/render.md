...
- Lua API surface: `134` functions, `17` types, `136` methods
- User-facing: `true`
...
### builtin_shape_catalog_generated.rs
...
- `lurek.render.drawPath(path, modeOrOpts?, close?) -> nil`: Draws a vector path composed of moveTo, lineTo, quadTo, cubicTo, and close segments.
...
- `lurek.render.getBuiltinShapeInfo(id) -> table`: Returns metadata for one native shape template.
- `lurek.render.getCanvas() -> LCanvas`: Returns the active canvas, or nil when drawing to the screen.
- `lurek.render.getCanvasSize(ud) -> number, number`: Returns the pixel dimensions of a canvas.
...
- `lurek.render.listBuiltinShapes(filter?) -> table`: Lists the 96 native primitive-based shape templates.
- `lurek.render.loadBuiltinShape(id, opts?) -> LShape`: Creates and compiles one native shape template into the shape registry.
...
- `lurek.render.setColorMask(args) -> nil`: Sets which color channels are written during draw calls. Call with no args to enable all.
...
- `LReadbackRequest:type() -> string`: Returns the userdata type name for this surface readback request handle.
...
- `msaa_fallback` (`boolean`): True when 4x MSAA was unavailable and 1x was selected.
- `msaa_samples` (`integer`): Active screen/canvas MSAA sample count (1 or 4).
...
- `shape_cache_hits` (`integer`): Compiled shape cache hits this frame.
- `shape_cache_misses` (`integer`): Compiled shape cache misses this frame.
- `shape_instances` (`integer`): Retained-shape instances submitted this frame.
- `shape_tessellations` (`integer`): CPU retained-shape tessellations performed this frame.
- `shape_uploads` (`integer`): Compiled shape GPU uploads this frame.
...
- `LShaderPrewarmRequest:type() -> string`: Returns the userdata type name for this shader prewarm request handle.
...
- `LShape:addShape(child, opts?) -> nil`: Snapshots another shape's IR under an optional local transform.
- `LShape:arc(mode, x, y, r, astart, aend, segments?) -> nil`: Adds a filled or outlined arc command to the shape.
- `LShape:arrow(mode, x1, y1, x2, y2, width?, head?) -> nil`: Adds an arrow from `(x1,y1)` to `(x2,y2)` with a triangular head.
- `LShape:capsule(mode, x, y, w, h, radius?) -> nil`: Adds a rounded capsule (horizontal or vertical according to its dimensions).
...
- `LShape:compile(opts?) -> boolean`: Compiles the shape once on the CPU; the next frame uploads one static GPU mesh.
- `LShape:draw(x, y, rotation?, sx?, sy?, ox?, oy?) -> nil`: Renders the accumulated shape commands to the screen with optional transform.
- `LShape:drawMany(instances) -> nil`: Queues up to 250,000 instances of one compiled shape for a single compatible draw.
- `LShape:ellipse(mode, x, y, rx, ry) -> nil`: Adds an ellipse command to the shape.
- `LShape:getBounds() -> table`: Returns `{x,y,w,h,minX,minY,maxX,maxY}` in local shape coordinates.
- `LShape:getCommandCount() -> number`: Returns the number of drawing commands accumulated in this shape.
- `LShape:getDiagnostics() -> table`: Returns compile revision, geometry counters, tolerance, and warnings.
- `LShape:line(x1, y1, x2, y2) -> nil`: Adds a line segment command to the shape.
- `LShape:path(segments, opts?) -> nil`: Adds a native path with move, line, quadratic, cubic, and close verbs.
- `LShape:point(x, y, size?) -> nil`: Adds one point or disc primitive to the shape.
- `LShape:points() -> nil`: Adds many point/disc primitives from flat coordinates or `{x,y}` pairs.
...
- `LShape:regularPolygon(mode, x, y, radius, sides, rotation?) -> nil`: Adds a regular polygon, useful for icons and data-viz marks.
- `LShape:release() -> boolean`: Releases the shape slot and makes this handle stale.
- `LShape:ring(mode, x, y, outer, inner, segments?) -> nil`: Adds a ring or annulus approximation from two circles.
- `LShape:roundedRectangle(mode, x, y, w, h, rx, ry?) -> nil`: Adds a rounded rectangle command to the shape.
- `LShape:sector(mode, x, y, radius, angle1, angle2, segments?) -> nil`: Adds a filled sector or stroked arc.
- `LShape:setColor(r, g, b, a?) -> nil`: Sets the drawing color for subsequent shape commands.
- `LShape:setColorRole(role) -> nil`: Selects a semantic palette role for subsequent commands.
- `LShape:setLineWidth(w) -> nil`: Sets the line width for subsequent line-mode shape commands.
- `LShape:setPalette(palette) -> nil`: Replaces one or more semantic palette roles with normalized RGBA colors.
- `LShape:setStrokeStyle(opts) -> nil`: Sets cap, join, miter and dash parameters for subsequent stroke commands.
- `LShape:star(mode, x, y, outer, inner, points, rotation?) -> nil`: Adds a regular star with alternating outer/inner radii.
- `LShape:symbol(name, x, y, size, mode?) -> nil`: Adds a data-viz marker symbol (circle, square, diamond, triangle, cross, plus, times, asterisk, wye).
- `LShape:trail(coords, width_table) -> nil`: Adds a variable-width trail as a strip between two point lists.
...
- `LVoxelModel:getBounds() -> table`: Returns local model bounds as `{minX, minY, minZ, maxX, maxY, maxZ}`.
- `LVoxelModel:getVoxelCount() -> integer`: Returns the number of occupied source voxels.
