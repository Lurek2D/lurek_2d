...
All `lurek.*` namespace registrations use a trait-based system defined in `src/lua_api/lua_module.rs`:

- **`LuaModule` trait** — each `*_api.rs` file implements `LuaModule` with a `register(lua, state)` method that mounts its namespace onto the Lua VM.
- **`ModuleEntry` struct** — a function-pointer alternative (`ModuleEntry { name, register_fn }`) used in the static `MODULES` slice for modules that don't need a full trait impl.
- **`always!` / `gated!` macros** (in `register.rs`) — declarative helpers that build the `MODULES` slice. `always!` includes a module unconditionally; `gated!` wraps it in a `#[cfg(feature = "...")]` gate.
- **Static `MODULES` slice** — the composition root iterates this slice to register all modules in deterministic order.
- **Feature-gated modules** — modules behind `#[cfg]` attributes that cannot appear in the static slice are registered separately in `register_gated_modules()`.
