...
- Per-module reference: each module under `src/<module>/` has a matching `docs/specs/<module>.md` produced by [tools/docs/gen_module_specs.py](../../tools/docs/gen_module_specs.py). The generator fills `General Info`, `Imports`, `Files`, optional generated `Callbacks`, and `Lua API Ref`, while `Summary` remains hand-curated. Specs are the canonical module reference; [docs/architecture/](../architecture/) holds narrative architecture and [logs/reports/](../../logs/reports/) holds generated reports.
...
Each spec follows [SPEC_TEMPLATE.md](../templates/SPEC_TEMPLATE.md). Sections split into two layers:

| Layer                                     | Sections                                                       | Owner                                           | Regenerated by                                                         |
| ----------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------- | ---------------------------------------------------------------------- |
| **Manual prose** (preserved across regen) | `## Summary` | Hand-written by the module owner / `Doc-Writer` | Never overwritten by the generator |
| **Auto-collected** (rebuilt from source)  | `## General Info`, `## Imports`, `## Files`, optional `## Callbacks`, `## Lua API Ref` | Source code + Lua binding metadata | [tools/docs/gen_module_specs.py](../../tools/docs/gen_module_specs.py) |
...
| `grep`       | [grep.md](grep.md)             | Literal-first text search engine for game content with lightweight regex/glob/fuzzy helpers.            | â€”               |
...
3. **Hand-write the manual sections** â€” primarily `Summary` â€” using [SPEC_TEMPLATE.md](../templates/SPEC_TEMPLATE.md) as the structural reference.
...
- [SPEC_TEMPLATE.md](../templates/SPEC_TEMPLATE.md) â€” canonical section list and authoring guide for a single spec.
