...
- Per-module reference: each module under `src/<module>/` has a matching `docs/specs/<module>.md` produced by [tools/docs/gen_module_specs.py](../../tools/docs/gen_module_specs.py). The generator fills `General Info`, `Imports`, `Files`, and `Lua API Ref`, while `Summary` remains hand-curated. Specs are the canonical module reference; [docs/architecture/](../architecture/) holds narrative architecture and [logs/reports/](../../logs/reports/) holds generated reports.
...
| **Auto-collected** (rebuilt from source)  | `## General Info`, `## Imports`, `## Files`, `## Lua API Ref` | Source code + Lua binding metadata | [tools/docs/gen_module_specs.py](../../tools/docs/gen_module_specs.py) |
