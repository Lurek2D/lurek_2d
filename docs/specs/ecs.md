...
- Organization boundary: use `lurek.ecs` for entities, components, systems, tags, blueprints, layers, relationships, and snapshots inside a world or scene. It does not choose the active game state or run screen transitions; use [`lurek.scene`](scene.md) for menus, screens, and stack navigation, and let a scene own or reference an ECS universe when needed.
