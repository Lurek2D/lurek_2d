...
> **Note:** DSP effects (`dsp.rs`), offline processing (`offline.rs`), visualizer (`visualizer.rs`), and MIDI (`midi.rs`, `midi_player.rs`) have been extracted to dedicated modules `src/dsp/` and `src/midi/`. See [dsp.md](dsp.md) and [midi.md](midi.md). Backward-compatible re-exports remain in `src/audio/mod.rs`.
