...
Registered by `src/lua_api/signal_api.rs` under the `lurek.signal` namespace. Provides eight top-level functions and a `Signal` UserData type with seven methods.
...
| `input`   | Related      | `input` manages hardware-level key/mouse/gamepad state; `signal` provides a user-programmable message queue and pub-sub layer |
...
| `lua_api`  | Imported by  | `src/lua_api/signal_api.rs` registers `lurek.signal.*` and wraps `Signal` as `LuaSignal` UserData |
