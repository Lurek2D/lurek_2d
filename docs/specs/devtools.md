...
- The `devtools` module provides runtime diagnostics for development: logging, profiling, frame stats, REPL support, and file-watch utilities.
...
The `devtools` module is the runtime diagnostics workspace for development sessions. It provides one consolidated surface for logs, profiling, frame timing, REPL evaluation, and watch-style inspection so teams can understand live behavior without scattering debug logic across feature modules.

Its logging path gives structured history with severity filtering, bounded retention, and optional output targets. This helps developers track what happened, in what order, and at what importance level when debugging runtime issues.

Performance visibility is handled through frame statistics and hierarchical profiling. The module records timing samples, computes aggregate metrics, and captures zone trees that expose expensive code paths. This makes it practical to identify hotspots and regressions during normal iteration.

REPL and display helpers shorten the debug loop. Scripts can evaluate expressions in the active runtime and inspect values through stable pretty-print behavior, which is useful for quick hypothesis checks without rebuilding gameplay logic.

Watcher features support content-heavy workflows. File change tracking and exposed watch values make hot iteration faster when scripts and data assets are updated frequently. Teams can react to changes with less manual restart friction.

The boundary is intentionally operational. `devtools` provides instrumentation and observability contracts, while gameplay behavior remains in dedicated domain modules. In practice, `lurek.devtools` serves as the shared diagnostic control plane for local debugging, QA runs, and integration troubleshooting.
