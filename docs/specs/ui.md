...
- UI boundary: `lurek.ui` is the native retained-mode widget toolkit for widgets, themes, focus/input routing, TOML layouts, charts, tables, menus, HUDs, and editor-like panels. Use [`lurek.html`](html.md) instead for document/CSS-oriented screens authored as HTML markup with DOM queries and CSS selectors.
...
> **Note:** Chart and graph rendering has moved to the standalone `charts` module. See [charts.md](charts.md).
