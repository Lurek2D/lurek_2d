...
- `TileMapLimits` defaults are `maxLayers=256`, `maxTiles=16,777,216`, `maxImportBytes=8 MiB`, `maxDecodedBytes=64 MiB`, `maxChunkCells=1,048,576`, `maxChunks=1,048,576`, `maxTileOperationCells=1,048,576`, `maxObjects=100,000`, `maxPointsPerObject=16,384`, and `maxTotalObjectPoints=1,000,000`. Option tables may lower or raise these ceilings only when the resulting configuration is non-zero and addressable.
