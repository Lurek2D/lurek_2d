...
function Tests.check_chart_widgets(C)
    ...
    local area = lurek.charts.newArea({ width = 64, height = 64, title = "area" })
    local area_count = area:addLayerFromDataFrame("expense", df, "expense", { 0.2, 0.6, 1.0 })
    local line = lurek.charts.newLine({ width = 64, height = 64, title = "line" })
    local line_count = line:addSeriesFromDataFrame("income", df, "x", "income", { 0.2, 0.6, 1.0 })
    local bar = lurek.charts.newBar({ width = 64, height = 64, title = "bar" })
    bar:addSeries("v", {}, { 0.2, 0.8, 0.4 })
    local bar_count = bar:addCategoriesFromDataFrame(df, "label", { "expense" })
    local pie = lurek.charts.newPie({ width = 64, height = 64, title = "pie" })
    local pie_count = pie:addSegmentsFromDataFrame(df, "label", "amount")
    local scatter = lurek.charts.newScatter({ width = 64, height = 64, title = "scatter" })
    local scatter_count = scatter:addSeriesFromDataFrame("points", df, "x", "amount", { 0.8, 0.3, 0.2 })
    lurek.filesystem.write(C.SAVE_DIR .. "/chart_check.txt", "Charts module DataFrame helper APIs covered; drawToImage covered by runtime smoke")
