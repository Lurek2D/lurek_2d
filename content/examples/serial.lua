...
--@api-stub: lurek.serial.fromJson -- Parses a JSON string into a Lua table
do -- lurek.serial.fromJson
  local data = lurek.serial.fromJson('{"name":"hero","hp":30}')
  lurek.log.info(data.name .. " hp=" .. data.hp, "serial")
end

--@api-stub: lurek.serial.toJson -- Serializes a Lua value (table, string, number, boolean, or nil) into a JSON string
do -- lurek.serial.toJson
  local body = lurek.serial.toJson({ event = "level_up", level = 5 }, true)
  lurek.log.info(body, "serial")
end

--@api-stub: lurek.serial.fromToml -- Parses a TOML string into a Lua table
...
--@api-stub: lurek.serial.toToml -- Serializes a Lua table into a TOML-formatted string
do -- lurek.serial.toToml
  local text = lurek.serial.toToml({ audio = { master = 0.8 }, fullscreen = false })
  lurek.log.info(text, "serial")
end

--@api-stub: lurek.serial.fromIni -- Parses an INI-format string into a Lua table
do -- lurek.serial.fromIni
  local cfg = lurek.serial.fromIni("[player]\nname=hero\n")
  lurek.log.info(cfg.player.name, "serial")
end

--@api-stub: lurek.serial.fromCsv -- Parses a CSV string into a Lua table (array of rows)
do -- lurek.serial.fromCsv
  local rows = lurek.serial.fromCsv("name,score\nada,10\nlin,20\n")
  lurek.log.info(rows[1].name .. " score=" .. rows[1].score, "serial")
end

--@api-stub: lurek.serial.toCsv -- Serializes a Lua table (array of row tables) into a CSV-formatted string
do -- lurek.serial.toCsv
  local csv = lurek.serial.toCsv({ { name = "ada", score = "10" } })
  lurek.log.info(csv, "serial")
end

--@api-stub: lurek.serial.encodeMsgPack -- Encodes a Lua table into a compact binary MessagePack string
...
--@api-stub: lurek.serial.decodeMsgPack -- Decodes a binary MessagePack string back into a Lua table
...
--@api-stub: lurek.serial.decodeXml -- Parses an XML string into a Lua table structure
...
--@api-stub: lurek.serial.validate -- Validates a Lua value against a schema table
...
--@api-stub: lurek.serial.detectFormat -- Attempts to auto-detect the serialization format of a string by inspecting its content (e
do -- lurek.serial.detectFormat
  local kind = lurek.serial.detectFormat('{"k":1}')
  lurek.log.info("format=" .. tostring(kind), "serial")
end

--@api-stub: lurek.serial.decode -- Universal decoder that parses a string payload into a Lua table using the specified format
...
--@api-stub: lurek.serial.encode -- Universal encoder that serializes a Lua value into the specified format
...
--@api-stub: lurek.serial.applyDefaults -- Merges a schema's default values into a data table, filling in any missing fields without overwriting existing ones
