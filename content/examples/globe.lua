...
-- =============================================================================
-- REAL EXAMPLES: Generated to satisfy API usage coverage
...
local function demo_Globe_addProvince()
    local g = lurek.globe.new("t1", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
end
...
local function demo_Globe_findPath()
    local g = lurek.globe.new("t2", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={2}, base_color={1,0,0,1}})
    g:addProvince({id=2, centroid={0,1}, vertices={{0,1},{1,1},{0,2}}, neighbors={1}, base_color={1,0,0,1}})
    local p = g:findPath(1, 2)
end
...
local function demo_GlobeRegistry_get()
    local g = lurek.globe.new("t3", {radius=100})
    local g_ref = lurek.globe.get("t3")
end
...
local function demo_Globe_getCamera()
    local g = lurek.globe.new("t5", {radius=100})
    g:setCamera(10, 20, 2.0)
    local lat, lon, zoom = g:getCamera()
end
...
local function demo_Globe_getLod()
    local g = lurek.globe.new("t6", {radius=100})
    g:setCamera(10, 20, 2.0)
    local lod = g:getLod()
end
...
local function demo_Globe_getMarkerAttr()
    local g = lurek.globe.new("t7", {radius=100})
    local id = g:addMarker("typeA", 10, 10, "m1")
    g:setMarkerAttr(id, "attr", "val")
    local val = g:getMarkerAttr(id, "attr")
end
...
local function demo_Globe_getName()
    local g = lurek.globe.new("t8", {radius=100})
    local n = g:getName()
end
...
local function demo_Globe_getNeighbors()
    local g = lurek.globe.new("t10", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={2}, base_color={1,0,0,1}})
...
local function demo_Globe_getProvinceAttr()
    local g = lurek.globe.new("t11", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
    g:setProvinceAttr(1, "owner", "p1")
    local owner = g:getProvinceAttr(1, "owner")
end
...
local function demo_Globe_getTimeOfDay()
    local g = lurek.globe.new("t12", {radius=100})
    g:setTimeOfDay(12.0)
...
local function demo_lurek_globe_greatCircleDistance()
    local d = lurek.globe.greatCircleDistance(0, 0, 10, 10)
    print("dist:", d)
end
...
local function demo_lurek_globe_greatCirclePath()
    local p = lurek.globe.greatCirclePath(0, 0, 10, 10, 5)
    print("path len:", #p)
end
...
local function demo_Globe_hideProvince()
    local g = lurek.globe.new("t13", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
...
local function demo_Globe_isVisible()
    local g = lurek.globe.new("t14", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
    g:revealProvince("p1", 1)
    local vis = g:isVisible("p1", 1)
end
...
local function demo_lurek_globe_latLonToUnit()
    local vec = lurek.globe.latLonToUnit(10, 20)
    print("vec:", vec[1], vec[2], vec[3])
end
...
local function demo_lurek_globe_loadFromTOML()
    ...
    local g = lurek.globe.loadFromTOML("t15", src, {radius=100})
end
...
local function demo_Globe_moveMarker()
    local g = lurek.globe.new("t16", {radius=100})
    local id = g:addMarker("type", 0, 0, "test")
    g:moveMarker(id, 10, 10)
end
...
local function demo_GlobeRegistry_names()
    local n = lurek.globe.names and lurek.globe.names() or {}
    print("Count:", #n)
end
...
local function demo_lurek_globe_new()
    local g = lurek.globe.new("t17", {radius=100, ambient=0.2})
    print("Created:", g:getName())
end
...
local function demo_Globe_pan()
    local g = lurek.globe.new("t18", {radius=100})
    g:pan(10.0, 5.0)
end
...
local function demo_Globe_pick()
    local g = lurek.globe.new("t19", {radius=100})
...
local function demo_Globe_pickLatLon()
    local g = lurek.globe.new("t20", {radius=100})
    local lat, lon = g:pickLatLon(400, 300)
end
...
local function demo_Globe_provinceCount()
    local g = lurek.globe.new("t21", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
    local c = g:provinceCount()
end
...
local function demo_GlobeRegistry_remove()
    local g = lurek.globe.new("t22", {radius=100})
    local ok = lurek.globe.remove and lurek.globe.remove("t22")
end
...
local function demo_Globe_removeArc()
    local g = lurek.globe.new("t23", {radius=100})
    local id = g:addArc(0,0, 10,10, 5)
    g:removeArc(id)
end
...
local function demo_Globe_removeLabel()
    local g = lurek.globe.new("t24", {radius=100})
    local id = g:addLabel("type", 0,0, "test")
    g:removeLabel(id)
end
...
local function demo_Globe_removeLayer()
    local g = lurek.globe.new("t25", {radius=100})
    g:addLayer("layer1", 1)
    g:removeLayer("layer1")
end
...
local function demo_Globe_removeMarker()
    local g = lurek.globe.new("t26", {radius=100})
    local id = g:addMarker("type", 0,0, "test")
    g:removeMarker(id)
end
...
local function demo_Globe_removeProvince()
    local g = lurek.globe.new("t27", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
    g:removeProvince(1)
end
...
local function demo_Globe_revealAll()
    local g = lurek.globe.new("t28", {radius=100})
    g:revealAll("p1")
end
...
local function demo_Globe_revealProvince()
    local g = lurek.globe.new("t29", {radius=100})
    g:addProvince({id=1, centroid={0,0}, vertices={{0,0},{1,0},{0,1}}, neighbors={}, base_color={1,0,0,1}})
    g:revealProvince("p1", 1)
end
...
local function demo_Globe_setActiveViewer()
    local g = lurek.globe.new("t30", {radius=100})
    g:setActiveViewer("p1")
end
...
local function demo_Globe_setBorders()
    local g = lurek.globe.new("t31", {radius=100})
    g:setBorders(false)
end
...
local function demo_Globe_setCamera()
    local g = lurek.globe.new("t32", {radius=100})
    g:setCamera(30.0, 40.0, 1.5)
end
...
local function demo_Globe_setLabelText()
    local g = lurek.globe.new("t33", {radius=100})
    local id = g:addLabel("type", 0,0, "test")
    g:setLabelText(id, "new test")
end
...
local function demo_Globe_setLabelVisible()
    local g = lurek.globe.new("t34", {radius=100})
    local id = g:addLabel("type", 0,0, "test")
...
local function demo_Globe_setLayerAlpha()
    local g = lurek.globe.new("t35", {radius=100})
    g:addLayer("layer1", 1)
    g:setLayerAlpha("layer1", 0.5)
end
...
local function demo_Globe_setLayerVisible()
    local g = lurek.globe.new("t36", {radius=100})
    g:addLayer("climate", 1)
    g:setLayerVisible("climate", true)
end
...
local function demo_Globe_setMarkerVisible()
    local g = lurek.globe.new("t37", {radius=100})
    local id = g:addMarker("type", 0,0, "test")
...
local function demo_Globe_setRotation()
    local g = lurek.globe.new("t38", {radius=100})
    g:setRotation(0.5)
end
...
local function demo_Globe_setTimeOfDay()
    local g = lurek.globe.new("t39", {radius=100})
    g:setTimeOfDay(14.5)
end
...
local function demo_Globe_update()
    local g = lurek.globe.new("t40", {radius=100})
    g:update(1.0)
end
...
local function demo_Globe_zoom()
    local g = lurek.globe.new("t41", {radius=100})
    g:zoom(1.2)
end
...
local function demo_lurek_globe_get()
    local g = lurek.globe.new("t42", {radius=100})
    local g_ref = lurek.globe.get("t42")
end
local _ok, _err = pcall(demo_lurek_globe_get)
