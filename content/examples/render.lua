...
do
  ...
  function lurek.init()
    local ok, img = pcall(lurek.render.newImage, "img/panel.png")
    if ok then panel = lurek.render.newNineSlice(img, 8, 8, 8, 8) end
    if panel and panel:typeOf("NineSlice") then
...
do
  ...
  function lurek.init()
    img = lurek.render.newImage("img/hero.png")
    if img:typeOf("Image") then lurek.log.debug("confirmed Image type") end
  end
end
...
do
  ...
  function lurek.init()
    f = lurek.render.newFont("assets/fonts/Inter.ttf", 18)
    if f:typeOf("Font") then lurek.log.debug("confirmed Font type") end
  end
end
...
do
  ...
  function lurek.init()
    c = lurek.render.newCanvas(320, 240)
    if c:typeOf("Canvas") then lurek.log.debug("confirmed Canvas type") end
  end
end
...
do
  ...
  function lurek.init()
    ...
    if batch:typeOf("SpriteBatch") then lurek.log.debug("confirmed SpriteBatch") end
  end
end
...
do
  ...
  function lurek.init()
    ...
    if m:typeOf("Mesh") then lurek.log.debug("confirmed Mesh type") end
  end
end
...
do
  ...
  function lurek.init()
    sh = lurek.render.newShader("// shader source")
    if sh:typeOf("Shader") then lurek.log.debug("confirmed Shader type") end
  end
end
...
do
  ...
  function lurek.init()
    q = lurek.render.newQuad(0, 0, 32, 32, 256, 256)
    if q:typeOf("Quad") then lurek.log.debug("confirmed Quad type") end
  end
end
...
do
  ...
  function lurek.init()
    s = lurek.render.newShape()
    if s:typeOf("Shape") then lurek.log.debug("confirmed Shape") end
  end
end
...
do
  ...
  function lurek.init()
    dl = lurek.render.newDrawLayer()
    if dl:typeOf("DrawLayer") then lurek.log.debug("confirmed DrawLayer") end
  end
end
