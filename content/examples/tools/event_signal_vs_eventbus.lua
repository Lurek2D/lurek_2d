...
health_signal:connect("changed", function(hp, reason)
    ...
    player.hp = hp
    lurek.log.info("signal health changed to " .. hp .. " after " .. reason)
end)
...
bus:on("inventory.item_used", function(item_name)
    ...
    lurek.log.info("analytics saw item use: " .. item_name)
end, 10)
bus:on("inventory.item_used", function(item_name)
    lurek.log.info("quest system checked: " .. item_name)
end)
bus:emit("inventory.item_used", "small_potion")

lurek.log.info("ui dirty = " .. tostring(player.ui_dirty))
lurek.log.info("log entries = " .. #player.log)
