...
lurek.log.info("=== Position tracking at 120 BPM (4/4 time) ===")
local t = 0
...
for i = 1, 24 do
    ...
    if #crossings > 0 then
        ...
        lurek.log.info(string.format("  t=%.2fs  beat=%.2f  bar=%d  beat_in_bar=%d",
...
bc:setBpm(180.0)
lurek.log.info(string.format("180 BPM: %.4f sec/beat", bc:secondsPerBeat()))
bc:setBpm(120.0)
...
for i = 1, 50 do
    ...
    if #fired > 0 then
        for _, beat in ipairs(fired) do
            ...
            lurek.log.info(string.format("  EVENT  beat=%.1f  fired at t=%.3fs", beat, t))
        end
    end
end
lurek.log.info(string.format("  Total events fired: %d (expected 3)", fired_total))
...
lurek.log.info(string.format("  Estimated BPM: %.1f  (expected ~140)", last_bpm))

-- ── 5. Quantisation ───────────────────────────────────────────────────────────

lurek.log.info("\n=== Quantisation ===")
local q = tap_clock:quantise(1.3, 0.25)
lurek.log.info(string.format("  beat 1.3 quantised to 1/4-beat grid = %.2f", q))
q = tap_clock:quantise(2.7, 0.5)
lurek.log.info(string.format("  beat 2.7 quantised to 1/2-beat grid = %.2f", q))
...
for i = 1, 9 do
    ...
    lurek.log.info(string.format("  beat %d  bar=%d  beat_in_bar=%d", i, pos.bar, pos.beat_in_bar))
end
