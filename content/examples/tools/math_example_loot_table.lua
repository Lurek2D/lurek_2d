...
lurek.log.info("=== 10 random drops ===")
for i = 1, 10 do
    local entry = drops:sample()
    lurek.log.info(string.format("  drop %2d: %s (weight=%.0f)", i, entry.id, entry.weight))
end
...
for id, n in pairs(counts) do
    lurek.log.info(string.format("  %-22s %d%%", id, n / 10))
end
...
for i, e in ipairs(chest:sampleUnique(3)) do
    lurek.log.info(string.format("  slot %d: %s", i, e.id))
end
...
for i = 1, 20 do
    ...
    if pity:isPrimed() then
        ...
        pity:reset()
        lurek.log.info(string.format("  pull %2d: [PITY] epic  (miss counter was %d)", i, pity:counter()))
    else
        entry = loot:sample()
        local just_primed = pity:notice(entry.id)
        local flag = just_primed and " <-- pity primed!" or ""
        lurek.log.info(string.format("  pull %2d: %-10s (misses=%d)%s", i, entry.id, pity:counter(), flag))
    end
end
...
pt:notice("common")
pt:notice("rare")
lurek.log.info("Before save: counter=" .. pt:counter())
local blob = pt:export()
local pt2 = lurek.math.newPityTracker("epic", 5)
pt2:import(blob)
lurek.log.info("After restore: counter=" .. pt2:counter() .. " isPrimed=" .. tostring(pt2:isPrimed()))
