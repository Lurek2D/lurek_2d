...
//! and `newGlyphData` factory functions matching the L├ľVE2D `love.font` module
...
///
/// # Fields
/// - `width` ÔÇö Pixel width of the rasterized glyph bitmap.
/// - `height` ÔÇö Pixel height of the rasterized glyph bitmap.
/// - `advance` ÔÇö Horizontal advance (cursor movement after this glyph).
/// - `bearing_x` ÔÇö Horizontal bearing (offset from cursor to left edge).
/// - `bearing_y` ÔÇö Vertical bearing (offset from baseline to top edge).
/// - `glyph` ÔÇö Unicode code point as a Lua integer.
/// - `glyph_string` ÔÇö Single UTF-8 character string.
#[derive(Clone)]
pub struct GlyphData {
    ...
    pub advance: f32,
    /// Horizontal bearing offset (cursor Ôćĺ left edge of glyph).
    pub bearing_x: f32,
    /// Vertical bearing offset (baseline Ôćĺ top edge of glyph).
    pub bearing_y: f32,
    /// Unicode code point.
    pub glyph: u32,
...
pub fn register(
    ...
    /// module for L├ľVE2D compatibility.
    ...
    /// effect ÔÇö fontdue uses its own hinting strategy internally.
    let state_cl = state.clone();
    font_table.set(
