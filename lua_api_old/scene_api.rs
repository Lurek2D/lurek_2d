...
impl LuaUserData for LuaDepthSorter {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        ///
        /// # Parameters
        /// - `callback` ÔÇö `function`: Draw callback `function()` called when flushing this layer.
        /// - `depth` ÔÇö `number`: Depth value determining draw order (lower = drawn first).
        ...
        ///
        /// # Parameters
        /// - `obj` ÔÇö `table`: Object with a `draw()` method. Uses `obj.depth` if no explicit depth is provided.
        ...
        ///
        /// # Returns
        /// `integer` ÔÇö number of registered draw entries.
...
pub fn register(lua: &Lua, luna: &LuaTable) -> LuaResult<()> {
    ...
    // luna.scene.popTo(name) Ôćĺ bool
    {
        let st = state.clone();
        /// Removes to.
        ///
    ...
    // luna.scene.getStackSize() Ôćĺ number
    {
        let st = state.clone();
    ...
    // luna.scene.getCurrent() Ôćĺ table|nil
    {
        let st = state.clone();
        /// Returns the current.
        ///
    ...
    // luna.scene.isTransitioning() Ôćĺ bool
    {
        let st = state.clone();
    ...
    // luna.scene.getTransitionProgress() Ôćĺ number [0,1]
    {
        let st = state.clone();
    ...
    // luna.scene.getRegistered(name) Ôćĺ table|nil
    {
        let st = state.clone();
    ...
    // luna.scene.hasRegistered(name) Ôćĺ bool
    {
        let st = state.clone();
    ...
    // luna.scene.getRegisteredNames() Ôćĺ {string}
    {
        let st = state.clone();
    ...
    // luna.scene.getData(key) Ôćĺ any|nil
    {
        let st = state.clone();
        /// Returns the data.
        ///
    ...
    // luna.scene.hasData(key) Ôćĺ bool
    {
        let st = state.clone();
    ...
    // luna.scene.newDepthSorter() Ôćĺ DepthSorter
    /// New depth sorter.
    ///
    /// @return any
    scene_table.set(
