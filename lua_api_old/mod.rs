...
pub mod compute_api;
/// Registers the `luna.data.*` L├ľVE2D-compatible binary data, pack, compress, hash, encode, and TOML API.
pub mod data_api;
...
pub mod serial_api;
// dialog_api removed ÔÇö dialog system is now library/dialog/init.lua
...
// math_ext_api removed ÔÇô merged into math_api
...
// sound_api removed ÔÇô functions merged into audio_api
...
// battle_api removed ÔÇö battle system is now library/battle/init.lua
// cardgame_api removed ÔÇö cardgame system is now library/cardgame/init.lua
// combat_api removed ÔÇö combat system is now library/combat/init.lua
// crafting_api removed ÔÇö crafting system is now library/crafting/init.lua
// economy_api removed ÔÇö economy system is now library/economy/init.lua
// inventory_api removed ÔÇö inventory system is now library/inventory/init.lua
// item_api removed ÔÇô cardgame covers this domain (doc comment removed)
/// UserData type utilities for Luna2D Lua objects.
pub mod lua_types;
// quest_api removed ÔÇö quest system is now library/quest/init.lua
// stats_api removed ÔÇö stats system is now library/stats/init.lua
...
///
/// # Parameters
/// - `state` ÔÇö Shared engine state passed (via `Rc<RefCell>` clone) to every Lua closure.
/// - `modules` ÔÇö Module enable/disable flags read from `conf.lua`. Mandatory APIs
///   (`math`, `log`, `event`) are always registered regardless of flags.
///
/// # Returns
/// `LuaResult<Lua>` ÔÇö A configured Lua VM with `luna.*` as a global, or a Lua error if
...
pub fn create_lua_vm(state: Rc<RefCell<SharedState>>, modules: &ModulesConfig) -> LuaResult<Lua> {
    ...
    // Mandatory ÔÇö always registered
    ...
    //   1. cwd-relative  ÔÇö works during development (cargo run -- examples/X)
    //   2. exe-relative  ÔÇö works in distribution (luna2d.exe next to library/)
    {
