...
pub fn register(lua: &Lua, luna: &LuaTable, state: Rc<RefCell<SharedState>>) -> LuaResult<()> {
    ...
    ///
    /// # Parameters
    /// - `title` ÔÇö New title string to display.
    let s = state.clone();
    /// @param title : string
    window.set(
        "setTitle",
    ...
    // Returns the physical pixel dimensions of the window (logical size ├Ś DPI scale).
    ...
    ///
    /// # Parameters
    /// - `mode` ÔÇö Scale mode: `"none"`, `"letterbox"`, `"stretch"`, or `"pixel"`.
    let s = state.clone();
    /// @param mode : string
    window.set(
        "setScaleMode",
