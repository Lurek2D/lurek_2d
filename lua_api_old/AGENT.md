# `lua_api` ÔÇö Agent Reference

| Property | Value |
|----------|-------|
| **Tier** | Bridge Layer |
| **Lua API** | `N/A ÔÇö this IS the binding layer` |
| **Source** | `src/lua_api/` |
| **Tests** | `tests/lua_api_tests.rs` |

## Summary

The lua_api module is the complete Lua interface layer ÔÇö it re-exports
...
to populate it ÔÇö one file per subsystem, each file covering a coherent surface
area.
...
SharedState (Rc<RefCell<SharedState>>)
  Ôöé
  ÔöťÔöÇÔöÇ 90+ pub fields ÔöÇÔöÇ textures, fonts, sounds, physics world,
  Ôöé     input states, filesystem, renderer, draw commands, etc.
  Ôöé
  ÔöťÔöÇÔöÇ create_lua_vm() ÔöÇÔöÇ Lua VM factory
  Ôöé     ÔöťÔöÇÔöÇ Creates mlua::Lua instance (LuaJIT or Lua 5.4)
  Ôöé     ÔöťÔöÇÔöÇ Creates `luna` global table
  Ôöé     ÔööÔöÇÔöÇ Calls register() on all API modules
  Ôöé
  ÔööÔöÇÔöÇ API modules (each has pub fn register())
        Ôöé
        ÔöťÔöÇÔöÇ Graphics ÔöÇÔöÇ luna.graphics.* (120+ functions)
        Ôöé     ÔöťÔöÇÔöÇ Drawing: rectangle, circle, line, polygon, print, ...
        Ôöé     ÔöťÔöÇÔöÇ Textures: newImage, draw(image), newCanvas, setCanvas
        Ôöé     ÔöťÔöÇÔöÇ Fonts: newFont, setFont, printf (aligned text)
        Ôöé     ÔöťÔöÇÔöÇ Transforms: push, pop, translate, rotate, scale
        Ôöé     ÔöťÔöÇÔöÇ Stencil: setStencilTest, stencil (masking)
        Ôöé     ÔöťÔöÇÔöÇ Shaders: newShader, setShader, shader:send
        Ôöé     ÔöťÔöÇÔöÇ Meshes: newMesh, mesh:setVertex
        Ôöé     ÔööÔöÇÔöÇ SpriteBatch: newSpriteBatch, batch:add
        Ôöé
        ÔöťÔöÇÔöÇ Audio ÔöÇÔöÇ luna.audio.* (25+ functions)
        Ôöé     ÔöťÔöÇÔöÇ newSource, play, pause, stop, setVolume
        Ôöé     ÔöťÔöÇÔöÇ Bus: newBus, setBusVolume
        Ôöé     ÔööÔöÇÔöÇ MIDI: newMidiPlayer, loadMidi
        Ôöé
        ÔöťÔöÇÔöÇ Input ÔöÇÔöÇ luna.keyboard/mouse/gamepad/touch.*
        Ôöé     ÔöťÔöÇÔöÇ Keyboard: isDown, getPressed, scancodes
        Ôöé     ÔöťÔöÇÔöÇ Mouse: getPosition, isDown, setCursor
        Ôöé     ÔöťÔöÇÔöÇ Gamepad: isDown, getAxis, getName
        Ôöé     ÔööÔöÇÔöÇ Touch: getTouches, getTouch
        Ôöé
        ÔöťÔöÇÔöÇ Physics ÔöÇÔöÇ luna.physics.* (70+ functions)
        Ôöé     ÔöťÔöÇÔöÇ World: newWorld, step, setGravity
        Ôöé     ÔöťÔöÇÔöÇ Bodies: newBody (rect/circle/polygon/edge/chain)
        Ôöé     ÔöťÔöÇÔöÇ Joints: 10 types (revolute, distance, prismatic, ...)
        Ôöé     ÔööÔöÇÔöÇ Queries: raycast, queryAABB, getContacts
        Ôöé
        ÔöťÔöÇÔöÇ Filesystem ÔöÇÔöÇ luna.filesystem.* (sandboxed I/O)
        ÔöťÔöÇÔöÇ Timer ÔöÇÔöÇ luna.timer.* (frame timing, scheduler)
        ÔöťÔöÇÔöÇ Window ÔöÇÔöÇ luna.window.* (window lifecycle)
        ÔöťÔöÇÔöÇ Math ÔöÇÔöÇ luna.math.* (vector utilities)
        ÔöťÔöÇÔöÇ Data ÔöÇÔöÇ luna.data.* (compress, hash, TOML)
        ÔöťÔöÇÔöÇ DataFrame ÔöÇÔöÇ luna.data.* (tabular, SQL)
        ÔöťÔöÇÔöÇ Entity ÔöÇÔöÇ luna.entity.* (ECS)
        ÔöťÔöÇÔöÇ Scene ÔöÇÔöÇ luna.scene.* (scene stack)
        ÔöťÔöÇÔöÇ Particle ÔöÇÔöÇ luna.particle.* (particle systems)
        ÔöťÔöÇÔöÇ Tilemap ÔöÇÔöÇ luna.tilemap.* (tilemaps, autotile)
        ÔöťÔöÇÔöÇ AI ÔöÇÔöÇ luna.ai.* (FSM, BT, steering, pathfinding)
        ÔöťÔöÇÔöÇ Compute ÔöÇÔöÇ luna.compute.* (NdArray)
        ÔöťÔöÇÔöÇ Graph ÔöÇÔöÇ luna.graph.* (directed graph)
        ÔöťÔöÇÔöÇ Image ÔöÇÔöÇ luna.image.* (pixel data)
        ÔöťÔöÇÔöÇ Sound ÔöÇÔöÇ luna.sound.* (raw samples)
        ÔöťÔöÇÔöÇ Modding ÔöÇÔöÇ luna.modding.* (mod management)
        ÔöťÔöÇÔöÇ Savegame ÔöÇÔöÇ luna.savegame.* (save/load)
        ÔöťÔöÇÔöÇ Pathfinding ÔöÇÔöÇ luna.pathfinding.* (A*, flow fields)
        ÔöťÔöÇÔöÇ Event ÔöÇÔöÇ luna.event.* (custom events)
        ÔöťÔöÇÔöÇ System ÔöÇÔöÇ luna.system.* (OS info, clipboard)
        ÔöťÔöÇÔöÇ Thread ÔöÇÔöÇ luna.thread.* (multi-threading)
        ÔöťÔöÇÔöÇ Graphics Ext ÔöÇÔöÇ additional graphics functions
        ÔööÔöÇÔöÇ Math Ext ÔöÇÔöÇ additional math functions
```
...
- **`LuaTrail`** (struct): Lua UserData wrapper for Trail (internal ÔÇö no factory).
- **`LuaDecalSurface`** (struct): Lua UserData wrapper for DecalSurface (internal ÔÇö no factory).
- **`LuaPaletteLUT`** (struct): Lua UserData wrapper for PaletteLUT (internal ÔÇö no factory).
...
Lua UserData wrapper for DecalSurface (internal ÔÇö no factory).
...
Lua UserData wrapper for PaletteLUT (internal ÔÇö no factory).
...
Lua UserData wrapper for Trail (internal ÔÇö no factory).
...
- **`add_type_methods()`** `lua_types::` ÔÇö Adds standard `type()` and `typeOf()` methods to a UserData definition.
- **`batch_key_from_value()`** `graphics_api::helpers::` ÔÇö Extract a `SpriteBatchKey` from either a `LuaSpriteBatch` UserData or a numeric ID.
- **`body_index_from_value()`** `physics_api::helpers::` ÔÇö Extract a body index from either a `LuaBody` UserData or an integer.
- **`canvas_key_from_value()`** `graphics_api::helpers::` ÔÇö Extract a `CanvasKey` from either a `LuaCanvas` UserData or a numeric ID.
- **`create_lua_vm()`** ÔÇö Creates and configures the Lua VM, registers all `luna.*` sub-APIs, and returns the ready `Lua` instance.
- **`emission_shape_to_lua()`** `particle_api::helpers::` ÔÇö Convert an `EmissionShape` to a Lua table with type and parameter fields.
- **`ensure_particle_exists()`** `particle_api::helpers::` ÔÇö Return `LuaError` if the particle does not exist in the pool.
- **`font_key_from_value()`** `graphics_api::helpers::` ÔÇö Extract a `FontKey` from either a `LuaFont` UserData or a numeric ID.
- **`get_memory_size()`** `system_api::` ÔÇö Returns total system RAM in MiB using the `sysinfo` crate.
- **`get_power_info()`** `system_api::` ÔÇö Returns power/battery information: (state, percent, seconds).  On desktop platforms this returns `(Unknown, None,...
- **`get_preferred_locales()`** `system_api::` ÔÇö Returns the user's preferred locale strings.
- **`get_processor_count()`** `system_api::` ÔÇö Returns the number of logical processors available.
- **`invalid_batch_handle()`** `graphics_api::helpers::` ÔÇö Returns a `LuaError` for an invalid batch handle.
- **`invalid_canvas_handle()`** `graphics_api::helpers::` ÔÇö Returns a `LuaError` for an invalid canvas handle.
- **`invalid_font_handle()`** `graphics_api::helpers::` ÔÇö Returns a `LuaError` for an invalid font handle.
- **`invalid_mesh_handle()`** `graphics_api::helpers::` ÔÇö Returns a `LuaError` for an invalid mesh handle.
- **`invalid_particle_handle()`** `particle_api::helpers::` ÔÇö Returns a `LuaError` for an invalid particle handle.
- **`invalid_texture_handle()`** `graphics_api::helpers::` ÔÇö Returns a `LuaError` for an invalid texture handle.
- **`lua_value_to_f64()`** `particle_api::helpers::` ÔÇö Helper to extract an f64 from a `LuaValue`.
- **`open_url()`** `system_api::` ÔÇö Opens a URL in the default browser/application.  Only `http://`, `https://`, and `mailto:` schemes are allowed.
- **`parse_body_type()`** `physics_api::helpers::` ÔÇö Parses a body type string into a `BodyType` enum value.
- **`parse_color()`** `particle_api::helpers::` ÔÇö Helper: parse a Lua color table `{r, g, b, a}` into `[f32; 4]`.
- **`parse_edge()`** `tilemap_api::helpers::` ÔÇö Parse an edge tag string into a `TileEdge` enum variant.
- **`parse_emission_shape()`** `particle_api::helpers::` ÔÇö Parse an emission shape from a Lua string name and optional parameters table.
- **`parse_script_step()`** `tilemap_api::helpers::` ÔÇö Parses a Lua table into a [`ScriptStep`].
- **`particle_key_from_value()`** `particle_api::helpers::` ÔÇö Extract a `ParticleKey` from either a `LuaParticleSystem` UserData or a numeric ID.  Callers validate liveness against...
- **`particle_system()`** `particle_api::helpers::` ÔÇö Borrow the particle system from shared state.
- **`particle_system_mut()`** `particle_api::helpers::` ÔÇö Borrow the particle system (mutable) from shared state.
- **`rect_to_table()`** `tilemap_api::helpers::` ÔÇö Convert a `Rect` into a Lua table `{x, y, w, h}`.
- **`register()`** `ai_api::` ÔÇö Registers the `luna.ai.*` game AI toolkit API.
- **`register()`** `audio_api::` ÔÇö Registers all `luna.audio.*` functions into the Lua VM.
- **`register()`** `battle_api::` ÔÇö Registers the `luna.battle.*` table. Panics in debug mode if the same entity is registered twice.
- **`register()`** `cardgame_api::` ÔÇö Registers the `luna.cardgame.*` API. Panics in debug mode if the same entity is registered twice.
- **`register()`** `combat_api::` ÔÇö Register the `luna.combat.*` vehicle combat table.
- **`register()`** `compute_api::` ÔÇö Registers the `luna.compute` table with array factory functions.
- **`register()`** `crafting_api::` ÔÇö Register the `luna.crafting.*` table. Panics in debug mode if the same entity is registered twice.
- **`register()`** `data_api::` ÔÇö Registers the `luna.data` table on the provided `luna` namespace.
- **`register()`** `dataframe_api::` ÔÇö Register the `luna.dataframe` namespace.
- **`register()`** `debug_api::` ÔÇö Registers the `luna.devtools` namespace.
- **`register()`** `debugbridge_api::` ÔÇö Registers the `luna.debugbridge` namespace.
- **`register()`** `dialog_api::` ÔÇö Registers the `luna.dialog` module with the Lua VM.
- **`register()`** `docs_api::` ÔÇö Registers the `luna.docs` namespace. Panics in debug mode if the same entity is registered twice.
- **`register()`** `entity_api::` ÔÇö Registers the `luna.entity` table with the `newUniverse` factory function.
- **`register()`** `event_api::` ÔÇö Registers `luna.event.quit()` and related engine lifecycle functions into the Lua VM.
- **`register()`** `filesystem_api::` ÔÇö Registers `luna.filesystem.*` functions into the Lua VM.
- **`register()`** `graph_api::` ÔÇö Register the `luna.graph` API table. Panics in debug mode if the same entity is registered twice.
- **`register()`** `graphics_api::` ÔÇö Register `luna..*` bindings in the Lua state.
- **`register()`** `graphics_ext_api::` ÔÇö Registers Phase 24 graphics extension factory functions on `luna.graphics`.
- **`register()`** `image_api::` ÔÇö Registers the `luna.image` table on the provided `luna` namespace.
- **`register()`** `input_api::` ÔÇö Registers `luna.keyboard.*` and `luna.mouse.*` query functions into the Lua VM.
- **`register()`** `inventory_api::` ÔÇö Register the `luna.inventory` module. Panics in debug mode if the same entity is registered twice.
- **`register()`** `localization_api::` ÔÇö Registers `luna.localization.*` functions.
- **`register()`** `log_api::` ÔÇö Registers the `luna.log.*` namespace into the shared `luna` table.
- **`register()`** `math_api::` ÔÇö Registers `luna.math.*` helpers (Vec2, distance, random, noise, transforms, etc.) into the Lua VM.
- **`register()`** `minimap_api::` ÔÇö Register the `luna.minimap` module. Panics in debug mode if the same entity is registered twice.
- **`register()`** `modding_api::` ÔÇö Registers `luna.modding.*` functions into the Lua VM.
- **`register()`** `particle_api::` ÔÇö Register `luna..*` bindings in the Lua state.
- **`register()`** `pathfinding_api::` ÔÇö Register the `luna.pathfinding` namespace.
- **`register()`** `patterns_api::` ÔÇö Registers `luna.patterns.*` factory functions.
- **`register()`** `physics_api::` ÔÇö Register `luna..*` bindings in the Lua state.
- **`register()`** `postfx_api::` ÔÇö Registers the `luna.postfx.*` API. Panics in debug mode if the same entity is registered twice.
- **`register()`** `quest_api::` ÔÇö Register `luna.quest.*` API with the Lua VM.
- **`register()`** `economy_api::` ÔÇö Registers the `luna.economy` module with the Lua VM.
- **`register()`** `savegame_api::` ÔÇö Registers `luna.savegame.*` functions into the Lua VM.
- **`register()`** `scene_api::` ÔÇö Registers the `luna.scene` table with scene stack, registry, data store,
- **`register()`** `stats_api::` ÔÇö Register the `luna.stats` module into the Lua state.
- **`register()`** `system_api::` ÔÇö Registers `luna.system.*` platform query functions into the Lua VM.
- **`register()`** `thread_api::` ÔÇö Registers all `luna.thread.*` functions into the Lua VM.
- **`register()`** `tilemap_api::` ÔÇö Register `luna..*` bindings in the Lua state.
- **`register()`** `timer_api::` ÔÇö Registers `luna.timer.*` functions into the Lua VM.  Provides frame-timing utilities: delta time, FPS, total time,...
- **`register()`** `window_api::` ÔÇö Registers `luna.window.*` functions into the Lua VM.
- **`register_ext()`** `graphics_api::ext::` ÔÇö Register extended Lua bindings for this sub-module.
- **`register_ext()`** `particle_api::ext::` ÔÇö Register extended Lua bindings for this sub-module.
- **`register_ext()`** `physics_api::ext::` ÔÇö Register extended Lua bindings for this sub-module.
- **`register_ext()`** `tilemap_api::ext::` ÔÇö Register extended Lua bindings for this sub-module.
- **`require_batch_key()`** `graphics_api::helpers::` ÔÇö Resolve and validate a batch key, returning `LuaError` if missing.
- **`require_canvas_key()`** `graphics_api::helpers::` ÔÇö Resolve and validate a canvas key, returning `LuaError` if missing.
- **`require_font_key()`** `graphics_api::helpers::` ÔÇö Resolve and validate a font key, returning `LuaError` if missing.
- **`require_mesh_key()`** `graphics_api::helpers::` ÔÇö Resolve and validate a mesh key, returning `LuaError` if missing.
- **`require_particle_key()`** `particle_api::helpers::` ÔÇö Resolve and validate a particle key, returning `LuaError` if missing.
- **`require_texture_key()`** `graphics_api::helpers::` ÔÇö Resolve and validate a texture key, returning `LuaError` if missing.
- **`step_to_table()`** `tilemap_api::helpers::` ÔÇö Converts a [`ScriptStep`] back to a Lua table.
- **`texture_key_from_value()`** `graphics_api::helpers::` ÔÇö Extract a `TextureKey` from either a `LuaImage` UserData or a numeric ID.
- **`world_index_from_value()`** `physics_api::helpers::` ÔÇö Extract a world index from either a `LuaWorld` UserData or an integer.
