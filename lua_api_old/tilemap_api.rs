...
///
/// # Fields
/// - `inner` ├ö├ç├Â `Rc<RefCell<TileMap>>`.
#[derive(Clone)]
...
///
/// # Fields
/// - `inner` ÔÇö `Rc<RefCell<AutoTileSheet>>`.
#[derive(Clone)]
...
///
/// # Fields
/// - `inner` ÔÇö `Rc<RefCell<MapScript>>`.
#[derive(Clone)]
...
///
/// # Fields
/// - `inner` ÔÇö `Rc<RefCell<MapGen>>`.
/// - `group` ÔÇö `Rc<RefCell<MapGroup>>`.
#[derive(Clone)]
...
impl LuaUserData for LuaTileMap {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        ///
        /// # Parameters
        /// - `dt` ├ö├ç├Â Delta time in seconds since the last update.
        ...
        ///
        /// # Returns
        /// `integer` ├ö├ç├Â tile pixel width.
        methods.add_method("getTileWidth", |_, this, ()| {
            Ok(this.inner.borrow().get_tile_width())
        });
        ...
        ///
        /// # Returns
        /// `integer` ├ö├ç├Â tile pixel height.
        methods.add_method("getTileHeight", |_, this, ()| {
            Ok(this.inner.borrow().get_tile_height())
        });
...
// ---------------------------------------------------------------------------
// LuaMapScript UserData ├ö├ç├Â helpers
// ---------------------------------------------------------------------------
...
pub(super) fn parse_script_step(t: &LuaTable) -> LuaResult<ScriptStep> {
    ...
    // Helper for 1-based ├ö─ç─║ 0-based conversion on coordinate/index fields
...
pub(super) fn step_to_table<'lua>(lua: &'lua Lua, step: &ScriptStep) -> LuaResult<LuaTable<'lua>> {
    ...
    /// # Returns
    /// The result.
    t.set("x", step.x + 1)?; // 0-based ├ö─ç─║ 1-based
    /// Y on this MapGroup.
    ///
    /// # Returns
    /// The result.
    ...
    /// # Returns
    /// The result.
    t.set("tileLayer", step.tile_layer + 1)?; // 0-based ├ö─ç─║ 1-based
    t.set(
        "zoneStartY",
...
impl LuaUserData for LuaMapGen {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        ///
        /// # Parameters
        /// - `index` ├ö├ç├Â 1-based index into the zone list.
        ///
        /// # Returns
        ...
        methods.add_method("getZone", |lua, this, index: usize| {
            ...
            match inner.get_zone(index - 1) {
                Some(zone) => {
                    ...
                    /// # Returns
                    /// The result.
                    t.set("startRow", zone.start_row + 1)?; // 0-based ├ö─ç─║ 1-based
...
impl LuaUserData for LuaIsoMap {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        ///
        /// # Parameters
        /// - `level` ├ö├ç├Â Level definition table or level index integer.
...
fn register_ext(lua: &Lua, tilemap_table: &LuaTable) -> LuaResult<()> {
    ...
    ///
    /// # Parameters
    /// - `q` ├ö├ç├Â Integer q (column) coordinate of the center cell.
    /// - `r` ├ö├ç├Â Integer r (row) coordinate of the center cell.
    ///
    /// # Returns
    ...
    ///
    /// # Parameters
    /// - `q` ├ö├ç├Â Fractional q (column) coordinate.
    /// - `r` ├ö├ç├Â Fractional r (row) coordinate.
    ///
    /// # Returns
    ...
    ///
    /// # Parameters
    /// - `q` ├ö├ç├Â Center q coordinate.
    /// - `r` ├ö├ç├Â Center r coordinate.
    /// - `radius` ├ö├ç├Â Ring distance in hex steps.
    ///
    /// # Returns
    ...
    /// - `q` ├ö├ç├Â Center q coordinate.
    /// - `r` ├ö├ç├Â Center r coordinate.
    /// - `radius` ├ö├ç├Â Radius in hex steps (1 = immediate ring, 2 = two rings, etc.).
    ///
    /// # Returns
...
pub fn register(lua: &Lua, luna: &LuaTable) -> LuaResult<()> {
    ...
    ///
    /// # Parameters
    /// - `name` ├ö├ç├Â Display name for the group layer.
    ///
    /// # Returns
    ...
    ///
    /// # Parameters
    /// - `name` ├ö├ç├Â Identifier name for the script component.
    ///
    /// # Returns
